...
namespace App\View\Components\AdvancedPay;
...
class AdvancedPay extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $finalfromdate = '';
		...
		if($datatype != NULL && $datatype != '')
		{
			...
			if($whereraw == '')
			{
				$whereraw = "requested_at >= '".$fromDate."' and requested_at <= '".$toDate."'";
			}
			else
			{
				$whereraw .= " And requested_at >= '".$fromDate."' and requested_at <= '".$toDate."'";
		...
		if($whereraw != '')
		{
			...
			$totaldata= AdvancedPayRequest::whereRaw($whereraw)->where('approved_reject_status',1)->groupBy('emp_id')->orderBy("id","DESC")->get();
		...
		else
		{
			...
			$toDate = date("Y-m-d");
			$fromDate = date("Y").'-'.date("m").'-'.'01';
			$whererawdefault = "requested_at >= '".$fromDate."' and requested_at <= '".$toDate."'";
			// echo "h1";
			// print_r($whereraw);
			// exit;
			...
			//echo "h2";
			//$totaldata= DocumentCollectionDetails::select("department")->groupBy('department')->get();
			//$totaldata= RequestedLeaves::where('approved_reject_status',1)->where('final_status',1)->groupBy('emp_id')->get();	
			$totaldata= AdvancedPayRequest::whereRaw($whererawdefault)->where('approved_reject_status',1)->groupBy('emp_id')->orderBy("id","DESC")->get();	
		...
		//$empDetails= RequestedLeaves::whereIn("id",$finalemp)->get();
    ...
    {
        return view('components.AdvancedPay.advancedpay');
    }
...
public static function getTeamListsSelectedName($dept){
	...
	public static function getDepartmenttname($empid)
	{
		$empDetails= Employee_details::where('emp_id',$empid)->orderBy("id","DESC")->first();

		if($empDetails)
		{
			$name = Department::where("id",$empDetails->dept_id)->first();
			if($name!="")
			{
	...
	public static function getLeaveType($empid,$rowid)
	{
		$leaveEmpDetails= RequestedLeaves::where('emp_id',$empid)->where('id',$rowid)->orderBy("id","DESC")->first();	

		if($leaveEmpDetails)
		{
			$leaveType= LeaveTypes::where('id',$leaveEmpDetails->leave_id)->orderBy("id","DESC")->first();

			if($leaveType)
			{
				return $leaveType->leaves_title;
			}
			else
			{
				return "--";
