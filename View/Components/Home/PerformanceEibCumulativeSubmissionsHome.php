...
class PerformanceEibCumulativeSubmissionsHome extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if($whereraw != '')
		{
		$totaldata= EIBDepartmentFormEntry::groupBy('tl_name')->selectRaw('count(*) as total, tl_name')->whereRaw($whereraw)->get();
   ...
   $finaldata= json_encode($finalarray);
		...
		if($whereraw != '')
		{
		$totaldata= EIBDepartmentFormEntry::groupBy('application_date')->selectRaw('count(*) as total, application_date')->whereRaw($whereraw)->get();
 ...
 $finaldata= json_encode($finalarray);
    ...
    {
        return view('components.Home.performanceeibcumulativesubmissionshome');
    }
...
Private function getTeamdata($team,$widgetId)
	{
		...
		if($whereraw != '')
		{
		$totaldata= EIBDepartmentFormEntry::groupBy('application_date')->selectRaw('count(*) as total, application_date')->whereRaw($whereraw)->where("tl_name",$team)->get();
