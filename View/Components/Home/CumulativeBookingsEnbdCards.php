...
class CumulativeBookingsEnbdCards extends Component
{
    /**
	...
	public $nonprocessorSelecteddata;
	public $WeekList;
	public $lagendData;
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
			...
			if($datatype != NULL && $datatype != '')
		{
			...
			if($whereraw == '')
			{
				$whereraw = "application_created >= '".$fromDate."' and application_created <= '".$toDate."'";
			}
			else
			{
				$whereraw .= " And application_created >= '".$fromDate."' and application_created <= '".$toDate."'";
			}
		}
		else{
			$toDate = date("Y-m-d");
		...
		if(Request::session()->get('widgetBookingsFilterAleNaleStatus['.$widgetId.']') != '' && Request::session()->get('widgetBookingsFilterAleNaleStatus['.$widgetId.']') != NULL)
		{
			$aleIds =  Request::session()->get('widgetBookingsFilterAleNaleStatus['.$widgetId.']');
			...
			if($whereraw == '')
			{
			$whereraw = 'ale_nonale IN('.$finalcname.')';
			}
			else
			{
				$whereraw .= ' AND ale_nonale IN('.$finalcname.')';
			}
					
		}


		if(Request::session()->get('widgetBookingsFilterNonAleNaleStatus['.$widgetId.']') != '' && Request::session()->get('widgetBookingsFilterNonAleNaleStatus['.$widgetId.']') != NULL)
		{
			$aleIds =  Request::session()->get('widgetBookingsFilterNonAleNaleStatus['.$widgetId.']');
		...
		$totaldata= MainMisReportTab::where("application_status","COMPLETED")->whereRaw($whereraw)->get();
  ...
  $finaldata= json_encode($finalarray);
		...
		if($datatype != NULL && $datatype != '')
		{
			...
			{
				$toDate = date("Y").'-'.date("m").'-'.'20';
				$fromDate = date("Y",strtotime("-3 month ".date("Y-m-d"))).'-'.date("m",strtotime("-3 month ".date("Y-m-d"))).'-'.'21';
		...
		$totaldata= MainMisReportTab::groupBy('application_created')->selectRaw('count(*) as total, application_created')->where("application_status","COMPLETED")->whereRaw($whereraw)->orderBy('application_created', 'ASC')->get();
		...
		foreach($totaldata as $_totaldata)
		{
			//echo $_totaldata->application_created;exit;
				
			//echo $fromdate3;
			//echo $todate3;exit;
			if(strtotime($_totaldata->application_created) >= strtotime($fromdate1) && strtotime($_totaldata->application_created) <= strtotime($todate1))
			{
				$month=date('Y-m', strtotime($fromdate1));
				
				$monthlevel=date('M', strtotime($fromdate1));
				$datezero=date('j', strtotime($_totaldata->application_created));
			...
			else if(strtotime($_totaldata->application_created) >= strtotime($fromdate2) && strtotime($_totaldata->application_created) <= strtotime($todate2))
			{
			...
			else if(strtotime($_totaldata->application_created) >= strtotime($fromdate3) && strtotime($_totaldata->application_created) <= strtotime($todate3))
			{
 ...
 $finaldata= json_encode($finalarray);
		...
		if(Request::session()->get('widgetBookingsFilterAleNaleStatus['.$widgetId.']') != '' && Request::session()->get('widgetBookingsFilterAleNaleStatus['.$widgetId.']') != NULL)	
		{
			$this->processorSelecteddata = Request::session()->get('widgetBookingsFilterAleNaleStatus['.$widgetId.']');
		}
		else
		{
		...
		if(Request::session()->get('widgetBookingsFilterNonAleNaleStatus['.$widgetId.']') != '' && Request::session()->get('widgetBookingsFilterNonAleNaleStatus['.$widgetId.']') != NULL)	
		{
			$this->nonprocessorSelecteddata = Request::session()->get('widgetBookingsFilterNonAleNaleStatus['.$widgetId.']');
		}
		else
		{
			$this->nonprocessorSelecteddata ='';
		}
    ...
    {
        return view('components.Home.cumulativebookings_enbdcards');
    }
...
Private function getTeamdata($team,$widgetId)
	{
		...
		$totaldata= MainMisReportTab::groupBy('application_created')->selectRaw('count(*) as total, application_created')->where("application_status","COMPLETED")->whereRaw($whereraw)->get();
		...
		foreach($totaldata as $_totaldata)
		{
			...
			//print_r($_totaldata);exit;
			$date=explode("-",$_totaldata->application_created);
			$datezero=date('m-j', strtotime($_totaldata->application_created));
			$monthlevel=date('M', strtotime($_totaldata->application_created));//exit;
			$month=date('Y-m', strtotime($_totaldata->application_created));
			//$bookingAsDateCompime[$month]=0;
			if(isset($bookingAsDateCompime[$month]))
				{
				$bookingAsDateCompime[$month]  = $bookingAsDateCompime[$month]+$_totaldata->total;
...
Private function getProcessordata($processor,$widgetId)
	{
		...
		if($whereraw != '')
		{
		//$totaldata= DepartmentFormEntry::groupBy('application_created')->selectRaw('count(*) as total, application_created')->where("form_id",7)->whereRaw($whereraw)->get();
		$totaldata= MainMisReportTab::groupBy('application_created')->selectRaw('count(*) as total, application_created')->where("application_status","COMPLETED")->whereRaw($whereraw)->get();
