...
class EmployeeAttendanceHome extends Component
{
    /**
	   ...
	   $finalfromdate = '';
		...
		if(Request::session()->get('widgetFilterHiring['.$widgetId.'][department]') != '' && Request::session()->get('widgetFilterHiring['.$widgetId.'][department]') != NULL)
		{
			...
			if($empDataGetting!=''){
				...
				if($empid!='' && $empid->job_function==3){
					$id=$empid->id;
					$tldata=Employee_details::where("tl_id",$id)->get();
					if($tldata!=''){
						$empid=array();
						foreach($tldata as $tlids){
						$empid[]=$tlids->emp_id;	
						}
					}
					$empIdsFinal=implode(",", $empid);
					if($whereraw == '')
					{
					$whereraw = 'emp_id IN('.$empIdsFinal.')';
					}
					else
					{
						$whereraw .= ' AND emp_id IN('.$empIdsFinal.')';
					}
				}
			}
    ...
    {
        return view('components.Home.empattendancehome');
...
public static function getTeamListsSelectedName($dept){
	...
	public static function getPresentAttendanceTotal($empid,$widgetId)
	{	
		$datatype = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
		if($datatype != NULL && $datatype != '')
		{
	...
	public static function getAbsentAttendanceTotal($empid,$widgetId)
	{	
		$datatype = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
		if($datatype != NULL && $datatype != '')
		{
	...
	public static function getTodayAttendance($empid,$widgetId)
	{	
