...
class PerformanceScbCumulativeBookingsHome extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
		...
		if($user!=''){
			$empdata=Employee_details::where("emp_id",$user->employee_id)->where("job_function",3)->first();
					   ...
					   if($empdata!=''){
						   //$TeamLists = SCBDepartmentFormParentEntry::groupBy('emp_id',$user->employee_id)->first();
							Request::session()->put('widgetFiltermolTeam['.$widgetId.']',$user->employee_id);
					   }
		}*/
    ...
    {
        return view('components.Home.performancescbcumulativebookingshome');
    }
