...
class AutoloanInternalData extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
			...
			if($datatype != NULL && $datatype != '')
		{
			...
			if($whereraw == '')
			{
				$whereraw = "login_date >= '".$fromDate."' and login_date <= '".$toDate."'";
			}
			else
			{
				$whereraw .= " And login_date >= '".$fromDate."' and login_date <= '".$toDate."'";
			}
		}
		else{
			$toDate = date("Y-m-d");
		...
		if($whereraw != '')
		{
		$totaldata= AutoloanMIS::groupBy('team')->selectRaw('count(*) as total, team')->whereRaw($whereraw)->get();
  ...
  $finaldata= json_encode($finalarray);
		...
		if($whereraw != '')
		{
		$totaldata= AutoloanMIS::groupBy('login_date')->selectRaw('count(*) as total, login_date')->whereRaw($whereraw)->get();
		...
		foreach($totaldata as $_totaldata)
		{
			...
			$date=explode("-",$_totaldata->login_date);
			$datezero=date('m-j', strtotime($_totaldata->login_date));
			$monthlevel=date('M', strtotime($_totaldata->login_date));//exit;
			$month=date('Y-m', strtotime($_totaldata->login_date));
  ...
  $finaldata= json_encode($finalarray);
		...
		$TeamLists = AutoloanMIS::whereRaw("team !=''")->groupBy('team')->selectRaw('count(*) as total, team')->get();
    ...
    {
        return view('components.TestRavi.autoloan-internal-data');
    }
...
public static function getTeamListsSelectedName($dept){
		...
		foreach($dept as $r)
		{
			if($name == '')
			{
				$name = AutoloanMIS::where("team",$r)->first()->team;
			}
			else
			{
				$name = $name.','.AutoloanMIS::where("team",$r)->first()->team;
			}
		}
		return $name;
	}
...
Private function getTeamdata($team,$widgetId)
	{
		...
		if($whereraw != '')
		{
		$totaldata= AutoloanMIS::groupBy('login_date')->selectRaw('count(*) as total, login_date')->whereRaw($whereraw)->where("team",$team)->get();
