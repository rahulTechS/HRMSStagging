...
class ReportData extends Component
{
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
			...
			if($datatype != NULL && $datatype != '')
		{
			...
			if($whereraw == '')
			{
				$whereraw = "date_of_request >= '".$fromDate."' and date_of_request <= '".$toDate."'";
			}
			else
			{
				$whereraw .= " And date_of_request >= '".$fromDate."' and date_of_request <= '".$toDate."'";
			}
		}
		else{
			$toDate = date("Y-m-d");
		...
		if($whereraw != '')
		{
		$totaldata= ENBDLoanMISAECB::groupBy('team')->selectRaw('count(*) as total, team')->whereRaw($whereraw)->get();
  ...
  $finaldata= json_encode($finalarray);
		...
		if($whereraw != '')
		{
		$totaldata= ENBDLoanMISAECB::groupBy('date_of_request')->selectRaw('count(*) as total, date_of_request')->whereRaw($whereraw)->get();
		...
		foreach($totaldata as $_totaldata)
		{
			...
			$date=explode("-",$_totaldata->date_of_request);
			$datezero=date('m-j', strtotime($_totaldata->date_of_request));
			$monthlevel=date('M', strtotime($_totaldata->date_of_request));//exit;
			$month=date('Y-m', strtotime($_totaldata->date_of_request));
  ...
  $finaldata= json_encode($finalarray);
		...
		$TeamLists = ENBDLoanMISAECB::groupBy('team')->selectRaw('count(*) as total, team')->get();
    ...
    {
        return view('components.TestRavi.report-data');
    }
...
public static function getTeamListsSelectedName($dept){
		...
		foreach($dept as $r)
		{
			if($name == '')
			{
				$name = ENBDLoanMISAECB::where("team",$r)->first()->team;
			}
			else
			{
				$name = $name.','.ENBDLoanMISAECB::where("team",$r)->first()->team;
			}
		}
		return $name;
	}
...
Private function getTeamdata($team,$widgetId)
	{
		...
		if($whereraw != '')
		{
		$totaldata= ENBDLoanMISAECB::groupBy('date_of_request')->selectRaw('count(*) as total, date_of_request')->whereRaw($whereraw)->where("team",$team)->get();
