...
class RecommendationsSales extends Component
{
    /**
	...
	// 	$empData= Employee_details::where('emp_id',$userData->employee_id)->where('job_function',3)->first();
	   ...
	   $whereraw = '';
		...
		if($whereraw != '')
		{
			$totaldata= MashreqBookingMIS::whereIn("emp_id",$finalemp)->whereRaw($whereraw)->groupBy('emp_id')->get();
    ...
    public function render()
    {
		...
		return view('components.SalesManagement.recommendationsales');
...
public static function getSourceCode($empid)
{
	...
	public static function getBookingCount($empid,$fromfilterdate,$month,$tofilterdate)
	{
		...
		$whererawdefault = "dateofdisbursal >= '".$fromDate."' and dateofdisbursal <= '".$toDate."'";
		
		$empbookingdata= MashreqBookingMIS::whereRaw($whererawdefault)->where('emp_id', $empid)->orderBy("id","DESC")->get()->count();		
	...
	public static function getSecondPreviousBookingCount($empid,$fromfilterdate,$month,$tofilterdate)
	{
		...
		else
		{
			...
			$form=date('Y-m',strtotime('-60days'));
			$fromDate=$form."-01";
