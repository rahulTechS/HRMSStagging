...
class ScbRecommendationsSales extends Component
{
    /**
    ...
    public function render()
    {
		...
		return view('components.SalesManagement.scbrecommendationsales');
...
public static function getTeamListsSelectedName($dept){
		...
		foreach($dept as $r)
		{
			if($name == '')
			{
				$name = CBDBankMis::where("sm_manager",$r)->first()->sm_manager;
			}
			else
			{
				$name = $name.','.CBDBankMis::where("sm_manager",$r)->first()->sm_manager;
			}
		}
		return $name;
	}
...
Private function getTeamdata($team,$widgetId)
	{
		...
		if($whereraw != '')
		{
		$totaldata= CBDBankMis::groupBy('approval_date')->selectRaw('count(*) as total, approval_date')->whereRaw($whereraw)->where("sm_manager",$team)->get();
...
public static function getSourceCode($empid)
{
	...
	public static function getBookingCount($empid,$fromfilterdate,$month,$tofilterdate)
	{
		...
		$whererawdefault = "approved_date >= '".$fromDate."' and approved_date <= '".$toDate."'";
		
		$empbookingdata= SCBDepartmentFormParentEntry::whereIn("form_status",array("Approved"))->whereRaw($whererawdefault)->where('emp_id', $empid)->orderBy("id","DESC")->get()->count();		
