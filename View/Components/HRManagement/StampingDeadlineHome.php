...
namespace App\View\Components\HRManagement;
...
class StampingDeadlineHome extends Component
{
    /**
		...
		if($user!=''){
			$empdata=Employee_details::where("emp_id",$user->employee_id)->where("job_function",3)->first();
					   ...
					   if($empdata!=''){
						...
						$documentCollectiondetails= DocumentCollectionDetails::whereIn("id",$finaldocidarray)->whereRaw($whereraw)->orderByRaw("-sort_date DESC")->get();
    ...
    {
        return view('components.HRManagement.stampingdeadlinehome');
    }
	...
	$departmentDetails = EmployeeOnboardLogdata::where("document_id",$docId)->first();
		   ...
		   else
		   {
				...
				if($empd!=""){
					...
					$departmentdata = EmployeeOnboardData::where("document_id",$docId)->first();
				   if($departmentdata != '' && $departmentdata->emp_id!='')
					{
				   return $departmentdata->emp_id;
					}else{
						return "--";
					}
...
public static function getVisaStageName($id)
 ...
 public static function getRecruiterName($recruiterId)
	   ...
	   public static function getChangeStatus($docId){
		...
		public static function getEmailStatus($id)
		{
			$docsData= DocumentCollectionDetails::where('id', $id)->orderBy("id","DESC")->first();
	
			if($docsData!='')
			{
				if($docsData->email_sent==0)
				{
					return 1;
				}
				if($docsData->email_sent==1)
				{
					return 2;
				}
			}
