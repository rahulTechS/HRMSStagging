...
class StampingDeadline extends Component
{
		...
		if($user!=''){
			$empdata=Employee_details::where("emp_id",$user->employee_id)->where("job_function",3)->first();
					   ...
					   if($empdata!=''){
						...
						$documentCollectiondetails= DocumentCollectionDetails::whereIn("id",$finaldocidarray)->whereRaw($whereraw)->where("sort_dateBY","Stamping Deadline")->orderByRaw("-sort_date DESC")->get();
    ...
    {
        return view('components.MyTeam.stampingdeadline');
    }
