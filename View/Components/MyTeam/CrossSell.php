...
class CrossSell extends Component
{
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if($empData!=''){
			...
			$totalempdata= Employee_details::where('offline_status',1)->where('dept_id',$empData->dept_id)->get();
		}
		...
		if($whereraw != '')
		{
			...
			// print_r($whereraw);
			// exit;
			$totaldata= CrossSellScenariosAllocation::whereRaw($whereraw)->whereIn("emp_id",$finalemp1)->where("allocate_to",'Agent')->get();
		...
		else
		{
			...
			$totaldata= CrossSellScenariosAllocation::whereIn("emp_id",$finalemp1)->where("allocate_to",'Agent')->get();	
    ...
    {
        return view('components.MyTeam.crosssell');
    }
