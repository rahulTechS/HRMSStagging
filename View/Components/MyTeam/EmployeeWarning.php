...
class EmployeeWarning extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if($datatype != NULL && $datatype != '')
		{
			...
			if($whereraw == '')
			{
				$whereraw = "warning_letter_issued_on >= '".$fromDate."' and warning_letter_issued_on <= '".$toDate."'";
			}
			else
			{
				$whereraw .= " And warning_letter_issued_on >= '".$fromDate."' and warning_letter_issued_on <= '".$toDate."'";
			}
		}
		else{
	...
	$this->loggedinuser = Request::session()->get('EmployeeId');
		...
		if($empData!=''){
			$totalempdata= Employee_details::where('tl_id',$empData->id)->where('dept_id',$empData->dept_id)->where('offline_status',1)->get();
		}
		else{
			$totalempdata= Employee_details::where('offline_status',1)->get();
		}
		...
		if($whereraw != '')
		{
			...
			// print_r($whereraw);
			// exit;
			$totaldata= WarningLetterRequest::whereRaw($whereraw)->whereIn("emp_id",$finalemp1)->where('warning_letter_status', 1)
			->where('final_status', 1)->groupBy('emp_id')->orderBy("id","DESC")->get();
		...
		else
		{
			...
			$totaldata= WarningLetterRequest::whereRaw($whererawdefault)->whereIn("emp_id",$finalemp1)->where('warning_letter_status', 1)
			->where('final_status', 1)->groupBy('emp_id')->orderBy("id","DESC")->get();	
    ...
    {
        return view('components.MyTeam.warningletterMyTeam');
    }
