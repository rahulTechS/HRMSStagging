...
class EmployeeWarning extends Component
{
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if($whereraw != '')
		{
			...
			// print_r($whereraw);
			// exit;
			$totaldata= WarningLetterRequest::whereRaw($whereraw)->where('warning_letter_status', 1)
		...
		else
		{
			...
			$totaldata= WarningLetterRequest::whereRaw($whererawdefault)->where('warning_letter_status', 1)
			->where('final_status', 1)->groupBy('emp_id')->orderBy("id","DESC")->get();	
    ...
    {
        return view('components.Warning.warningletter');
    }
...
public static function getTeamListsSelectedName($dept){
	...
	{
		//$empid='11100000';
		$approvedAmt = MasterPayout::where('employee_id',$empid)->orderBy('id', 'desc')->take(3)->get();


		if (count($approvedAmt) === 0) {
			return "Empty";
	   }
	   else{
		return "Non Empty";
