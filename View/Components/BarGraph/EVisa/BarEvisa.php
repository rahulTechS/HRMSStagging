...
namespace App\View\Components\BarGraph\EVisa;
...
use Session;
class BarEvisa extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $widgetData = WidgetBarEvisa::where("widget_id",$widgetId)->first();
	   ...
	   if(Request::session()->get('widgetFilterHiring['.$widgetId.'][job_opening]') != '' && Request::session()->get('widgetFilterHiring['.$widgetId.'][job_opening]') != NULL)	
		...
		foreach($jobOpeningArray as $_opening)
		{
			...
			$graphArray[$index]['Total'] = $this->getMOLTyped($_opening,$widgetId);
		...
		if(Request::session()->get('widgetFiltermolDept['.$widgetId.']') != '' && Request::session()->get('widgetFiltermolDept['.$widgetId.']') != NULL)	
		{
			$this->DepartmentSelected = explode(",",Request::session()->get('widgetFiltermolDept['.$widgetId.']'));
		}
		else
		{
    ...
    {
        return view('components.bargraph.bar_evisa');
    }
	...
	Private function getMOLTyped($jobId,$widgetId)
	{
		...
		if($whereraw != '')
		{
		return DocumentCollectionDetails::where("evisa_start_date","!=",NULL)->where("job_opening",$jobId)->where("backout_status",1)->where("visa_stage_steps","Stage2")->whereRaw($whereraw)->get()->count();
		}
			
		else
		{
		return DocumentCollectionDetails::where("evisa_start_date","!=",NULL)->whereRaw($whereraw)->where("job_opening",$jobId)->where("backout_status",1)->where("visa_stage_steps","Stage2")->get()->count();	
