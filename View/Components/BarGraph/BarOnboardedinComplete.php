	...
	Private static function getOnboarded($jobId,$widgetId)
	{
		...
		if($whereraw  != '')
		{
			if(Request::session()->get('widgetFilterHiring['.$widgetId.'][recruiterId]') != '' && Request::session()->get('widgetFilterHiring['.$widgetId.'][recruiterId]') != NULL)
			{
				...
				return DocumentCollectionDetails::where("job_opening",$jobId)->whereIn("recruiter_name",$recruiterIdsArray)->where("onboard_status",1)->whereRaw($whereraw)->get()->count();
			}
			else
			{
		...
		else
		{
			if(Request::session()->get('widgetFilterHiring['.$widgetId.'][recruiterId]') != '' && Request::session()->get('widgetFilterHiring['.$widgetId.'][recruiterId]') != NULL)
			{
				...
				$recruiterIdsArray = explode(",",$recruiterIds);
				return DocumentCollectionDetails::where("job_opening",$jobId)->whereIn("recruiter_name",$recruiterIdsArray)->where("onboard_status",1)->get()->count();
			}
			else
			{
