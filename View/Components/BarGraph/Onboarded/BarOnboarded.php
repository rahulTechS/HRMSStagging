...
class BarOnboarded extends Component
{
	...
	Private static function getOnboarded($jobId,$widgetId)
	{
		...
		if($datatype != NULL && $datatype != '')
		{
			...
			if($whereraw == '')
			{
				$whereraw = "onboard_date >= '".$fromDate."' and onboard_date <= '".$toDate."'";
			}
			else
			{
				$whereraw .= " And onboard_date >= '".$fromDate."' and onboard_date <= '".$toDate."'";
			}
		}
		else{
			$toDate = date("Y-m-d");
		...
		if($whereraw  != '')
		{			
			return DocumentCollectionDetails::where("job_opening",$jobId)->where("onboard_status",2)->whereRaw($whereraw)->get()->count();
