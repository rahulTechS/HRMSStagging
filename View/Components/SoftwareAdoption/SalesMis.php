...
class SalesMis extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		{
			//echo "h1";
		$totaldata= DepartmentFormEntry::select("team")->where("form_id",1)->whereRaw($whereraw)->whereNull("customer_mobile")->groupBy('team')->get();
		}
		...
		$datatype1 = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
					...
					{
						//echo "hello";
			$totaldata= DepartmentFormEntry::select("team")->where("form_id",1)->whereNull("customer_mobile")->groupBy('team')->get();	
					}
					else{
						$totaldata= DepartmentFormEntry::select("team")->whereRaw($whereraw)->where("form_id",1)->whereNull("customer_mobile")->groupBy('team')->get();	
					}
		}
		$graphdata= $totaldata;
    ...
    {
        return view('components.SoftwareAdoption.salesmis');
    }
	...
	$whereraw = '';
		...
		else
		{
			...
			return $totaldata= DepartmentFormEntry::whereIn("team",$teamarray)->where("form_id",1)->whereNull("customer_mobile")->get()->count();	
					}
					else{
	...
	$whereraw = '';
		...
		else
		{
			$datatype1 = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
					if($datatype1 == 'MAX')
					{
						return $totaldata= DepartmentFormEntry::where("team",$team)->where("form_id",1)->whereNull("customer_mobile")->get()->count();	
...
Private function getTeamdataGraph4to6($team,$widgetId)
	...
	public static function getTeamListsSelectedNameMis($TeamListsSelected){
		//$departmentName = Department::where("id",$data->department)->first()->department_name;
		...
		foreach($TeamListsSelected as $r)
		{
			...
			if($name == '')
			{
				$fname = DepartmentFormEntry::where("team",$r)->first();
				if($fname!=''){
				$name=	$fname->team;
				}
			}
	...
	public static function getTotalmissingmobilenoSPTotal($team,$widgetId){
	$whereraw = '';
		$datatype = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
		if($datatype != NULL && $datatype != '')
		{
		...
		{
			//echo "h1";
		return $totaldata= DepartmentFormEntry::whereIn("team",$teamarray)->where("form_id",1)->whereRaw($whereraw)->get()->count();
		}
		...
		else
		{
			$datatype1 = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
					...
					return $totaldata= DepartmentFormEntry::whereIn("team",$teamarray)->where("form_id",1)->get()->count();	
					}
					else{
						return $totaldata= DepartmentFormEntry::whereIn("team",$teamarray)->whereRaw($whereraw)->where("form_id",1)->get()->count();	
					}
		

		}		
	}
	public static function getTotalmissingmobilenoTotal($team,$widgetId){
	$whereraw = '';
		$datatype = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
		if($datatype != NULL && $datatype != '')
		{
		...
		{
			//echo "h1";
		return $totaldata= DepartmentFormEntry::where("team",$team)->where("form_id",1)->whereRaw($whereraw)->get()->count();
		}
		...
		else
		{
			$datatype1 = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
					if($datatype1 == 'MAX')
					{
						return $totaldata= DepartmentFormEntry::where("team",$team)->where("form_id",1)->get()->count();	
	
					}
					else{
						return $totaldata= DepartmentFormEntry::where("team",$team)->whereRaw($whereraw)->where("form_id",1)->get()->count();	
