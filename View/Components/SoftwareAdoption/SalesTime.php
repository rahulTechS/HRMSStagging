...
class SalesTime extends Component
{
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		{
			//echo "h1";
		$totaldata= MasterPayout::select("bank_name")->whereRaw($whereraw)->groupBy('bank_name')->get();
		}
			
		else
		{
			//echo "h2";
		$totaldata= MasterPayout::select("bank_name")->groupBy('bank_name')->get();	
	  ...
	  //print_r(json_encode($finalvalue));exit;
		...
		$TeamLists = MasterPayout::select("bank_name")->groupBy('bank_name')->get();
    ...
    {
        return view('components.SoftwareAdoption.salestime');
    }
	...
	public static function getsalesTime($bank,$widgetId){
	$total= MasterPayout::where("bank_name",$bank)->orderBy("sort_order","DESC")->first();
	
	
		echo $total->sales_time;
...
Private function getTeamdataGraph4to6($team,$widgetId)
	...
	public static function getTeamListsSelectedNameTime($TeamListsSelected){
		...
		foreach($TeamListsSelected as $r)
		{
			...
			if($name == '')
			{
				$fname = MasterPayout::where("bank_name",$r)->first();
				if($fname!=''){
				$name=	$fname->bank_name;
				}
			}
			else
			{
				$name = $name.','.DepartmentFormEntry::where("bank_name",$r)->first()->bank_name;
