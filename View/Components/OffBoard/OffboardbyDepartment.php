...
class OffboardbyDepartment extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		{
			//echo "h1";
		$totaldata= EmpOffline::select("department")->whereRaw($whereraw)->groupBy('department')->get();
		}
			
		else
		{
			//echo "h2";
		$totaldata= EmpOffline::select("department")->groupBy('department')->get();	
    ...
    {
        return view('components.OffBoard.offboardbydepartment');
    }
	Private function getJobOpeningName($jobId)
	{
		...
		public static function getTotalDataLocation($dept,$widgetId)
	{
		$whereraw = '';
		$datatype = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
		if($datatype != NULL && $datatype != '')
		...
		{
			//echo "h2";
			return EmpOffline::select("id")->where("department",$dept)->get()->count();
