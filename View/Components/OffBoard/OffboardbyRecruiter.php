...
namespace App\View\Components\Offboard;
...
class OffBoardByRecruiter extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		{
			//echo "h1";
		$totaldata= EmpOffline::select("recruiter_name")->whereRaw($whereraw)->groupBy('recruiter_name')->get();
		}
			
		else
		{
			//echo "h2";
		$totaldata= EmpOffline::select("recruiter_name")->groupBy('recruiter_name')->get();	
    ...
    {
        return view('components.OffBoard.offboardbyrecruiter');
    }
	Private function getJobOpeningName($jobId)
	{
		...
		public static function getTotalDataRecruiter($dept,$widgetId)
	{
		$whereraw = '';
		$datatype = Request::session()->get('widgetFilterHiring['.$widgetId.'][data_type]');
		if($datatype != NULL && $datatype != '')
		...
		{
			//echo "h1";
		return EmpOffline::select("id")->whereRaw($whereraw)->where("recruiter_name",$dept)->get()->count();
		}
			
		else
		{
			//echo "h2";
			return EmpOffline::select("id")->where("recruiter_name",$dept)->get()->count();
...
public static function getTeamListsSelectedName($dept){
	...
	public static function getRecruiter($rec){
		
		$name = RecruiterDetails::where("id",$rec)->first();
		if($name!=""){
