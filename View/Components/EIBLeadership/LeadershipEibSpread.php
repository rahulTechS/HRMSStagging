...
namespace App\View\Components\EIBLeadership;
...
class LeadershipEibSpread extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		{
		//echo "h1";
		// $totaldata= EibBankMis::select("tl_name")->where("form_id",2)->whereRaw($whereraw)->where('tl_name','!=','#N/A')->groupBy('tl_name')->get();
		...
		{
		//$totaldata= EibBankMis::select("tl_name")->where("form_id",2)->where('tl_name','!=','#N/A')->groupBy('tl_name')->get();	
		$totaldata= EibBankMis::groupBy('tl_name')->selectRaw('count(*) as total, tl_name')->where("matched_status",1)->where("final_decision","Approve")->get();
...
//print_r($TeamLists);exit;
    ...
    {
        return view('components.EIBLeadership.Leadershipeibspread');
    }
...
public static function getTeamListsSelectedName($dept){
		...
		foreach($dept as $r)
		{
			if($name == '')
			{
				$namedata = EIBDepartmentFormEntry::where("tl_name",$r)->first();
				if($namedata!=''){
					$name=$namedata->tl_name;
