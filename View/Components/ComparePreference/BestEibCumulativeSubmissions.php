...
class BestEibCumulativeSubmissions extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		$TeamLists = EIBDepartmentFormEntry::groupBy('tl_name')->selectRaw('count(*) as total, tl_name')->whereNotNull('tl_name')->get();
	   ...
	   $this->recruiterCategory = $recruiterCategory;
		...
		$this->salestime=MasterPayout::select('sales_time')->where("dept_id",52)->orderBy("sort_order","DESC")->get()->unique('sales_time');
    ...
    {
        return view('components.ComparePreference.eibbestcumulativesubmissionslastmonth');
    }
		...
		if(Request::session()->get('widgetFilterBYSubmissions['.$widgetId.']') != '' && Request::session()->get('widgetFilterBYSubmissions['.$widgetId.']')=="Submissions" )
			{
				if($whereraw != '')
				{
				return $totaldata= EIBDepartmentFormEntry::where("emp_id",$emp_id)->whereRaw($whereraw)->get()->count();
				...
				if($whereraw != '')
				{
				return $totaldata= EibBankMis::where("emp_id",$emp_id)->whereRaw($whereraw)->where('final_decision','Approve')->get()->count();
