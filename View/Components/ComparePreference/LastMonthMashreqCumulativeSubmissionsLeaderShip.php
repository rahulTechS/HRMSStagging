...
class LastMonthMashreqCumulativeSubmissionsLeaderShip extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		$design=Designation::where("tlsm",2)->where("department_id",36)->where("status",1)->get();
				...
				if($whereraw != '')
				{
				$empdetailsdata = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereIn("designation_by_doc_collection",$designarray)->where("dept_id",36)->where("offline_status",1)->whereNotIn('emp_id', [102482])->get();
				...
				{
					$empdetailsdata = Employee_details::orderBy("id","DESC")->whereIn("designation_by_doc_collection",$designarray)->where("dept_id",36)->where("offline_status",1)->whereNotIn('emp_id', [102482])->get();
    ...
    {
        return view('components.ComparePreference.mashreqcumulativesubmissionslastmonthleadership');
    }
