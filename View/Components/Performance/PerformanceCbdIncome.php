...
class PerformanceCbdIncome extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		//echo $whereraw;
			...
			{
				//echo "h1";
			$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary")->where("application_id","!=",NULL)->where("form_id",1)->whereRaw($whereraw)->get();
			}
				
			else
			{
				//echo "h2";
			$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary")->where("application_id","!=",NULL)->whereRaw($whereraw)->where("form_id",2)->get();	
				...
				{
					//echo "h1";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary")->where("application_id","!=",NULL)->where("form_id",2)->whereIn("form_status",array("Missing(Approved)","Welcome Calling,Archive on Approval","Approved","Pending with Onboarder","Pending with COC"))->whereRaw($whereraw)->get();
				}
					
				else
				{
					//echo "h2";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary")->where("application_id","!=",NULL)->whereRaw($whereraw)->where("form_id",2)->whereIn("form_status",array("Missing(Approved)","Welcome Calling,Archive on Approval","Approved","Pending with Onboarder","Pending with COC"))->get();	
	   ...
	   $range=array();
		...
		foreach($totaldata as $_totaldata)
		{
			//echo $_totaldata->application_id;//exit;
				$rangesal=$_totaldata->cdafinalsalary;
			...
			//print_r($rangesal);exit;
				...
				if($sal>=0 && $sal<=7000){
					//echo $sal."r1";
				$salary5to7total=$salary5to7total+1;	
				}
				else if($sal>=7001 && $sal<=10000){
					//echo $sal."r2";
				$salary7to10total=$salary7to10total+1;	
				}
				else if($sal>=10001 && $sal<=15000){
					//echo $sal."r3";
				$salary10to15total=$salary10to15total+1;	
				}
				else if($sal>15001 && $sal<=25000){
					//echo $sal."r4";
				$salary15to25total=$salary15to25total+1;	
				}
				else if($sal>25001 && $sal<=50000){
					//echo $sal."r4";
				$salary25to50total=$salary25to50total+1;	
				}
				else if($sal>50000){
					//echo $sal."r4";
				$salary50plustotal=$salary50plustotal+1;	
				}
				else{
		...
		if($countd>=1){
			...
			$salary5to7totalfinal=round(($salary5to7total/$totaldata)*100);
			$salary7to10totalfinal=round(($salary7to10total/$totaldata)*100);
			$salary10to15totalfinal=round(($salary10to15total/$totaldata)*100);
			$salary15to25totalfinal=round(($salary15to25total/$totaldata)*100);
			$salary25to50totalfinal=round(($salary25to50total/$totaldata)*100);
			$salary50plustotalfinal=round(($salary50plustotal/$totaldata)*100);
			//$graphArray=array();
			$graphArray = array(
						array(
							"range" => "5-7k (".$salary5to7totalfinal."%)",
							"salery" => $salary5to7totalfinal
						),
						array(
							"range" => "7-10k (".$salary7to10totalfinal."%)",
							"salery" => $salary7to10totalfinal
						),
						array(
							"range" => "10-15k (".$salary10to15totalfinal."%)",
							"salery" => $salary10to15totalfinal
						)
						,
						
						array(
							"range" => "15k-25k (".$salary15to25totalfinal."%)",
							"salery" => $salary15to25totalfinal
						)
						,
						array(
							"range" => "25-50k (".$salary25to50totalfinal."%)",
							"salery" => $salary25to50totalfinal
						)
						,
						array(
							"range" => "50k+ (".$salary50plustotalfinal."%)",
							"salery" => $salary50plustotalfinal
						)
					);
    ...
    {
        return view('components.Performance.performancemashreqincome');
    }
...
public static function getTeamListsSelectedName($dept){
		...
		foreach($dept as $r)
		{
			if($name == '')
			{
				$data=DepartmentFormEntry::where("team",$r)->first();
				if($data!=''){
				$name = $data->team;
				}
			}
			...
			{
				$abc=DepartmentFormEntry::where("team",$r)->first();
				if($abc!=''){
				$nnamed = $abc->team;
				$name = $name.','.$nnamed;				
				}
