...
class PerformanceMashreqCumulativeBookings extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if($whereraw != '')
		{
		$totaldata= MashreqBookingMIS::groupBy('team')->selectRaw('count(*) as total, team')->where("instanceid","!=",NULL)->whereRaw($whereraw)->get();
    ...
    {
        return view('components.Performance.performancemashreqcumulativebookings');
    }
...
Private function getTeamdata($team,$widgetId)
	{
		...
		if($whereraw != '')
		{
		$totaldata= MashreqBookingMIS::groupBy('dateofdisbursal')->selectRaw('count(*) as total, dateofdisbursal')->where("instanceid","!=",NULL)->whereRaw($whereraw)->where("team",$team)->get();
