...
class PerformanceMashreqStpPecentages extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		{
			//echo "h1";
		$totaldata= DepartmentFormEntry::select("team")->where("application_id","!=",NULL)->where("form_id",1)->whereRaw($whereraw)->groupBy('team')->get();
		}
		...
		$STPDisbursedarray=array();
		foreach($totaldata as $_totaldata)
		{			
			
			$bookingcount = DepartmentFormEntry::select("department_form_parent_entry.id","department_form_parent_entry.application_id","department_form_parent_entry.submission_date")
					->join("mashreq_booking_mis","mashreq_booking_mis.instanceid","=","department_form_parent_entry.application_id")
					->where("department_form_parent_entry.form_id",1)
					->where("department_form_parent_entry.application_id","!=",NULL)->where("department_form_parent_entry.team",$_totaldata->team)->get()->count();			
			$STPDisbursedcount = DepartmentFormEntry::select("department_form_parent_entry.id","department_form_parent_entry.application_id","department_form_parent_entry.submission_date")
					->join("mashreq_login_data","mashreq_login_data.applicationid","=","department_form_parent_entry.application_id")
					->where("department_form_parent_entry.form_id",1)->where("mashreq_login_data.application_status","STP Disbursed")
					->where("department_form_parent_entry.application_id","!=",NULL)->where("department_form_parent_entry.team",$_totaldata->team)->get()->count();
		if($bookingcount>0){
		...
		$Finaldata=round(($STPDisbursedcount/$bookingcount)*100);
		$STPDisbursedarray[$_totaldata->team]=$Finaldata;
		}
		//print_r($STPDisbursedarray);exit;
		$graphdata= $STPDisbursedarray;
    ...
    {
        return view('components.Performance.performancemashreqstppecentages');
    }
