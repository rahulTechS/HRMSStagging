...
class PerformanceCbdCumulativeSubmissions extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if($whereraw != '')
		{
		$totaldata= DepartmentFormEntry::groupBy('team')->selectRaw('count(*) as total, team')->where("form_id",2)->whereRaw($whereraw)->get();
   ...
   $finaldata= json_encode($finalarray);
		...
		if($whereraw != '')
		{
		$totaldata= DepartmentFormEntry::groupBy('application_date')->selectRaw('count(*) as total, application_date')->where("form_id",2)->whereRaw($whereraw)->get();
  ...
  $finaldata= json_encode($finalarray);
    ...
    {
        return view('components.Performance.performancecbdcumulativesubmissions');
    }
...
Private function getTeamdata($team,$widgetId)
	{
		...
		if($whereraw != '')
		{
		$totaldata= DepartmentFormEntry::groupBy('application_date')->selectRaw('count(*) as total, application_date')->where("form_id",2)->whereRaw($whereraw)->where("team",$team)->get();
