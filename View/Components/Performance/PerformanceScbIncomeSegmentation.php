...
class PerformanceScbIncomeSegmentation extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $range=array();
		...
		foreach($totaldata as $_totaldata)
		{
			...
			{
				$status=array('Approved','Welcome Calling','Archive on Approval','Pending with Onboarder','Pending with COC');
			$rangesal=CBDBankMis::select("declared_salary")->where("ref_no",$_totaldata->ref_no)->whereIn('Status',$status)->first();
			//print_r($rangesal);exit;
			if($rangesal!=''){
				$sal=$rangesal->declared_salary;
				if($sal<=7000){
					//echo $sal."r1";
			...
			$rangesal=CBDBankMis::select("declared_salary")->where("ref_no",$_totaldata->ref_no)->first();
			//print_r($rangesal);exit;
			if($rangesal!=''){
				$sal=$rangesal->declared_salary;
				if($sal<=7000){
    ...
    {
        return view('components.Performance.performancescbincomesegmentation');
    }
