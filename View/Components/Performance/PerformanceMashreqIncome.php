...
class PerformanceMashreqIncome extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		//echo $whereraw;
			...
			{
				//echo "h2";
			$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary")->where("application_id","!=",NULL)->whereRaw($whereraw)->where("form_id",1)->get();	
				...
				{
					//echo "h1";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary")->where("application_id","!=",NULL)->where("form_id",1)->where("form_status","Booked")->whereRaw($whereraw)->get();
				}
					
				else
				{
					//echo "h2";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary")->where("application_id","!=",NULL)->whereRaw($whereraw)->where("form_id",1)->where("form_status","Booked")->get();	
