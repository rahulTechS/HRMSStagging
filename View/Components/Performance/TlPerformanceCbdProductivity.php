...
class TlPerformanceCbdProductivity extends Component
{
    /**
    ...
    {
        return view('components.Performance.TLperformancecbdproductivity');
    }
...
//$totalreturndata=$count;
	...
	public static function getTotalProductivity($team,$widgetId,$fromDate,$toDate)
	{
		...
		//echo $whereraw."r";exit;
			...
			//echo $salestime;exit;
			$totalmasterdata=MasterPayout::whereRaw($whereraw)->where("sales_time",$salestime)->where("dept_id",49)->where("tl_name",$team)->get();
			if(count($totalmasterdata)>0){
			$totalemp=count($totalmasterdata);
			$countdata=MasterPayout::whereRaw($whereraw)->where("sales_time",$salestime)->where("dept_id",49)->where("tl_name",$team)->sum('card_cbd');
				if($countdata>0){
			...
			$totalempdata= DepartmentFormEntry::where("ref_no","!=",NULL)->where("form_id",2)->whereRaw($whererawmis)->where("team",$team)->get();
			$totalemp =0;
			...
			foreach($totalempdata as $_totalempdata){
				...
				$totalemp = Employee_details::where("tl_id",$employeeExist->tl_id)->where("dept_id",49)->count();
				break;
				}
				else{
					continue;
			...
			$totaldata= DepartmentFormEntry::select("ref_no")->where("ref_no","!=",NULL)->where("form_id",2)->whereRaw($whererawmis)->where("team",$team)->get();
		...
		$totalbooking=0;
		 ...
		 $countdata=CBDBankMis::whereIn("ref_no",$finalarray)->count();
