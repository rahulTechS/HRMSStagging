...
class PerformanceMashreqPlasticDistrbution extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if(Request::session()->get('widgetFilterBYSubmissions['.$widgetId.']') != '' && Request::session()->get('widgetFilterBYSubmissions['.$widgetId.']')=="Submissions" )
		{
			...
			{
				//echo "h1";
			$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type")->where("application_id","!=",NULL)->where("form_id",1)->whereRaw($whereraw)->get();
			}
				
			else
			{
				//echo "h2";
			$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type")->where("application_id","!=",NULL)->whereRaw($whereraw)->where("form_id",1)->get();	
			...
			{
				//echo "h1";
			$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type")->where("application_id","!=",NULL)->where("form_status","Booked")->where("form_id",1)->whereRaw($whereraw)->get();
			}
				
			else
			{
				//echo "h2";
			$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type")->where("application_id","!=",NULL)->where("form_status","Booked")->whereRaw($whereraw)->where("form_id",1)->get();	
	   ...
	   $range=array();
		...
		foreach($totaldata as $_totaldata)
		{
				...
				else if($cardtype=='Cash Back Platinum'){
					//echo $sal."r2";
				$CashBackPlatinum=$CashBackPlatinum+1;	
				}
				else if($cardtype=='Visa Smart Saver Platinum'){
					//echo $sal."r3";
				$VisaSmartSaverPlatinum=$VisaSmartSaverPlatinum+1;	
				}
				else if($cardtype=='Infinite'){
					//echo $sal."r4";
				$Infinite=$Infinite+1;	
				}
				else if($cardtype=='MAI Platinum Elite'){
					//echo $sal."r4";
				$MAIPlatinumElite=$MAIPlatinumElite+1;	
				}
				else if($cardtype=='Mashreq noon Yellow'){
					//echo $sal."r4";
				$MashreqnoonYellow=$MashreqnoonYellow+1;	
				}
				else if($cardtype=='Approve'){
					//echo $sal."r4";
				$Approve=$Approve+1;	
				}
				else if($cardtype=='CARD_TYPE'){
					//echo $sal."r4";
		...
		if($countd>=1){
			...
			$Platinumfinal=round(($Platinum/$totaldata)*100);
			$CashBackPlatinumfinal=round(($CashBackPlatinum/$totaldata)*100);
			$VisaSmartSaverPlatinumfinal=round(($VisaSmartSaverPlatinum/$totaldata)*100);
			$Infinitefinal=round(($Infinite/$totaldata)*100);
			$MAIPlatinumElitefinal=round(($MAIPlatinumElite/$totaldata)*100);
			$MashreqnoonYellowfinal=round(($MashreqnoonYellow/$totaldata)*100);
			$Approvefinal=round(($Approve/$totaldata)*100);
			$CARD_TYPEfinal=round(($CARD_TYPE/$totaldata)*100);
			//$graphArray=array();
			$graphArray = array(
						array(
							"range" => "Platinum (".$Platinumfinal."%)",
							"salery" => $Platinumfinal
						),
						array(
							"range" => "Cash Back Platinum (".$CashBackPlatinumfinal."%)",
							"salery" => $CashBackPlatinumfinal
						),
						array(
							"range" => "Visa Smart Saver Platinum (".$VisaSmartSaverPlatinumfinal."%)",
							"salery" => $VisaSmartSaverPlatinumfinal
						)
						,
						array(
							"range" => "Infinite (".$Infinitefinal."%)",
							"salery" => $Infinitefinal
						)
						,
						array(
							"range" => "MAI Platinum Elite (".$MAIPlatinumElitefinal."%)",
							"salery" => $MAIPlatinumElitefinal
						)
						,
						array(
							"range" => "Mashreq noon Yellow (".$MashreqnoonYellowfinal."%)",
							"salery" => $MashreqnoonYellowfinal
						)
						,
						array(
							"range" => "Approve (".$Approvefinal."%)",
							"salery" => $Approvefinal
						)
						,
						array(
							"range" => "CARD TYPE (".$CARD_TYPEfinal."%)",
							"salery" => $CARD_TYPEfinal
						)
					);
    ...
    {
        return view('components.Performance.performancemashreqplasticdistrbution');
    }
