...
class PerformanceMashreqBureauSegmentation extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
		...
		if(Request::session()->get('widgetFilterBYSubmissions['.$widgetId.']') != '' && Request::session()->get('widgetFilterBYSubmissions['.$widgetId.']')=="Submissions" )
			{
				...
				{
					//echo "h1";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary","bureau_segmentation")->where("application_id","!=",NULL)->where("form_id",1)->whereRaw($whereraw)->get();
				}
					
				else
				{
					//echo "h2";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary","bureau_segmentation")->where("application_id","!=",NULL)->whereRaw($whereraw)->where("form_id",1)->get();	
				...
				{
					//echo "h1";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary","bureau_segmentation")->where("application_id","!=",NULL)->where("form_id",1)->where("form_status","Booked")->whereRaw($whereraw)->get();
				}
					
				else
				{
					//echo "h2";
				$totaldata= DepartmentFormEntry::select("application_id","form_status","card_type","cdafinalsalary","bureau_segmentation")->where("application_id","!=",NULL)->whereRaw($whereraw)->where("form_id",1)->where("form_status","Booked")->get();	
	   ...
	   $range=array();
		...
		$RICH=0;
		$CashBackPlatinum=0;
		$BUREAU_SEGMENTATION=0;
		$bookingdata='';
		foreach($totaldata as $_totaldata)
		{
			
			//$rangesal=MashreqLoginMIS::select("bureau_segmentation")->where("applicationid",$_totaldata->application_id)->first();
			$rangesal=$_totaldata->bureau_segmentation;
			
				$bureausegmentation=$rangesal;
				if($bureausegmentation=='POOR'){
					//echo $sal."r1";
				$POOR=$POOR+1;	
				}
				else if($bureausegmentation=='THIN1'){
					//echo $sal."r2";
				$THIN1=$THIN1+1;	
				}
				else if($bureausegmentation=='NO HIT'){
					//echo $sal."r3";
				$NOHIT=$NOHIT+1;	
				}
				else if($bureausegmentation=='THIN2'){
					//echo $sal."r4";
				$THIN2=$THIN2+1;	
				}
				else if($bureausegmentation=='RICH'){
					//echo $sal."r4";
				$RICH=$RICH+1;	
				}
				else if($bureausegmentation=='Cash Back Platinum'){
					//echo $sal."r4";
				$CashBackPlatinum=$CashBackPlatinum+1;	
				}
				else if($bureausegmentation=='BUREAU_SEGMENTATION'){
					//echo $sal."r4";
				$BUREAU_SEGMENTATION=$BUREAU_SEGMENTATION+1;	
				}
				else{
		...
		if($countd>=1){
			...
			$POORfinal=round(($POOR/$totaldata)*100);
			$THIN1final=round(($THIN1/$totaldata)*100);
			$NOHITfinal=round(($NOHIT/$totaldata)*100);
			$THIN2final=round(($THIN2/$totaldata)*100);
			$RICHfinal=round(($RICH/$totaldata)*100);
			$CashBackPlatinumfinal=round(($CashBackPlatinum/$totaldata)*100);
			$BUREAU_SEGMENTATIONfinal=round(($BUREAU_SEGMENTATION/$totaldata)*100);
			$graphArray = array(
						array(
							"bureau_segmentation" => "POOR (".$POORfinal."%)",
							"bureau_total" => $POORfinal
						),
						array(
							"bureau_segmentation" => "THIN1 (".$THIN1final."%)",
							"bureau_total" => $THIN1final
						),
						array(
							"bureau_segmentation" => "NO HIT (".$NOHITfinal."%)",
							"bureau_total" => $NOHITfinal
						)
						,
						array(
							"bureau_segmentation" => "THIN2 (".$THIN2final."%)",
							"bureau_total" => $THIN2final
						)
						,
						array(
							"bureau_segmentation" => "RICH (".$RICHfinal."%)",
							"bureau_total" => $RICHfinal
						)
						,
						array(
							"bureau_segmentation" => "BUREAU SEGMENTATION (".$BUREAU_SEGMENTATIONfinal."%)",
							"bureau_total" => $BUREAU_SEGMENTATIONfinal
						)
						,
						array(
							"bureau_segmentation" => "Cash Back Platinum (".$CashBackPlatinumfinal.")",
							"bureau_total" => $CashBackPlatinumfinal
						)
    ...
    {
        return view('components.Performance.performancemashreqbureausegmentation');
    }
