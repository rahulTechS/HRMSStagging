...
class PerformanceScbCumulativeBookings extends Component
{
    /**
	...
	$finaldata= json_encode($finalweekarray);//exit;
		...
		if($whereraw != '')
		{
		$totaldata= SCBDepartmentFormParentEntry::groupBy('approved_date')->selectRaw('count(*) as total, approved_date')->whereIn("form_status",array("Approved"))->whereRaw($whereraw)->get();
		...
		foreach($totaldata as $_totaldata)
		{
			/*$date=explode("-",$_totaldata->approved_date);
			$month=date('M', strtotime($_totaldata->approved_date));//exit;
			...
			*/
			$date=explode("-",$_totaldata->approved_date);
			$datezero=date('m-j', strtotime($_totaldata->approved_date));
			$monthlevel=date('M', strtotime($_totaldata->approved_date));//exit;
			$month=date('Y-m', strtotime($_totaldata->approved_date));
			//$bookingAsDateCompime[$month]=0;
			if(isset($bookingAsDateCompime[$month]))
				{
				$bookingAsDateCompime[$month]  = $bookingAsDateCompime[$month]+$_totaldata->total;
	...
	$finaldata= json_encode($finalarray);//exit;
    ...
    {
        return view('components.Performance.performancescbcumulativebookings');
    }
...
Private function getTeamdata($team,$widgetId)
	{
		...
		if($whereraw != '')
		{
		$totaldata= SCBDepartmentFormParentEntry::groupBy('approved_date')->selectRaw('count(*) as total, approved_date')->whereIn("form_status",array("Approved"))->whereRaw($whereraw)->where("team",$team)->get();
