...
class PerformanceEibIncome extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		//echo $whereraw;
			...
			{
				//echo "h1";
			$totaldata= EIBDepartmentFormEntry::select("application_no","salary")->whereRaw($whereraw)->get();
			}
			...
			{
				//echo "h1";
			$totaldataval= EIBDepartmentFormEntry::select("application_no","salary")->whereRaw($whereraw)->get();
			}
			...
			$totaldata= EIBDepartmentFormEntry::select("application_no","salary")->whereIn("application_no",$finalarray)->get();
			}
...
//		print_r($totaldata);//exit;
		foreach($totaldata as $_totaldata)
		{
			//echo $_totaldata->application_id;//exit;
				 $rangesal=str_replace(",","",$_totaldata->salary);//exit;
    ...
    {
        return view('components.EIB.performanceeibincome');
    }
