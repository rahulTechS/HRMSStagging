...
class StampingDeadline extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		if($whereraw != '')
				{
					//echo "hello";exit;
					$documentCollectiondetaildata = DocumentCollectionDetails::select("id")->whereIn("visa_process_status",array(0,1,2))->where("visa_documents_status",2)->where("offer_letter_onboarding_status",2)->where("backout_status",1)->where("sort_dateBY","Stamping Deadline")->whereRaw($whereraw)->orderByRaw("-sort_date DESC")->get();
				...
				{
					//echo "hello1";
					$documentCollectiondetaildata = DocumentCollectionDetails::select("id")->whereIn("visa_process_status",array(0,1,2))->where("visa_documents_status",2)->where("offer_letter_onboarding_status",2)->where("backout_status",1)->where("sort_dateBY","Stamping Deadline")->orderByRaw("-sort_date DESC")->get();
		...
		$finaldocidarray=array();
				foreach($documentCollectiondetaildata as $_documentCollectiondetails){
					$EMPdetailsdata =  Employee_details::select("offline_status")->where("document_collection_id",$_documentCollectiondetails->id)->first();
				...
				foreach($finaldeadlinearray as $_deadlnef){
					$val=DocumentCollectionDetails::select("current_visa_status")->where("id",$_deadlnef)->first();
				...
				$documentCollectiondetails= DocumentCollectionDetails::select("id")->whereIn("id",$finalarray)->orderByRaw("-sort_date DESC")->get();
    ...
    {
        return view('components.DeadLine.stampingdeadline');
    }
