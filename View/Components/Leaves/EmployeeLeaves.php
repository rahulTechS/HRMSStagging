...
namespace App\View\Components\Leaves;
...
class EmployeeLeaves extends Component
{
    ...
    public function __construct($widgetId)
    {
	   ...
	   $finalfromdate = '';
		...
		if($whereraw != '')
		{
			...
			$totaldata= RequestedLeaves::whereRaw($whereraw)->where('approved_reject_status',1)->where('final_status',1)->orderBy("id","DESC")->get();
		...
		else
		{
			...
			$totaldata= RequestedLeaves::where('approved_reject_status',1)->where('final_status',1)->orderBy("id","DESC")->get();	
		...
		{
			if($emp->updated_from_date==NULL && $emp->updated_to_date==NULL)
			{
				if($emp->from_date <= $toDate && $emp->to_date >= $fromDate)
				{
					$finalemp[]=$emp->id;
				}                       
			}
			...
			{
				if($emp->updated_from_date <= $toDate && $emp->updated_to_date >= $fromDate)
				{
					$finalemp[]=$emp->id;
				}   
    ...
    {
        return view('components.Leaves.empleaves');
    }
