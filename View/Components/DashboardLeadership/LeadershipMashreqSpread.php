...
class LeadershipMashreqSpread extends Component
{
    /**
    ...
    {
        return view('components.DashboardLeadership.Leadershipmashreqspread');
    }
	...
	public static function getNumberHeadcount($team,$widgetId){
		...
		if(Request::session()->get('widgetFiltermolRange['.$widgetId.']') != 'All' && Request::session()->get('widgetFiltermolRange['.$widgetId.']') != '')
		{
			...
			if($datatype == 'current_month' || $datatype == ''){				
				...
				if($whererawrange != '')
				{
				return	$empcountdata  = Employee_details::where('tl_id',$empdata->id)->where('offline_status',1)->whereRaw($whererawrange)->where('dept_id',36)->get()->count();
				}
				else{
				...
				return $totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->get()->count();
				}else{
				
				return $totalmastercard=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->get()->count();
				}
			}
		...
		if($datatype == 'current_month' || $datatype == ''){
		 ...
		 if($empdata!=""){
			 foreach($empdata as $_id){
				...
				return	$empcountdata  = Employee_details::whereIn('tl_id',$ids)->where('offline_status',1)->whereRaw($whererawrange)->where('dept_id',36)->get()->count();
			...
			if($whererawsales != '')
				...
				return $totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
				}else{
				
				return $totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
				}
		}
	...
	if($datatype == 'current_month' || $datatype == ''){
		...
		//echo $fromDate;exit;
			...
			$salestime=date("n-Y", strtotime($fromDate));
			//echo $salestime;exit;
			$totalmasterTL=MasterPayoutPre::whereRaw($whereraw1)->where("sales_time",$salestime)->where('dept_id',36)->where("TL",$team)->sum('tc');
			$totalmastercard=MasterPayoutPre::where("sales_time",$salestime)->where("TL",$team)->where('dept_id',36)->sum('tc');
...
//print_r($totaldata);exit;
		...
		return round(($totalcard/$count)*100,2);
			...
			if($whererawsales != '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->where("tc","=",0)->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::where("TL",$team)->where("tc","=",0)->where('dept_id',36)->get()->count();
			$totalmastercard=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->get()->count();
			}
...
//$totalreturndata=$count;
		...
		return round(($count/$totalcard)*100,2);
			...
			if($whererawsales != '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->whereBetween('tc', [1,3])->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->whereBetween('tc', [1,3])->get()->count();
			$totalmastercard=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
...
//$totalreturndata=$count;
	...
	public static function getTotalsubmission4to6($team,$widgetId)
	{
		...
		return round(($count/$totalcard)*100,2);
			...
			if($whererawsales!='')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->whereBetween('tc', [4, 6])->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->whereBetween('tc', [4, 6])->get()->count();
			$totalmastercard=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
		...
		public static function getTotalsubmission7to10($team,$widgetId)
	{
		...
		//echo $whereraw;
			if($whererawsales != '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->whereBetween('tc', [7, 10])->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->whereBetween('tc', [7, 10])->get()->count();
			$totalmastercard=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
	...
	public static function getTotalsubmission10plus($team,$widgetId)
	{
		...
		return round(($count/$totalcard)*100,2);
			...
			if($whererawsales != '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->where("tc",">",10)->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where("TL",$team)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->where("tc",">",10)->get()->count();
			$totalmastercard=MasterPayoutPre::where("TL",$team)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
...
//$totalreturndata=$count;
	...
	//$rangeidsdata=implode(',',$rangeids);
		...
		return round(($count/$totalcard)*100,2);
			...
			if($whererawsales!= '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->whereBetween('tc', [1, 3])->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->whereBetween('tc', [1, 3])->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
			}
...
//$totalreturndata=$count;
	...
	//$count;
		...
		return round(($count/$totalcard)*100,2);
			...
			if($whererawsales != '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->whereBetween('tc', [4, 6])->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::whereBetween('tc', [4, 6])->where('dept_id',36)->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
	...
	//$count;
		...
		return $count;
			...
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->whereBetween('tc', [4, 6])->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where('dept_id',36)->whereIn("TL",$teamdataval)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::whereBetween('tc', [4, 6])->where('dept_id',36)->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}
	...
	//$count;
		...
		return round(($count/$totalcard)*100,2);
			...
			if($whererawsales!= '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->whereBetween('tc', [7, 10])->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->where('dept_id',36)->whereIn("TL",$teamdataval)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->whereBetween('tc', [7, 10])->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
...
public static function getTotalsubmission10plusprocesser($processer,$widgetId)
	...
	//$count;
		...
		return round(($count/$totalcard)*100,2);
			...
			if($whererawsales!= '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->where("tc",">",10)->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->where("tc",">",10)->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
	...
	//$count;
		...
		return $count;
			...
			$totalmasterTL=MasterPayoutPre::whereIn("TL",$teamdataval)->where("tc",">",10)->where('dept_id',36)->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->get()->where('dept_id',36)->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
			}
...
public static function getTotalsubmissionzerocardprocesser($processer,$widgetId)
	...
	//$rangeidsdata=implode(',',$rangeids);
		...
		return round(($totalcard/$count)*100,2);
			...
			if($whererawsales!= '')
			{
			$totalmasterTL=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->where("tc","=",0)->get()->count();
			$totalmastercard=MasterPayoutPre::whereRaw($whererawsales)->whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}else{
			$totalmasterTL=MasterPayoutPre::whereIn("TL",$teamdataval)->where("tc","=",0)->where('dept_id',36)->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
	...
	//$rangeidsdata=implode(',',$rangeids);
		...
		return $totalcard;
			...
			$totalmasterTL=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->where("tc","=",0)->get()->count();
			$totalmastercard=MasterPayoutPre::whereIn("TL",$teamdataval)->where('dept_id',36)->get()->count();
			}
			if($totalmastercard>0){
				$totalmastercard=$totalmastercard;
