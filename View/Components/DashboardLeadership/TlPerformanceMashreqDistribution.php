...
class TlPerformanceMashreqDistribution extends Component
{
    /**
    ...
    public function __construct($widgetId)
    {
	   ...
	   $whereraw = '';
		...
		{
			//echo "h1";
		$totaldata= DepartmentFormEntry::select("team")->whereNull('vintage_status')->where("form_id",1)->whereRaw($whereraw)->groupBy('team')->get();
		}
    ...
    {
        return view('components.DashboardLeadership.TLperformancemashreqdistribution');
    }
...
//$totalreturndata=$count;
	...
	public static function getNumberHeadcount($team,$widgetId,$salestime){
...
//echo $salestime;
				...
				return $totalmastercard=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->sum('tc');
	...
	public static function getTenPercent($team,$widgetId,$salestime){
		
		     $totalmastercard=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->sum('tc');
		 
	      $total10card=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->get()->count(); 
		  
		  //3.4
		   
		   //$totalcountdata=($total10card/100)*10;
		   
		$totalcountdata=($total10card / 100)* 10;
		
		//3
			 
		$totalcountdataperctage=round($totalcountdata);
			
			$totalmastercardcalcuated=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->limit($totalcountdataperctage)->orderBy('tc', 'DESC')->get();
			
			$count=0;
			foreach($totalmastercardcalcuated as $countdatanew){
			$count=$count+$countdatanew->tc	;
			...
			$totalcountnewdataten=($count /$totalmastercard)* 100;
			
			
		$totalcountdataperctagetendata=round($totalcountnewdataten);
			
			 //$count/$totalmastercard;
			
			//print_r($totalmastercardcalcuated);exit;
		 
 return  $totalcountdataperctagetendata;
...
//echo $salestime;
			...
			//$total10card=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->sum('tc');
	...
	public static function getthirtyPercent($team,$widgetId,$salestime){
		...
		$totalcountdata=($total10card / 100)* 10;
			 ...
			 // $total30card=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->get()->count(); 
			  
		 // $total30card=$totalcountdata;
				
				
				 $totalcountdatathirty=($total10card / 100)* 30;
		
		
   $totalcountdataperctagethirty=round($totalcountdatathirty);
		//10	 
			 
				//$totalmastercardcalcuated=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->limit($totalcountdataperctagethirty)->offset($totalcountdataperctage)->orderBy('tc','DESC')->get();
			$totalmastercardcalcuated=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->limit($totalcountdataperctagethirty)->offset($totalcountdataperctage)->orderBy('tc','DESC')->get();
			...
			$totalcountnewdatathrity=($count /$totalmastercard)* 100;
			
			
			$totalcountdataperctagethirtydata=round($totalcountnewdatathrity);
			
			 //$count/$totalmastercard;
			
			//print_r($totalmastercardcalcuated);exit;
		 
 return  $totalcountdataperctagethirtydata;
	...
	public static function getsixtyPercent($team,$widgetId,$salestime){
	   ...
	   $totalcountdataperctagethirty=round($totalcountdatathirty);
		...
		//$totalcountdata=($total10card / 100)* 10;
		 ...
		 // $total30card=$totalcountdata;
				 ...
				 $totalcountdatasixty=($total10card / 100)* 60;
		
		
   $totalcountdataperctagesixty=round($totalcountdatasixty);
			 //20
			 
				//$totalmastercardcalcuated=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->limit($totalcountdataperctagethirty)->offset($totalcountdataperctage)->orderBy('tc','DESC')->get();
			$totalmastercardcalcuated=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->limit($totalcountdataperctagesixty)->offset($totalcountdataperctagethirty)->orderBy('tc','DESC')->get();
			...
			$totalcountnewdatasixty=($count /$totalmastercard)* 100;
			
			
			$totalcountdataperctagesixtydata=round($totalcountnewdatasixty);
			
			 //$count/$totalmastercard;
			
			//print_r($totalmastercardcalcuated);exit;
		 
 return  $totalcountdataperctagesixtydata;
	...
	public static function geteightfivePercent($team,$widgetId,$salestime){
	   ...
	   $totalcountdataperctagesixty=round($totalcountdatasixty);
		...
		//$totalcountdata=($total10card / 100)* 10;
		 ...
		 // $total30card=$totalcountdata;
				 ...
				 $totalcountdataeightfive=($total10card / 100)* 85;
		
		
	  $totalcountdataperctageeightfive=round($totalcountdataeightfive);
			 //29
			 
				//$totalmastercardcalcuated=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->limit($totalcountdataperctagethirty)->offset($totalcountdataperctage)->orderBy('tc','DESC')->get();
			$totalmastercardcalcuated=MasterPayoutPre::where("TL",$team)->where("sales_time",$salestime)->limit($totalcountdataperctageeightfive)->offset($totalcountdataperctagesixty)->orderBy('tc','DESC')->get();
			...
			$totalcountnewdataeightfive=($count /$totalmastercard)* 100;
			
			
			$totalcountdataperctageeightfivedata=round($totalcountnewdataeightfive);
			
			 //$count/$totalmastercard;
			
			//print_r($totalmastercardcalcuated);exit;
		 
 return  $totalcountdataperctageeightfivedata;
