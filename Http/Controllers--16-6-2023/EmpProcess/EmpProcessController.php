			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				if(!empty($request->session()->get('fname_emp_filter_inner_list')) && $request->session()->get('fname_emp_filter_inner_list') != 'All')
				{
					$fname = $request->session()->get('fname_emp_filter_inner_list');
					 ...
					 if($whereraw == '')
					{
						$whereraw = 'first_name like "%'.$fname.'%"';
					}
					else
					{
						$whereraw .= ' And first_name like "%'.$fname.'%"';
					}
				}
		...
		public function UpdateEMPPersonalData(Request $request)
		   {
				...
				$attributesDetailswarningletter = Attributes::where("department_id",'All')->where(["parent_attribute"=>0])->where("tab_name","warning_letter")->where("status",1)->orderBy("sort_order","ASC")->get();
	...
	public function workLocationEMP(Request $req){
		...
		public function updateEmpDepartmentData(Request $request){
		 ...
		 public function OfflineEMPData(Request $request){
			 ...
			 $offlineObj->document_collection_id=$empdetails->document_collection_id;			 
