...
use App\Http\Controllers\Reporting\InternalReportController;
...
{
	$filename = 'JONUS CCD REPORT DAILY BASIS_'.date("d-m-Y").'.xlsx';
		/*
			*Export Logs
			*/
			$newMonth = date("m");
					 ...
					 if($currentDate > 20)
					 {
						 ...
						 $newMonth = date("m",strtotime('+ 1 months'));
						 $newMonthName = date("M",strtotime('+ 1 months'));
					 }
					 $selectedMonth = $newMonth-1;
						$yearSelected = $currentYear;
						 ...
						 $dateFrom = $yearSelected.'-'.$selectedMonth.'-21';
						
						$dateTo = $currentYear.'-'.$newMonth.'-20';
			$exportLogsObj = new ExportReportLogs();
			$exportLogsObj->download_area = 'JONUS CCD REPORT DAILY BASIS Report';
			$exportLogsObj->download_filename = $filename;
			$exportLogsObj->downloaded_by = $request->session()->get('EmployeeId');
			$exportLogsObj->save();
			/*
			...
			*/
				...
				$sheet->setCellValue('A1', 'CCD END REPORT TL WISE - '.strtoupper($newMonthName).' ENDS')->getStyle('A1')->getAlignment()->setHorizontal('center')->setVertical('top');
			...
			foreach($tL_details as $TLData)
			{
				...
				$phyWip = ReportingController::getWipCount($leader,$dateFrom,$dateTo);
				$total_phyWip = $total_phyWip+$phyWip;
				$tabWip = ReportingController::getWipCountTab($leader,$dateFrom,$dateTo);
				$total_tabWip = $total_tabWip+$tabWip;
				$phyEnd = ReportingController::getEndCount($leader,$dateFrom,$dateTo);
				$total_phyEnd = $total_phyEnd+$phyEnd;
				$tabEnd = ReportingController::getEndCountTab($leader,$dateFrom,$dateTo);
...
public function excelDailyCCDInternal(Request $request)
{
	$filename = 'INTERNAL CCD REPORT DAILY BASIS_'.date("d-m-Y").'.xlsx';
		/*
			*Export Logs
			*/
			 $newMonth = date("m");
					 ...
					 $selectedMonth = $newMonth-1;
						...
						$dateTo = $currentYear.'-'.$newMonth.'-20';
			$exportLogsObj = new ExportReportLogs();
			$exportLogsObj->download_area = 'INTERNAL CCD REPORT DAILY BASIS Report';
			$exportLogsObj->download_filename = $filename;
			$exportLogsObj->downloaded_by = $request->session()->get('EmployeeId');
			$exportLogsObj->save();
			/*
			...
			*/
			...
			foreach($tL_details as $TLData)
			{
				...
				$tabLogin = ReportingController::getReportMisCards($newMonth,$currentYear,'login',$leader,'Tab');
				$total_tabLogin = $total_tabLogin+$tabLogin;
				$phyWip = InternalReportController::getWipCount($leader,$dateFrom,$dateTo);
				$total_phyWip = $total_phyWip+$phyWip;
				$tabWip = ReportingController::getWipCountTab($leader,$dateFrom,$dateTo);
				$total_tabWip = $total_tabWip+$tabWip;
				$phyEnd = InternalReportController::getEndCount($leader,$dateFrom,$dateTo);
				$total_phyEnd = $total_phyEnd+$phyEnd;
				$tabEnd = ReportingController::getEndCountTab($leader,$dateFrom,$dateTo);
				$total_tabEnd = $total_tabEnd+$tabEnd;
				$awaiting = ReportingController::getWaiting($newMonth,$currentYear,$leader,'manual');
...
public function printToFileInternal(Request $request)
{
	...
	$leadersArray = explode(",",$leaders);
	  $currentDate = date("d");
					  if($currentDate > 20)
					 {
						 $currentYear = date("Y",strtotime('+ 1 months'));
						 $newMonth = date("m",strtotime('+ 1 months'));
						 $dateCur = '01-'.$newMonth.'-'.$currentYear; 
					 }
					 else
					 {
						 $newMonth =  date('m');
	...
	$filename = 'Internal DAILY BASIS REPORT_'.date("d-m-Y").'.xlsx';
	$spreadsheet = new Spreadsheet();
				$sheet = $spreadsheet->getActiveSheet();
		 ...
		 $currentDate = date("d");
					...
					for($i=0;$i<3;$i++)
					{
						$newMonth = date('m', strtotime($dateCur.'-'.$i.' month'));
						$currentYear = date('Y', strtotime($dateCur.'-'.$i.' month'));
					...
					foreach($leadersArray as $leader)
					{
						...
						*/
						...
						$dateTo = $currentYear.'-'.$newMonth.'-20';
								$newIndex++;
								$wipM =  InternalReportController::getWipCount($leader,$dateFrom,$dateTo);
								$wipT =  ReportingController::getWipCountTab($leader,$dateFrom,$dateTo);
						$wip = $wipM+$wipT;
...
public function printToFile(Request $request)
{
	...
	$filename = 'Jonus DAILY BASIS REPORT_'.date("d-m-Y").'.xlsx';
	$spreadsheet = new Spreadsheet();
				$sheet = $spreadsheet->getActiveSheet();
		 ...
		 $currentDate = date("d");
					...
					foreach($leadersArray as $leader)
					{
						...
						*/
						...
						$dateTo = $currentYear.'-'.$newMonth.'-20';
								$newIndex++;
								$wipM =  ReportingController::getWipCount($leader,$dateFrom,$dateTo);
								$wipT =  ReportingController::getWipCountTab($leader,$dateFrom,$dateTo);
								$wip = $wipM+$wipT;
								$sheet->setCellValue($startCellNameNew.$newIndex, $wip)->getStyle($startCellNameNew.$newIndex)->getAlignment()->setHorizontal('center')->setVertical('top'); 
								$spreadsheet->getActiveSheet()->getStyle($startCellNameNew.$newIndex)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('bdd7ee');
