...
use App\Models\SEPayout\AgentPayoutdeemRange;



class SEPayoutDeemController extends Controller
{
			...
			public function UpdateDeemTime()
			{
				$agentData=AgentPayoutDeem::get();
				foreach($agentData as $_agent){
				$payoutId=$_agent->id;
				
				$updateMod = AgentPayoutDeem::find($payoutId);
			...
			public function UpdateDeemTimeRange(){
			...
			public function AgentMidPointDeembyRange(){
				
				
				$salestimeUniq = AgentPayoutDeem::groupBy('range_id')->selectRaw('count(*) as total, range_id')->get();
				$rangeArray = array();
				...
				$tc= array();
					...
					$saletimeList =AgentPayoutDeem::where("agent_product","CARD")->where("range_id",$saletime->range_id)->get();
					...
					foreach($saletimeList as $allList){
						...
						if($allList->total_card=="-" || $allList->total_card==""){
							$tc[] = 0;
						}
						else{
						$tc[] = $allList->total_card;
					...
					$payoutData = new AgentPayoutdeemRange();
