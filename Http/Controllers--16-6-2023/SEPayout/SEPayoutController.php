...
class SEPayoutController extends Controller
{
			...
			public function UpdateloanAmount()
			{
				...
				$payoutId=$_agent->id;
				if(trim($_agent->personal_loan)=='-' || trim($_agent->personal_loan)=='' || trim($_agent->personal_loan)==NULL ){
				...
				if(trim($_agent->auto_loan)=='-' || trim($_agent->auto_loan)=='' || trim($_agent->personal_loan)==NULL  ){
				...
				//echo $p_loan;
				//str_replace(",","",$p_loan);exit;
				$finalamount=str_replace(",","",trim($p_loan))+str_replace(",","",trim($a_loan));//exit;
				$updateMod = AgentPayout::find($payoutId);
			...
			public function UpdateloanData(){
				...
				$loanData=AgentPayout::where('match_employee',2)->get();
				//$index = 0;
