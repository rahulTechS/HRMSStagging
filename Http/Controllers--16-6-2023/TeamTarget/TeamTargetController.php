<?php
namespace App\Http\Controllers\TeamTarget;
...
class TeamTargetController extends Controller
{
		...
		{			
			return view("TeamTarget/TeamTarget");
			
		}
		public function setOffSetForTargetLeader(Request $request)
			{
				$offset = $request->offset;
				$request->session()->put('offset_team_leader_inner_mis',$offset);
				 return  redirect('TargetList');
			}
			public function setFileSourceForTeamLeader(Request $request)
		   {
			   //print_r($request);//exit;
			   $request->session()->put('file_source_type_filter_team_leader',$request->fileSource);
			   return  redirect('TargetList');
		   }
		
		public function TargetList(Request $request){
			
			if(!empty($request->session()->get('offset_team_leader_inner_mis')))
				{
					$paginationValue = $request->session()->get('offset_team_leader_inner_mis');
				}
				else
				{
					$paginationValue = 10;
				...
				$whereraw='';
				if(!empty($request->session()->get('file_source_type_filter_team_leader'))){
				
				//$filesource='';
					$filesource = $request->session()->get('file_source_type_filter_team_leader');
					...
					$empdetails = Team_Leader::paginate($paginationValue);
				}
				//print_r($filesource);//exit;
			$reportsCount = Team_Leader::get()->count();
			if($whereraw != '')
			{
			$empdetails = Team_Leader::orderBy("id","DESC")->whereRaw($whereraw)->paginate($paginationValue);	
			}
			else
			{
				$empdetails = Team_Leader::orderBy("id","DESC")->paginate($paginationValue);	
			}
			//$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/TargetList'));
			return view("TeamTarget/TargetList",compact('empdetails','paginationValue','reportsCount'));
		}
		
		public function addTeamTarget(Request $request)
		{
			return view("TeamTarget/addTeamTarget");
		}
		public function SalesExecutiveForm(Request $request)
			{
				
				$teamlead = Employee_details::where("dept_id",9)->where("job_role","Team Leader")->orderBy("id","ASC")->get();
				$salesex = Employee_details::where("dept_id",9)->where("job_role","Sales Executive")->orderBy("id","ASC")->get();
				
				return view("TeamTarget/SalesExecutiveForm" ,compact('teamlead','salesex'));
			}
			public function TeamLeaderForm(Request $request)
			{
				
				 
				$empdetails = Employee_details::where("dept_id",9)->where("job_role","Team Leader")->orderBy("id","ASC")->get();		
				
				return view("TeamTarget/TeamLeaderForm",compact('empdetails'));
			}
		

		public function saveTeamLeaderData(Request $req)
		{
			...
			$year=$inputData['year'];
			$team_target=$inputData['team_target'];
			$data = Team_Leader::where("team_id",$team_id)->where("product","$product")->where("month","$month")->where("year","$year")->first();
		
			if($data !=''){
			$req->session()->flash('message','Data Already Exit.');
            return redirect('TeamTarget');
			}
			else{
				
					$tregetdata = new Team_Leader();
					$tregetdata->team_id = $team_id;
					$tregetdata->product = $product;
					$tregetdata->month = $month;
					$tregetdata->year = $year;
					$tregetdata->team_target = $team_target;
					$tregetdata->type = "Team Leader";
					$tregetdata->agent_id=0;
					$tregetdata->save();
		...
		public function saveSalesExcData(Request $req)
		{
			
			$inputData = $req->input();
			//print_r($inputData);exit;
			$team_id=$inputData['team_id'];
			$salesex=$inputData['salesex'];
			$type="Agent";
			$product=$inputData['product'];
			$month=$inputData['month'];
			$year=$inputData['year'];
			$team_target=$inputData['team_target'];
			$teamlead = Team_Leader::where("team_id",$team_id)->where("product","$product")->where("month","$month")->where("year","$year")->first();
			$agentdata = Team_Leader::where("team_id",$team_id)->where("agent_id","$salesex")->where("product","$product")->where("month","$month")->where("year","$year")->first();			
			if($teamlead !=''){
				...
				return redirect('TeamTarget');
						...
						$tregetdata->year = $year;
						$tregetdata->team_target = $team_target;
						$tregetdata->type = $type;
						$tregetdata->agent_id = $salesex;
						$tregetdata->save();
				$req->session()->flash('message','Data Saved Successfully.');
				return redirect('TeamTarget');
				}
				...
				$req->session()->flash('message','Team Leader Not Exit.');
				return redirect('TeamTarget');
			}
			//echo "DAta Saved";
			//exit;
		...
		public static function getTeamLeaderName($deptId)
	{
		$empRequiredDetails =  Employee_details::where('id',$deptId)->first();
		$data=$empRequiredDetails->first_name.' '.$empRequiredDetails->middle_name.' '.$empRequiredDetails->last_name;
		return $data;
	}
	
	public static function getagentName($agent_id)
	{
		$agentDetails =  Employee_details::where('id',$agent_id)->first();
		//print_r($agentDetails);exit;
		$agentname=$agentDetails->first_name.' '.$agentDetails->middle_name.' '.$agentDetails->last_name;
		return $agentname;
	}
	public function updateTeamaLeaderdata(Request $request){
			$rowid=$request->rowid;
			$updateleader = Team_Leader::where("id",$rowid)->first();
			$empdetails = Employee_details::where("dept_id",9)->where("job_role","Team Leader")->orderBy("id","ASC")->get();
			
			return view("TeamTarget/TeamLeaderFormUpdate",compact('updateleader','empdetails'));
		}
	public function saveTeamLeaderDataUpdate(Request $req)
		{
			
			$inputData = $req->input();
			$id=$inputData['id'];
			$team_leader = Team_Leader::find($id);
			$team_leader->team_target=$inputData['team_target'];
			$team_leader->save();
			$req->session()->flash('message','Data Saved Successfully.');
            return redirect('TeamTarget');
		...
		public function updateSaleshExcdata(Request $request){
			$rowid=$request->rowid;
			$updateleader = Team_Leader::where("id",$rowid)->first();
			$teamlead = Employee_details::where("dept_id",9)->where("job_role","Team Leader")->orderBy("id","ASC")->get();
			$salesex = Employee_details::where("dept_id",9)->where("job_role","Sales Executive")->orderBy("id","ASC")->get();
			
			return view("TeamTarget/SalesExecutiveFormUpdate",compact('updateleader','teamlead','salesex'));
		}
		
		public function saveTeamSalesDataUpdate(Request $req)
		{
