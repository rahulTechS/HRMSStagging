...
class BaronboardedInCompleteLayoutController extends Controller
{
	...
	public function searchOnboardedInComplete(Request $request)
	{
		...
		if(isset($parametersInput['department']) && $parametersInput['department'] != '' && $parametersInput['department'] != NULL )
		{
			...
			$request->session()->put('widgetFilteronboardDept['.$widgetID.']',$departmentData);	
		}
		else
		{
	...
	public function resetOnboardedInComplete(Request $request)
	{
	...
	public function reloadmeBarOnboardedInComplete(Request $request)
	{
		$wid = $request->wid;
		
		return view("components/bargraph/reloadmeBarOnboardedincomplete",compact('wid'));
	}
	
	
	public function OnboardedInCompleteBarLink(Request $request)
	{
		...
		*/
		...
		if($dataType == 'current_month')
		{
			...
			$request->session()->put('datefrom_candonboard_filter_inner_list',$fromData);
				$request->session()->put('dateto_candonboard_filter_inner_list',$toDate);
		}
		...
		else if($dataType == 'custom')
		{
			...
			$request->session()->put('datefrom_candonboard_filter_inner_list',$from_salesTime);
			$request->session()->put('dateto_candonboard_filter_inner_list',$to_salesTime);
		}
		else
		{
			$toDate = date("Y-m-d");
	...
	public function SearchLastMonthOnboardInComplete(Request $request)
	{
		//echo "hello";exit;
		$widgetID = $request->wid;
	...
	public function SearchLastMonthOnboardInCompletetotal(Request $request){
		$widgetID = $request->wid;
		$request->session()->put('widgetFilterHiring['.$widgetID.'][data_type]','total');	
		return redirect('reloadmeBarOnboardedInComplete/'.$widgetID);
	}
	
	public function SearchLastMonthOnboardInCompleteCM(Request $request)
	{
		//echo "hello";exit;
		$widgetID = $request->wid;
	...
	public function SearchLastMonthOnboardInCompleteLM(Request $request)
	{
		//echo "hello";exit;
		$widgetID = $request->wid;
		$request->session()->put('widgetFilterHiring['.$widgetID.'][data_type]','last_month');	
	...
	public function SearchLastMonthOnboardInComplete3M(Request $request)
	{
		//echo "hello";exit;
		$widgetID = $request->wid;
