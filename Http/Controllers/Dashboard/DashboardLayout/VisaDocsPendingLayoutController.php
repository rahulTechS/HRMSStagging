...
class VisaDocsPendingLayoutController extends Controller
{
	
	public function searchVisaDocsPending(Request $request)
	{
	...
	public function resetsearchVisaDocsPending(Request $request)
	{
		//echo "hello";exit;
	...
	public function reloadmeVisaDocsPending(Request $request)
	{
		 $wid = $request->wid;
		
		return view("components/OnboardingReport/reloadmevisadocspending",compact('wid'));
	}
	
	
	public function expandVisaDocsPending(Request $request)
	{
		$wid = $request->wid;
		$request->session()->put('open_section_status_'.$wid,'Yes');
		return redirect('reloadmeVisaDocsPending/'.$wid);
	}
	
	public function compressVisaDocsPending(Request $request)
	{
		$wid = $request->wid;
	...
	public function VisaDocsPendingLastMonth(Request $request)
	{
		//echo "hello";exit;
		$widgetID = $request->wid;
	...
	public function candidateVisadocsPendingLink(Request $request)
	{
		//echo "hello";exit;
		$dept = $request->dept;
		$wid = $request->wid;
		$request->session()->put('departmentId_candofferlettercomplete_filter_inner_list',$dept);
		$request->session()->put('filtervisa_documents_status_filter_inner_list',1);
		$request->session()->put('tabOpenByWidget',"OfferletterComplete");		
		return redirect('documentcollectionAjax');
	}
	public function candidateVisadocsPendingTotelLink(Request $request)
	{
		//echo "hello";exit;
