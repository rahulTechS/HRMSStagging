...
namespace App\Http\Controllers\Dashboard\HeadCountAnalysis;
...
class HeadCountAnalysisController extends Controller
{
	
	public function searchTLPerformanceMashreqProductivityWid(Request $request)
	{
		...
		if(isset($parametersInput['data_type_headcount']) && $parametersInput['data_type_headcount'] != '' && $parametersInput['data_type_headcount'] != NULL )
		{
			$data_type = $parametersInput['data_type_headcount'];
		...
		return redirect('reloadHeadCountAnalysisProcess/'.$widgetID);
	}
	
	public function resetsearchTLPerformanceMashreqProductivity(Request $request)
	{
		//echo "hello";exit;
	...
	public function reloadHeadCountAnalysisContentData(Request $request)
	{
		 $wid = $request->wid;
		
		return view("components/HeadcountProductivity/reloadHeadCountAnalysis",compact('wid'));
	}
	
	
	public function expandTLPerformanceMashreqProductivity(Request $request)
	{
		$wid = $request->wid;
		$request->session()->put('open_section_status_'.$wid,'Yes');
		return redirect('reloadmeLeadershipTLPerformanceMashreqProductivity/'.$wid);
	}
	
	public function compressTLPerformanceMashreqProductivity(Request $request)
	{
		$wid = $request->wid;
	...
	public function searchHeadCountLMData(Request $request)
	{
		//echo "hello";exit;
		//$request->session()->put('widgetFilterHiring['.$widgetID.'][data_type]','');
		$widgetID = $request->wid;
	...
	public function searchHeadCountCurrentData(Request $request)
	{
		//echo "hello";exit;
		//$request->session()->put('widgetFilterHiring['.$widgetID.'][data_type]','');
		$widgetID = $request->wid;
