...
class EibRecommendationsSalesLayoutController extends Controller
{
	...
	public function searchEibWarningLetter(Request $request)
	{
		...
		if(isset($parametersInput['team']) && $parametersInput['team'] != '' && $parametersInput['team'] != NULL )
		{
			...
			$request->session()->put('widgetFiltermolTeamSalesRecommend['.$widgetID.']',$team);	
		}
		else
		{
			$request->session()->put('widgetFiltermolTeamSalesRecommend['.$widgetID.']','');	
		}
		...
		if(isset($parametersInput['processor']) && $parametersInput['processor'] != '' && $parametersInput['processor'] != NULL )
		{
			...
			$request->session()->put('widgetFilterprocessorSalesRecommend['.$widgetID.']',$processor);	
		}
		else
		{
			$request->session()->put('widgetFilterprocessorSalesRecommend['.$widgetID.']','');	
		}
		...
		return redirect('reloadEibRecommendationSales/'.$widgetID);
	}
	...
	public function resetEibWarningLetter(Request $request)
	{
		...
		$request->session()->put('widgetFilterHiring['.$widgetID.'][department]','');
		$request->session()->put('widgetFilterHiring['.$widgetID.'][teamLeaders]','');			
		return redirect('reloadRecommendationSales/'.$widgetID);
	}
	...
	public function reloadEibWarningLetter(Request $request)
	{
		...
		return view("components/SalesManagement/reloadrecommendationseib",compact('wid'));
	}
	...
	public function expandEibWarning(Request $request)
	{
		...
		return redirect('reloadEibRecommendationSales/'.$wid);
	}
	...
	public function PerformanceEibCBDspreadByGraph(Request $request)
	{
		//echo "hello";exit;
		$widgetID = $request->wid;
	...
	public function PerformanceEibCBDByTable(Request $request)
	{
		$widgetID = $request->wid;
	...
	public function OLDocsPendingEibLastMonth(Request $request)
	{
		//echo "hello";exit;
		$widgetID = $request->wid;
	...
	public function empAttendanceEibDeptLink(Request $request)
	{
		//echo "hello";exit;
		$dept = $request->dept;
		$wid = $request->wid;
