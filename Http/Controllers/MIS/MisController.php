...
use App\Models\MIS\EnbdCardsList;
...
ini_set("display_errors", 1);
        ...
        return back()
		   ...
		   public function savePostENBDCARDSMIS(Request $request)
		   {
			   ...
			   $misReportObj->fv_company_name = strtoupper($requestParameters['fv_company_name']);
			   $misReportObj->company_name_as_per_visa = strtoupper($requestParameters['company_name_as_per_visa']);
			   ...
			   $misReportObj->Card_Name = $requestParameters['Card_Name'];
			  ...
			  $misReportObj->type_data = 'generated';
			   ...
			   $misReportObj->REFERENCE_NAME = strtoupper($requestParameters['REFERENCE_NAME']);
			   $misReportObj->REFERENCE_MOBILE_NO = $requestParameters['REFERENCE_MOBILE_NO'];
			 
			   $misReportObj->cm_name = strtoupper($requestParameters['cm_name']);
			   ...
			   $misReportObj->DESIGNATION = strtoupper($requestParameters['DESIGNATION']);
			   $misReportObj->MMN = strtoupper($requestParameters['MMN']);
			   $misReportObj->EIDA = strtoupper($requestParameters['EIDA']);
		   ...
		   public static function getapproved_notapproved($id)
		   {
			    if(!empty($id))
			   {
				   if(ENDBCARDStatus::where("id",$id)->first() != '')
				   {
		   ...
		   public function manageMISENBDCardsTab(Request $request)
			...
			public function listingENBDCardManual(Request $request)
			{
			  ...
			  $selectedFilter['internalStatus'] = '';
			  $selectedFilter['Mobile'] = '';
			  $selectedFilter['Passport'] = '';
			  ...
			  if(!empty($request->session()->get('file_source_type_filter')))
				...
				if(!empty($request->session()->get('update_jonus_filter')))
				{
					$update_jonus_filter = $request->session()->get('update_jonus_filter');
					$selectedFilter['jonus_filter'] = $update_jonus_filter;
					if($update_jonus_filter  != 'All')
					{
						if($whereraw == '')
						{
							$whereraw = "match_status = '".$update_jonus_filter."'";
						}
						else
						{
							$whereraw .= " And match_status = '".$update_jonus_filter."'";
						}
					}
				}
				if(!empty($request->session()->get('emp_jonus_filter')))
				{
					$emp_id = $request->session()->get('emp_jonus_filter');
					$selectedFilter['employee_id'] = $emp_id;
					if($emp_id  != 'All')
					{
						if($whereraw == '')
						{
							$whereraw = "employee_id = '".$emp_id."'";
						}
						else
						{
							$whereraw .= " And employee_id = '".$emp_id."'";
						}
					}
				}
				
				
				if(!empty($request->session()->get('cutomer_jonus_filter')))
				{
					$customerName = $request->session()->get('cutomer_jonus_filter');
					$selectedFilter['customer_name'] = $customerName;
					if($customerName  != 'All')
					{
						if($whereraw == '')
						{
							$whereraw = "cm_name = '".$customerName."'";
						}
						else
						{
							$whereraw .= " And cm_name = '".$customerName."'";
						}
					}
				}
				
				if(!empty($request->session()->get('appid_jonus_filter')))
				{
					$appid = $request->session()->get('appid_jonus_filter');
					$selectedFilter['APPID'] = $appid;
					if($appid  != 'All')
					{
						if($whereraw == '')
						{
							$whereraw = "application_id = '".$appid."'";
						}
						else
						{
							$whereraw .= " And application_id = '".$appid."'";
						}
					}
				}
				if(!empty($request->session()->get('product_jonus_filter')))
				{
					$product = $request->session()->get('product_jonus_filter');
				...
				if(!empty($request->session()->get('cards_dateOfSubmission_from')) && !empty($request->session()->get('cards_dateOfSubmission_to')))
				{
					$submission_from = $request->session()->get('cards_dateOfSubmission_from');
					$submission_to = $request->session()->get('cards_dateOfSubmission_to');
					 $selectedFilter['submission_from'] = $submission_from;
			  $selectedFilter['submission_to'] = $submission_to;
					if($whereraw == '')
					{
						$whereraw = 'submission_format >= "'.date("Y-m-d",strtotime($submission_from)).'" and submission_format <= "'.date("Y-m-d",strtotime($submission_to)).'"';
					
					}
					else
					{
					$whereraw .= ' And submission_format >= "'.date("Y-m-d",strtotime($submission_from)).'" and submission_format <= "'.date("Y-m-d",strtotime($submission_to)).'"';
					
					}
				}
				
				if(!empty($request->session()->get('submission_date_one')) && !empty($request->session()->get('submission_date_one')))
				{
					$submission_date_one = $request->session()->get('submission_date_one');
					
					 $selectedFilter['submission_date_one'] = $submission_date_one;
					if($submission_date_one != 'All')
					{
					if($whereraw == '')
					{
						$whereraw = 'submission_format like "%'.date("Y-m-d",strtotime($submission_date_one)).'%"';
					
					}
					else
					{
						$whereraw .= ' And submission_format like "%'.date("Y-m-d",strtotime($submission_date_one)).'%"';
					
					}
					}
				}
				
				if(!empty($request->session()->get('internal_Status')) && !empty($request->session()->get('internal_Status')))
				{
					$internal_Status = $request->session()->get('internal_Status');
					
					 $selectedFilter['internalStatus'] = $internal_Status;
					if($internal_Status != 'All')
					{
					if($whereraw == '')
					{
						if($internal_Status == 7)
						{
							$whereraw = 'approved_notapproved_internal = "'.$internal_Status.'" OR mothly_end_internal = "AWATING REPORT"';
						}
						else
						{
						$whereraw = 'approved_notapproved_internal = "'.$internal_Status.'"';
						}
					...
					else
					{
						if($internal_Status == 7)
						{
							$whereraw .= ' And approved_notapproved_internal = "'.$internal_Status.'" OR mothly_end_internal = "AWATING REPORT"';
						}
						else
						{
						$whereraw .= ' And approved_notapproved_internal = "'.$internal_Status.'"';
						}
					
					}
					}
				}
				if(!empty($request->session()->get('customer_mobile_cards')) && !empty($request->session()->get('customer_mobile_cards')))
				{
					$customer_mobile_cards = $request->session()->get('customer_mobile_cards');
					
					 $selectedFilter['Mobile'] = $customer_mobile_cards;
					if($customer_mobile_cards != 'All')
					{
					if($whereraw == '')
					{
						$whereraw = 'CV_MOBILE_NUMBER = "'.$customer_mobile_cards.'"';
					
					}
					else
					{
						$whereraw .= ' And CV_MOBILE_NUMBER = "'.$customer_mobile_cards.'"';
					
					}
					}
				}
				if(!empty($request->session()->get('customer_passport_cards')) && !empty($request->session()->get('customer_passport_cards')))
				{
					$customer_passport_cards = $request->session()->get('customer_passport_cards');
					
					 $selectedFilter['Passport'] = $customer_passport_cards;
					if($customer_passport_cards != 'All')
					{
					if($whereraw == '')
					{
						$whereraw = 'PASSPORT_NO = "'.$customer_passport_cards.'"';
					
					}
					else
					{
						$whereraw .= ' And PASSPORT_NO = "'.$customer_passport_cards.'"';
					
					}
					}
				}
				
				if(!empty($request->session()->get('location_name')) && !empty($request->session()->get('location_name')))
				{
					$location_name = $request->session()->get('location_name');
					
					 $selectedFilter['location'] = $location_name;
					if($location_name != 'All')
					{
					$empAData = Employee_attribute::where("attribute_code","work_location")->where("attribute_values",$location_name)->get();
					$empidList = array();
					foreach($empAData as $empA)
					{
						$empIdA = $empA->emp_id;
						$empModelA = Employee_details::where("emp_id",$empIdA)->first();
						if($empModelA != '')
						{
						$empidList[] = $empModelA->id;
						}
					}
					if(count($empidList) >0 )
					{
						$empidListstring = implode(",",$empidList);
						
						if($whereraw == '')
						{
							$whereraw = 'employee_id in ('.$empidListstring.')';
						
						}
						else
						{
							$whereraw .= ' And employee_id in ('.$empidListstring.')';
			...
			/* 	echo $whereraw;exit; */
				...
				/*
				*get all employee list from Cards mis
				*start code
				*/
				$employeeListArray = array();
				if($whereraw == '')
				{
				$employeeList = MainMisReport::groupBy("employee_id")->selectRaw('count(*) as total, employee_id')->get();
				}
				else
				{
					$employeeList = MainMisReport::groupBy("employee_id")->selectRaw('count(*) as total, employee_id')->whereRaw($whereraw)->get();
				...
				*/
				...
				if($whereraw == '')
				{
				$customerList = MainMisReport::groupBy("cm_name")->selectRaw('count(*) as total, cm_name')->get();
				}
				else
				{
					
					$customerList = MainMisReport::groupBy("cm_name")->selectRaw('count(*) as total, cm_name')->whereRaw($whereraw)->get();
				...
				*/
				...
				if($whereraw == '')
				{
				$appidGet = MainMisReport::groupBy("application_id")->selectRaw('count(*) as total, application_id')->get();
				}
				else
				{
					
					$appidGet = MainMisReport::groupBy("application_id")->selectRaw('count(*) as total, application_id')->whereRaw($whereraw)->get();
				...
				foreach($appidGet as $_d)
				{
						...
						$appIdArray[$_d->application_id] = $_d->application_id;
				...
				*/
				...
				/*
				*get all Product from Cards mis
				*start code
				*/
				$productArray = array();
				if($whereraw == '')
				{
				$products = MainMisReport::groupBy("PRODUCT")->selectRaw('count(*) as total, PRODUCT')->get();
				}
				else
				{
					
					$products = MainMisReport::groupBy("PRODUCT")->selectRaw('count(*) as total, PRODUCT')->whereRaw($whereraw)->get();
				...
				foreach($products as $_pro)
				{
						...
						$productArray[$_pro->PRODUCT] = $_pro->PRODUCT;
				...
				*/
				...
				if($whereraw == '')
				{
				$submissionGet = MainMisReport::groupBy("submission_format")->selectRaw('count(*) as total, submission_format')->orderBy("id","DESC")->get();
				}
				else
				{
					
					$submissionGet = MainMisReport::groupBy("submission_format")->selectRaw('count(*) as total, submission_format')->whereRaw($whereraw)->orderBy("id","DESC")->get();
					
				}
				
				foreach($submissionGet as $_d)
				{
					$submissionArray[$_d->submission_format] = date("d M Y",strtotime($_d->submission_format));
				}
				...
				*/
				...
				/*
				*get all location from Cards mis
				*start code
				*/
				$locationArray = array();
				if($whereraw == '')
				{
					$locationEmp = MainMisReport::groupBy("employee_id")->selectRaw('count(*) as total, employee_id')->orderBy("id","DESC")->get();
				}
				else
				{
					
					$locationEmp = MainMisReport::groupBy("employee_id")->selectRaw('count(*) as total, employee_id')->whereRaw($whereraw)->orderBy("id","DESC")->get();
					
				}
				$emp_idList = array();
				foreach($locationEmp as $_emp)
				{
					$empId = Employee_details::where("id",$_emp->employee_id)->selectRaw('emp_id')->first();
					if($empId != '')
					{
					$emp_idList[] = $empId->emp_id;
					}
					
				}
				$datas = Employee_attribute::groupBy("attribute_values")->whereIn("emp_id",$emp_idList)->where("attribute_code","work_location")->selectRaw('attribute_values')->get();
				
				foreach($datas as $data1)
				{
					$locationArray[$data1->attribute_values] = $data1->attribute_values;
				}
				/*
				*get all location from Cards mis
				*end code
				...
				*start code
				*/
				$misInternalStatusArray = array();
				if($whereraw == '')
				{
				$internalStatus = MainMisReport::orderBy("id","DESC")->distinct('approved_notapproved_internal')->get();
				}
				else
				{
					$internalStatus = MainMisReport::whereRaw($whereraw)->orderBy("id","DESC")->distinct('approved_notapproved_internal')->get();
					
				}
				
				foreach($internalStatus as $_stat)
				{
					if($_stat->approved_notapproved_internal != '' || $_stat->approved_notapproved_internal != NULL)
					{
						$statusMod =  ENDBCARDStatus::where("id",$_stat->approved_notapproved_internal)->first();
						if($statusMod != '')
						{
							$misInternalStatusArray[$_stat->approved_notapproved_internal] = $statusMod->status_name;
						}
					}
					else if($_stat->mothly_end_internal == 'AWATING REPORT')
					{
						$misInternalStatusArray[7] = 'Wip';
					}
				}
				...
				*/
				...
				/*
				*get all employee Mobile from Cards mis
				*start code
				*/
				$customerMobileArray = array();
				if($whereraw == '')
				{
				$customerMobileMod = MainMisReport::groupBy("CV_MOBILE_NUMBER")->selectRaw('count(*) as total, CV_MOBILE_NUMBER')->orderBy("id","DESC")->get();
				}
				else
				{
					
					$customerMobileMod = MainMisReport::groupBy("CV_MOBILE_NUMBER")->selectRaw('count(*) as total, CV_MOBILE_NUMBER')->whereRaw($whereraw)->orderBy("id","DESC")->get();
					
				}
				
				foreach($customerMobileMod as $_mobile)
				{
					if($_mobile->CV_MOBILE_NUMBER != '' && $_mobile->CV_MOBILE_NUMBER != NULL)
					{
					$customerMobileArray[$_mobile->CV_MOBILE_NUMBER] = $_mobile->CV_MOBILE_NUMBER;
					}
				}
				...
				*/
				...
				/*
				*get all employee Passport from Cards mis
				*start code
				*/
				$customerPassportArray = array();
				if($whereraw == '')
				{
				$customerPassMod = MainMisReport::groupBy("PASSPORT_NO")->selectRaw('count(*) as total, PASSPORT_NO')->orderBy("id","DESC")->get();
				}
				else
				{
					
					$customerPassMod = MainMisReport::groupBy("PASSPORT_NO")->selectRaw('count(*) as total, PASSPORT_NO')->whereRaw($whereraw)->orderBy("id","DESC")->get();
					
				}
				
				foreach($customerPassMod as $_pass)
				{
					if($_pass->PASSPORT_NO != '' && $_pass->PASSPORT_NO != NULL)
					{
					$customerPassportArray[$_pass->PASSPORT_NO] = $_pass->PASSPORT_NO;
					}
				}
				...
				*end code
				*/
				Cache::put('reports', $reports, now()->addMinutes(30));
				return view("MIS/listingENBDCardManual",compact('reports','reportsCount','paginationValue','employees','selectedFilter','autoSelectRowId','employeeListArray','customerNameArray','appIdArray','productArray','submissionArray','locationArray','misInternalStatusArray','customerMobileArray','customerPassportArray'));
			}
		   ...
		   public function getManualENBDCardsUpdate(Request $request)
			{
				...
				*get card name
				*/
				$product = $misRecords->PRODUCT;
				$cardListing = EnbdCardsList::where("product",$product)->get();
				$cardList = array();
				foreach($cardListing as $_cards)
				{
					$cardList[$_cards->id] = $_cards->card_name;
				}
				/*
				*get card name
				*/
				return view("MIS/getManualENBDCardsUpdate",compact('currentActMod','enbdCardsStatus','MonthlyEndsValues','tL_details','agent_details','natValues','banks','misRecords','rowId','cList','cardList'));
			}
			...
			public function savePostENBDCARDSMISUpdate(Request $request)
		   {
			  ...
			  $misReportObjUpdate->date_of_submission = $requestParameters['date_of_submission'];
			   ...
			   $misReportObjUpdate->fv_company_name = strtoupper($requestParameters['fv_company_name']);
			   $misReportObjUpdate->company_name_as_per_visa = strtoupper($requestParameters['company_name_as_per_visa']);
			   ...
			   $misReportObjUpdate->approved_notapproved_internal = $requestParameters['approved_notapproved'];
			   if($requestParameters['approved_notapproved'] == 3)
			   {
					$mothly_end_internal = $requestParameters['mothly_end_internal'];
					
					$currentMonth = date("m",strtotime($mothly_end_internal));
					$currentYear = date("Y",strtotime($mothly_end_internal));
					$currentDate = date("d",strtotime($mothly_end_internal));
					if($currentDate > 20)
					{
					...
					$monthNameSmall = date('M', mktime(0, 0, 0, $currentMonth, 10));
					$mothly_end_internalFinalValue = 'End - '.$monthNameSmall.'-'.$currentYear;
					$misReportObjUpdate->mothly_end_internal = $mothly_end_internalFinalValue;
					$misReportObjUpdate->monthly_end_number = $requestParameters['mothly_end_internal'];
			   }
			   else
			   {
				   $misReportObjUpdate->mothly_end_internal = NULL;
					$misReportObjUpdate->monthly_end_number = NULL;
			   }
			  ...
			  $misReportObjUpdate->type_data = 'generated';
			   $misReportObjUpdate->REFERENCE_NAME = strtoupper($requestParameters['REFERENCE_NAME']);
			   $misReportObjUpdate->REFERENCE_MOBILE_NO = $requestParameters['REFERENCE_MOBILE_NO'];
			 
			   $misReportObjUpdate->cm_name = strtoupper($requestParameters['cm_name']);
			   ...
			   $misReportObjUpdate->DESIGNATION = strtoupper($requestParameters['DESIGNATION']);
			   $misReportObjUpdate->MMN = strtoupper($requestParameters['MMN']);
			   $misReportObjUpdate->EIDA = strtoupper($requestParameters['EIDA']);
		   ...
		   public function uploadDocCardsManual(Request $request)
			...
			public function dailyReportTeamsCardsInternal(Request $request)
				{
				...
				$selectedTL = array();
					...
					return view("MIS/dailyReportTeamsCardsInternal",compact('tL_details','selectedTL'));
				}
...
public function dailyReportTeamsCardsCCDInternal(Request $request)
				{
				...
				$selectedTL = array();
					...
					return view("MIS/dailyReportTeamsCardsCCDInternal",compact('tL_details','selectedTL'));
				}	
		...
		public function reportingTeamDisplayInternal(Request $request)
				{
				
					$leaders = $request->leaders;
					$leadersArray = explode(",",$leaders);
					return view("MIS/reportingTeamDisplayInternal",compact('leadersArray'));
				}			
		  ...
		  public function downloadPdfCards(Request $request)
		  {
			  $misid = $request->misid;
			  $type = $request->type;
			  $misDocuments = MisDocuments::where("mis_id",$misid)->where("type",$type)->orderBy("id","DESC")->get();
			  return view("MIS/downloadPdfCards",compact('misDocuments'));
	
		  }
		  
		  
		  public function downloadPdfCardsBackup(Request $request)
		  {
			  $misid = $request->misid;
			  $type = $request->type;
...
//$merge->output();
		...
		public function setUpdatedJonus(Request $request)
		{
			$update_jonus = $request->update_jonus;
			 $request->session()->put('update_jonus_filter',$update_jonus);
			   return  redirect('manageMISENBDCards');
		}
		  
		public function filterByEmpCards(Request $request)
		{
			$empSearch = $request->empSearch;
			 $request->session()->put('emp_jonus_filter',$empSearch);
			   return  redirect('manageMISENBDCards');
			
		}
		
		public function filterByCustomerNameCards(Request $request)
		{
			$cName = $request->cName;
			$request->session()->put('cutomer_jonus_filter',$cName);
			   return  redirect('manageMISENBDCards');
		}
		public function filterByAppidCards(Request $request)
		{
			$appidfilter = $request->appidfilter;
			$request->session()->put('appid_jonus_filter',$appidfilter);
			   return  redirect('manageMISENBDCards');
		}
		public function filterByProduct(Request $request)
		{
			$productfilter = $request->productfilter;
			$request->session()->put('product_jonus_filter',$productfilter);
			   return  redirect('manageMISENBDCards');
		}
		
		public function searchLoanDateRangeCards(Request $request)
		{
			$dateOfSubmission_from = $request->dateOfSubmission_from;
			$dateOfSubmission_to = $request->dateOfSubmission_to;
			$request->session()->put('cards_dateOfSubmission_from',$dateOfSubmission_from);
			$request->session()->put('cards_dateOfSubmission_to',$dateOfSubmission_to);
			   return  redirect('manageMISENBDCards');
		}
		
		public function filterBySubmissionCards(Request $request)
		{
			$submission_date_one = $request->submission_date_one;
			
			$request->session()->put('submission_date_one',$submission_date_one);
			
			   return  redirect('manageMISENBDCards');
		}
		
		public function resetAllCards(Request $request)
		{
			$request->session()->put('cards_dateOfSubmission_from','');
			$request->session()->put('cards_dateOfSubmission_to','');
			   return  redirect('manageMISENBDCards');
		}
		...
		public function filterBylocation(Request $request)
		{
			...
			$request->session()->put('location_name',$locationName);
			
			   return  redirect('manageMISENBDCards');
		}
		public function filterByInternalStatus(Request $request)
		{
			$internalStatus = $request->internalStatus;
			
			$request->session()->put('internal_Status',$internalStatus);
			
			   return  redirect('manageMISENBDCards');
		}
		public function filterByCustomerMobile(Request $request)
		{
			$cus_mobile = $request->cus_mobile;
			
			$request->session()->put('customer_mobile_cards',$cus_mobile);
			
			   return  redirect('manageMISENBDCards');
		}
		public function filterByCustomerPassport(Request $request)
		{
			$cus_passport = $request->cus_passport;
			
			$request->session()->put('customer_passport_cards',$cus_passport);
		...
		public function getCardName(Request $request)
		{
			...
			$cardNameList = EnbdCardsList::where("product",$product)->get();
			$html = ' <option value="">Select Card Name..</option>';
			foreach($cardNameList as $_cardN)
			{
				$html .= ' <option value="'.$_cardN->id.'">'.$_cardN->card_name.'</option>';
			}
			echo $html;exit;
