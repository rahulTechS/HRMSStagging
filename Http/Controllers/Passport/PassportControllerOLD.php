...
class PassportController extends Controller
{
			...
			if($whereraw != '')
			{
				...
				if($departmentDetails != '')
				{
					...
					if($empDetails!='')
					{
						...
						->whereRaw($whereraw)
						->where('passport.passport_status',1)
						->whereNull('passport.pre_release_request_status')
						// ->where('passport.pre_release_request_approved_reject_status',2)
						// ->where('passport.collection_status',2)
						->whereRaw($whereraw)
						// ->where('finalstatus',0)
						// ->orWhere('finalstatus', '>', 0)
						->orderBy('passport.updated_at','desc')->paginate($paginationValue);
			...
			else
			{
				...
				if($departmentDetails != '')
				{
					...
					if($empDetails!='')
					{
						...
						$reportsCount = Passport::join('employee_details', 'employee_details.emp_id', '=', 'passport.emp_id')->where('employee_details.dept_id',$empDetails->dept_id)
						->where('passport.passport_status',1)
						->whereNull('passport.pre_release_request_status')
						// ->where('passport.pre_release_request_approved_reject_status',2)
						// ->where('passport.collection_status',2)
		...
		$selectedFilter['Recruiter'] = '';
			...
			if($whereraw != '')
			{
				...
				if($departmentDetails != '')
				{
					...
					->where('passport.pre_release_request_approved_reject_status',1)
					->where('passport.collection_status',1)
						// ->where('finalstatus',0)
						// ->orWhere('finalstatus', '>', 0)
						->orderBy('passport.updated_at','desc')->paginate($paginationValue);
	...
	public function listingReleasedPassportsData(Request $request)
	{
		...
		$selectedFilter['Recruiter'] = '';
			...
			$userDetails=User::where("id",$empsessionId)->first();

			$empDetailsData=Employee_details::where("emp_id",$userDetails->employee_id)->first();

			if($empDetailsData->job_function==3 || $empDetailsData->job_function==4)
			{
				$releasedButtonShow = 2;				
			}
			else
			{
				$releasedButtonShow = 1;
			}
				...
				if($whereraw != '')
				{
					...
					if($departmentDetails != '')
					{
						...
						if($empDetails!='')
						{
							...
							->where('passport.pre_release_request_approved_reject_status',2)
							->where('passport.collection_status',2)
							//->where('passport.pre_release_request_status',2)
							->whereIn('passport.pre_release_request_status',$statusArr)	
							->where('employee_details.dept_id',$empDetails->dept_id)
							->orderBy('passport.updated_at','desc')->paginate($paginationValue);
				...
				else
				{
					...
					{
						$passportDetails = Passport::select('passport.id as rowid','passport.*')
		...
		return view("Passport/listingRealeasedPassports",compact('passportDetails','departmentLists','productDetails','designationDetails','reportsCount','filterList','paginationValue','releasedButtonShow'));
	...
	public function listingApprovalReleasedQueueData(Request $request)
	{
		//return "Hello";
		$whereraw = '';
		$whereraw1 = '';
			...
			else
			{
				...
				if($whereraw != '')
				{
						...
						else
						{
								...
								->where('collection_status',2)
								->whereIn('passport.pre_release_request_status',$statusArr)
								//->whereNotIn('approveRejectAction', [1])
								->orderBy('passport.updated_at','desc')->paginate($paginationValue);
				...
				else
				{
					...
					else
					{
						...
						$reportsCount = Passport::where('passport.pre_release_request_approved_reject_status',2)
						->where('collection_status',2)
						->whereIn('passport.pre_release_request_status',$statusArr)						
						->orderBy('passport.id','desc')->get()->count();
					}	
		...
		return view("Passport/listingApprovalPassports",compact('passportDetails','departmentLists','productDetails','designationDetails','reportsCount','filterList','paginationValue','releasedButtonShow'));
	...
	public function exportNotAvailableCompletePassportsReport(Request $request)
	{
		...
		$parameters = $request->input(); 
			...
			$spreadsheet->getActiveSheet()->getStyle('A1:J1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('cbddf7');
				...
				$logObj->user_id =$request->session()->get('EmployeeId');
				$logObj->download_date =date("Y-m-d");
				$logObj->tilte ="Passport-Available";					
				$logObj->save();
				$writer = new Xlsx($spreadsheet);
				$writer->save(public_path('uploads/exportPassport/'.$filename));	
				echo $filename;
	...
	public function availableVisaCompletedReleasedData(Request $request)
	{
				...
				$selectedFilter['Recruiter'] = '';
					...
					if($empsessionId==61 || $empsessionId==72)
					{
						$statusArr=array(1,3);
						if($whereraw != '')
						{
				...
				return view("Passport/listingRealeasedPassportsVisaComplete",compact('passportDetails','departmentLists','productDetails','designationDetails','reportsCount','filterList','paginationValue','releasedButtonShow'));
	}
	...
	public function availableinProgressVisaReleasedData(Request $request)
	{
				...
				return view("Passport/listingRealeasedPassportsVisaInProgress",compact('passportDetails','departmentLists','productDetails','designationDetails','reportsCount','filterList','paginationValue','releasedButtonShow'));
	}
	...
	// 	$passportDetails->pre_release_request_approved_reject_status = 1;
	// 	$passportDetails->pre_release_request_status = 2;
	// 	$passportDetails->release_request_approved_rejected_at = date('Y-m-d');	
	// 	$passportDetails->release_request_approved_rejected_by = $userid;					
	...
	// 	$passportDetails->pre_release_request_approved_reject_status = 2;
	// 	$passportDetails->pre_release_request_status = 1;
	// 	$passportDetails->release_request_approved_rejected_at = date('Y-m-d');	
	// 	$passportDetails->release_request_approved_rejected_by = $userid;					
	// 	$passportDetails->save();
	...
	public function requestReleasePassportApprovedActionProcess(Request $request)
	{
		...
		$passportDetails->approveRejectAction = 1;
		$passportDetails->pre_release_request_status = 2;
		$passportDetails->release_request_approved_rejected_at = date('Y-m-d');	
		$passportDetails->release_request_approved_rejected_by = $userid;					
		$passportDetails->save();
	...
	public function requestReleasePassportRejectedActionProcess(Request $request)
	{
		...
		$passportDetails->approveRejectAction = 2;
		$passportDetails->pre_release_request_status = 3;
		$passportDetails->release_request_approved_rejected_at = date('Y-m-d');	
		$passportDetails->release_request_approved_rejected_by = $userid;					
		$passportDetails->save();
	...
	public function saveReleaseRequestfromTopData(Request $request)
	{
		...
		else
		{
			...
			if($passportData)
			{
				...
				$passportData->approveRejectAction = NULL;
	...
	// 		$passportData->pre_release_request_status = NULL;
	// 		$passportData->release_request_approved_rejected_at = NULL;	
	// 		$passportData->release_request_approved_rejected_by = NULL;	
	...
	public function searchPassportDetailsData(Request $request)
		...
		{
			$emp_details = Employee_details::whereNull('passport_update_status')->where('offline_status',1)->orderBy("id","desc")->get();
			...
			if(count($emp_details) > 0)
			{
				...
				foreach($emp_details as $emp)
				{
					...
					else
					{
						...
						$passportDetails->pre_release_request_approved_reject_status = 1;
						$passportDetails->collection_status = 1;
						$passportDetails->save();
				...
				return response()->json(['success'=>'Passport Module Updated Successfully. '.$count.' Employee Added Successfully']);	
			}
			else
			{
				return response()->json(['success'=>'No Any New Employee found to Update in Passport.']);
			}
