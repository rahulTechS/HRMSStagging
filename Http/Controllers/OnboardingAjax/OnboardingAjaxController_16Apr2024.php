 ...
 */			$whereraw1 = '';
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				$documentCollectiondetails= DocumentCollectionDetails::whereIn("id",$finaldocidarray)->orderByRaw("-sort_date DESC")->paginate($paginationValue);
				$reportsCountdeadline = DocumentCollectionDetails::whereIn("id",$finaldocidarray)->get()->count();
