...
class TabsAjaxController extends Controller
{
	   ...
	   public function summaryTabWithFullViewAjax(Request $request)
	   {
			...
			*/
			...
			$stepsAll[7]['name'] = 'Insurance';
			
			if($documentCollectionDetails->status >= 6 && $documentCollectionDetails->insurance_status!= 1)
			{
				 $stepsAll[7]['stage'] = 'inprogress'; 
				 $stepsAll[7]['Tab'] = 'active'; 
			}
			else if($documentCollectionDetails->status >= 6 && $documentCollectionDetails->insurance_status==1)
			{
				$completedStep++;
				 $stepsAll[7]['stage'] = 'active'; 
				 $OnboardingProgress = 'insurance ';
				  $stepsAll[7]['Tab'] = 'active'; 
			}
			else
			{
			...
			$stepsAll[7]['slagURL'] = 'insurance '; 
			$stepsAll[7]['onclick'] = 'tabinsurancePanel();'; 	

			//sanjeetadd
			
			//tab 8 start
			$stepsAll[8]['name'] = 'Training'; 
			...
			$stepsAll[8]['slagURL'] = 'tab6';
			 $stepsAll[8]['onclick'] = 'tabtrainingProcessPanel();';
			 $stepsAll[9]['name'] = 'On-Boarding'; 
			...
			$stepsAll[9]['slagURL'] = 'onboard';  
			$stepsAll[9]['onclick'] = 'onboardPanel();'; 
			
			$stepsAll[10]['name'] = 'Bank Code Generation Process'; 
			...
			$stepsAll[10]['slagURL'] = 'tab7';
		    $stepsAll[10]['onclick'] = 'codeGeneration();'; 	
			
			
			$stepsAll[11]['name'] = 'On-Boarding Completed'; 
			...
			{
				$stepsAll[11]['stage'] = 'inprogress'; 
				$stepsAll[11]['Tab'] = 'active';
			}
			else if($documentCollectionDetails->onboard_status == 2)
			{
				$completedStep++;
				$stepsAll[11]['stage'] = 'active';
				$stepsAll[11]['Tab'] = 'active';				
			}
			else
			{
				$stepsAll[11]['stage'] = 'pending'; 
				 $stepsAll[11]['Tab'] = 'disabled-tab';
			}
			
		    $stepsAll[11]['slagURL'] = 'tab8';
			 $stepsAll[11]['onclick'] = 'finalization();'; 	
			$totalStep = 11;
	   ...
	   public function getVisaDocumentTouristVisa($documentCollectionID=NULL)
	   {
		   ...
		   foreach($documentAttributesDetails as $_documentCUpload)
		   {
			...
			print_r($uploadDetails);
			exit; */
		   return view("OnboardingAjax/RequestTabsAjax/uploadVisaDocumentByOnchange",compact('medicalAttributes','documentDetails','documentAttributes','uploadDetails'));
$medicalAttributes = DocumentCollectionAttributes::where("attribute_area","medical")->where("attribute_category","alltypevisa")->where("status",1)->get();
	   }
	   ...
	   public function getVisaDocumentResidenceVisaCom($documentCollectionID=NULL)
	   {
		   ...
		   foreach($documentAttributesDetails as $_documentCUpload)
		   {
			...
			print_r($uploadDetails);
			exit; */
		   return view("OnboardingAjax/RequestTabsAjax/uploadVisaDocumentByOnchangeCom",compact('medicalAttributes','documentDetails','documentAttributes','uploadDetails'));
	   }
	   ...
	   public function getVisaDocumentPartnershipVisa($documentCollectionID=NULL)
	   {
		   $uploadDetails = array();
		   $id = $documentCollectionID; 
		   $documentDetails = DocumentCollectionDetails::where("id",$id)->first();
	   ...
	   public function tab2EmpDetailsAjax(Request $request)
	   {
		      $documentCollectId = $request->documentCollectionId;
			 $documentCollectionDetails = DocumentCollectionDetails::where("id",$documentCollectId)->first();
			 $documentCollectionValues = DocumentCollectionDetailsValues::where("document_collection_id",$documentCollectId)->where("attribute_code",14)->first();
				
			
			 return view("OnboardingAjax/RequestTabsAjax/employeeDetailsAjax",compact('documentCollectionDetails','documentCollectionValues')); 
	   }
...
public static function getVisaTypenameData($id)
	...
	public function tababinsurancePanelAjax(Request $request)
	{
		$documentCollectId = $request->documentCollectionId;
		   $documentCollectionDetails = DocumentCollectionDetails::where("id",$documentCollectId)->first();
		  $documentCollectionValues = DocumentCollectionDetailsValues::where("document_collection_id",$documentCollectId)->get();
		  ...
		  foreach($documentCollectionValues as $_docCollectionValue)
		  {
			  ...
			  if($docAttributes != '')
			  {
				  if($docAttributes->attribute_area== 'onboardinginsurance')
				  {
				  $attributeName = $docAttributes->attribute_name.'^'.$docAttributes->attrbute_type_id;
				  $attributeValue = $_docCollectionValue->attribute_value;
				  $docCollectionDetails[$attributeName] = $attributeValue;
		  ...
		  foreach($documentCollectionValues as $_docCollectionValueMedical)
		  {
			  ...
			  if($docAttributesmedical != '')
			  {
				  if($docAttributesmedical->attribute_area== 'onboardinginsurancemedical')
				  {
				  $attributeName = $docAttributesmedical->attribute_name.'^'.$docAttributesmedical->attrbute_type_id;
				  $attributeValue = $_docCollectionValueMedical->attribute_value;
				  $docCollectionDetailsMedical[$attributeName] = $attributeValue;
		  ...
		  exit;  */
		   return view("OnboardingAjax/RequestTabsAjax/insuranceDocumentAjax",compact('documentCollectionDetails','docCollectionDetails','docCollectionDetailsMedical'));
	}	
	

	public function uploadInsuranceDocumentAjax(Request $request)
	{
		$uploadDetails = array();
		$id = $request->id; 
		$documentDetails = DocumentCollectionDetails::where("id",$id)->first();
		$documentAttributes = DocumentCollectionAttributes::where("status",1)->Where("attribute_area","onboardinginsurance")->get();
		//print_r($documentAttributes);exit;
		$documentAttributesmedical = DocumentCollectionAttributes::where("status",1)->Where("attribute_area","onboardinginsurancemedical")->get();
		$documentAttributesDetails =DocumentCollectionDetailsValues::where("document_collection_id",$id)->get();
		foreach($documentAttributesDetails as $_documentCUpload)
		{
			$uploadDetails[$_documentCUpload->attribute_code] = $_documentCUpload->attribute_value;
		 ...
		 print_r($uploadDetails);
		 exit; */
		return view("OnboardingAjax/RequestTabsAjax/uploadInsuranceDocumentAjax",compact('documentDetails','documentAttributes','documentAttributesmedical','uploadDetails'));
	}




	public function uploadInsuranceDocumentStartAjax(Request $request)
	{
		 $selectedFilter = $request->input();
		// print_r($_FILES);
	   ...
	   /*
		 ...
		 */
		 /* update after change inside to out side*/
		 //$changecountry=DocumentCollectionDetails::where("id",$selectedFilter['documentCollectionID'])->first();
		 if(isset($selectedFilter[105]) && $selectedFilter[105] != 'undefined'){
					 $documentMod = DocumentCollectionDetails::find($selectedFilter['documentCollectionID']);
					 $documentMod->insurance_status =1;
					 if($documentMod->save()){
						$finaljsondata = json_encode(array('Policy Number' =>$selectedFilter[105]), JSON_PRETTY_PRINT);
						$logObj = new DocumentCollectionDetailsLog();
						$logObj->document_id =$selectedFilter['documentCollectionID'];
						$logObj->created_by=$request->session()->get('EmployeeId');
						$logObj->title ="Insurance";
						$logObj->response =$finaljsondata;
						$logObj->category ="Insurance";
						$logObj->save();
					}
				}
