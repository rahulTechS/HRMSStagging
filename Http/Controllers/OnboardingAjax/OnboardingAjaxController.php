...
use App\Models\InsuranceProcess\InsuranceOnboardMapping;
	  ...
	  public function getSalaryBreakupDocumentAjax(Request $request)
	   ...
	   public function editDocumentCollectionAjax(Request $request)
	   {
		   ...
		   $recruiterList = RecruiterDetails::where("status",1)->orderBy("id","DESC")->get();
			...
			//$existDocument = DocumentCollectionDetailsValues::where("document_collection_id",$selectedFilter['documentCollectionEdit']['id'])->where("attribute_code",14)->first();

			$existDocument = DocumentCollectionDetailsValues::where("document_collection_id",$dCollectionId)->where("attribute_code",14)->first();
//print_r($existDocument);
			return view("OnboardingAjax/editDocumentCollectionAjax",compact('interviewer_name','departmentDetails','designationDetails','salaryBreakUpdetails','documentCollectionData','hiringSourceList','recruiterList','jobOpeningList','existDocument'));
	   }
	   ...
	   public function editdocumentCollectionPostAjax(Request $request)
	   {
		    ...
		    $selectedFilter = $request->input();
			//echo $selectedFilter['documentCollectionEdit']['id'];exit;
...
//print_r($_FILES);exit;
			...
			$filesAttributeInfo = array();
				...
				$fileIndex = 0;
				$LastInsertId=$selectedFilter['documentCollectionEdit']['id'];
				foreach($keys as $key)
				{
					...
					$vKey = $key;
					$newFileName = $key.'-'.$LastInsertId.'.'.$fileExtension;
					...
					$existDocument = DocumentCollectionDetailsValues::where("document_collection_id",$selectedFilter['documentCollectionEdit']['id'])->where("attribute_code",$key)->first();
					if($existDocument != '')
					{
					...
					$objDocument->document_collection_id = $selectedFilter['documentCollectionEdit']['id'];
					$objDocument->attribute_code = $key;
					$objDocument->attribute_value = $newFileName;
		   ...
		   if($documentCollectionModel->save())
		   {
				 ...
				 if($docdata!=''){
					 ...
					 if($newdata!=''){
						 ...
						 if($interviewObJ->save()){
							if($processdata->interviewer_name!=$selectedFilter['documentCollectionEdit']['interviewer_name']){
			   ...
			   if($processdata->proposed_salary != $selectedFilter['documentCollectionEdit']['proposed_salary']){
			   $logObj = new DocumentCollectionDetailsLog();
				$logObj->document_id =$selectedFilter['documentCollectionEdit']['id'];
				$logObj->created_by=$request->session()->get('EmployeeId');
	...
	public function uploadofferletterIncentiveLetterDocumentStartAjax(Request $request)
			...
			foreach($keys as $key)
			{
				if(in_array($key,$listOfAttribute))
				{
					...
					if($objDocument->save())
					{
						...
						if($key == 81)
						{
							$data=DocumentCollectionDetails::where("id",$documentCollectionId)->first();
							$documentCollectionMod = DocumentCollectionDetails::find($documentCollectionId);
							$documentCollectionMod->offer_letter_onboarding_status=2;
							if($data->location=="Karachi" || $data->location=="Rawalpindi"){
							$documentCollectionMod->city_status=2;	
							}
	   ...
	   public function uploadonboardDocumentStartAjax(Request $request)
	   {
		...
		$empattributesMod = OnboardCandidateKyc::where('docId',$documentCollectionId)->first();
			...
			$onboardKYCOBJ->marital_status =$request->input('marital_status');
			...
			*/
			if($selectedFilter[84] == "YES")
			{
				...
				if($documentDetailsForOnboarding->current_visa_status=="Outside Country"){
					$date = $selectedFilter[83];
					$newdate = strtotime ( '+50 days' , strtotime ( $date ) ) ;
					$newdate = date ( 'd-m-Y' , $newdate );

						
							$_documentCollectionModcaption = DocumentCollectionDetails::find($documentCollectionId);
				...
				*/
				...
				if($empId != '')
				{
					...
					if($documentDetailsForOnboarding->location == 'AUH')
									...
									elseif($documentDetailsForOnboarding->location == 'Rawalpindi')
									{
									$worklocation = 'Rawalpindi';
									}
									else
									{
										$worklocation = 'NA';
									}
					$newEmpModel->work_location = $selectedFilter[94];
					...
					$newEmpModel->actual_salary = $selectedFilter[95];
					$newEmpModel->offline_status = 1;
					$newEmpModel->offboard_status = 2;
					$newEmpModel->vintage_updated_date=date("Y-m-d");
					/*
					*get Designation
					*/
						...
						if($designationOnboard  != '' && $designationOnboard != NULL)
							{
								...
								if($designationMod != '')
								{
									...
									elseif($documentDetailsForOnboarding->location == 'Rawalpindi')
									{
									$employeeAttribute->attribute_values = 'Rawalpindi';
									}
									else
									{
									...
									if($selectedFilter[95]!=''){
									$employeeAttribute = new Employee_attribute();
									$employeeAttribute->emp_id = $newEMPID;
									$employeeAttribute->dept_id = $deptId;
									$employeeAttribute->attribute_code = 'actual_salary';
									$employeeAttribute->attribute_values = $selectedFilter[95];
									$employeeAttribute->status = 1;
									$employeeAttribute->save();
									}
									if($selectedFilter[96]!=''){
									$employeeAttribute = new Employee_attribute();
									$employeeAttribute->emp_id = $newEMPID;
									$employeeAttribute->dept_id = $deptId;
									$employeeAttribute->attribute_code = 'PP_NO';
									$employeeAttribute->attribute_values = $selectedFilter[96];
									$employeeAttribute->status = 1;
									$employeeAttribute->save();
									}
									...
									if($documentkycdata->marital_status != '')
									{										
									$employeeAttribute = new Employee_attribute();
									$employeeAttribute->emp_id = $newEMPID;
									$employeeAttribute->dept_id = $deptId;
									$employeeAttribute->attribute_code = 'marital_status';
									$employeeAttribute->attribute_values = $documentkycdata->marital_status;									
									$employeeAttribute->status = 1;
									$employeeAttribute->save();
									}									
									...
									$employeeAttribute->attribute_values = strtoupper($documentkycdata->country);									
									$employeeAttribute->status = 1;
									$employeeAttribute->save();
									}
									...
									$employeeAttribute->save();
										...
										$existDocumentemail = DocumentCollectionDetailsValues::where("document_collection_id",$documentCollectionId)->where("attribute_code",93)->first();
									if($existDocumentemail!=''){
										$employeeAttribute = new Employee_attribute();
										$employeeAttribute->emp_id = $newEMPID;
										$employeeAttribute->dept_id = $deptId;
										$employeeAttribute->attribute_code = 'email';
										$employeeAttribute->attribute_values = $existDocumentemail->attribute_values;									
										$employeeAttribute->status = 1;
										$employeeAttribute->save();
									}
									}
									...
									$detailsObj->email_sent =0;
									$detailsObj->doj = date("Y-m-d",strtotime($selectedFilter[83]));
									if($detailsObj->save()){
		...
		/*   echo '<pre>';
		  ...
		  //print_r($_FILES);exit;
		   ...
		   $num = $documentCollectionId;
			...
			/* ravi Sir Code  */
			$updateEmpAttrravi = new InsuranceOnboardMapping();
			$updateEmpAttrravi->document_collection_id=$documentCollectionId;
			$updateEmpAttrravi->status=1;
			$updateEmpAttrravi->save();
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('dateto_visaapprove_approval_filter_inner_list')) && $request->session()->get('dateto_visaapprove_approval_filter_inner_list') != 'All')
				{
					$datetoapproval = $request->session()->get('dateto_visaapprove_approval_filter_inner_list');
					 if($whereraw == '')
					{
						$whereraw = 'created_at<= "'.$datetoapproval.' 23:59:59"';
					}
					else
					{
						$whereraw .= ' And created_at<= "'.$datetoapproval.' 23:59:59"';
					}
				}
				if(!empty($request->session()->get('datefrom_visaapprove_expected_filter_inner_list')) && $request->session()->get('datefrom_visaapprove_expected_filter_inner_list') != 'All')
				{
					$datefromexpected = $request->session()->get('datefrom_visaapprove_expected_filter_inner_list');
					 if($whereraw == '')
					{
						$whereraw = 'expected_date_joining>= "'.$datefromexpected.'"';
					}
					else
					{
						$whereraw .= ' And expected_date_joining>= "'.$datefromexpected.'"';
					}
				}
				if(!empty($request->session()->get('dateto_visaapprove_expected_filter_inner_list')) && $request->session()->get('dateto_visaapprove_expected_filter_inner_list') != 'All')
				{
					$datetoexpected = $request->session()->get('dateto_visaapprove_expected_filter_inner_list');
					 if($whereraw == '')
					{
						$whereraw = 'expected_date_joining<= "'.$datetoexpected.'"';
					}
					else
					{
						$whereraw .= ' And expected_date_joining<= "'.$datetoexpected.'"';
					}
				}
				if(!empty($request->session()->get('flighttkt_list_Visaapprove')) && $request->session()->get('flighttkt_list_Visaapprove') != 'All')
				{
					$flighttkt = $request->session()->get('flighttkt_list_Visaapprove');
					
					 if($whereraw == '')
					{
						$whereraw = 'ticket_status IN('.$flighttkt.')';
					}
					else
					{
						$whereraw .= ' And ticket_status IN('.$flighttkt.')';
					}
				}
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
		...
		public function requestdocmentdataPost(Request $request)
	   {
		...
		$parameters = $request->input(); 
			...
			$sheet->setCellValue('AA'.$indexCounter, strtoupper('Country'))->getStyle('AA'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AB'.$indexCounter, strtoupper('Skilled/ Unskilled'))->getStyle('AB'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AC'.$indexCounter, strtoupper('Attestation'))->getStyle('AC'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AD'.$indexCounter, strtoupper('Notes'))->getStyle('AD'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AE'.$indexCounter, strtoupper('Expected joining Type'))->getStyle('AE'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AF'.$indexCounter, strtoupper('Fligh Ticket'))->getStyle('AF'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AG'.$indexCounter, strtoupper('Degree Custody'))->getStyle('AG'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AH'.$indexCounter, strtoupper('Offer Letter Salary'))->getStyle('AH'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			...
			foreach ($selectedId as $sid) {
				...
				if($misData->training_process_status == 4){
				 ...
				 $olsal = DocumentCollectionDetailsValues::where("document_collection_id",$sid)->where("attribute_code",99)->first();
				 if($olsal!=''){
					 $offerlettersal=$olsal->attribute_value;
				 }
				 else{
					$offerlettersal=0; 
				 }
				...
				$sheet->setCellValue('AB'.$indexCounter, $skill)->getStyle('AB'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AC'.$indexCounter, $attestation)->getStyle('AC'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AD'.$indexCounter, $misData->notes)->getStyle('AD'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AE'.$indexCounter, $misData->expected_joining_type)->getStyle('AE'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AF'.$indexCounter, $fligh)->getStyle('AF'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AG'.$indexCounter, $misData->degree_custody)->getStyle('AG'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AH'.$indexCounter, $offerlettersal)->getStyle('AH'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				...
				if((!empty($request->session()->get('datefrom_candonboard_filter_inner_list')) && $request->session()->get('datefrom_candonboard_filter_inner_list') != 'All') && (!empty($request->session()->get('dateto_candonboard_filter_inner_list')) && $request->session()->get('dateto_candonboard_filter_inner_list') != 'All'))
				{
					...
					if($documentValues!=''){
						...
						foreach($documentValues as $_filterdate){
							if($_filterdate->document_collection_id!='' || $_filterdate->document_collection_id!=NULL){
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				{
					//echo "hello";exit;
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("created_at DESC")->whereRaw($whereraw)->where("offer_letter_onboarding_status",1)->where("backout_status",1)->paginate($paginationValue);
					//$departmentLists =  Department::where("status",1)->whereRaw($whereraw)->orderBy("id","DESC")->get();
					//$productDetails =  Product::where("status",1)->whereRaw($whereraw)->orderBy('id','DESC')->get();
					//$designationDetails =  Designation::where("status",1)->whereRaw($whereraw)->orderBy('id','DESC')->get();
				}
				...
				{
					//echo "hello1";
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("created_at DESC")->where("offer_letter_onboarding_status",1)->where("backout_status",1)->paginate($paginationValue);
					//$departmentLists =  Department::where("status",1)->orderBy("id","DESC")->get();
					//$productDetails =  Product::where("status",1)->orderBy('id','DESC')->get();
					//$designationDetails =  Designation::where("status",1)->orderBy('id','DESC')->get();
				}
				...
				if($whereraw != '')
				{
					...
					$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("offer_letter_onboarding_status",1)->where("backout_status",1)->get()->count();
				}
				else
				{
					$reportsCount = DocumentCollectionDetails::where("offer_letter_onboarding_status",1)->where("backout_status",1)->get()->count();
				}
...
public function departmentListDataofferlettercomplete(Request $request)
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				{
					$current_visa_status = $request->session()->get('current_visa_status_filter_inner_list_offerlettercomlete');
				...
				if(!empty($request->session()->get('current_visa_details_filter_inner_list_offerletterscomplete')) && $request->session()->get('current_visa_details_filter_inner_list_offerletterscomplete') != 'All')
				{
					$current_visa_detail = $request->session()->get('current_visa_details_filter_inner_list_offerletterscomplete');
				...
				{
					//echo "hello";exit;
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("offer_letter_document_date DESC")->whereRaw($whereraw)->where("offer_letter_onboarding_status",2)->whereIn("visa_process_status",array(0,1))->where("backout_status",1)->paginate($paginationValue);
					//$departmentLists =  Department::where("status",1)->whereRaw($whereraw)->orderBy("id","DESC")->get();
					//$productDetails =  Product::where("status",1)->whereRaw($whereraw)->orderBy('id','DESC')->get();
					//$designationDetails =  Designation::where("status",1)->whereRaw($whereraw)->orderBy('id','DESC')->get();
				}
				...
				{
					//echo "hello1";
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("offer_letter_document_date DESC")->where("offer_letter_onboarding_status",2)->where("backout_status",1)->whereIn("visa_process_status",array(0,1))->paginate($paginationValue);
					//$departmentLists =  Department::where("status",1)->orderBy("id","DESC")->get();
					//$productDetails =  Product::where("status",1)->orderBy('id','DESC')->get();
					//$designationDetails =  Designation::where("status",1)->orderBy('id','DESC')->get();
				}
				...
				if($whereraw != '')
				{
					...
					$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("offer_letter_onboarding_status",2)->where("backout_status",1)->whereIn("visa_process_status",array(0,1))->get()->count();
				}
				else
				{
					$reportsCount = DocumentCollectionDetails::where("offer_letter_onboarding_status",2)->where("backout_status",1)->whereIn("visa_process_status",array(0,1))->get()->count();
				}
 ...
 */			$whereraw1 = '';
		...
		$filterList['visa_process_status'] = '';
			...
			if(!empty($request->session()->get('flighttkt_list_visainprocessall')) && $request->session()->get('flighttkt_list_visainprocessall') != 'All')
				{
					$flighttkt = $request->session()->get('flighttkt_list_visainprocessall');
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('current_visa_status_filter_inner_list_pendingvisa')) && $request->session()->get('current_visa_status_filter_inner_list_pendingvisa') != 'All')
				{
					$current_visa_status = $request->session()->get('current_visa_status_filter_inner_list_pendingvisa');
					$current_visa_statusArray = explode(",",$current_visa_status);
				...
				if(!empty($request->session()->get('current_visa_type_value_filter_inner_list_pendingvisa')) && $request->session()->get('current_visa_type_value_filter_inner_list_pendingvisa') != 'All')
				{
					$visa_type_value = $request->session()->get('current_visa_type_value_filter_inner_list_pendingvisa');
				...
				if(!empty($request->session()->get('Visatype_visainprocessall_filter_inner_list')) && $request->session()->get('Visatype_visainprocessall_filter_inner_list') != 'All')
				{
					$visa_type = $request->session()->get('Visatype_visainprocessall_filter_inner_list');
					$visa_type_Array = explode(",",$visa_type);
					 
					 $visaProcessLists = Visaprocess::whereIn("visa_type",$visa_type_Array)->groupBy('document_id')->get();
					//print_r($visaProcessLists);exit;
					 $docidByVisatype=array();
					 if($visaProcessLists!=''){
						 foreach($visaProcessLists as $_visatype){
				...
				if(!empty($request->session()->get('current_visa_details_filter_inner_list_pendingvisa')) && $request->session()->get('current_visa_details_filter_inner_list_pendingvisa') != 'All')
				{
					$current_visa_detail = $request->session()->get('current_visa_details_filter_inner_list_pendingvisa');
					$current_visa_detailArray = explode(",",$current_visa_detail);
				...
				if(!empty($request->session()->get('datefrom_visainprocessall_moldate_filter_inner_list')) && $request->session()->get('datefrom_visainprocessall_moldate_filter_inner_list') != 'All')
				{
					 ...
					 if($whereraw == '')
					{
						...
						$whereraw = 'mol_date>= "'.$datefrommol.'"';
					}
					else
					{
						
						$whereraw .= ' And mol_date>= "'.$datefrommol.'"';
					}
				}
				$visainprocessto='';
				if(!empty($request->session()->get('dateto_visainprocessall_moldate_filter_inner_list')) && $request->session()->get('dateto_visainprocessall_moldate_filter_inner_list') != 'All')
				{
					 ...
					 if($whereraw == '')
					{
						...
						$whereraw = 'mol_date<= "'.$datetomol.'"';
					}
					else
					{
						
						$whereraw .= ' And mol_date<= "'.$datetomol.'"';
					}
				}
				$visainprocessevisafrom='';
				...
				{
					$visainprocessevisafrom=1;
					$datefromevisa = $request->session()->get('datefrom_visainprocessall_evisa_filter_inner_list');
					 if($whereraw == '')
					{
						
						$whereraw = 'evisa_start_date>= "'.$datefromevisa.'"';
					}
					else
					{
						
						$whereraw .= ' And evisa_start_date>= "'.$datefromevisa.'"';
					}
				}
				$visainprocessevisato='';
				...
				{
					$visainprocessevisato=1;
					$datetoevisa = $request->session()->get('dateto_visainprocessall_evisa_filter_inner_list');
					 if($whereraw == '')
					{
						
						$whereraw = 'evisa_start_date<= "'.$datetoevisa.'"';
					}
					else
					{
						
						$whereraw .= ' And evisa_start_date<= "'.$datetoevisa.'"';
					}
				}
				...
				if(!empty($request->session()->get('dateto_visainprocessall_approval_filter_inner_list')) && $request->session()->get('dateto_visainprocessall_approval_filter_inner_list') != 'All')
				{
					$datetoapproval = $request->session()->get('dateto_visainprocessall_approval_filter_inner_list');
					 if($whereraw == '')
					{
						$whereraw = 'created_at<= "'.$datetoapproval.' 23:59:590"';
					}
					else
					{
						$whereraw .= ' And created_at<= "'.$datetoapproval.' 23:59:59"';
				...
				if($whereraw != '')
				{
					...
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->whereiN("visa_process_status",array(2,4))->where("backout_status",1)->paginate($paginationValue);
						...
						if(($visainprocessto==1 && $visainprocessfrom==1) || ($visainprocessevisafrom==1 && $visainprocessevisato==1)){
							//echo "he";exit;
						$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("backout_status",1)->paginate($paginationValue);
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('current_visa_status_filter_inner_list_visascomplete')) && $request->session()->get('current_visa_status_filter_inner_list_visascomplete') != 'All')
				{
					$current_visa_status = $request->session()->get('current_visa_status_filter_inner_list_visascomplete');
					$current_visa_statusArray = explode(",",$current_visa_status);
				...
				if(!empty($request->session()->get('current_visa_type_value_filter_inner_list_visacomplete')) && $request->session()->get('current_visa_type_value_filter_inner_list_visacomplete') != 'All')
				{
					$visa_type_value = $request->session()->get('current_visa_type_value_filter_inner_list_visacomplete');
					$visa_type_valueArray = explode(",",$visa_type_value);
					 $totalvisavalue = array();
					 $rvalue = array(7,8,14,19,20,21);
					$lvalue = array(11,15,22,23,24,25);
				...
				if(!empty($request->session()->get('Visatype_visacomplete_filter_inner_list')) && $request->session()->get('Visatype_visacomplete_filter_inner_list') != 'All')
				{
					$visa_type = $request->session()->get('Visatype_visacomplete_filter_inner_list');
					$visa_type_Array = explode(",",$visa_type);
				...
				if(!empty($request->session()->get('flighttkt_list_visacomplete')) && $request->session()->get('flighttkt_list_visacomplete') != 'All')
				{
					$flighttkt = $request->session()->get('flighttkt_list_visacomplete');
				...
				if(!empty($request->session()->get('current_visa_details_filter_inner_list_visascomplete')) && $request->session()->get('current_visa_details_filter_inner_list_visascomplete') != 'All')
				{
					$current_visa_detail = $request->session()->get('current_visa_details_filter_inner_list_visascomplete');
					$current_visa_detailArray = explode(",",$current_visa_detail);
				...
				{
					//echo "hello";exit;
					if(($molfromcomplete==1 && $moltocomplete==1) || ($evisafromcomplete==1 && $evisatocomplete==1)){
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_complete_date DESC")->whereRaw($whereraw)->where("backout_status",1)->paginate($paginationValue);
					}
					else{
	   ...
	   public function documentcollectionbyfilterBYinsurance(Request $request)
	   {
		   ...
		   $request->session()->put('departmentId_candAll_filter_inner_list_insurance',$department);
		  
			//return  redirect('listingPageonboarding');	
	   }
	   public function resetListDataFilterinsurance(Request $request){
		   $request->session()->put('departmentId_candAll_filter_inner_list_insurance','');
	   ...
	   public function listingPageonboardinginsurance(Request $request)
	   {
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('departmentId_candAll_filter_inner_list_insurance')) && $request->session()->get('departmentId_candAll_filter_inner_list_insurance') != 'All' && $request->session()->get('departmentId_candAll_filter_inner_list_insurance') !=  'null')
				{
					$departmentids = $request->session()->get('departmentId_candAll_filter_inner_list_insurance');
				...
				*/
				...
				$departmentLists =  Department::where("status",1)->orderBy("id","DESC")->get();
					...
					if($whereraw != ''){
						$documentCollectiondetailsdata = DocumentCollectionDetails::whereRaw($whereraw)->where("visa_process_status",2)->where("backout_status",1)->get();
						//$departmentLists =  Department::where("status",1)->whereRaw($whereraw)->orderBy("id","DESC")->get();
	
					}
					else{
						$documentCollectiondetailsdata = DocumentCollectionDetails::where("visa_process_status",2)->where("backout_status",1)->get();
	
	
					}
	
	
					$visadatarraydateinsurnace=array();
					foreach ($documentCollectiondetailsdata as $documentCollectiondetailsnewdatainscurance){
	
					
					$visastagepermissioninsurnace=VisaStagePermission::where("id",3)->first();
					$visadatarrayinsurnace=explode(',',$visastagepermissioninsurnace->stageid);
	
					$visastagepermissionstampinginsurnace=VisaStagePermission::where("id",4)->first();
					$visadatarraystampinginsurnace=explode(',',$visastagepermissionstampinginsurnace->stageid);
	
	
					$visaproccessstatusinsurnace=Visaprocess::whereIN("visa_stage", $visadatarrayinsurnace)->whereNotIn("visa_stage",$visadatarraystampinginsurnace)->where('document_id',$documentCollectiondetailsnewdatainscurance->id)->first();
	
			   if($visaproccessstatusinsurnace!=''){
	
				$currentdate=date("Y-m-d");
				$visaproccessclosing= $visaproccessstatusinsurnace->closing_date;
				$closingdate=date("Y-m-d", strtotime($visaproccessclosing));
	
				//print_r($closingdate);exit;
	
				  $newdate = strtotime ( '+60 days' , strtotime ( $closingdate ) ) ;
				 ...
				 if(strtotime($currentdate)> strtotime($newdate)){
				   //echo "yes";
					...
					$visadatarraydateinsurnace[]=$documentCollectiondetailsnewdata->id;
				 }
	
					  }
	
				}
				$documentCollectiondetails = DocumentCollectionDetails::whereIn("id",$visadatarraydateinsurnace)->paginate($paginationValue);
				$reportsCount = DocumentCollectionDetails::whereIn("id",$visadatarraydateinsurnace)->get()->count();




				$documentCollectiondetails->setPath(config('app.url/listingPageinsuranceAll'));
				
				
		
		 $salaryBreakUpdetails =  SalaryBreakup::where("status",1)->orderBy("id","DESC")->get();
		return view("OnboardingAjax/listingPageinsuranceAll",compact('departmentLists','productDetails','designationDetails','documentCollectiondetails','reportsCount','filterList','salaryBreakUpdetails','paginationValue'));
	   }
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('Visatype_visaall_filter_inner_list')) && $request->session()->get('Visatype_visaall_filter_inner_list') != 'All')
				{
					$visa_type = $request->session()->get('Visatype_visaall_filter_inner_list');
					$visa_type_Array = explode(",",$visa_type);
				...
				if(!empty($request->session()->get('flighttkt_list_visaprocessvisaall')) && $request->session()->get('flighttkt_list_visaprocessvisaall') != 'All')
				{
					$flighttkt = $request->session()->get('flighttkt_list_visaprocessvisaall');
				...
				if($whereraw != '')
				{
					if(($evisato==1 && $evisafrom==1) || ($moldatefrom==1 && $moldateto==1)){
					$whereraw .= 'AND (ok_visa =3 OR ok_visa =2 OR ok_visa =1)';
					}else{
...
public function OfferLetterReleasedStatus(Request $request){
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
		...
		public function documentcollectionbyfilterBYDateofferletterComplete(Request $request)
		{
			...
			$request->session()->put('current_visa_details_filter_inner_list_offerletterscomplete',$current_visa_details);
		...
		public function resetListDataFilterDateofferletterComplete(Request $request){
			...
			$request->session()->put('current_visa_details_filter_inner_list_offerletterscomplete','');
	...
	public function getEmpBackoutPopupData(Request $req)
	   ...
	   public function expecteddateUpdateData(Request $request){
			...
			$joining_type=$request->input('joining_type');
			...
			$expecteddateobj->expected_joining_type = $joining_type; 
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				*/
				...
				if($whereraw != ''){
					$documentCollectiondetails = DocumentCollectionDetails::whereRaw($whereraw)->where("visa_process_status",2)->where("backout_status",1)->get();
					//$departmentLists =  Department::where("status",1)->whereRaw($whereraw)->orderBy("id","DESC")->get();
		...
		public function listingPageonboardingVisainprocessStage1(Request $request)
	   {
		...
		$filterList['visa_process_status'] = '';
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('current_visa_status_filter_inner_list_visastage1')) && $request->session()->get('current_visa_status_filter_inner_list_visastage1') != 'All')
				{
					$current_visa_status = $request->session()->get('current_visa_status_filter_inner_list_visastage1');
					$current_visa_statusArray = explode(",",$current_visa_status);
				...
				if(!empty($request->session()->get('current_visa_type_value_filter_inner_list_visastage1')) && $request->session()->get('current_visa_type_value_filter_inner_list_visastage1') != 'All')
				{
					$visa_type_value = $request->session()->get('current_visa_type_value_filter_inner_list_visastage1');
					$visa_type_valueArray = explode(",",$visa_type_value);
					 $totalvisavalue = array();
					 $rvalue = array(7,8,14,19,20,21);
					$lvalue = array(11,15,22,23,24,25);
				...
				if(!empty($request->session()->get('Visatype_visastage1_filter_inner_list')) && $request->session()->get('Visatype_visastage1_filter_inner_list') != 'All')
				{
					$visa_type = $request->session()->get('Visatype_visastage1_filter_inner_list');
					$visa_type_Array = explode(",",$visa_type);
				...
				if(!empty($request->session()->get('current_visa_details_filter_inner_list_visastage1')) && $request->session()->get('current_visa_details_filter_inner_list_visastage1') != 'All')
				{
					$current_visa_detail = $request->session()->get('current_visa_details_filter_inner_list_visastage1');
					$current_visa_detailArray = explode(",",$current_visa_detail);
				...
				if(!empty($request->session()->get('flighttkt_list_visastage1')) && $request->session()->get('flighttkt_list_visastage1') != 'All')
				{
					$flighttkt = $request->session()->get('flighttkt_list_visastage1');
				...
				if($whereraw != '')
				{
					...
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->whereIn("visa_process_status",array(2,4))->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->where("backout_status",1)->paginate($paginationValue);
					$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->whereIn("visa_process_status",array(2,4))->where("backout_status",1)->get()->count();
					}
					else{
						$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("visa_process_status",2)->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->where("backout_status",1)->paginate($paginationValue);
						$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->where("visa_process_status",2)->where("backout_status",1)->get()->count();
					}
					}
					else{
						if(($moltostage1==1 && $molfromstage1==1) || ($evisatostage1==1 && $evisafromstage1==1)){
						$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("backout_status",1)->paginate($paginationValue);
						$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("visa_stage_steps","Stage1")->where("backout_status",1)->get()->count();
						}
						else{
							$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("visa_process_status",2)->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->where("backout_status",1)->paginate($paginationValue);
							$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->where("visa_process_status",2)->where("backout_status",1)->get()->count();
				...
				{
					//echo "hello1";
					$documentCollectiondetails = DocumentCollectionDetails::where("visa_process_status",2)->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->where("backout_status",1)->orderByRaw("visa_inprogress_date DESC")->paginate($paginationValue);
					$reportsCount = DocumentCollectionDetails::where("visa_process_status",2)->where("mol_date","!=",NULL)->where("evisa_start_date","=",NULL)->where("backout_status",1)->get()->count();
					//$departmentLists =  Department::where("status",1)->orderBy("id","DESC")->get();
					//$productDetails =  Product::where("status",1)->orderBy('id','DESC')->get();
					//$designationDetails =  Designation::where("status",1)->orderBy('id','DESC')->get();
				}
	...
	public function listingPageonboardingVisainprocessStage2(Request $request)
	   {
		...
		$filterList['visa_process_status'] = '';
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('current_visa_status_filter_inner_list_visastage2')) && $request->session()->get('current_visa_status_filter_inner_list_visastage2') != 'All')
				{
					$current_visa_status = $request->session()->get('current_visa_status_filter_inner_list_visastage2');
					$current_visa_statusArray = explode(",",$current_visa_status);
				...
				if(!empty($request->session()->get('flighttkt_list_visastage2')) && $request->session()->get('flighttkt_list_visastage2') != 'All')
				{
					$flighttkt = $request->session()->get('flighttkt_list_visastage2');
				...
				if(!empty($request->session()->get('current_visa_type_value_filter_inner_list_visastage2')) && $request->session()->get('current_visa_type_value_filter_inner_list_visastage2') != 'All')
				{
					$visa_type_value = $request->session()->get('current_visa_type_value_filter_inner_list_visastage2');
					$visa_type_valueArray = explode(",",$visa_type_value);
					 $totalvisavalue = array();
					 $rvalue = array(7,8,14,19,20,21);
					$lvalue = array(11,15,22,23,24,25);
				...
				if(!empty($request->session()->get('Visatype_visastage2_filter_inner_list')) && $request->session()->get('Visatype_visastage2_filter_inner_list') != 'All')
				{
					$visa_type = $request->session()->get('Visatype_visastage2_filter_inner_list');
					$visa_type_Array = explode(",",$visa_type);
				...
				if(!empty($request->session()->get('current_visa_details_filter_inner_list_visastage2')) && $request->session()->get('current_visa_details_filter_inner_list_visastage2') != 'All')
				{
					$current_visa_detail = $request->session()->get('current_visa_details_filter_inner_list_visastage2');
					$current_visa_detailArray = explode(",",$current_visa_detail);
				...
				if(!empty($request->session()->get('inprocessallstage2_visastage_status_filter_inner_list')) && $request->session()->get('inprocessallstage2_visastage_status_filter_inner_list') != 'All')
				{
					...
					$onBoardingStatusArraystage2 = $request->session()->get('inprocessallstage2_visastage_status_filter_inner_list');
					 ...
					 if(in_array("visa_c",$visastage_status))
						 {
					 $documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("evisa_start_date","!=",NULL)->whereIn("visa_process_status",array(2,4))->where("backout_status",1)->paginate($paginationValue);
					$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->whereIn("visa_process_status",array(2,4))->where("evisa_start_date","!=",NULL)->where("backout_status",1)->get()->count();						
				}
				else{
					echo "h2";
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("evisa_start_date","!=",NULL)->where("visa_process_status",2)->where("backout_status",1)->paginate($paginationValue);
					 $reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("visa_process_status",2)->where("evisa_start_date","!=",NULL)->where("backout_status",1)->get()->count();
					 }
				}
				else{
					echo "h3";
					if(($moltostage2==1 && $molfromstage2==1) || ($evisatostage2==1 && $evisafromstage2==1)){
						echo "ram";
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("backout_status",1)->paginate($paginationValue);
					 $reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("evisa_start_date","!=",NULL)->where("backout_status",1)->get()->count();
				...
				else
				{
					...
					$documentCollectiondetails = DocumentCollectionDetails::where("visa_process_status",2)->where("evisa_start_date","!=",NULL)->where("backout_status",1)->orderByRaw("visa_inprogress_date DESC")->paginate($paginationValue);
	...
	public function getVisaAllFilterData(Request $request){
		...
		$visatype=visaType::where("status",1)->get();
		return view("OnboardingAjax/VisaAllFilterpopup",compact('departmentLists','visastagestatuslist','visatype'));
...
public function documentcollectionfilterBYVisaALL(Request $request)
		{
			...
			$flighttkt='';
			if($request->input('flighttkt')!=''){
			 
			 $flighttkt=implode(",", $request->input('flighttkt'));
			...
			$visatype='';
			if($request->input('visatype')!=''){
			 
			 $visatype=implode(",", $request->input('visatype'));
			}
			...
			$request->session()->put('flighttkt_list_visaprocessvisaall',$flighttkt);
			$request->session()->put('Visatype_visaall_filter_inner_list',$visatype);
		...
		public function resetdocumentcollectionfilterBYVisaALL(Request $request){
			...
			$request->session()->put('flighttkt_list_visaprocessvisaall','');
			$request->session()->put('Visatype_visaall_filter_inner_list','');
...
public function documentcollectionfilterBYVisaapprove(Request $request)
		{
			...
			$request->session()->put('flighttkt_list_Visaapprove',$flighttkt);
		...
		public function resetdocumentcollectionfilterBYVisaapprove(Request $request){
			...
			$request->session()->put('flighttkt_list_Visaapprove','');
		}
public function getVisaInProcessFilterData(Request $request){
		...
		$documentColectionId = DocumentCollectionDetails::whereIn("visa_process_status",array(2,4))->get();
				...
				$visatype=visaType::where("status",1)->get();
		return view("OnboardingAjax/VisaInProcessFilterpopup",compact('visatype','visastagearray','departmentLists','visastagestatuslist'));
...
public function documentcollectionfilterBYvisainprocessall(Request $request)
		{
			...
			$request->session()->put('current_visa_status_filter_inner_list_pendingvisa',$currentVisa_status);
			$request->session()->put('current_visa_details_filter_inner_list_pendingvisa',$current_visa_details);
			$request->session()->put('current_visa_type_value_filter_inner_list_pendingvisa',$visa_type_value);
			$request->session()->put('flighttkt_list_visainprocessall',$flighttkt);
			$request->session()->put('Visatype_visainprocessall_filter_inner_list',$visatype);
		...
		public function resetdocumentcollectionfilterBYvisainprocess(Request $request){
			...
			$request->session()->put('current_visa_status_filter_inner_list_pendingvisa','');
			$request->session()->put('current_visa_details_filter_inner_list_pendingvisa','');
			$request->session()->put('current_visa_type_value_filter_inner_list_pendingvisa','');
			$request->session()->put('flighttkt_list_visainprocessall','');
			$request->session()->put('Visatype_visainprocessall_filter_inner_list','');
	...
	public function getVisaInProcessStage1FilterData(Request $request){
		...
		$documentColectionId = DocumentCollectionDetails::whereIn("visa_process_status",array(2,4))->where("visa_stage_steps","Stage1")->get();
				...
				$visatype=visaType::where("status",1)->get();
		return view("OnboardingAjax/VisaInProcessStage1Filterpopup",compact('visatype','visastagearray','departmentLists','visastagestatuslist'));
	...
	public function documentcollectionfilterBYvisainprocessallstage1(Request $request)
		{
			...
			$request->session()->put('current_visa_status_filter_inner_list_visastage1',$currentVisa_status);
			$request->session()->put('current_visa_details_filter_inner_list_visastage1',$current_visa_details);
			$request->session()->put('current_visa_type_value_filter_inner_list_visastage1',$visa_type_value);
			$request->session()->put('flighttkt_list_visastage1',$flighttkt);
			$request->session()->put('Visatype_visastage1_filter_inner_list',$visatype);
		...
		public function resetdocumentcollectionfilterBYvisainprocessstage1(Request $request){
			...
			$request->session()->put('current_visa_status_filter_inner_list_visastage1','');
			$request->session()->put('current_visa_details_filter_inner_list_visastage1','');
			$request->session()->put('current_visa_type_value_filter_inner_list_visastage1','');
			$request->session()->put('flighttkt_list_visastage1','');
			$request->session()->put('Visatype_visastage1_filter_inner_list','');
	...
	public function getVisaInProcessStage2FilterData(Request $request){
		...
		$documentColectionId = DocumentCollectionDetails::whereIn("visa_process_status",array(2,4))->where("visa_stage_steps","Stage2")->get();
				...
				$visatype=visaType::where("status",1)->get();
		return view("OnboardingAjax/VisaInProcessStage2Filterpopup",compact('visatype','visastagearray','departmentLists','visastagestatuslist'));
	...
	public function documentcollectionfilterBYvisainprocessallstage2(Request $request)
		{
			...
			$request->session()->put('current_visa_status_filter_inner_list_visastage2',$currentVisa_status);
			$request->session()->put('current_visa_details_filter_inner_list_visastage2',$current_visa_details);
			$request->session()->put('current_visa_type_value_filter_inner_list_visastage2',$visa_type_value);
			$request->session()->put('flighttkt_list_visastage2',$flighttkt);
			$request->session()->put('Visatype_visastage2_filter_inner_list',$visatype);
		...
		public function resetdocumentcollectionfilterBYvisainprocessstage2(Request $request){
			...
			$request->session()->put('datefrom_visainprocessallstage2_moldate_filter_inner_list','');
			$request->session()->put('dateto_visainprocessallstage2_moldate_filter_inner_list','');
			...
			$request->session()->put('current_visa_status_filter_inner_list_visastage2','');
			$request->session()->put('current_visa_details_filter_inner_list_visastage2','');
			$request->session()->put('current_visa_type_value_filter_inner_list_visastage2','');
			$request->session()->put('flighttkt_list_visastage2','');
			$request->session()->put('Visatype_visastage2_filter_inner_list','');
	...
	public function	documentcollectionSavevisaexpdate(Request $request){
		...
		$detailsObj = DocumentCollectionDetails::find($docId);
		$detailsObj->visa_expiry_date = date("Y-m-d",strtotime($date)); 
		$documentCollect = DocumentCollectionDetails::where("id",$docId)->first();
				if($documentCollect->current_visa_status=="Inside Country" && $documentCollect->sort_dateBY=="Change Status" || $documentCollect->sort_dateBY==NULL){
					$detailsObj->sort_date =date("Y-m-d",strtotime($date));
					$detailsObj->sort_dateBY ="Change Status";
					$detailsObj->email_sent =0;	
				}
		$detailsObj->save();
		$existDocument = DocumentCollectionDetailsValues::where("document_collection_id",$docId)->where("attribute_code",66)->first();
				if($existDocument != '')
				{
					$objDocument= DocumentCollectionDetailsValues::find($existDocument->id);
				}
				...
				$objDocument->document_collection_id = $docId;
				$objDocument->attribute_code = 66;
				$objDocument->attribute_value = $date;
				$objDocument->save();
		echo "done";
	}
 ...
 */			$whereraw1 = '';
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('current_visa_status_filter_inner_list_deadline')) && $request->session()->get('current_visa_status_filter_inner_list_deadline') != 'All')
				{
					$current_visa_status = $request->session()->get('current_visa_status_filter_inner_list_deadline');
					$current_visa_statusArray = explode(",",$current_visa_status);
				...
				if(!empty($request->session()->get('current_visa_details_filter_inner_list_deadline')) && $request->session()->get('current_visa_details_filter_inner_list_deadline') != 'All')
				{
					$current_visa_detail = $request->session()->get('current_visa_details_filter_inner_list_deadline');
					$current_visa_detailArray = explode(",",$current_visa_detail);
				...
				if(!empty($request->session()->get('filter_inner_list_deadline_type')))
				{
					$deadline_type = $request->session()->get('filter_inner_list_deadline_type');
					$todayDate = date('Y-m-d');
					if($deadline_type=="Not missed"){
						if($whereraw == '')
						{
					...
					$todayDate = date('Y-m-d');
					$request->session()->put('filter_inner_list_deadline_type',"Not missed");
					if($whereraw == '')
						{
							$whereraw = 'sort_date>= "'.$todayDate.'"';
						}
				...
				if(!empty($request->session()->get('filter_inner_list_deadline_sortby')) && $request->session()->get('filter_inner_list_deadline_sortby') != 'All')
				{
					$deadlinetypestatus = $request->session()->get('filter_inner_list_deadline_sortby');
					
					 if($whereraw == '')
					{
						$whereraw = 'sort_dateBY= "'.$deadlinetypestatus.'"';
					}
					else
					{
						$whereraw .= ' And sort_dateBY= "'.$deadlinetypestatus.'"';
					}
				}
		...
		//echo $whereraw;
				...
				{
					//echo "hello";exit;
					$documentCollectiondetaildata = DocumentCollectionDetails::whereIn("visa_process_status",array(0,1,2))->where("visa_documents_status",2)->where("offer_letter_onboarding_status",2)->where("backout_status",1)->whereRaw($whereraw)->orderByRaw("-sort_date DESC")->get();
				...
				{
					//echo "hello1";
					$documentCollectiondetaildata = DocumentCollectionDetails::whereIn("visa_process_status",array(0,1,2))->where("visa_documents_status",2)->where("offer_letter_onboarding_status",2)->where("backout_status",1)->orderByRaw("-sort_date DESC")->get();
				...
				$finaldeadlinearray=array_unique($finalarray);
				$documentCollectiondetails= DocumentCollectionDetails::whereIn("id",$finaldeadlinearray)->orderByRaw("-sort_date DESC")->paginate($paginationValue);
				$reportsCountdeadline = DocumentCollectionDetails::whereIn("id",$finaldeadlinearray)->get()->count();
				$documentCollectiondetails->setPath(config('app.url/listingPageonboardingDeadline'));
				
				
				$documentCollectiondetails->setPath(config('app.url/listingPageonboardingDeadline'));
		 
		return view("OnboardingAjax/listingPageonboardingDeadline",compact('documentCollectiondetails','reportsCountdeadline','filterList','paginationValue','selectedFilter'));
	   }
...
public function getVisaInProcessCompleteFilterData(Request $request){
		...
		$documentColectionId = DocumentCollectionDetails::whereIn("visa_process_status",array(2,4))->where("visa_stage_steps","Stage2")->get();
				...
				$visatype=visaType::where("status",1)->get();
		return view("OnboardingAjax/VisaInProcessCompletFilterpopup",compact('visatype','visastagearray','departmentLists','visastagestatuslist'));
	...
	public function documentcollectionfilterBYvisaComplete(Request $request)
		{
			...
			$request->session()->put('current_visa_status_filter_inner_list_visascomplete',$currentVisa_status);
			$request->session()->put('current_visa_details_filter_inner_list_visascomplete',$current_visa_details);
			$request->session()->put('current_visa_type_value_filter_inner_list_visacomplete',$visa_type_value);
			$request->session()->put('flighttkt_list_visacomplete',$flighttkt);
			$request->session()->put('Visatype_visacomplete_filter_inner_list',$visatype);
		...
		public function resetdocumentcollectionfilterBYvisaComplete(Request $request){
			...
			$request->session()->put('current_visa_status_filter_inner_list_visascomplete','');
			$request->session()->put('current_visa_details_filter_inner_list_visascomplete','');
			$request->session()->put('current_visa_type_value_filter_inner_list_visacomplete','');
			$request->session()->put('flighttkt_list_visacomplete','');
			$request->session()->put('Visatype_visacomplete_filter_inner_list','');
...
public static function getReminderData($docId){
		 ...
		 $Documentdata = DocumentCollectionDetails::where("id",$docId)->first(); 
		if($Documentdata != '' && $Documentdata->stamping_deadline!='')
		   {
			   
			   $date = $Documentdata->stamping_deadline;
	...
	public function documentcollectionbyfilterBYDateOnboardDeadLine(Request $request)
		{
			...
			//$request->session()->put('filter_inner_list_deadline_type','');
			$type = $request->deadline_types;//exit;
			$typestatus = $request->deadline_types_status;//exit;
			$currentVisa_status='';
			if($request->input('currentVisa_status')!=''){
			...
			$request->session()->put('current_visa_status_filter_inner_list_deadline',$currentVisa_status);
			$request->session()->put('current_visa_details_filter_inner_list_deadline',$current_visa_details);
			$request->session()->put('filter_inner_list_deadline_type',$type);
			$request->session()->put('filter_inner_list_deadline_sortby',$typestatus);
			 //return  redirect('listingPageonboarding');	
		}
		public function resetListDataFilterDateOnboardDeadLine(Request $request){
			...
			$request->session()->put('current_visa_status_filter_inner_list_deadline','');
			$request->session()->put('current_visa_details_filter_inner_list_deadline','');
			$request->session()->put('filter_inner_list_deadline_type','');
			$request->session()->put('filter_inner_list_deadline_sortby','');
		}
 public function exportDocReportDeadLine(Request $request){
			...
			$sheet->setCellValue('Q'.$indexCounter, strtoupper('proposed salary'))->getStyle('Q'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('R'.$indexCounter, strtoupper('Evisa Date'))->getStyle('R'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('S'.$indexCounter, strtoupper('Total Gross Salary'))->getStyle('S'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('T'.$indexCounter, strtoupper('Change Status'))->getStyle('T'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('U'.$indexCounter, strtoupper('Stamping Deadline'))->getStyle('U'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('V'.$indexCounter, strtoupper('Date of Entry'))->getStyle('V'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('W'.$indexCounter, strtoupper('Reminder'))->getStyle('W'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('X'.$indexCounter, strtoupper('CURRENT VISA STAGE'))->getStyle('X'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('Y'.$indexCounter, strtoupper('CURRENT VISA STAGE DATE'))->getStyle('Y'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('Z'.$indexCounter, strtoupper('Notes'))->getStyle('Z'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AA'.$indexCounter, strtoupper('Missed/Not missed'))->getStyle('AA'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AB'.$indexCounter, strtoupper('Upcoming Deadline'))->getStyle('AB'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AC'.$indexCounter, strtoupper('Fligh Ticket'))->getStyle('AC'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			...
			foreach ($selectedId as $sid) {
				...
				if($misData->stamping_deadline != '')
				 ...
				 $todayDate = date('Y-m-d');
				 $finaldate=date("Y-m-d",strtotime(str_replace("/","-",$misData->sort_date)));
				...
				$sheet->setCellValue('Q'.$indexCounter, "AED ".$proposedsalary)->getStyle('Q'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('R'.$indexCounter, $evisa)->getStyle('R'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('S'.$indexCounter, $molsalary)->getStyle('S'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('T'.$indexCounter, $changestatus)->getStyle('T'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('U'.$indexCounter, $Stampingdeadline)->getStyle('U'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('V'.$indexCounter, $dateofentry)->getStyle('V'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('W'.$indexCounter, $Reminder)->getStyle('W'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('X'.$indexCounter, $visastagename)->getStyle('X'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('Y'.$indexCounter, $visastagedate)->getStyle('Y'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('Z'.$indexCounter, $misData->notes)->getStyle('Z'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AA'.$indexCounter, $missed)->getStyle('AA'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AB'.$indexCounter, $misData->sort_dateBY)->getStyle('Z'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AC'.$indexCounter, $fligh)->getStyle('AC'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
...
public function ListDataFilterBackout(Request $request)
	...
	public function getOfferletterEMPOfferletterskill(Request $req)
	   {
		  
		$docId =  $req->docId;
		$docData = DocumentCollectionDetails::where("id",$docId)->first();
		
		return view("OnboardingAjax/OfferletterSkillpopupUpdate",compact('docId','docData'));
	   }
	public function SubmitOfferletterSkillPost(Request $request){
			...
			$expecteddateobj->skill =$skill; 
			if($expecteddateobj->save()){
				$logObj = new DocumentCollectionDetailsLog();
				$logObj->document_id =$docid;
				$logObj->created_by=$request->session()->get('EmployeeId');
				$logObj->title ="Update Skill";
				$logObj->response ="Update Skill";
				$logObj->category ="Offer Letter";
				$logObj->save();
			}
			echo "Updated";
		}
	public function getOfferletterEMPOfferletterattestation(Request $req)
	   {
		  
		$docId =  $req->docId;
		$docData = DocumentCollectionDetails::where("id",$docId)->first();
		
		return view("OnboardingAjax/OfferletterattestationpopupUpdate",compact('docId','docData'));
	   }
	public function SubmitOfferletterattestationPost(Request $request){
			...
			$expecteddateobj->attestation =$attestation; 
			if($expecteddateobj->save()){
				$logObj = new DocumentCollectionDetailsLog();
				$logObj->document_id =$docid;
				$logObj->created_by=$request->session()->get('EmployeeId');
				$logObj->title ="Update attestation";
				$logObj->response ="Update attestation";
				$logObj->category ="Offer Letter";
				$logObj->save();
			}
			echo "Updated";
		}
public static function getCountry($cont){
	$cList = WpCountries::where("id",$cont)->first();
	if($cList!=''){
		return $cList->name;
	}
	else{
...
public function getdocNotedata($docid,$catname){
		
		$docData = DocumentCollectionDetails::where("id",$docid)->first();
		
		return view("OnboardingAjax/docNotes",compact('docid','docData','catname'));	
}
public function addDocumentsNotes(Request $request){
			$docid=$request->input('docId');
			$notes=$request->input('special_notes');
			$expecteddateobj = DocumentCollectionDetails::find($docid);
			
			$expecteddateobj->notes =$notes; 
			if($expecteddateobj->save()){
				$logObj = new DocumentCollectionDetailsLog();
				$logObj->document_id =$docid;
				$logObj->created_by=$request->session()->get('EmployeeId');
				$logObj->title ="Update Notes";
				$logObj->response ="Update Notes";
				$logObj->category ="EMP Onboarding";
				$logObj->save();
			}
			
		}
public function getdocVisaHistory($docid){
		
		$docData = VisaProcessHistroy::where("document_id",$docid)->get();
		
		return view("OnboardingAjax/Visahistorypopup",compact('docid','docData'));	
}
public static function getvisatypehistory($id){
...
public static function getvisatypestagehistory($stageid){
			$visa_stage = VisaStage::where("id",$stageid)->first();
			  if($visa_stage!=''){
				
				return $visa_stage->stage_name;			 
			  }
			  else{
				  return "";
			  }
		}
public static function getpvisadata($docid){
$docData = VisaProcessHistroy::where("document_id",$docid)->first();
if($docData!=''){
	return 1;
...
public static function getpvisatotalcost($docid){
$docData = VisaProcessHistroy::where("document_id",$docid)->sum('cost');
if($docData>0){
	return $docData;
...
public static function getpvisatotalfine($docid){
$docData = VisaProcessHistroy::where("document_id",$docid)->sum('cost_fine');
if($docData>0){
	return $docData;
 ...
 public function Stampingdeadline_export(Request $request){
			...
			$sheet->setCellValue('AB'.$indexCounter, strtoupper('Upcoming Deadline'))->getStyle('AB'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AC'.$indexCounter, strtoupper('Fligh Ticket'))->getStyle('AC'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('AD'.$indexCounter, strtoupper('Visa Type'))->getStyle('AD'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');

			$sn = 1;
			$documentCollectiondetaildata=DocumentCollectionDetails::whereIn("visa_process_status",array(0,1,2))->where("visa_documents_status",2)->where("offer_letter_onboarding_status",2)->where("backout_status",1)->where("sort_dateBY","Stamping Deadline")->orderByRaw("-sort_date DESC")->get();
			$finaldocidarray=array();
				foreach($documentCollectiondetaildata as $_documentCollectiondetails){
					$EMPdetailsdata =  Employee_details::where("document_collection_id",$_documentCollectiondetails->id)->first();
					if($EMPdetailsdata!='' && $EMPdetailsdata->offline_status==2){
				...
				$selectedId= DocumentCollectionDetails::whereIn("id",$finaldocidarray)->orderByRaw("-sort_date DESC")->get();
			foreach ($selectedId as $sidd) {
				...
				if($misData->stamping_deadline != '')
				 ...
				 $visaProcessstage = Visaprocess::where("document_id",$misData->id)->orderBy('id','DESC')->first();
					...
					if($visaProcessstage!=''){
						$visastageId=$visaProcessstage->visa_stage;
						$visatypeId=$visaProcessstage->visa_type;
						$visaTypeData = VisaStage::where("id",$visastageId)->first();
						$visaTypevalue = visaType::where("id",$visatypeId)->first();
						$visatypename=$visaTypevalue->title;
						$visastagename=$visaTypeData->stage_name;
						if($visaProcessstage->closing_date!=''){
						$visastagedate=date("d-M-Y",strtotime(str_replace("/","-",$visaProcessstage->closing_date)));
						}
				...
				$sheet->setCellValue('AB'.$indexCounter, $misData->sort_dateBY)->getStyle('Z'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AC'.$indexCounter, $fligh)->getStyle('AC'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('AD'.$indexCounter, $visatypename)->getStyle('AD'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			...
			$spreadsheet->getActiveSheet()->getStyle('A1:AD1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('cbddf7');
				...
				$logObj->user_id =$request->session()->get('EmployeeId');
				$logObj->download_date =date("Y-m-d");
				$logObj->tilte ="Onboard-Stampping";					
				$logObj->save();
				$writer = new Xlsx($spreadsheet);
				$writer->save(public_path('uploads/exportEmp/'.$filename));	
				echo $filename;
 ...
 public function ChangeStatus_export(Request $request){
			...
			$sn = 1;
			$documentCollectiondetaildata = DocumentCollectionDetails::whereIn("visa_process_status",array(0,1,2))->where("visa_documents_status",2)->where("offer_letter_onboarding_status",2)->where("backout_status",1)->where("sort_dateBY","Change Status")->orderByRaw("-sort_date DESC")->get();$finaldocidarray=array();
				foreach($documentCollectiondetaildata as $_documentCollectiondetails){
					$EMPdetailsdata =  Employee_details::where("document_collection_id",$_documentCollectiondetails->id)->first();
					if($EMPdetailsdata!='' && $EMPdetailsdata->offline_status==2){
			...
			$spreadsheet->getActiveSheet()->getStyle('A1:AD1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('cbddf7');
				...
				$logObj->user_id =$request->session()->get('EmployeeId');
				$logObj->download_date =date("Y-m-d");
				$logObj->tilte ="Onboard-Change Status";					
				$logObj->save();
				$writer = new Xlsx($spreadsheet);
				$writer->save(public_path('uploads/exportEmp/'.$filename));	
				echo $filename;
 ...
 public function DateEntry_export(Request $request){
			...
			$spreadsheet->getActiveSheet()->getStyle('A1:AD1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('cbddf7');
				...
				$logObj->user_id =$request->session()->get('EmployeeId');
				$logObj->download_date =date("Y-m-d");
				$logObj->tilte ="Onboard-Date Of Entry";					
				$logObj->save();
				$writer = new Xlsx($spreadsheet);
				$writer->save(public_path('uploads/exportEmp/'.$filename));	
				echo $filename;
...
public function getTicketuploadData($empId,$index)
	{
		$empdata = DocumentCollectionDetails::where("id",$empId)->first();
		
		return view("OnboardingAjax/TicketUpload",compact('empId','empdata','index'));
	}
public function FlightTicketUploadPost(Request $request){
			...
			$detailsObj = DocumentCollectionDetails::find($rowId);
			$detailsObj->ticket_status = 2;
			$detailsObj->ticket_upload_date = $request->input('ticket_upload_date');
			if($newFileName!=''){
			$detailsObj->ticket_doc =$newFileName;
			}
			
			$detailsObj->save();
			echo "Save Data..";
		}
public function getOfferletterEMPOfferletterCustodyData($empId)
	{
		$docData = DocumentCollectionDetails::where("id",$empId)->first();
		
		return view("OnboardingAjax/OfferletterdegreecustodypopupUpdate",compact('empId','docData'));
	}
public function SubmitOfferletterCustodyDataPost(Request $request){
			
			
			$docid=$request->input('documentCollectionID');
			
			$degree_custody=$request->input('degree_custody');
			
				
			$expecteddateobj = DocumentCollectionDetails::find($docid);
			
			$expecteddateobj->degree_custody =$degree_custody; 
			if($expecteddateobj->save()){
				$logObj = new DocumentCollectionDetailsLog();
				$logObj->document_id =$docid;
				$logObj->created_by=$request->session()->get('EmployeeId');
				$logObj->title ="Update degree custody";
				$logObj->response ="Update degree custody";
				$logObj->category ="Offer Letter";
				$logObj->save();
			}
			echo "Updated";
		...
		public function changeOnboardStatusActionProcess(Request $request)
		{
			...
			{
				$successmsg='';
				$failedmsg="To Update Visa Status, Please firstly login in Portal and then again click on Update Visa Status through Email.";
				return view("OnboardingAjax/email_process",compact('failedmsg','successmsg'));
			}
	...
	$data = Visaprocess::where("document_id",$id)->orderBy('id','DESC')->first();
	  ...
	  if($data != '')
	  {
		  $visa_stage = visaType::where("id",$data->visa_type)->first();
		  if($visa_stage!=''){
			return "<p>Current Visa Type: ".$visa_stage->title."</p>";
		  }else{
			  return '';
		  }
	  }
	...
	public function getdocVisaStatus($docid,$catname){
		
		$docData = DocumentCollectionDetails::where("id",$docid)->first();
		
		return view("OnboardingAjax/docVisaStatus",compact('docid','docData','catname'));	
}
public function addDocumentsCurrentStatus(Request $request){
			$docid=$request->input('docId');
			$current_visa_status=$request->input('current_visa_status');
			$expecteddateobj = DocumentCollectionDetails::find($docid);
			
			$expecteddateobj->current_visa_status =$current_visa_status; 
			if($expecteddateobj->save()){
				$logObj = new DocumentCollectionDetailsLog();
				$logObj->document_id =$docid;
				$logObj->created_by=$request->session()->get('EmployeeId');
				$logObj->title ="Update Visa status from Widget";
				$logObj->response ="Update Visa status from Widget";
				$logObj->category ="EMP Onboarding";
				$logObj->save();
			}
			
		}
public function ResetGlobalFilters(Request $request){
	
$this->documentresetfilter($request);
$this->resetListDataFilterNotification($request);
$this->resetListDataFilterDateALL($request);
$this->resetListDataFilterDateofferletter($request);
$this->resetListDataFilterDateofferletterComplete($request);
$this->resetdocumentcollectionfilterBYVisaALL($request);
$this->resetdocumentcollectionfilterBYVisarequest($request);
$this->resetdocumentcollectionfilterBYVisaapprove($request);
$this->resetdocumentcollectionfilterBYvisainprocess($request);
$this->resetdocumentcollectionfilterBYvisainprocess($request);
$this->resetdocumentcollectionfilterBYvisainprocessstage1($request);
$this->resetdocumentcollectionfilterBYvisainprocessstage2($request);
$this->resetdocumentcollectionfilterBYvisaComplete($request);
$this->resetListDataFilterDateOnboard($request);
$this->resetListDataFilterBackout($request);
$this->resetListDataFilterDateOnboardDeadLine($request);
echo "done";
...
public static function getinsuranceappflag($docids)
{

          $insuranceappflagData = DocumentCollectionDetailsValues::where("document_collection_id",$docids)->where("attribute_code",106)->first(); 
					if($insuranceappflagData != '')
					   {     // $insuranceFlag="Yes";
							$insuranceappflag= date("d M Y",strtotime($insuranceappflagData->attribute_value)) ;
					   }
					   else
					   {     // $insuranceFlag="No";
						     $insuranceappflag= "--";
			...
			public static function getmedicaldate($docmeducal)
          {


	  $insurancecardflagData = DocumentCollectionDetailsValues::where("document_collection_id",$docmeducal)->where("attribute_code",107)->first(); 
	   if($insurancecardflagData != '')
	   {     /// $insurancecardFlag="Yes";
			$insurancecardflag= date("d M Y",strtotime($insurancecardflagData->attribute_value)) ;
	   }
	   else
	   {      //$medicalFlag="No";
			$insurancecardFlag= "--";
					...
					public function listingPageonboardinglistingevisabackout(Request $request)
					{
			  ...
			  if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
						 ...
						 if(!empty($request->session()->get('onboading_page_limit')))
							 ...
							 */
							 ...
							 {
								 //echo "hello";exit;
								 $documentCollectiondetailsdata = DocumentCollectionDetails::whereRaw($whereraw)->where("onboard_status",1)->where("visa_process_status",2)->where("backout_status",1)->get();
								 //$departmentLists =  Department::where("status",1)->whereRaw($whereraw)->orderBy("id","DESC")->get();
								 //$productDetails =  Product::where("status",1)->whereRaw($whereraw)->orderBy('id','DESC')->get();
								 //$designationDetails =  Designation::where("status",1)->whereRaw($whereraw)->orderBy('id','DESC')->get();
							 }
							 else
							 {
								...
								$documentCollectiondetailsdata = DocumentCollectionDetails::where("onboard_status",1)->where("visa_process_status",2)->where("backout_status",1)->get();
							 ...
							 $visadatarraydate=array();
                    foreach ($documentCollectiondetailsdata as $documentCollectiondetailsnewdata){

                    $visastagepermission=VisaStagePermission::where("id",3)->first();
                  $visadatarray=explode(',',$visastagepermission->stageid);
				 $visaproccessstatus= Visaprocess::whereIN("visa_stage", $visadatarray)->where('document_id',$documentCollectiondetailsnewdata->id)->first();
                
				if($visaproccessstatus!=''){

                     $currentdate=date("Y-m-d");
					 $visaproccessclosing= $visaproccessstatus->closing_date;
					 $closingdate=date("Y-m-d", strtotime($visaproccessclosing));
					 ...
					 //print_r($closingdate);exit;
					  ...
					  if(strtotime($currentdate)> strtotime($newdate)){
						//echo "yes";
						$visadatarraydate[]=$documentCollectiondetailsnewdata->id;
				 ...
				 $documentCollectiondetails = DocumentCollectionDetails::whereIn("id",$visadatarraydate)->where("visa_process_status",2)->where("backout_status",1)->paginate($paginationValue);
							 ...
							 $departmentLists =  Department::where("status",1)->orderBy("id","DESC")->get();
								 ...
								 $reportsCount = DocumentCollectionDetails::whereIn("id",$visadatarraydate)->get()->count();
							
							 $documentCollectiondetails->setPath(config('app.url/listingPageonboardinglistingevisabackout'));
							 
							 
					 
					  $salaryBreakUpdetails =  SalaryBreakup::where("status",1)->orderBy("id","DESC")->get();
					 return view("OnboardingAjax/listingPageonboardinglistingevisabackout",compact('departmentLists','productDetails','designationDetails','documentCollectiondetails','reportsCount','filterList','salaryBreakUpdetails','paginationValue','selectedFilter'));
					}
