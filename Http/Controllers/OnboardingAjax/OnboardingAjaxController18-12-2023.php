						...
						if($designationOnboard  != '' && $designationOnboard != NULL)
							{
								...
								if($designationMod != '')
								{
									...
									$employeeAttribute->save();
										...
										$existDocumentemail = DocumentCollectionDetailsValues::where("document_collection_id",$documentCollectionId)->where("attribute_code",94)->first();
 ...
 */			$whereraw1 = '';
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if($whereraw != '')
				{
					if(($evisato==1 && $evisarom==1) || ($moldatefrom==1 && $moldateto==1)){
					$whereraw .= 'AND (ok_visa =3 OR ok_visa =2)';
					}else{
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if($whereraw != '')
				{
					...
					$reportsCount = DocumentCollectionDetails::whereRaw($whereraw)->where("visa_stage_steps","Stage1")->whereIn("visa_process_status",array(2,4))->where("backout_status",1)->get()->count();
						...
						$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("visa_stage_steps","Stage1")->where("backout_status",1)->paginate($paginationValue);
		...
		$filterList['visa_process_status'] = '';
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('inprocessallstage2_visastage_status_filter_inner_list')) && $request->session()->get('inprocessallstage2_visastage_status_filter_inner_list') != 'All')
				{
					...
					$documentCollectiondetails = DocumentCollectionDetails::orderByRaw("visa_inprogress_date DESC")->whereRaw($whereraw)->where("visa_stage_steps","Stage2")->where("backout_status",1)->paginate($paginationValue);
