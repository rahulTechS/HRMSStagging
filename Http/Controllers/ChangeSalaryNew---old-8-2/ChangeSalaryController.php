...
class ChangeSalaryController extends Controller
{
	...
	public function Index(Request $req)
	{
		...
		return view("ChangeSalaryNew/Index",compact('ReasonsForLeavingDetails','departmentLists','tL_details','empId','Designation'));
	}
	...
	{
		$changesalaryRequest = ChangeSalary::orderBy('id','DESC')->get();

		//return $changesalaryRequest;

		foreach($changesalaryRequest as $salaryRequest)
		{
			if($salaryRequest->request_type==1 && $salaryRequest->incrementstatus==1)
			{
				$empDetailsModel = Employee_attribute::where("emp_id",$salaryRequest->emp_id)->where("attribute_code","total_gross_salary")->first();

				if($empDetailsModel->attribute_values!=$salaryRequest->newsalary)
				{
					$empDetailsModel->attribute_values = $salaryRequest->newsalary;
					$empDetailsModel->save();
				}

			}

			if($salaryRequest->request_type==2 && $salaryRequest->molstatus==1)
			{
				$empDetailsModel = Employee_attribute::where("emp_id",$salaryRequest->emp_id)->where("attribute_code","total_gross_salary")->first();
			...
			if($salaryRequest->request_type==3 && $salaryRequest->incrementstatus==1 && $salaryRequest->molstatus==1)
			{
				$empDetailsModel = Employee_attribute::where("emp_id",$salaryRequest->emp_id)->where("attribute_code","total_gross_salary")->first();
...
if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
	...
	public function addFullandFinalSettelement($offboardingId)
	{
		...
		$empData = Employee_details::orderBy("id","DESC")->where("offline_status",1)->where('emp_id',$offboardingId)->first();
		$requestTypes = SalaryRequest::where("status",1)->get();
		return view("ChangeSalaryNew/SettelementForm",compact('empData'));
	}
...
public static function getsSttelementHr($id)
	...
	public static function getsSttelementPayment($id)
		...
		public function getleavingTypePopupData()
		{
			$requestTypes = SalaryRequest::where("status",1)->get();			
...
if(!empty($request->session()->get('ReasonofAttrition_empoffboard_filter_list')) && $request->session()->get('ReasonofAttrition_empoffboard_filter_list') != 'All')
			...
			if($whereraw != '')
			{
				...
				// ->orWhere('change_salary_request.molstatus', 1)
				// ->whereRaw($whereraw)
