			...
			foreach ($selectedId as $sid) {
			  ...
			  if($attr != '')
			  {
				...
				if($hrstatus == '' && $financestatus == ''){
					 $status="<p>F&F Calculation Pending </p>";					
				}else if($paymentstatus == 1){
					 $status="<p>Amount Confirmed</p>";
				}else if($paymentstatusdispute == 1 && $paymentstatus != 1){
					 $status="<p>F&F Calculation Disputed</p>";
				}else if($paymentpaid==1){
					$status="<p>F&F Calculation Paid</p>";
				}else{
					 $status="<p>F&F Calculation Pending </p>";	
				}
					if($misData->condition_leaving == 4){	
					$status1=" <p>Visa Cancellation Pending </p>";
				    }else if($misData->condition_leaving == 5){
					$status1= "<p>Visa Cancellation Complete </p>";
					}else{
					$status1='';
					}
				 ...
				 if($lwd!=''){
					...
					$days=0; 
						 ...
						 if($days>=30){
						$status2="<p>Last Working Day Complete </p>";
						 }else{
						$status2="<p>Last Working Day Incomplete </p>";
						 }
						
					if($misData->exit_interview_status == '' || $misData->exit_interview_status ==NULL){
					 $status3="<p>Questionnaire Pending</p>";
					}else if($misData->exit_interview_status == 1){
						$status3="<p>Questionnaire Complete and Waiting Interview</p>";
					}else if($misData->exit_interview_status == 2){
					$status3="Interview Complete";
					}	
					if($misData->condition_leaving == 1){
					$status4="<p>Condition for Leaving Not Selected</p>";
					}else{
					$status4="<p>Condition for Leaving Selected</p>";
					}
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('empoffboard_status_filter_list')) && $request->session()->get('empoffboard_status_filter_list') != 'All')
				...
				if($whereraw == '')
					...
					{
						$whereraw .= ' And last_working_day_resign is not null And  last_working_day_terminate is not null ';
					}
