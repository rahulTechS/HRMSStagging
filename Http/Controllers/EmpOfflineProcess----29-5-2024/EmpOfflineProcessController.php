 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
			...
			if(!empty($request->session()->get('empoffboard_status_filter_list')) && $request->session()->get('empoffboard_status_filter_list') != 'All')
				...
				if($whereraw != '')
				{
					//echo $whereraw;
					//echo "hello";exit;
					$documentCollectiondetails = EmpOffline::whereRaw($whereraw)->where("condition_leaving",1)->where("retain",2)->where("exit_interview_approved",2)->where("fnf_approved",2)->orderBy("created_at","DESC")->paginate($paginationValue);
					$reportsCount = EmpOffline::whereRaw($whereraw)->where("condition_leaving",1)->where("retain",2)->where("exit_interview_approved",2)->where("fnf_approved",2)->get()->count();
					
				}
				else
				{
					//echo "hello1";
					$documentCollectiondetails = EmpOffline::where("condition_leaving",1)->where("retain",2)->where("exit_interview_approved",2)->where("fnf_approved",2)->orderBy("created_at","DESC")->paginate($paginationValue);
					$reportsCount = EmpOffline::where("condition_leaving",1)->where("retain",2)->where("exit_interview_approved",2)->where("fnf_approved",2)->get()->count();
					//$departmentLists =  Department::where("status",1)->orderBy("id","DESC")->get();
...
if(!empty($request->session()->get('ReasonofAttrition_empoffboard_filter_list')) && $request->session()->get('ReasonofAttrition_empoffboard_filter_list') != 'All')
				...
				{
					//echo "hello";exit;
					$documentCollectiondetails = EmpOffline::whereIn("condition_leaving",array(2,3,4,5,6))->orderBy("id","DESC")->whereRaw($whereraw)->paginate($paginationValue);
				...
				{
					//echo "hello1";
					$documentCollectiondetails = EmpOffline::whereIn("condition_leaving",array(2,3,4,5,6))->orderBy("id","DESC")->paginate($paginationValue);
					//$departmentLists =  Department::where("status",1)->orderBy("id","DESC")->get();
					//$productDetails =  Product::where("status",1)->orderBy('id','DESC')->get();
					//$designationDetails =  Designation::where("status",1)->orderBy('id','DESC')->get();
				}
				...
				if($whereraw != '')
				{
					...
					$reportsCount = EmpOffline::whereRaw($whereraw)->whereIn("condition_leaving",array(2,3,4,5,6))->get()->count();
				}
				else
				{
					$reportsCount = EmpOffline::whereIn("condition_leaving",array(2,3,4,5,6))->get()->count();
				}
...
public function SettelementfromPost(Request $rq)
	{
			...
			if($newFileName!=''){
			$detailsObj->upload_doc =$newFileName;
			}
