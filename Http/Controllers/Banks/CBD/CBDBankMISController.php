...
public function searchCBDBankInnerCurrentMonth(Request $request)
			{
				$requestParameters = $request->input();

				$start_date_creation_Bank = '';
				$end_date_creation_Bank = '';
				$start_date_approval_bank = '';
				$end_date_approval_bank = '';
				$AECB_Status_Bank = '';
				$statusBank = '';
				$ref_no_bank = '';
				$employee_id_Bank = '';
				$sm_managerBankHistory = '';
				$submission_type_Bank = '';

				if(@isset($requestParameters['ref_noBankHistoric']))
				{
					$ref_no_bank = @$requestParameters['ref_noBankHistoric'];
				}

				if(isset($requestParameters['AECB_StatusBankHistoric']))
				{
					$AECB_Status_Bank = @$requestParameters['AECB_StatusBankHistoric'];
				}

				if(isset($requestParameters['statusBankHistoric']))
				{
					$statusBank = @$requestParameters['statusBankHistoric'];
				}
				
				if(isset($requestParameters['employee_idBankHistoric']))
				{
					$employee_id_Bank = @$requestParameters['employee_idBankHistoric'];
				}
				if(isset($requestParameters['sm_managerBankHistoric']))
				{
					$sm_managerBankHistory = @$requestParameters['sm_managerBankHistoric'];
				}

				if(isset($requestParameters['start_date_creationBankHistoric']))
				{
					$start_date_creation_Bank = @$requestParameters['start_date_creationBankHistoric'];
				}
				if(isset($requestParameters['end_date_creationBankHistoric']))
				{
					$end_date_creation_Bank = @$requestParameters['end_date_creationBankHistoric'];
				}
				
				if(isset($requestParameters['start_date_approval_BankHistoric']))
				{
					$start_date_approval_bank = @$requestParameters['start_date_approval_BankHistoric'];
				}
				if(isset($requestParameters['end_date_approval_BankHistoric']))
				{
					$end_date_approval_bank = @$requestParameters['end_date_approval_BankHistoric'];
				}
				
				if(isset($requestParameters['submission_typeBankHistoric']))
				{
					$submission_type_Bank = @$requestParameters['submission_typeBankHistoric'];
				}
				
				$request->session()->put('ref_no_CBD_bankCurrentMonth',$ref_no_bank);
				$request->session()->put('AECB_Status_CBD_bankCurrentMonth',$AECB_Status_Bank);
				$request->session()->put('status_CBD_bankCurrentMonth',$statusBank);
				$request->session()->put('employee_id_CBD_bankCurrentMonth',$employee_id_Bank);
				$request->session()->put('smManager_CBD_bankCurrentMonth',$sm_managerBankHistory);
				$request->session()->put('start_date_creation_CBD_bankCurrentMonth',$start_date_creation_Bank);
				$request->session()->put('end_date_creation_CBD_bankCurrentMonth',$end_date_creation_Bank);
				
				$request->session()->put('start_date_approval_CBD_bankCurrentMonth',$start_date_approval_bank);
				$request->session()->put('end_date_approval_CBD_bankCurrentMonth',$end_date_approval_bank);
				$request->session()->put('master_cbd_search_bankCurrentMonth','');
				$request->session()->put('submission_type_CBD_BankCurrentMonth',$submission_type_Bank);
				return redirect("loadBankContentsCBDCardBankSideCurrentMonth");
			}
			
			public function resetLoginInnerCBDCurrentMonth(Request $request)
			{
				$request->session()->put('ref_no_CBD_bankCurrentMonth','');
				$request->session()->put('AECB_Status_CBD_bankCurrentMonth','');
				$request->session()->put('status_CBD_bankCurrentMonth','');
				$request->session()->put('start_date_creation_CBD_bankCurrentMonth','');
				$request->session()->put('end_date_creation_CBD_bankCurrentMonth','');
				$request->session()->put('start_date_approval_CBD_bankCurrentMonth','');
				$request->session()->put('end_date_approval_CBD_bankCurrentMonth','');
				$request->session()->put('employee_id_CBD_bankCurrentMonth','');
				$request->session()->put('smManager_CBD_bankCurrentMonth','');
				$request->session()->put('submission_type_CBD_BankCurrentMonth','');
				$request->session()->put('master_cbd_search_bankCurrentMonth',2);
				return redirect("loadBankContentsCBDCardBankSideCurrentMonth");
			}
