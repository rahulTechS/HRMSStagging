...
class CBDController extends Controller
{
 ...
 public function editCBDFormEntryPost(Request $request)
 {
			...
			*/
			...
			$entry_objUpdate->approval_date = date("Y-m-d",strtotime($postDataInput['approval_date_cbd']));
			...
			*/
			/*
			*update bank mis for approval Date
			*/
			$ref = trim($postDataInput['ref_no']);
			$refMatch = CBDBankMis::where("ref_no",$ref)->first();
			if($refMatch != '')
			{
				$updateCBDBank = CBDBankMis::find($refMatch->id);
				$updateCBDBank->approval_date = date("Y-m-d",strtotime($postDataInput['approval_date_cbd']));
				$updateCBDBank->save();
			}
			/*
			*update bank mis for approval date
 ...
 public function setPaginationValueCBDCard(Request $request)
 {
 	...
 	public function loadBankContentsCBDCard(Request $request)
	{
		...
		else
		{
				...
				if($request->session()->get('start_date_approval_CBD_internal') != '')
				{
					$start_date_approval_CBD_internal = $request->session()->get('start_date_approval_CBD_internal');			
					$whereRaw .= " AND approval_date >='".date('Y-m-d',strtotime($start_date_approval_CBD_internal))."'";
					$searchValues['start_date_approval_CBD_internal'] = $start_date_approval_CBD_internal;			
				}

				if($request->session()->get('end_date_approval_CBD_internal') != '')
				{
					$end_date_approval_CBD_internal = $request->session()->get('end_date_approval_CBD_internal');			
					$whereRaw .= " AND approval_date <='".date('Y-m-d',strtotime($end_date_approval_CBD_internal))."'";
					$searchValues['end_date_approval_CBD_internal'] = $end_date_approval_CBD_internal;			
				}
		...
		$departmentFormParentTotal = DB::table('department_form_parent_entry')->whereRaw($whereRaw)->whereBetween('application_date', [$startDate, $endDate])->orderby('application_date','DESC')->get()->count();

		$departmentFormParentDetails = DB::table('department_form_parent_entry')->whereRaw($whereRaw)->whereBetween('application_date', [$startDate, $endDate])->orderby('application_date','DESC')->paginate($paginationValue);
		...
		*/
			...
			$masterAttributesStatus = MasterAttribute::select("option_values")->where("attribute_code","status_cbd")->first();
		/*
		*complete Status CBD
		*/
        return view("Banks/CBD/loadBankContentsCBDCard",compact('id','departmentFormDetails','DepartmentNameDetails','departmentFormParentDetails','departmentFormParentTotal','searchValues','employeeIdList','formStatusData','teamData','channel_cbd','status_AECB_cbd','card_type_cbd','masterAttributesStatus'));
	}
	...
	public function searchCBDInternalInner(Request $request)
	{
				...
				if(isset($requestParameters['start_date_approval']))
				{
					$start_date_approval = @$requestParameters['start_date_approval'];
				}
				if(isset($requestParameters['end_date_approval']))
				{
					$end_date_approval = @$requestParameters['end_date_approval'];
				}
				...
				$request->session()->put('start_date_approval_CBD_internal',$start_date_approval);
				$request->session()->put('end_date_approval_CBD_internal',$end_date_approval);
				return redirect("loadBankContentsCBDCard");
	}
	...
	public function resetLoginInnerCBDInternal(Request $request)
	{
			$request->session()->put('ref_no_CBD_internal','');
				...
				$request->session()->put('start_date_approval_CBD_internal','');
				$request->session()->put('end_date_approval_CBD_internal','');
	...
	public function loadBankContentsCBDCardBankSide(Request $request)
	{
		...
		else
		{
					...
					if($request->session()->get('start_date_approval_CBD_bank') != '')
					{
						$start_date_approval_CBD_bank = $request->session()->get('start_date_approval_CBD_bank');			
						$whereRaw .= " AND approval_date >='".date('Y-m-d',strtotime($start_date_approval_CBD_bank))."'";
						$searchValues['start_date_approval_CBD_bank'] = $start_date_approval_CBD_bank;			
					}

					if($request->session()->get('end_date_approval_CBD_bank') != '')
					{
						$end_date_approval_CBD_bank = $request->session()->get('end_date_approval_CBD_bank');			
						$whereRaw .= " AND approval_date <='".date('Y-m-d',strtotime($end_date_approval_CBD_bank))."'";
						$searchValues['end_date_approval_CBD_bank'] = $end_date_approval_CBD_bank;			
					}
		...
		//echo $whereRaw;exit;
		$datasCBDMainCount = CBDBankMis::whereRaw($whereRaw)->whereBetween('creation_date', [$startDate, $endDate])->get()->count();
		
		$datasCBDMain = CBDBankMis::whereRaw($whereRaw)->whereBetween('creation_date', [$startDate, $endDate])->orderBy("creation_date","DESC")->paginate($paginationValue);
	...
	public function loadBankContentsCBDCardBankSideMTD(Request $request)
	{
		...
		$datasCBDMainCount = BankCBDMTD::whereRaw($whereRaw)->whereBetween('CD_OPN_DT', [$startDate, $endDate])->get()->count();
		
		$datasCBDMain = BankCBDMTD::whereRaw($whereRaw)->whereBetween('CD_OPN_DT', [$startDate, $endDate])->orderBy("CD_OPN_DT","DESC")->paginate($paginationValue);
...
public function margeMissingCBD()
{
	$cbdMissingFinalApproved = DepartmentFormEntry::where("form_id",2)
								->where("missing_internal",4)
								->where("form_status","Missing(Approved)")
								->whereNull("marge_missing_status")
								->whereNotNull("ref_no")
								->get();
	/* echo count($cbdMissingFinalApproved);
	exit; */
	/* echo "<pre>";
	//echo count($cbdMissingFinalApproved);
	print_r($cbdMissingFinalApproved);
	exit; */
	foreach($cbdMissingFinalApproved as $_cbdMissing)
	{
		$bankCBDMod = new CBDBankMis();
		$bankCBDMod->ref_no = $_cbdMissing->ref_no;
		$bankCBDMod->sm_manager = $_cbdMissing->team;
		$bankCBDMod->employee_id = $_cbdMissing->emp_id;
		$bankCBDMod->customer_name = $_cbdMissing->customer_name;
		$bankCBDMod->creation_date = $_cbdMissing->application_date;
		$bankCBDMod->card_type = $_cbdMissing->card_type_cbd;
		$bankCBDMod->Status = 'Missing(Approved)';
		$bankCBDMod->missing_marge_status = 1;
		$bankCBDMod->cbd_marging_status = 2;
		if($_cbdMissing->emp_id != '' && $_cbdMissing->emp_id != NULL)
		{
			$bankCBDMod->update_emp_status = 2;
		}
		$bankCBDMod->save();
		/*
		*update Status
		...
		*/
		...
		$cbdMissingMod = DepartmentFormEntry::find($_cbdMissing->id);
		$cbdMissingMod->cbd_marging_status = 2;
		$cbdMissingMod->marge_missing_status = 1;
		$cbdMissingMod->save();
...
public static function getJobFunc($usersId)
{
	$employee_id = Employee::where("id",$usersId)->first()->employee_id;
	if($employee_id != '' && $employee_id != NULL)
	{
		$empData = Employee_details::where("emp_id",$employee_id)->first();
		if($empData != '' && $empData != NULL)
		{
			return $empData->job_function;
		}
		else
		{
			return '';
	...
	public function loadBankContentsCBDCardCurrentMonth(Request $request)
	{
		 //$request->session()->put('paginationValue','');
		$form_id = 2;
		$searchValues = array();
		...
		{
				if(@$request->session()->get('ref_no_CBD_internalCurrentMonth') != '')
				{
					$refNO = $request->session()->get('ref_no_CBD_internalCurrentMonth');
				...
				if(@$request->session()->get('form_status_CBD_internalCurrentMonth') != '')
				{
					$status = $request->session()->get('form_status_CBD_internalCurrentMonth');
					$strStatus = '';
					foreach($status  as $s)
					{
						if($strStatus == '')
				...
				if(@$request->session()->get('team_CBD_internalCurrentMonth') != '')
				{
					$teamL = $request->session()->get('team_CBD_internalCurrentMonth');
					$teamstr = '';
					foreach($teamL  as $lS)
					{
						if($teamstr == '')
				...
				if(@$request->session()->get('emp_id_CBD_internalCurrentMonth') != '')
				{
					$empIds = $request->session()->get('emp_id_CBD_internalCurrentMonth');
					$empStr = '';
					foreach($empIds  as $eid)
					{
						if($empStr == '')
				...
				if(@$request->session()->get('channel_CBD_internalCurrentMonth') != '')
				{
					$channelInternalL = $request->session()->get('channel_CBD_internalCurrentMonth');
					$channelInternalstr = '';
					foreach($channelInternalL  as $cL)
					{
						if($channelInternalstr == '')
				...
				if(@$request->session()->get('status_AECB_CBD_internalCurrentMonth') != '')
				{
					$statusAECBL = $request->session()->get('status_AECB_CBD_internalCurrentMonth');
					$statusAECBstr = '';
					foreach($statusAECBL  as $sL)
					{
						if($statusAECBstr == '')
				...
				if(@$request->session()->get('card_type_CBD_internalCurrentMonth') != '')
				{
					$cardTypeInterbalL = $request->session()->get('card_type_CBD_internalCurrentMonth');
					$cardTypeInterbalstr = '';
					foreach($cardTypeInterbalL  as $CY)
					{
						if($cardTypeInterbalstr == '')
				...
				if(@$request->session()->get('submission_type_internalCurrentMonth') != '')
				{
					$submission_type_internalL = $request->session()->get('submission_type_internalCurrentMonth');
					$submission_type_internalStr = '';
					foreach($submission_type_internalL  as $ST)
					{
						if($submission_type_internalStr == '')
				...
				if($request->session()->get('start_date_application_CBD_internalCurrentMonth') != '')
				{
					$start_date_application_CBD_internalCurrentMonth = $request->session()->get('start_date_application_CBD_internalCurrentMonth');			
					$whereRaw .= " AND application_date >='".date('Y-m-d',strtotime($start_date_application_CBD_internalCurrentMonth))."'";
					$searchValues['start_date_application_CBD_internalCurrentMonth'] = $start_date_application_CBD_internalCurrentMonth;			
				}

				if($request->session()->get('end_date_application_CBD_internalCurrentMonth') != '')
				{
					$end_date_application_CBD_internalCurrentMonth = $request->session()->get('end_date_application_CBD_internalCurrentMonth');			
					$whereRaw .= " AND application_date <='".date('Y-m-d',strtotime($end_date_application_CBD_internalCurrentMonth))."'";
					$searchValues['end_date_application_CBD_internalCurrentMonth'] = $end_date_application_CBD_internalCurrentMonth;			
				}
				
				if($request->session()->get('start_date_approval_CBD_internalCurrentMonth') != '')
				{
					$start_date_approval_CBD_internalCurrentMonth = $request->session()->get('start_date_approval_CBD_internalCurrentMonth');			
					$whereRaw .= " AND approval_date >='".date('Y-m-d',strtotime($start_date_approval_CBD_internalCurrentMonth))."'";
					$searchValues['start_date_approval_CBD_internalCurrentMonth'] = $start_date_approval_CBD_internalCurrentMonth;			
				}

				if($request->session()->get('end_date_approval_CBD_internalCurrentMonth') != '')
				{
					$end_date_approval_CBD_internalCurrentMonth = $request->session()->get('end_date_approval_CBD_internalCurrentMonth');			
					$whereRaw .= " AND approval_date <='".date('Y-m-d',strtotime($end_date_approval_CBD_internalCurrentMonth))."'";
					$searchValues['end_date_approval_CBD_internalCurrentMonth'] = $end_date_approval_CBD_internalCurrentMonth;			
				}
		...
		*complete Status CBD
		*/
        return view("Banks/CBD/loadBankContentsCBDCardCurrentMonth",compact('id','departmentFormDetails','DepartmentNameDetails','departmentFormParentDetails','departmentFormParentTotal','searchValues','employeeIdList','formStatusData','teamData','channel_cbd','status_AECB_cbd','card_type_cbd','masterAttributesStatus'));
	}




	public function searchCBDInternalInnerCurrentMonth(Request $request)
	{
				$requestParameters = $request->input();
				...
				if(isset($requestParameters['channel_cbdCurrentMonth']))
				{
					$channel_cbd = @$requestParameters['channel_cbdCurrentMonth'];
				}
				
				if(isset($requestParameters['status_AECB_cbdCurrentMonth']))
				{
					$status_AECB_cbd = @$requestParameters['status_AECB_cbdCurrentMonth'];
				}
				
				if(isset($requestParameters['card_type_cbdCurrentMonth']))
				{
					$card_type_cbd = @$requestParameters['card_type_cbdCurrentMonth'];
				}

				if(@isset($requestParameters['ref_noCurrentMonth']))
				{
					$ref_no = @$requestParameters['ref_noCurrentMonth'];
				}

				if(isset($requestParameters['teamCurrentMonth']))
				{
					$team = @$requestParameters['teamCurrentMonth'];
				}

				if(isset($requestParameters['form_statusCurrentMonth']))
				{
					$form_status = @$requestParameters['form_statusCurrentMonth'];
				}
				
				if(isset($requestParameters['emp_idCurrentMonth']))
				{
					$emp_id = @$requestParameters['emp_idCurrentMonth'];
				}

				if(isset($requestParameters['start_date_applicationCurrentMonth']))
				{
					$start_date_application = @$requestParameters['start_date_applicationCurrentMonth'];
				}
				if(isset($requestParameters['end_date_applicationCurrentMonth']))
				{
					$end_date_application = @$requestParameters['end_date_applicationCurrentMonth'];
				}
				
				
				if(isset($requestParameters['start_date_approvalCurrentMonth']))
				{
					$start_date_approval = @$requestParameters['start_date_approvalCurrentMonth'];
				}
				if(isset($requestParameters['end_date_approvalCurrentMonth']))
				{
					$end_date_approval = @$requestParameters['end_date_approvalCurrentMonth'];
				}
				if(isset($requestParameters['submission_type_innerCurrentMonth']))
				{
					$submission_type_inner = @$requestParameters['submission_type_innerCurrentMonth'];
				}
				$request->session()->put('master_cbd_search_internal','');
				$request->session()->put('ref_no_CBD_internalCurrentMonth',$ref_no);
				$request->session()->put('form_status_CBD_internalCurrentMonth',$form_status);
				$request->session()->put('team_CBD_internalCurrentMonth',$team);
				$request->session()->put('emp_id_CBD_internalCurrentMonth',$emp_id);
				$request->session()->put('start_date_application_CBD_internalCurrentMonth',$start_date_application);
				$request->session()->put('end_date_application_CBD_internalCurrentMonth',$end_date_application);
				$request->session()->put('channel_CBD_internalCurrentMonth',$channel_cbd);
				$request->session()->put('status_AECB_CBD_internalCurrentMonth',$status_AECB_cbd);
				$request->session()->put('card_type_CBD_internalCurrentMonth',$card_type_cbd);
				$request->session()->put('submission_type_internalCurrentMonth',$submission_type_inner);
				$request->session()->put('start_date_approval_CBD_internalCurrentMonth',$start_date_approval);
				$request->session()->put('end_date_approval_CBD_internalCurrentMonth',$end_date_approval);
				return redirect("loadBankContentsCBDCardCurrentMonth");
	}



	public function resetLoginInnerCBDInternalCurrentMonth(Request $request)
	{
			$request->session()->put('ref_no_CBD_internalCurrentMonth','');
				$request->session()->put('form_status_CBD_internalCurrentMonth','');
				$request->session()->put('team_CBD_internalCurrentMonth','');
				$request->session()->put('emp_id_CBD_internalCurrentMonth','');
				$request->session()->put('start_date_application_CBD_internalCurrentMonth','');
				$request->session()->put('end_date_application_CBD_internalCurrentMonth','');
				$request->session()->put('start_date_approval_CBD_internalCurrentMonth','');
				$request->session()->put('end_date_approval_CBD_internalCurrentMonth','');
				$request->session()->put('channel_CBD_internalCurrentMonth','');
				$request->session()->put('status_AECB_CBD_internalCurrentMonth','');
				$request->session()->put('card_type_CBD_internalCurrentMonth','');
				$request->session()->put('submission_type_internalCurrentMonth','');
	...
	public function loadBankContentsCBDCardBankSideCurrentMonthData(Request $request)
	{
		$paginationValue = 20;
		$searchValues = array();
		if(@$request->session()->get('paginationValue') != '')
		...
		$whereRaw = " ref_no!=''";	
		if(@$request->session()->get('master_cbd_search_bankCurrentMonth') != '' && @$request->session()->get('master_cbd_search_bankCurrentMonth') == 2)
		{
		...
		else
		{
					...
					if(@$request->session()->get('ref_no_CBD_bankCurrentMonth') != '')
					{
						$refNO = $request->session()->get('ref_no_CBD_bankCurrentMonth');
					...
					if(@$request->session()->get('status_CBD_bankCurrentMonth') != '')
					{
						$status = $request->session()->get('status_CBD_bankCurrentMonth');
						$strStatus = '';
						foreach($status  as $s)
						{
							if($strStatus == '')
					...
					if(@$request->session()->get('AECB_Status_CBD_bankCurrentMonth') != '')
					{
						$statusAECB = $request->session()->get('AECB_Status_CBD_bankCurrentMonth');
						$strStatusAECB = '';
						foreach($statusAECB  as $sAECB)
						{
							if($strStatusAECB == '')
					...
					if(@$request->session()->get('employee_id_CBD_bankCurrentMonth') != '')
					{
						$employeeMod = $request->session()->get('employee_id_CBD_bankCurrentMonth');
						$employeeModStr = '';
						foreach($employeeMod  as $modID)
						{
							if($employeeModStr == '')
					...
					if(@$request->session()->get('smManager_CBD_bankCurrentMonth') != '')
					{
						$SMMod = $request->session()->get('smManager_CBD_bankCurrentMonth');
						$smStr = '';
						foreach($SMMod  as $SM)
						{
							if($smStr == '')
					...
					if($request->session()->get('start_date_creation_CBD_bankCurrentMonth') != '')
					{
						$start_date_creation_CBD_bankCurrentMonth = $request->session()->get('start_date_creation_CBD_bankCurrentMonth');			
						$whereRaw .= " AND creation_date >='".date('Y-m-d',strtotime($start_date_creation_CBD_bankCurrentMonth))."'";
						$searchValues['start_date_creation_CBD_bankCurrentMonth'] = $start_date_creation_CBD_bankCurrentMonth;			
					}

					if($request->session()->get('end_date_creation_CBD_bankCurrentMonth') != '')
					{
						$end_date_creation_CBD_bankCurrentMonth = $request->session()->get('end_date_creation_CBD_bankCurrentMonth');			
						$whereRaw .= " AND creation_date <='".date('Y-m-d',strtotime($end_date_creation_CBD_bankCurrentMonth))."'";
						$searchValues['end_date_creation_CBD_bankCurrentMonth'] = $end_date_creation_CBD_bankCurrentMonth;			
					}
					if($request->session()->get('start_date_approval_CBD_bankCurrentMonth') != '')
					{
						$start_date_approval_CBD_bankCurrentMonth = $request->session()->get('start_date_approval_CBD_bankCurrentMonth');			
						$whereRaw .= " AND approval_date >='".date('Y-m-d',strtotime($start_date_approval_CBD_bankCurrentMonth))."'";
						$searchValues['start_date_approval_CBD_bankCurrentMonth'] = $start_date_approval_CBD_bankCurrentMonth;			
					}

					if($request->session()->get('end_date_approval_CBD_bankCurrentMonth') != '')
					{
						$end_date_approval_CBD_bankCurrentMonth = $request->session()->get('end_date_approval_CBD_bankCurrentMonth');			
						$whereRaw .= " AND approval_date <='".date('Y-m-d',strtotime($end_date_approval_CBD_bankCurrentMonth))."'";
						$searchValues['end_date_approval_CBD_bankCurrentMonth'] = $end_date_approval_CBD_bankCurrentMonth;			
					}
					if($request->session()->get('submission_type_CBD_BankCurrentMonth') != '')
					{
						
						$submission_type = $request->session()->get('submission_type_CBD_BankCurrentMonth');
						if($submission_type == 'Linked')
						{
							$whereRaw .= " AND update_emp_status =2";
						}
		...
		*/
		 ...
		 return view("Banks/CBD/loadBankContentsCBDCardBankSideCurrentMonth",compact('datasCBDMainCount','datasCBDMain','paginationValue','searchValues','appStatusMod','appAECB_StatusMod','employeeIdList','smManageData'));
	}
	...
	public function loadBankContentsCBDCardBankSideMTDCurrentMonthdata(Request $request)
	{
		$paginationValue = 20;
		$searchValues = array();
		if(@$request->session()->get('paginationValue') != '')
		...
		$whereRaw = " Appl_Nb !=''";	
		if(@$request->session()->get('master_cbd_search_MTDHistoric') != '' && @$request->session()->get('master_cbd_search_MTDHistoric') == 2)
		{
				  if(@$request->session()->get('ref_no_CBD_master') != '')
					{
						$refNO = $request->session()->get('ref_no_CBD_master');
		...
		{
					if(@$request->session()->get('ref_no_CBD_mtdHistorical') != '')
					{
						$refNO = $request->session()->get('ref_no_CBD_mtdHistorical');
					...
					if(@$request->session()->get('employee_id_CBD_mtdHistorical') != '')
					{
						$employeeMod = $request->session()->get('employee_id_CBD_mtdHistorical');
						$employeeModStr = '';
						foreach($employeeMod  as $modID)
						{
							if($employeeModStr == '')
					...
					if(@$request->session()->get('smManager_CBD_mtdHistorical') != '')
					{
						$SMMod = $request->session()->get('smManager_CBD_mtdHistorical');
						$smStr = '';
						foreach($SMMod  as $SM)
						{
							if($smStr == '')
					...
					if($request->session()->get('start_CD_OPN_DT_CBD_bankHistorical') != '')
					{
						$start_CD_OPN_DT_CBD_bankHistorical = $request->session()->get('start_CD_OPN_DT_CBD_bankHistorical');			
						$whereRaw .= " AND CD_OPN_DT >='".date('Y-m-d',strtotime($start_CD_OPN_DT_CBD_bankHistorical))."'";
						$searchValues['start_CD_OPN_DT_CBD_bankHistorical'] = $start_CD_OPN_DT_CBD_bankHistorical;			
					}

					if($request->session()->get('end_CD_OPN_DT_CBD_bankHistorical') != '')
					{
						$end_CD_OPN_DT_CBD_bankHistorical = $request->session()->get('end_CD_OPN_DT_CBD_bankHistorical');			
						$whereRaw .= " AND CD_OPN_DT <='".date('Y-m-d',strtotime($end_CD_OPN_DT_CBD_bankHistorical))."'";
						$searchValues['end_CD_OPN_DT_CBD_bankHistorical'] = $end_CD_OPN_DT_CBD_bankHistorical;			
					}
					if($request->session()->get('submission_type_CBD_MTDHistorical') != '')
					{
						
						$submission_type = $request->session()->get('submission_type_CBD_MTDHistorical');
						if($submission_type == 'Linked_in_internal')
						{
							$whereRaw .= " AND update_status =2 And match_bank_status =1";
						}
		...
		*Sm Manager
		*/
		 return view("Banks/CBD/loadBankContentsCBDCardBankSideMTDCurrentMonth",compact('datasCBDMainCount','datasCBDMain','paginationValue','searchValues','employeeIdList','smManageData'));
	}




	public function searchCBDMTDInnerHistoricalData(Request $request)
	{
				$requestParameters = $request->input();
				
				$ref_no_mtd_Historical = '';
				$sm_manager_mtd_Historical = '';
				$employee_id_mtd_Historical = '';
				$start_cd_opn_Historical = '';
				$end_cd_opn_Historical = '';
				$submission_type_mtd_Historical = '';
				
				
				if(isset($requestParameters['ref_no_mtdHistorical']))
				{
					$ref_no_mtd_Historical = @$requestParameters['ref_no_mtdHistorical'];
				}
				if(isset($requestParameters['sm_manager_mtdHistorical']))
				{
					$sm_manager_mtd_Historical = @$requestParameters['sm_manager_mtdHistorical'];
				}
				if(isset($requestParameters['employee_id_mtdHistorical']))
				{
					$employee_id_mtd_Historical = @$requestParameters['employee_id_mtdHistorical'];
				}
				if(isset($requestParameters['start_cd_opnHistorical']))
				{
					$start_cd_opn_Historical = @$requestParameters['start_cd_opnHistorical'];
				}
				if(isset($requestParameters['end_cd_opnHistorical']))
				{
					$end_cd_opn_Historical = @$requestParameters['end_cd_opnHistorical'];
				}
				
				if(isset($requestParameters['submission_type_mtdHistorical']))
				{
					$submission_type_mtd_Historical = @$requestParameters['submission_type_mtdHistorical'];
				}
			
				$request->session()->put('master_cbd_search_MTD','');
				$request->session()->put('ref_no_CBD_mtdHistorical',$ref_no_mtd_Historical);
				$request->session()->put('smManager_CBD_mtdHistorical',$sm_manager_mtd_Historical);
				$request->session()->put('employee_id_CBD_mtdHistorical',$employee_id_mtd_Historical);
				$request->session()->put('start_CD_OPN_DT_CBD_bankHistorical',$start_cd_opn_Historical);
				$request->session()->put('end_CD_OPN_DT_CBD_bankHistorical',$end_cd_opn_Historical);
				$request->session()->put('submission_type_CBD_MTDHistorical',$submission_type_mtd_Historical);
			
				return redirect("loadBankContentsCBDCardBankSideMTDCurrentMonth");
	}


	public function resetCBDMTDInnerFilterHistorical(Request $request)
	{
				$request->session()->put('ref_no_CBD_mtdHistorical','');
				$request->session()->put('smManager_CBD_mtdHistorical','');
				$request->session()->put('employee_id_CBD_mtdHistorical','');
				$request->session()->put('start_CD_OPN_DT_CBD_bankHistorical','');
				$request->session()->put('end_CD_OPN_DT_CBD_bankHistorical','');
				$request->session()->put('submission_type_CBD_MTDHistorical','');
				$request->session()->put('master_cbd_search_MTDHistoric',2);
				return redirect("loadBankContentsCBDCardBankSideMTDCurrentMonth");
	}
