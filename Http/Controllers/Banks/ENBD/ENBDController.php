...
public function loadBankContentsENBDCards(Request $request)
{
		...
		$startDate = date("Y",strtotime(date("Y-m-d"))).'-'.date("m",strtotime("-1 Month")).'-'.'21';
		

		$departmentFormParentTotal = DB::table('enbd_department_form_parent_entry')->whereRaw($whereRaw)->whereBetween('submission_date', [$startDate, $endDate])->orderby('submission_date','DESC')->get()->count();

		$departmentFormParentDetails = DB::table('enbd_department_form_parent_entry')->whereRaw($whereRaw)->whereBetween('submission_date', [$startDate, $endDate])->orderby('submission_date','DESC')->paginate($paginationValue);

		//return $departmentFormParentDetails;
	...
	public function loadJonusENBDCardsContentData(Request $request)
	{
		...
		$datasCBDMainCount = ENBDCardsMisReport::whereRaw($whereRaw)->whereBetween('DATEOFSOURCING', [$startDate, $endDate])->get()->count();

		//return $datasCBDMainCount;
		
		$datasCBDMain = ENBDCardsMisReport::whereRaw($whereRaw)->whereBetween('DATEOFSOURCING', [$startDate, $endDate])->orderBy("id","DESC")->paginate($paginationValue);
...
public function loadBankContentsENBDCardsHistoricData(Request $request)
{
		 //$request->session()->put('paginationValue','');
		...
		*/
		...
		return view("Banks/ENBD/loadBankContentsENBDCardsHistoric",compact('departmentFormDetails','DepartmentNameDetails','departmentFormParentDetails','departmentFormParentTotal','searchValues','formStatusData','teamData','channel_cbd','status_AECB_cbd','card_type_cbd','current_activityData','productsData','empNameData','sourceCodeData'));
}
...
public function loadJonusENBDCardsContentHistoricData(Request $request)
{
		$paginationValue = 20;
		$searchValues = array();
		if(@$request->session()->get('paginationValue') != '')
		...
		{
					if(@$request->session()->get('app_no_ENBD_jonusPhysical_Historic') != '')
					{
						$refNO = $request->session()->get('app_no_ENBD_jonusPhysical_Historic');
					...
					if($request->session()->get('start_date_application_JonusPhysical_Historic') != '')
					{
						$start_date_application_JonusPhysical_Historic = $request->session()->get('start_date_application_JonusPhysical_Historic');			
						$whereRaw .= " AND date_sourcing >='".date('Y-m-d',strtotime($start_date_application_JonusPhysical_Historic))."'";
						$searchValues['start_date_application_JonusPhysical_Historic'] = $start_date_application_JonusPhysical_Historic;			
					}

					if($request->session()->get('end_date_application_JonusPhysiacal_Historic') != '')
					{
						$end_date_application_JonusPhysiacal_Historic = $request->session()->get('end_date_application_JonusPhysiacal_Historic');			
						$whereRaw .= " AND date_sourcing <='".date('Y-m-d',strtotime($end_date_application_JonusPhysiacal_Historic))."'";
						$searchValues['end_date_application_JonusPhysiacal_Historic'] = $end_date_application_JonusPhysiacal_Historic;			
					}
					if(@$request->session()->get('submission_type_internal_jonusPhysical_Historic') != '')
				{
					$submission_type_internalL = $request->session()->get('submission_type_internal_jonusPhysical_Historic');
					$submission_type_internalStr = '';
					foreach($submission_type_internalL  as $ST)
					{
						if($submission_type_internalStr == '')
		...
		*Sm Manager
		*/
		 return view("Banks/ENBD/loadBankContentsENBDCardJonusPhysicalHistoric",compact('datasCBDMainCount','datasCBDMain','paginationValue','searchValues','appStatusMod','appAECB_StatusMod','employeeIdList','smManageData'));
	}
