...
class ENBDLinkMISController extends Controller
{
...
public function updateMISStatusENBDData()
{
	//return "Hello" jonus card physical;
	$eibBankMod = ENBDCardsMisReport::whereNull("update_status")->whereNull("enbd_marging_status")->whereNull("matched_status")->get();

	// return $eibBankMod;																					
	// echo count($eibBankMod);
	// exit; 


	if (count($eibBankMod) === 0) 
	{
	...
	else
	{
		...
		{
			$formidarr = array(6,7);
			$application_no = $eib->application_no;
			$detailsInternal = ENBDDepartmentFormEntry::where("application_no",$application_no)->whereIn("form_id",$formidarr)->first();
			if($detailsInternal != '')
			{
				$file_values = array();
				$file_values['matched_status'] = 1;
				$file_values['form_status'] = 1;
				$file_values['current_activity'] = $eib->CURRENTACTIVITY;
				$file_values['status'] = $eib->STATUS;	
						
				DB::table('enbd_department_form_parent_entry')->where('application_no', $application_no)->update($file_values);
				$file_values1 = array();
				$file_values1['update_status'] = 2;
				$file_values1['matched_status'] = 1;	
				DB::table('ENBD_Jonus_Cards_physical')->where('application_no', $application_no)->update($file_values1);
			}
			else
			{
				$file_values1 = array();
				$file_values1['update_status'] = NULL;
				$file_values1['matched_status'] = NULL;
				DB::table('ENBD_Jonus_Cards_physical')->where('application_no', $application_no)->update($file_values1);
			}
		}
...
public function updateMISStatusENBDDataSecond()
{
	$eibBankModTab = MainMisReportTab::whereNull("update_status")->whereNull("enbd_marging_status")->whereNull("matched_status")->get();
	...
	if (count($eibBankModTab) === 0) 
	{
		return response()->json(['success'=>'Nothing to Update.']);
	}
	else
	{
		foreach($eibBankModTab as $eib)
		{
			$formidarr = array(6,7);
			$application_no = $eib->application_no;
			$detailsInternal = ENBDDepartmentFormEntry::where("application_no",$application_no)->whereIn("form_id",$formidarr)->first();
			if($detailsInternal != '')
			{
				...
				$file_values1['update_status'] = 2;
				$file_values1['matched_status'] = 1;	
				DB::table('Jonus_Enbd_Mis_Cards_Tab')->where('application_no', $application_no)->update($file_values1);
			}
			else
			{
				$file_values1 = array();
		...
		return response()->json(['success'=>'Updated Successfully. Records Count: '.count($eibBankModTab)]);
...
public function updateMISStatusENBDDataThird()
{
	$eibBankModData = ENBDDataCut::whereNull("update_status")->whereNull("enbd_marging_status")->whereNull("matched_status")->get();

	//return $eibBankModData;																					
	//echo count($eibBankModData);
	//exit; 


	if (count($eibBankModData) === 0) 
	{
		return response()->json(['success'=>'Nothing to Update.']);
	}
	else
	{
		foreach($eibBankModData as $eib)
		{
			$formidarr = array(6,7);
			$application_no = $eib->application_no;
			$detailsInternal = ENBDDepartmentFormEntry::where("application_no",$application_no)->whereIn("form_id",$formidarr)->first();
			if($detailsInternal != '')
			{
				...
				$file_values1['update_status'] = 2;
				$file_values1['matched_status'] = 1;	
				DB::table('ENBD_DataCut')->where('application_no', $application_no)->update($file_values1);
			}
			else
			{
				$file_values1 = array();
		...
		return response()->json(['success'=>'Updated Successfully. Records Count: '.count($eibBankModData)]);
