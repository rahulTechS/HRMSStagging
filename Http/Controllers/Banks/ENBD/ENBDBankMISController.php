...
protected function lastMonthBooking($empId,$start_date_application_SCB_internal)
{
	...
	protected function sheet2Performance($spreadsheet,$whereRaw,$whereRawBank,$start_date_application_SCB_internal,$end_date_application_SCB_internal)
	{
		...
		$sheet->setCellValue('A1', 'TL Performance ENBD Cards - from -'.date("d M Y",strtotime($start_date_application_SCB_internal)).' to -'.date("d M Y",strtotime($end_date_application_SCB_internal)))->getStyle('A1')->getAlignment()->setHorizontal('center')->setVertical('top');
...
public function searchJonusPhysiaclInnerHistoricData(Request $request)
			{
				$requestParameters = $request->input();
				...
				if(@isset($requestParameters['app_no_historic']))
				{
					$ref_no = @$requestParameters['app_no_historic'];
				}
				...
				if(isset($requestParameters['submission_type_inner_jonusPhysical_Historic']))
				{
					$submission_type = @$requestParameters['submission_type_inner_jonusPhysical_Historic'];
				}
				
				$request->session()->put('app_no_ENBD_jonusPhysical_Historic',$ref_no);
				$request->session()->put('AECB_Status_CBD_bank',$AECB_Status);
				$request->session()->put('status_CBD_bank',$status);
				$request->session()->put('employee_id_CBD_bank',$employee_id);
				$request->session()->put('smManager_CBD_bank',$sm_manager);
				...
				$request->session()->put('end_date_approval_CBD_bank',$end_date_approval_bank);
				$request->session()->put('master_cbd_search_bank','');
				$request->session()->put('submission_type_internal_jonusPhysical_Historic',$submission_type);
				return redirect("loadJonusENBDCardsContentHistoric");
			}
			
			public function resetJonusPhysiaclHistoricData(Request $request)
			{
				$request->session()->put('app_no_ENBD_jonusPhysical_Historic','');
				$request->session()->put('AECB_Status_CBD_bank','');
				$request->session()->put('status_CBD_bank','');
				$request->session()->put('start_date_application_JonusPhysical','');
				$request->session()->put('end_date_application_JonusPhysiacal','');
				...
				$request->session()->put('employee_id_CBD_bank','');
				$request->session()->put('smManager_CBD_bank','');
				$request->session()->put('submission_type_internal_jonusPhysical_Historic','');
				$request->session()->put('master_cbd_search_bank',2);
				return redirect("loadJonusENBDCardsContentHistoric");
			}
