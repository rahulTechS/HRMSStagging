					...
					if(!empty($sheetData))
					{
						for($k=0;$k<count($sheetData);$k++)
						{
							...
							$file_values = array(
												...
												'SIGNEDDATE' => trim($sheetData[$k][6]),
												'PRODUCT' => trim($sheetData[$k][11]),
												//'application_created_at' => ($sheetData[$k][4]?date('Y-m-d',strtotime($sheetData[$k][4])):'0000-00-00'),												
												'SCHEMEGROUP' => trim($sheetData[$k][12]),
												'SCHEME' => trim($sheetData[$k][13]),
												'DSA_BRANCH' => trim($sheetData[$k][16]),
												'DME_RBE' => ($sheetData[$k][17]),
												'CASSTATUS' => ($sheetData[$k][37]),
												'REFERRAL_CODE' => trim($sheetData[$k][57]),
												'REFERRALNAME' => trim($sheetData[$k][58]),
												'P1CODE' => trim($sheetData[$k][59]),
												'SOURCED_ON' => trim($sheetData[$k][55]),
												'STATUS' => ($sheetData[$k][35]),
							...
							*/							
							...
							exit;		 */		
							 $application_number = trim($sheetData[$k][0]);
