...
public function updateMISStatusEIBData()
{
	...
	{
		$application_no = $eib->application_number;
		$detailsInternal = EIBDepartmentFormEntry::where("application_no",$application_no)->where("form_id",3)->first();
		if($detailsInternal != '')
		{
			$file_values = array();
			...
			DB::table('EIB_bank_mis')->where('application_number', $application_no)->update($file_values1);
		}
		else
		{
			$file_values1 = array();
