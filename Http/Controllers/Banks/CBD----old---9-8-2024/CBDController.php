...
public function loadBankContentsCBDCardCurrentMonth(Request $request)
	{
		...
		$departmentFormParentTotal = DB::table('department_form_parent_entry')->whereRaw($whereRaw)->whereBetween('approval_date', [$startDate, $endDate])->orderby('application_date','DESC')->get()->count();

		$departmentFormParentDetails = DB::table('department_form_parent_entry')->whereRaw($whereRaw)->whereBetween('approval_date', [$startDate, $endDate])->orderby('application_date','DESC')->paginate($paginationValue);
		...
		*complete Status CBD
		*/
        return view("Banks/CBD/loadContentCBDCardBankSideCurrentMonth",compact('id','departmentFormDetails','DepartmentNameDetails','departmentFormParentDetails','departmentFormParentTotal','searchValues','employeeIdList','formStatusData','teamData','channel_cbd','status_AECB_cbd','card_type_cbd','masterAttributesStatus'));
	}
