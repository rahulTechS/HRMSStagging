...
class EIBLinkMISController extends Controller
{
...
public function updateMISStatusEIBData()
{
	$eibBankMod = EibBankMis::whereNull("update_status")->whereNull("eib_marging_status")->whereNull("matched_status")->get();
	...
	else
	{
		foreach($eibBankMod as $eib)
		{
			...
			$detailsInternal = EIBDepartmentFormEntry::where("application_no",$application_no)->where("form_id",4)->first();
			if($detailsInternal != '')
			{
				$file_values = array();
				$file_values['matched_status'] = 2;
				$file_values['form_status'] = 1;
				$file_values['final_decision'] = $eib->final_decision;
				$file_values['application_status'] = $eib->application_status;			
				DB::table('eib_department_form_parent_entry')->where('application_no', $application_no)->update($file_values);
				$file_values1 = array();
				$file_values1['update_status'] = 2;
				$file_values1['matched_status'] = 1;
				$file_values1['emp_id'] = $detailsInternal->emp_id;
				$file_values1['emp_name'] = $detailsInternal->emp_name;
				$file_values1['se_name'] = $detailsInternal->se_name;
				$file_values1['actual_se_name'] = $detailsInternal->actual_se_name;
				$file_values1['actual_se_emp_id'] = $detailsInternal->actual_se_emp_id;
				$file_values1['tl_name'] = $detailsInternal->tl_name;		
				
				DB::table('EIB_bank_mis')->where('application_no', $application_no)->update($file_values1);
			}
			else
			{
				$file_values1 = array();
