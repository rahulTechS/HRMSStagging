...
class OpeningController extends Controller
{
	   ...
	   public function addJobOpeningPost(Request $request)
	   {
		   ...
		   $jobOpeningMod = new JobOpening();
			...
			$jobOpeningMod->designation = $parameterInput['jobOpening']['designation'];
	   ...
	   public function updateJobOpeningPost(Request $request)
	   {
		   $parameterMeters = $request->input();
			...
			$jobOpeningUpdateMod->designation = $datas['designation'];
	   ...
	   function getDesignationOfJobopning(Request $request)
	   {
		     $deptId = $request->id;
			  $designationDetails =  Designation::where("status",1)->where("department_id",$deptId)->orderBy('id','DESC')->get();
			  return view("Opening/getDesignationOfDocumentation",compact('designationDetails'));
	   }
	   public static function getdesignName($id)
			{	
