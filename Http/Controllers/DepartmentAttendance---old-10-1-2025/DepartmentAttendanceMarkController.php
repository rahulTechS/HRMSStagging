...
class DepartmentAttendanceMarkController extends Controller
{
	...
	{		
		...
		if(!empty($request->session()->get('attendance_type')) && $request->session()->get('attendance_type') != 'All')
		{
			...
			if($whereraw == '')
			{
				$whereraw = "attribute_value IN('".$teamlead."')";
				$attendanceFilter=1;
			}
			else
			{
				$whereraw .= " And attribute_value IN('".$teamlead."')";
				$attendanceFilter=1;
			}
		}
		...
		if(!empty($request->session()->get('emp_attendance_fromdate')) && $request->session()->get('emp_attendance_fromdate') != 'All')
		{
			...
			if($whereraw == '')
			{
				$whereraw = 'attendance_date >= "'.$datefrom.'"';
			}
			else
			{
				$whereraw .= ' And attendance_date >= "'.$datefrom.'"';
			}
		}
		if(!empty($request->session()->get('emp_attendance_todate')) && $request->session()->get('emp_attendance_todate') != 'All')
		{
			...
			if($whereraw == '')
			{
				$whereraw = 'attendance_date <= "'.$dateto.'"';
			}
			else
			{
				$whereraw .= ' And attendance_date <= "'.$dateto.'"';
			}		
		}
...
//exit;
        ...
        if($whereraw != '')
		{
			...
			if($attendanceFilter==1)
			{
				$whereraw = str_replace(",","','",$whereraw);
			}
			//echo "<pre>";
			// print_r($whereraw);
			// exit;
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					if($attendanceDateFilter==1)
					{
						...
						$empAttendanceData = EmpAttendance::whereRaw($whereraw)->orderBy('id','desc')->get();
					}
					else
					{
						$currDate = date('Y-m-d');
						$empAttendanceData = EmpAttendance::whereRaw($whereraw)->where('attendance_date',$currDate)->orderBy('id','desc')->get();
					}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('emp_id','emp_name')->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
				->paginate($paginationValue);
