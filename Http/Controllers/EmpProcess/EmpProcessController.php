...
class EmpProcessController extends Controller
{
	...
	public function viewEmployeeProfile($empid=NULL)
	{
		...
		{			
			...
			return view("EmpProcess/EmpList",compact('Designation','empId','EmpName','design','jobfun','recdata','EmpTLlist'));
		...
		public function AjaxEmpList(Request $request){
			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				if(!empty($request->session()->get('active_emp_filter_inner_list')) && $request->session()->get('active_emp_filter_inner_list') != 'All')
				{
					$activeemp = $request->session()->get('active_emp_filter_inner_list');
					if($activeemp==1){
						 if($whereraw == '')
							{
								$whereraw = 'offboard_status IN (2,4)';
							}
							else
							{
								$whereraw .= ' And offboard_status IN (2,4)';
							}
						
					}elseif($activeemp==2){
						if($whereraw == '')
							{
								$whereraw = 'offboard_status IN (3)';
							}
							else
							{
								$whereraw .= ' And offboard_status IN (3)';
							}
				...
				if(!empty($request->session()->get('tl_emp_filter_inner_list')) && $request->session()->get('tl_emp_filter_inner_list') != 'All')
				{
					$tl_emp = $request->session()->get('tl_emp_filter_inner_list');
					 //$selectedFilter['designation'] = $design;
					 if($whereraw == '')
					{
						$whereraw = 'tl_id IN ('.$tl_emp.')';
					}
					else
					{
						$whereraw .= ' And tl_id IN('.$tl_emp.')';
					}
				}
				...
				if(!empty($request->session()->get('appkyc_emp_filter_inner_list')) && $request->session()->get('appkyc_emp_filter_inner_list') != 'All')
				{
					$appkyc = $request->session()->get('appkyc_emp_filter_inner_list');
					 if($appkyc==1){
						  if($whereraw == '')
							{
								$whereraw = 'app_journey_status=1';
							}
							else
							{
								$whereraw .= ' And app_journey_status=1';
							}
					 }
					 else{
						if($whereraw == '')
							{
								$whereraw = 'app_journey_status IS NULL';
							}
							else
							{
								$whereraw .= ' And app_journey_status IS NULL';
							}
					 }
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderByRaw("-vintage_days ASC")->whereRaw($whereraw)->where("offline_status",1)->whereNotIn('emp_id', array(102392,102547))->paginate($paginationValue);
				$reportsCount = Employee_details::whereRaw($whereraw)->where("offline_status",1)->whereNotIn('emp_id', array(102392,102547))->get()->count();
					$activeCount = Employee_details::whereRaw($whereraw)->where("offline_status",1)->where('status',1)->whereNotIn('emp_id', array(102392,102547))->get()->count();
					$inactiveCount = Employee_details::whereRaw($whereraw)->where("offline_status",1)->where('status',2)->whereNotIn('emp_id', array(102392,102547))->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderByRaw("-vintage_days DESC")->where("offline_status",1)->whereNotIn('emp_id', array(102392,102547))->paginate($paginationValue);
					$reportsCount = Employee_details::where("offline_status",1)->whereNotIn('emp_id', array(102392,102547))->get()->count();	
					$activeCount = Employee_details::where('status',1)->where("offline_status",1)->whereNotIn('emp_id', array(102392,102547))->get()->count();
					$inactiveCount = Employee_details::where('status',2)->where("offline_status",1)->whereNotIn('emp_id', array(102392,102547))->get()->count();					
				}
			...
			return view("EmpProcess/AjaxEmpListall",compact('exportemployeestatus','empdetails','paginationValue','departmentLists','deptID','reportsCount','activeCount','inactiveCount'));
		}
...
//print_r($filesAttributeInfo['37']);exit;
			...
			public function updateEmpdata(Request $req)
				{
					...
					$empNameArray = explode(" ",$req->input('emp_name'));
				
					if(count($empNameArray) >1)
					{
						$empdetails->first_name=$empNameArray[0];
						$empdetails->middle_name=$empNameArray[1];
						$empdetails->last_name=$empNameArray[2];
					}
					else
					{
						$empdetails->first_name=$empNameArray[0];
					}
					$empdetails->emp_name= $req->input('emp_name');
		...
		$parameters = $request->input(); 
			...
			$sheet->setCellValue('AL'.$indexCounter, strtoupper('KYC Status'))->getStyle('AL'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			...
			foreach ($selectedId as $sid) {
				 ...
				 *get ESTABLISHMENT NAME
				 */
				 if($misData->app_journey_status==1){
						$kycdata="Approved";
				 }else{
						$kycdata="Pending";
				 }
				...
				$sheet->setCellValue('AL'.$indexCounter, $kycdata)->getStyle('AL'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');				
			...
			$spreadsheet->getActiveSheet()->getStyle('A1:AL1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('cbddf7');
	...
	public function workLocationEMP(Request $req){
		 ...
		 if(count($Collection)>0)
			...
			$empattributesMod = Employee_attribute::where('emp_id',$empId)->where('attribute_code','work_location')->where('dept_id',$dept_id)->first();
				 //print_r($empattributesMod);exit;
				if(!empty($empattributesMod) && $empattributesMod->attribute_values!=''){	
				$updateOBJ = Employee_details::find($_coll->id);				
				
				$updateOBJ->work_location = $empattributesMod->attribute_values;
				$updateOBJ->save();
			...
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			$exportemployeestatus=ExportEmployeeStatus::where('id',1)->first();
			return view("EmpProcess/AjaxEmpListenbd",compact('exportemployeestatus','empdetails','paginationValue','departmentLists','deptID','reportsCountenbd','activeCountenbd','inactiveCountenbd'));
		}
		...
		public function searchbyempName(Request $request)
		{
			...
			$tl_empName='';
			if($request->tl_emp!=''){
			$tl_emparray=array_filter($request->tl_emp);		
			$tl_empName=implode(",", $tl_emparray);
			}
			$appkyc = $request->appkyc;
			$activeemp = $request->activeemp;
			...
			$request->session()->put('appkyc_emp_filter_inner_list',$appkyc);
			$request->session()->put('tl_emp_filter_inner_list',$tl_empName);
			$request->session()->put('active_emp_filter_inner_list',$activeemp);
		...
		public function empFilterreset(Request $request)
		{
			...
			$request->session()->put('appkyc_emp_filter_inner_list','');
			$request->session()->put('tl_emp_filter_inner_list','');
			$request->session()->put('active_emp_filter_inner_list','');
			  redirect('AjaxEmpList');	
		}
		public function getdesigndata($deptId,$empId){
			...
			return view("EmpProcess/designationdropdown",compact('designationMod','empdetails','tL_details'));
		...
		public function updateEmpDepartmentData(Request $request){
		 ...
		 public function OfflineEMPData(Request $request){
			...
			// print_r($request->input());exit;
			 ...
			 // New Code Start for PDF Download
				...
				$warnCountDetails = WarningLetterRequest::where("emp_id",$offEmpDetails->emp_id)->orderBy("id","DESC")->first();

				if($warnCountDetails)
				{
					$warnCount = $warnCountDetails->counter;
				}
				else
				{
					$warnCount = "";
				...
				$dojDetails = Employee_details::where("emp_id",$offEmpDetails->emp_id)->orderBy("id","DESC")->first();

				if($dojDetails)
				{
					if($dojDetails->doj!='' || $dojDetails->doj!=NULL)
					{
						$emp_doj = date("d M, Y", strtotime($dojDetails->doj));
					}
					else
					{
						$emp_doj = "--";
				...
				if($offEmpDetails->last_working_day_resign)
				{
					$resigndate = $offEmpDetails->last_working_day_resign;
					$month = date('m', strtotime($resigndate));
					$year = date('Y', strtotime($resigndate));	
					$datetime1 = strtotime($year."-".$month."-01");
					$your_date = strtotime($resigndate);
					$datediff = $your_date - $datetime1;	
					$workingDayCount = round($datediff / (60 * 60 * 24));
				...
				if($offEmpDetails->last_working_day_resign)
				{
					$emp_last = date("d M, Y", strtotime($offEmpDetails->last_working_day_resign));
					$this->fpdf->Text($x+132,$y+55,$emp_last);
				}
				else
				{
				...
				if($offEmpDetails->doj)
				{
					$dojdate = $offEmpDetails->doj;
					$dojdate = str_replace("/","-",$dojdate);
					$doj = date("Y-m-d",strtotime($dojdate));
					$empl_doj = date("d M, Y", strtotime($doj));
					$this->fpdf->Text($x+128,$y+65,$empl_doj);
				}
				else
				{
				...
				$this->fpdf->Text($x+82,$y+127,$workingDayCount);			
				$this->fpdf->line($x-10,$y,$x-10,$y-20);//SN line
				...
				$this->fpdf->Text($x+82,$y+134,$warnCount);				
				$this->fpdf->line($x-10,$y,$x-10,$y-20);//SN line
...
public function viewEmployeeCompensationDetails($empid=NULL)
	...
	public static function getTeamLeader($id = NULL)
	   ...
	   public function AjaxEmpListDUCData(Request $request){
			...
			$exportemployeestatus=ExportEmployeeStatus::where('id',1)->first();
			return view("EmpProcess/AjaxEmpListDUC",compact('exportemployeestatus','empdetails','paginationValue','reportsCountduc','activeCountduc','inactiveCountduc'));
		}
	...
	public function AjaxEmpListHSBCData(Request $request){
			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				else$exportemployeestatus=ExportEmployeeStatus::where('id',1)->first();
				{
					$paginationValue = 10;
				}
				$whereraw='';
		...
		public function AjaxEmpListUAEData(Request $request){
			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				//$whereraw = 'type,Team Leader';
					...
					$empdetails = Employee_details::where("dept_id",56)->paginate($paginationValue);	
					$reportsCountenbd = Employee_details::where("dept_id",56)->get()->count();
					$activeCountenbd = Employee_details::where("dept_id",56)->where('status',1)->get()->count();
					$inactiveCountenbd = Employee_details::where("dept_id",56)->where('status',2)->get()->count();
				}
				if(!empty($request->session()->get('empid_emp_filter_inner_list')) && $request->session()->get('empid_emp_filter_inner_list') != 'All')
				{
					$empId = $request->session()->get('empid_emp_filter_inner_list');
				...
				if($whereraw == '')
				{
				$appidGet = Employee_details::where("dept_id",56)->get();
				}
				else
				{
					
					$appidGet = Employee_details::whereRaw($whereraw)->where("dept_id",56)->get();
				...
				*/
				...
				if($whereraw == '')
				{
				$f_namedata = Employee_details::where("dept_id",56)->get();
				}
				else
				{
					
					$f_namedata = Employee_details::whereRaw($whereraw)->where("dept_id",56)->get();
				...
				if($whereraw == '')
				{
				$l_namedata = Employee_details::where("dept_id",56)->get();
				}
				else
				{
					
					$l_namedata = Employee_details::whereRaw($whereraw)->where("dept_id",56)->get();
				...
				if($whereraw == '')
				{
				$soursecode= Employee_details::where("dept_id",56)->get();
				}
				else
				{
					
					$soursecode = Employee_details::whereRaw($whereraw)->where("dept_id",56)->get();
				...
				if($whereraw == '')
				{
				$designation= Employee_details::where("dept_id",56)->get();
				}
				else
				{
					
					$designation = Employee_details::whereRaw($whereraw)->where("dept_id",56)->get();
				...
				if($whereraw == '')
				{
					$ventArray = Employee_details::where("dept_id",56)->orderBy("id", "DESC")->get();
					$ventList = array(); 
					foreach($ventArray as $_vent)
					  {
					  $ventList[]  = $_vent->vintage_days;
				...
				{
					//echo $whereraw;//exit;
					$ventArray = Employee_details::whereRaw($whereraw)->where("dept_id",56)->orderBy("id", "DESC")->get();
					$ventList = array(); 
					foreach($ventArray as $_vent)
					  {
					  $ventList[]  = $_vent->vintage_days;
				...
				if($whereraw == '')
				{
				$loc= Employee_details::where("dept_id",56)->get();
				}
				else
				{
					
					$loc = Employee_details::whereRaw($whereraw)->where("dept_id",56)->get();
				...
				{
				//$empdata= Employee_details::get();
				$Collection  = Employee_details::where("dept_id",56)->get();
				if(!empty($Collection)){
				$empid=array();
				foreach($Collection as $_coll)
				{
				...
				{
					$Collection = Employee_details::whereRaw($whereraw)->where("dept_id",56)->get();
					$empid=array();
					foreach($Collection as $_coll)
					{
					$empid[]=$_coll->emp_id;					
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->where("dept_id",56)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountuae = Employee_details::whereRaw($whereraw)->where("dept_id",56)->where("offline_status",1)->get()->count();
					$activeCountuae = Employee_details::whereRaw($whereraw)->where("dept_id",56)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountuae = Employee_details::whereRaw($whereraw)->where("dept_id",56)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::where("dept_id",56)->where("offline_status",1)->orderBy("id","DESC")->paginate($paginationValue);
					$reportsCountuae = Employee_details::where("dept_id",56)->where("offline_status",1)->get()->count();	
					$activeCountuae = Employee_details::where("dept_id",56)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountuae = Employee_details::where("dept_id",56)->where("offline_status",1)->where('status',2)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/AjaxEmpListUAEData'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			$exportemployeestatus=ExportEmployeeStatus::where('id',1)->first();
			return view("EmpProcess/AjaxEmpListUAE",compact('exportemployeestatus','VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountuae','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountuae','inactiveCountuae'));
		}
