...
//print_r($filesAttributeInfo['37']);exit;
			...
			$total_gross_salary = Employee_attribute::where('emp_id',$empId)->where('attribute_code','total_gross_salary')->where('dept_id',$dept_id)->first();
			 if($total_gross_salary!=''){
				 //$deptdata=$dpdata->dept_id;
					...
					$logObjbank->change_attribute_name ="Total Gross Salary";					
		...
		$parameters = $request->input(); 
			...
			$spreadsheet->getActiveSheet()->getStyle('A1:K1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('cbddf7');
...
function getDatesFromRangeLists($start, $end) {
	...
	public function workLocationEMP(Request $req){
		 ...
		 if(count($Collection)>0)
			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				*/
				...
				$reportsCountenbd = Employee_details::whereRaw($whereraw)->where("dept_id",9)->where("offline_status",1)->get()->count();
					$activeCountenbd = Employee_details::whereRaw($whereraw)->where("dept_id",9)->where("offline_status",1)->where('status',1)->get()->count();
		...
		public function getdesigndata($deptId,$empId){
			...
			$designationMod = Designation::where("department_id",$deptId)->get();
			return view("EmpProcess/designationdropdown",compact('designationMod','empdetails'));
		...
		public function updateEmpDepartmentData(Request $request){
			 ...
			 if($dpdata!=''){
				 $departmentMod = Department::where("id",$dept_id)->first();
				 $deptname=$departmentMod->department_name;
				 //$deptdata=$dpdata->dept_id;
					$logObj = new EmpChangeLog();
