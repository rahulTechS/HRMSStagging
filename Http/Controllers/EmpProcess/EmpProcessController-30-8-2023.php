...
class EmpProcessController extends Controller
{
	...
	public function viewEmployeeProfile($empid=NULL)
	{
		...
		{			
			...
			return view("EmpProcess/EmpList",compact('Designation'));
		...
		public function AjaxEmpList(Request $request){
			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				if(!empty($request->session()->get('empid_emp_filter_inner_list')) && $request->session()->get('empid_emp_filter_inner_list') != 'All')
				{
					$empId = $request->session()->get('empid_emp_filter_inner_list');
					 ...
					 if($whereraw == '')
					{
						$whereraw = 'emp_id = "'.$empId.'"';
					}
					else
					{
						$whereraw .= ' And emp_id = "'.$empId.'"';
					}
				}
				if(!empty($request->session()->get('fname_emp_filter_inner_list')) && $request->session()->get('fname_emp_filter_inner_list') != 'All')
				{
					$fname = $request->session()->get('fname_emp_filter_inner_list');
					 $selectedFilter['f_name'] = $fname;
					 if($whereraw == '')
					{
			...
			return view("EmpProcess/AjaxEmpListall",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCount','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCount','inactiveCount'));
		}
...
//print_r($filesAttributeInfo['37']);exit;
		...
		$parameters = $request->input(); 
			...
			$sheet->setCellValue('L'.$indexCounter, strtoupper('Full Name'))->getStyle('L'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			...
			foreach ($selectedId as $sid) {
				//echo $sid;
				 ...
				 $tld=Employee_details::where("tl_id",$tldata)->first();
				...
				$sheet->setCellValue('L'.$indexCounter, strtoupper($misData->emp_name))->getStyle('L'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sn++;
...
function getDatesFromRangeLists($start, $end) {
	...
	public function workLocationEMP(Request $req){
		 ...
		 if(count($Collection)>0)
			...
			return view("EmpProcess/AjaxEmpListenbd",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountenbd','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountenbd','inactiveCountenbd'));
		}
			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				*/
				...
				else
				{
					...
					$inactiveCountcbd = Employee_details::where("dept_id",449)->where("offline_status",1)->where('status',2)->get()->count();					
				}
