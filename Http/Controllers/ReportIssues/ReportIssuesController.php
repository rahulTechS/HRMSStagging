<?php
namespace App\Http\Controllers\ReportIssues;
...
class ReportIssuesController extends Controller
{
	public  function Index(Request $request)
	{
        ...
        return view("ReportIssues/index",compact('empDetailsIndex','departmentLists','moduleLists','tL_details'));
    }
    ...
    public function allReportedIssuesListing(Request $request)
    {
        $whereraw = '';
        $whererawother = '';
		$whereraw1 = '';
        ...
        if(!empty($request->session()->get('reportIssues_emp_name')) && $request->session()->get('reportIssues_emp_name') != 'All')
        {
            $fname = $request->session()->get('reportIssues_emp_name');
            if($fname==',')
            {               
            }
            else
            {
                ...
                $newResult3=array();
                $newResult=array();
                $empDetails = Employee_details::whereIn('emp_name',$cnameArray)->orderBy('id', 'desc')
                // ->toSql();
                // dd($empDetails);
                ->get();
                ...
                $empDetails2 = User::whereIn('employee_id',$newResult)->orderBy('id', 'desc')
                ->get();
    
                if(count($empDetails2) === 0)
                {
                    $newResult3[]=0;
                }
                else
                {
                    foreach($empDetails2 as $value)
                    {
                        $newResult3[]=$value->id;
                    }
                }
                $newempid2 = implode(",",$newResult);
                $newempid3 = implode(",",$newResult3);
                if($whereraw == '')
                {
                    //$whereraw = 'emp_name like "%'.$fname.'%"';
                    $whereraw = 'user_id IN ('.$newempid2.') OR user_id IN ('.$newempid3.')';
                }
                else
                {
                    $whereraw .= ' And user_id IN ('.$newempid2.') OR user_id IN ('.$newempid3.')';
                }
            }
        ...
        if(!empty($request->session()->get('reportIssues_emp_id')) && $request->session()->get('reportIssues_emp_id') != 'All')
        {
            $empId = $request->session()->get('reportIssues_emp_id');
            $newempid = explode(",",$empId);

           
            $newResult2=array();
            $newResult3=array();
            $newResult=array();
            $empDetails = Employee_details::whereIn('emp_id',$newempid)->orderBy('id', 'desc')
            // ->toSql();
            // dd($empDetails);
            ->get();
            ...
            $empDetails2 = User::whereIn('employee_id',$newempid)->orderBy('id', 'desc')
            ->get();
        ...
        if(!empty($request->session()->get('reportIssues_fromdate')) && $request->session()->get('reportIssues_fromdate') != 'All')
        {
            $datefrom = $request->session()->get('reportIssues_fromdate');
             if($whereraw == '')
            {
        ...
        if(!empty($request->session()->get('reportIssues_todate')) && $request->session()->get('reportIssues_todate') != 'All')
        {
            $dateto = $request->session()->get('reportIssues_todate');
             if($whereraw == '')
            {
        ...
        if($whereraw != '')
		{
            ...
            if($empData==1) // all
            {
                ...
                // echo "<pre>";
                // print_r($newResult);
                // print_r($whererawother);
                // print_r($newResult3);
                // exit;

                
                    $requestDetails = Reportissue::whereRaw($whereraw)->orderBy('id', 'desc')					
                    ->paginate($paginationValue);
                    //->toSql();
                    //dd($requestDetails);
    
                    $reportsCount = Reportissue::whereRaw($whereraw)->orderBy('id','desc')
                    ->get()->count();
        ...
        else
        {
            if($empData==1) // all
            {
                $requestDetails = Reportissue::orderBy('id', 'desc')
                //->toSql();	 
                //dd($documentCollectiondetails);						
                ->paginate($paginationValue);	
                
                $reportsCount = Reportissue::orderBy('id','desc')
                ->get()->count();
            ...
            else // specific dept
            {
                ...
                $requestDetails = Reportissue::whereIn('emp_id',$newResult)->orderBy('id', 'desc')
                //->toSql();	 
                //dd($documentCollectiondetails);						
                ->paginate($paginationValue);
                ...
                $reportsCount = Reportissue::whereIn('emp_id',$newResult)->orderBy('id','desc')
                ->get()->count();
            }
        }
        ...
        return view("ReportIssues/listingAll",compact('requestDetails','paginationValue','reportsCount'));
    }
    ...
    public function searchReportedIssuesFilter(Request $request)
	{
			$department='';
			...
			$request->session()->put('reportIssues_emp_name',$name);
            $request->session()->put('reportIssues_emp_id',$empId);
            $request->session()->put('reportIssues_dept',$department);
            ...
            $request->session()->put('reportIssues_fromdate',$datefrom);
            $request->session()->put('reportIssues_todate',$dateto);
    ...
    public function resetReportedIssuesFilter(Request $request)
    {
        $request->session()->put('reportIssues_emp_name','');
        $request->session()->put('reportIssues_emp_id','');
        $request->session()->put('reportIssues_dept','');
        $request->session()->put('transfer_requests_new_dept','');
        $request->session()->put('reported_issues_modules','');
        ...
        $request->session()->put('reportIssues_fromdate','');
		$request->session()->put('reportIssues_todate','');
        
        
    }


    public static function getDepartment($empid,$from)
    {
        if($from!='')
        {
        ...
        else
        {
            ...
            if($empData)
            {
                $emp_details = Employee_details::where("emp_id",$empData->employee_id)->orderBy("id","desc")->first();
    ...
    public static function getEmpName($empid,$from)
    {
        ...
        else
        {
            ...
            if($empData)
            {
                $empData = Employee_details::where('emp_id', $empData->employee_id)->orderBy('id','desc')->first();
