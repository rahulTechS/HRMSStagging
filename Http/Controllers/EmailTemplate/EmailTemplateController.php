...
use App\Models\EmailTemplate\EmailTemplates;
...
class EmailTemplateController extends Controller
{
	public  function Index(Request $request)
	{
        ...
        return view("EmailTemplate/index",compact('empDetailsIndex','departmentLists','designationLists','tL_details'));
    }
    ...
    public function emailTemplatesListing(Request $request)
    {
        $whereraw = '';
		$whereraw1 = '';
        ...
        else
        {
            if($empData==1) // all
            {
                $requestDetails = EmailTemplates::orderBy('id', 'desc')
                //->toSql();	 
                //dd($documentCollectiondetails);						
                ->paginate($paginationValue);	
                
                $reportsCount = EmailTemplates::orderBy('id','desc')
                ->get()->count();
            }
            else // specific dept
            {
        ...
        return view("EmailTemplate/listingAll",compact('requestDetails','paginationValue','reportsCount'));
    }

    
    public static function addEmailTemplateRequest(Request $request)
    {
        return view("EmailTemplate/addRequest");
    }
    ...
    public function emailTemplateRequestPostSubmit(Request $request)
    {
        $validator = Validator::make($request->all(), 
        [			
			'templateName' => 'required',
            'templateSub' => 'required',
            'templateBody' => 'required', 
        ],
		[
			'templateName.required'=> 'Please Write Email Template Name',
		 	'templateSub.required'=> 'Please Write Email Template Subject',
			'templateBody.required'=> 'Please put code for Email Template',
		...
		else
		{
            ...
            $emailTemplateRequest = new EmailTemplates();
            $emailTemplateRequest->template_name = $request->templateName;
            $emailTemplateRequest->template_subject = $request->templateSub;
            $emailTemplateRequest->area_type = $request->areaType;
            $emailTemplateRequest->template_body = $request->templateBody;            
            $emailTemplateRequest->comments = $request->templateComments; 
            $emailTemplateRequest->created_at = date('Y-m-d H:i:s');
            $emailTemplateRequest->status = 1;
            $emailTemplateRequest->created_by = $usersessionId;
            $emailTemplateRequest->template_at = date('Y-m-d H:i:s');            
            $emailTemplateRequest->save();
            ...
            return response()->json(['success'=>'Email Template Added Successfully.']);
        }
    }
    ...
    public function editTemplateRequestContent(Request $request)
    {
        $rowid=$request->rowid;
        ...
        $requestData = EmailTemplates::where('id',$rowid)->orderBy('id', 'desc')->first();

        return view("EmailTemplate/editRequestContent",compact('requestData'));
    }
    ...
    public function updateEmailTemplate(Request $request)
    {
        $validator = Validator::make($request->all(), 
        [			
			'editTemplateName' => 'required',
            'editTemplateSub' => 'required',
            'editTemplateBody' => 'required', 
        ],
		[
			'editTemplateName.required'=> 'Please Write Email Template Name',
		 	'editTemplateSub.required'=> 'Please Write Email Template Subject',
			'editTemplateBody.required'=> 'Please put code for Email Template',
		...
		else
		{
            //return $request->all();
            $usersessionId=$request->session()->get('EmployeeId');
            $emailTemplateDetails = EmailTemplates::where('id',$request->rowid)->orderBy('id', 'desc')->first();

            
            //$advancedPayRequest->emp_id = $request->addRequestEmp;
            $emailTemplateDetails->template_name = $request->editTemplateName;
            $emailTemplateDetails->template_subject = $request->editTemplateSub;
            // $emailTemplateDetails->area_type = $request->areaType;
            $emailTemplateDetails->template_body = $request->editTemplateBody;            
            $emailTemplateDetails->comments = $request->editTemplateComments; 
            // $emailTemplateDetails->created_at = date('Y-m-d H:i:s');
            // $emailTemplateDetails->status = 1;
            $emailTemplateDetails->edit_by = $usersessionId;
            $emailTemplateDetails->edit_at = date('Y-m-d H:i:s');            
            $emailTemplateDetails->save();
            ...
            return response()->json(['success'=>'Email Template Updated Successfully.']);
        }
    }
    ...
    {
        //return $id;s
        $emailTemp = EmailTemplates::find($id)->delete();
