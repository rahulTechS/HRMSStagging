...
use App\Models\RedFlags\ENBDRevenueList;
use App\Models\RedFlags\ENBDRevenueRedflag;
...
class ENBDController extends Controller
{
	...
	public function revenueListENBD()
	{
		
		/*
		*overall revenue code
		*/
		   /*$collection = AgentPayout::get();
			
			foreach($collection as $data)
			{
					$mashreqRevenueModel = new ENBDRevenueList();
					...
					$mashreqRevenueModel->location = trim($data->location);
		...
		*overall revenue code
		*/
		$collectionSalesTime = AgentPayout::groupBy('sales_time')
			->selectRaw('count(*) as total, sales_time')
			->get();
			foreach($collectionSalesTime as $cSalesTime)
			{
				  $collectionRange = AgentPayout::groupBy('range_id')
					->selectRaw('count(*) as total, range_id')->where("sales_time",$cSalesTime->sales_time)
					->get();
				
					foreach($collectionRange as $range)
					{
							$collection = AgentPayout::where("sales_time",$cSalesTime->sales_time)->where("range_id",$range->range_id)
							->get();
							...
							foreach($collection as $data)
							{
									...
									$mashreqRevenueModel->sales_time = $cSalesTime->sales_time;
									$mashreqRevenueModel->redFlag_status = 1;
									$mashreqRevenueModel->doj = trim($data->doj);
									$mashreqRevenueModel->tl_name = trim($data->tl_name);
									$mashreqRevenueModel->agent_name = trim($data->agent_name);
	...
	public function revenuePerformanceENBD()
	{
		$allMashreqData = ENBDRevenueList::where("type","OverAll")->get();
		$takeReveue = array();
		$totalReveue = 0;
	...
	public function revenuePerformanceAsPerRangeENBD()
	{
		
		$collectionTime = ENBDRevenueList::groupBy('sales_time')
			->selectRaw('count(*) as total, sales_time')
			->get();
		foreach($collectionTime as $time)
		{
				$collectionRange = ENBDRevenueList::groupBy('range_id')
				...
				foreach($collectionRange as $range)
				{
					if($range->range_id != '')
					{
				$allMashreqData = ENBDRevenueList::where("range_id",$range->range_id)->where("sales_time",$time->sales_time)->get();
				$takeReveue = array();
				$totalReveue = 0;
				foreach($allMashreqData as $_mashreq)
				{
	...
	public function doingredFlagMENBD()
	{
		$agentPayoutMDetails = AgentPayout::get();
		/* echo '<pre>';
		...
		foreach($agentPayoutMDetails as $_mashreq)
		{
			...
			$sales_time = $_mashreq->sales_time;
			/*
			*overAll setting
			*/
			$mRedFlagDetails = ENBDRevenueRedflag::where("type","OverAll")->first();
			
			$mashreqUpdate = AgentPayout::find($mId);
			$mashreqUpdate->avg_overall = $mRedFlagDetails->mean;
			$mashreqUpdate->median_overall = $mRedFlagDetails->median;
			/*
			*mashreq lists
			*/
			$rLists = ENBDRevenueList::where("type","OverAll")->orderBy("revenue","DESC")->get();
			$currentarray = array();
			$position = 0;
			$index = 0;
		 ...
		 */
			...
			*range setting
			*/
			$mRedFlagDetails = ENBDRevenueRedflag::where("range_id",$range_id)->where("sales_time",$sales_time)->first();
			if($mRedFlagDetails != '')
			{
			...
			*mashreq lists
			*/
			$rLists = ENBDRevenueList::where("range_id",$range_id)->where("sales_time",$sales_time)->orderBy("revenue","DESC")->get();
	...
	public function salesTimeUpdateENDB()
	{
		$MashreqRevenueRedflagData = ENBDRevenueRedflag::get();
		foreach($MashreqRevenueRedflagData as $mData)
		{
			if($mData->sales_time != '' && $mData->sales_time != NULL)
			{
			$updateMod = ENBDRevenueRedflag::find($mData->id);
	...
	public function revenuePerformanceAsFiltersENDB(Request $request)
	{
		...
		if($range_id == "No")
		{
			if($whereRaw == '')
			{
				$allMashreqData = AgentPayout::whereIn("sales_time",$salesTimeArray)->get();
			}
			else
			{
				$allMashreqData = AgentPayout::whereIn("sales_time",$salesTimeArray)->whereraw($whereRaw)->get();
			}
		}
		else
		{
			...
			if($whereRaw == '')
			{
				$allMashreqData = AgentPayout::whereIn("sales_time",$salesTimeArray)->whereIn("range_id",$rangeIdArray)->get();
			}
			else
			{
				$allMashreqData = AgentPayout::whereIn("sales_time",$salesTimeArray)->whereIn("range_id",$rangeIdArray)->whereraw($whereRaw)->get();
			}
		}
		/* echo '<pre>';
	...
	public function revenuePerformanceAsFiltersENDBAgent(Request $request)
	{
		...
		if($range_id == "No")
		{
			if($whereRaw == '')
			{
				$allMashreqData = ENBDRevenueList::whereIn("sales_time",$salesTimeArray)->get();
			}
			else
			{
				$allMashreqData = ENBDRevenueList::whereIn("sales_time",$salesTimeArray)->whereraw($whereRaw)->get();
			}
		}
		else
		{
			...
			if($whereRaw == '')
			{
				$allMashreqData = ENBDRevenueList::whereIn("sales_time",$salesTimeArray)->whereIn("range_id",$rangeIdArray)->get();
			}
			else
			{
				$allMashreqData = ENBDRevenueList::whereIn("sales_time",$salesTimeArray)->whereIn("range_id",$rangeIdArray)->whereraw($whereRaw)->get();
			}
		}
		/* echo '<pre>';
		print_r($allMashreqData);
...
/*
		...
		*/
		...
		if($location != 'No')
		{
			if($whereRaw1 == '')
			{
				$whereRaw1 = "location = '".$location."'";
			}
			else
			{
				$whereRaw1 .= " AND location= '".$location."'";
			}
		}
		if($range_id == "No")
		{
			if($whereRaw1 != '')
			{
				$agentData = AgentPayout::whereIn("sales_time",$salesTimeArray)->where("agent_name",$agentName)->whereRaw($whereRaw1)->get();
			}
			else
			{
			$agentData = AgentPayout::whereIn("sales_time",$salesTimeArray)->where("agent_name",$agentName)->get();
			}
		}
		else
		{
			...
			if($whereRaw1 != '')
			{
				$agentData = AgentPayout::whereIn("sales_time",$salesTimeArray)->where("agent_name",$agentName)->whereIn("range_id",$rangeIdArray)->whereRaw($whereRaw1)->get();
			}
			else
			{
			$agentData = AgentPayout::whereIn("sales_time",$salesTimeArray)->where("agent_name",$agentName)->whereIn("range_id",$rangeIdArray)->get();
			}
		}
		/* echo '<pre>';
...
public function targetPerformanceAsFiltersENDB(Request $request)
{
		$salesMonthFrom =  $request->salesMonthFrom;
		$salesYearFrom =  $request->salesYearFrom;
		$salesMonthTo = $request->salesMonthTo;
		...
		else
		{
			...
			if($whereRaw == '')
			{
				$allMashreqData = AgentPayout::where("agent_target","!=",0)->whereIn("sales_time",$salesTimeArray)->whereIn("range_id",$rangeIdArray)->get();
			}
			else
			{
				$allMashreqData = AgentPayout::where("agent_target","!=",0)->whereIn("sales_time",$salesTimeArray)->whereIn("range_id",$rangeIdArray)->whereraw($whereRaw)->get();
			}
		}
		/* echo '<pre>';
...
public function updateENBDTable()
{
	$agentMod = AgentPayout::get();
	/* echo '<pre>';
	print_r($agentMod);
	exit; */
	foreach($agentMod as $mod)
	{
		$updateAgent = AgentPayout::find($mod->id);
		$tc_card = $mod->mass+$mod->premium+$mod->super_premium;
		$final_loan_amount =  $mod->personal_loan+$mod->auto_loan;
		$updateAgent->tc_card = $tc_card;
		$updateAgent->final_loan_amount = $final_loan_amount;
		$updateAgent->save();
	}
	echo "done";
	exit;
...
public function updateENDBData()
{
				$filename = 'enbd_data.csv';
				...
				$icsv = 0;
				foreach($dataFromCsv as $_dataCSV)
				{
					if($icsv != 0 && $_dataCSV != NULL)
					{
							$agentPayoutUpdate = AgentPayout::find($_dataCSV[0]);
							$agentPayoutUpdate->agent_target = $_dataCSV[1];
...
public function updateENBDFinalCards()
	{
		$agentPDatas = AgentPayout::get();
		
		foreach($agentPDatas as $agents)
		{
			$tc_card = $agents->tc_card;
			$final_loan_amount = $agents->final_loan_amount;
			$newCards = 0;
			if($final_loan_amount > 0)
			{
				$cardsIndex = ($final_loan_amount/50000);
				$newCards = floor($cardsIndex);
			}
			$agentPayoutUpdate = AgentPayout::find($agents->id);
			$agentPayoutUpdate->tc_final = $tc_card+$newCards;
			$agentPayoutUpdate->save();
...
public function updateENBDFinalLoan()
	{
		$agentPDatas = AgentPayout::get();
		/* echo'<pre>';
		print_r($agentPDatas);
		...
		foreach($agentPDatas as $agents)
		{
			...
			$super_premium = $agents->super_premium;
			$final_loan_amount = $agents->final_loan_amount;
			$newloan = 0;
			if(!empty($final_loan_amount) && $final_loan_amount != NULL)
			{
				$newloan = $final_loan_amount;
			}
			if($mass >0 && !empty($final_loan_amount) && $final_loan_amount != NULL)
			{
				$newloan = $newloan+$mass*50000;
			}
			if($premium >0 && !empty($final_loan_amount) && $final_loan_amount != NULL)
			{
				$newloan = $newloan+$premium*100000;
			}
			if($super_premium >0 && !empty($final_loan_amount) && $final_loan_amount != NULL)
			{
				$newloan = $newloan+$super_premium*150000;
			}
			$agentPayoutUpdate = AgentPayout::find($agents->id);
			$agentPayoutUpdate->final_loan_amount_r = $newloan;
			$agentPayoutUpdate->save();
	...
	public function targetPerformanceAsFiltersENDB_r(Request $request)
		{
				$salesMonthFrom =  $request->salesMonthFrom;
				$salesYearFrom =  $request->salesYearFrom;
				$salesMonthTo = $request->salesMonthTo;
				...
				*/
				...
				foreach($allMashreqData as $_tdata)
				{
					...
					if($_tdata->agent_product == 'CARD')
					{
						$totalCards = $totalCards+$_tdata->tc_final;
				$totalCardsCount++;
						if($agentTarget80 > $_tdata->tc_final)
						{
							$targetNotAchive80++;
						}
					}
					...
					{
							$totalLoan = $totalLoan+$_tdata->final_loan_amount_r;
				$totalLoanCount++;
				
						if($agentTarget80 > $_tdata->final_loan_amount_r)
						{
							$targetNotAchive80++;
						}
					}
				...
				*/
				...
				foreach($allMashreqData as $_tdata)
				{
					...
					if($_tdata->agent_product == 'CARDS')
					{
						if($AgentTarget <= $_tdata->tc_final)
						{
							$achivedCandidate++;
						}
					}
					...
					{
						if($AgentTarget <= $_tdata->final_loan_amount_r)
						{
							$achivedCandidate++;
						}
					}
				...
				*/
				...
				echo "<br />";
				echo "Agents Achived less than 80% of Target(Converted) - ".$targetNotAchive80;
				if($qt != 'No')
				{
					/*
					*count dynamic target not achive
					*/
					...
					foreach($allMashreqData as $_tdata)
					{
						...
						if($_tdata->agent_product == 'CARD')
						{
							if($agentTargetD > $_tdata->tc_final)
							{
								$targetNotAchiveD++;
							}
						}
						...
						{
							if($agentTargetD > $_tdata->final_loan_amount_r)
							{
								$targetNotAchiveD++;
							}
						}
					...
					echo "<br />";
					echo "Agents Achived less than ".$qt."% of Target(Converted) - ".$targetNotAchiveD;
				}
					echo "<br />";
					echo "Total Agents in Card(Converted) -".$totalCardsCount;
					echo "<br />";
					echo "Total Agents in loan(Converted) -".$totalLoanCount;
						echo "<br />";
					echo "Core Team Productivity (Cards-Converted) -".round($totalCards/$totalCardsCount,2);
					echo "<br />";
						echo "Core Team Productivity (PLs-Converted)-".round($totalLoan/$totalLoanCount,2);
					echo "<br />";
				/*
				*count dynamic target not achive
				*/
		...
		*/
			...
			if($agentId != "No")
			{
					...
					if($agentDetails != '')
					{
						...
						if($agentDetails->agent_product == 'CARD')
						{
							echo "Agent CARD(Converted)   <b>".$agentDetails->tc_final.'</b>';
							echo "<br />";
							if($agentDetails->agent_target > 0)
							{
								echo "Agent Achived Target(Converted) ".round(($agentDetails->tc_final/$agentDetails->agent_target)*100,2).'%';
							}
							else
							{
								echo "Agent Achived Target(Converted) 0%";
							}
						}
						else
						{
							echo "Agent Loan(Converted) AED<b>".$agentDetails->final_loan_amount_r.'</b>';
							echo "<br />";
							if($agentDetails->agent_target > 0)
							{
								echo "Agent Achived Target(Converted) ".round(($agentDetails->final_loan_amount_r/$agentDetails->agent_target)*100,2).'%';
							}
							else
							{
								echo "Agent Achived Target(Converted) 0%";
