...
class CommonController extends Controller
{
    ...
    {		
		...
		else if($fileType=='ENBDJonus')
		{
			return view("Common/commonTabsENBDJonus",compact('fileType'));
		}
	...
	public function ENBDTABJonusCalRenderTab(Request $request)
    {
	   $monthSelected = $request->m;
	   $yearSelected = $request->y;
	   return view("Common/ENBDTABJonusCalRender",compact('monthSelected','yearSelected'));
    }
	...
	public static function getENBDTABJonusFileLog($calendar_date=NULL)
    {
      return $getENBDJonusFileLog = DB::table('enbd_new_jonus_import_file')->where('calendar_date', $calendar_date)->orderBy('updated_at','DESC')->first();
    }
	...
	public static function getAllFileLogENBDTABJonus(Request $request)
    {
	  $d = $request->d;
	  $m = $request->m;
	  $y = $request->y;
      ...
      $getENBDJonusFileLog = DB::table('enbd_new_jonus_import_file')->where('calendar_date', $calendar_date)->orderBy('updated_at','DESC')->get();
	  

	  return view("Common/allFileLogENBDTABJonus",compact('getENBDJonusFileLog'));
...
public function FileUploadExcelENBDPLLink(Request $request)
				...
				public function ENBDPLLinkFileImport(Request $request)
				{
					...
					if(!empty($sheetData))
					{
						...
						{							
							...
							if($emp_name=='')
							{								
								$Employee_details_data = DepartmentFormController::getEmployeeDetailsBySorceCode($source_code);	

								$emp_name= @$Employee_details_data->first_name.(@$Employee_details_data->middle_name ? " ".@$Employee_details_data->middle_name:'').(@$Employee_details_data->last_name?" ".@$Employee_details_data->last_name:'');	
								
								$emp_id = @$Employee_details_data->emp_id;
							}
							...
							{			
									...
									ENBDLoanMIS::where('tab_id', $tab_id)
			...
			public function FileUploadExcelENBDTABJonus(Request $request)
				{
					$user_id = $request->session()->get('EmployeeId');
					...
					$fileName = $fileType.'_'.date("Y-m-d_h-i-s").'.xlsx';  
						...
						$highestRow = $worksheet->getHighestRow()-1; // e.g. 10							
							...
							$tableName = 'enbd_new_jonus_import_file';
				...
				public function ENBDTABJonusFileImport(Request $request)
				{
					$user_id = $request->session()->get('EmployeeId');
					...
					$fileInfo = DB::table('enbd_new_jonus_import_file')->where('id', $attr_f_import)->first();
					...
					if(!empty($sheetData))
					{
						...
						{							
							...
							{
								$fileInfo = DB::table('enbd_new_jonus_import_file')->where('id', $attr_f_import)->delete();
								$result['code'] = 300;
								echo json_encode($result);
								exit;
							}
							...
							/////////// Get from Internal /////////
							$applicationsid = $sheetData[$k][1];
							


							$file_values = array(	'applicationsid' => $applicationsid,
													'customer_type' => $sheetData[$k][0],
													'bidaya_pid' => $sheetData[$k][2],
													'product' => $sheetData[$k][3],
													'credit_status' => $sheetData[$k][4],									
													'rbe_name' => $sheetData[$k][5],
													'branch_name' => $sheetData[$k][6],
													'signeddatetime' => ($sheetData[$k][7]?date('Y-m-d',strtotime($sheetData[$k][7])):'0000-00-00'),
													'loan_type' => $sheetData[$k][8],
													'name' => $sheetData[$k][9],
													'loan_amount' => str_replace(',','',$sheetData[$k][10]),
													'topup_os_principal' => str_replace(',','',$sheetData[$k][11]),
													//'loan_amount' => $sheetData[$k][12],
													'tenure' => $sheetData[$k][13],
													'rate' => $sheetData[$k][14],
													'final_rate' => $sheetData[$k][15],
													'previous_liability' => $sheetData[$k][16],
													'p1code' => $sheetData[$k][17],
													'disbursal_datetime' => ($sheetData[$k][19]?date('Y-m-d',strtotime($sheetData[$k][19])):'0000-00-00'),
													'last_remarks_added' => $sheetData[$k][20],
													'submission_location' => $sheetData[$k][21],
													'dsa' => 'SMART UNION COMMERCIAL',
													'channel' => 'SMART UNION COMMERCIAL',
													'created_at' => date('Y-m-d H:i:s'),
													'updated_at' => date('Y-m-d H:i:s')
