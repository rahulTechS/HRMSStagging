				...
				public function ENDBJonusFileImport(Request $request)
				{
					...
					if(!empty($sheetData))
					{
						...
						{							
							...
							$check = DB::table('enbd_jonus_import_file_data')->whereRaw($whereRaw)->get();
						...
						{							
							...
							{	
								$file_valuesParent = array(
													...
													'ref_no' => $ref_no,
													'form_id' => 1,
													'form_title' => 'Credit Card Submission Form',
							...
							{			
									...
									DepartmentFormEntry::where('ref_no', $sheetData[$k][8])
										->update(['application_id' => $instanceid,'emp_id' => $emp_id,'emp_name' => $emp_name,'team' => $team]);
				...
				public function mashreqLoginFileImport(Request $request)
				{
					...
					if(!empty($sheetData))
					{
						for($k=0;$k<count($sheetData);$k++)
						{
							...
							$check = DB::table('mashreq_login_data')->whereRaw($whereRaw)->get();
			...
			public function mashreqBankFileImport(Request $request)
				{
					...
					if(!empty($sheetData))
					{
						for($k=0;$k<count($sheetData);$k++)
						{
							...
							$check = DB::table('mashreq_bank_mis')->whereRaw($whereRaw)->get();
				...
				{					
					...
					if(!empty($sheetData))
					{
						for($k=0;$k<count($sheetData);$k++)
						{
							...
							$check = DB::table('mashreq_booking_mis')->whereRaw($whereRaw)->get();
				...
				{					
					...
					if(!empty($sheetData))
					{
						for($k=0;$k<count($sheetData);$k++)
						{
							...
							$instanceid = $sheetData[$k][0];
							$whereRaw = " instanceid ='".$instanceid."'";
							$check = DB::table('mashreq_mtd_mis')->whereRaw($whereRaw)->get();
				...
				{					
					...
					if(!empty($sheetData))
					{
						for($k=0;$k<count($sheetData);$k++)
						{
							...
							$instanceid = $sheetData[$k][0];
							$whereRaw = " instanceid ='".$instanceid."'";
							$check = DB::table('mashreq_final_mtd_mis')->whereRaw($whereRaw)->get();
