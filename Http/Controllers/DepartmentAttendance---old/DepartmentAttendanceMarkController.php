...
class DepartmentAttendanceMarkController extends Controller
{
	...
	public static function getEmpid($userid)
	{
		
		//return $empid;
		$empDetails = User::where("id",$userid)->orderBy('id','desc')->first(); 


		if($empDetails)
		{
			return $empDetails->employee_id;
		}
		else
		{
			return '';
	...
	{
		$attendanceData = Attendance::orderBy('id','desc')->take(2)->get();
	...
	public function massMarkAttendanceRequestPostData(Request $request)
	{
		...
		else
		{
			...
			$emp_markoff = array_merge($request->emp_drop,$request->emp_drop2);

			foreach($request->emp_drop as $emp)
			{
				$emp_details = Employee_details::where("emp_id",$emp)->first(); 
					...
					if($emp_details)
					{
						...
						{
							if($emp_details->doj <= $request->attendanceDate)
							{
					...
					if($lastworkingdate)
					{
						if($request->attendanceDate > $lastworkingdate)
						{
							//echo "Hide";
						}
						else
			...
			foreach($request->emp_drop2 as $emp)
			{
				$emp_details = Employee_details::where("emp_id",$emp)->first(); 
					...
					if($emp_details)
					{
						...
						else
						{
							if($emp_details->doj <= $request->attendanceDate)
							{
								$empAttendanceData = EmpAttendance::where('emp_id',$emp)->where('attribute_code','attendance')->where('attendance_date',$request->attendanceDate)->orderBy('id','desc')->first();
								...
								else
								{
									...
									$attendanceData->emp_id = $emp;
									$attendanceData->attribute_code = 'attendance';
									$attendanceData->attribute_value = $request->attendanceType2;
									$attendanceData->attendance_date = $request->attendanceDate;
									$attendanceData->created_at = date('Y-m-d H:i:s');
									$attendanceData->status = 1;
									$attendanceData->attendance_mark_by = $usersessionId;
			...
			$emp_markoff = array_filter($emp_markoff, fn($value) => !is_null($value) && $value !== '');
			//return $emp_markoff;
			// for other employees start
			$emp_details = Employee_details::whereNotIn('emp_id', $emp_markoff)->get();
			...
			{
				$emp_details = Employee_details::where("emp_id",$emp->emp_id)->first(); 
	...
	public  function Index(Request $request)
	{
		...
		return view("DepartmentAttendance/index",compact('empData','departmentLists','designation'));
	}
	...
	{		
        ...
        if($whereraw != '')
		{
            ...
            if($departmentDetails != '')
            {
                ...
                if($empDetails!='')
                {
					...
					$empData = Employee_details::whereRaw($whereraw)
        ...
        else
        {
			...
			if($departmentDetails != '')
			{
				...
				if($empDetails!='')
				{
					$empData = Employee_details::where('employee_details.dept_id',$empDetails->dept_id)->orderBy('id','desc')->paginate($paginationValue);
	...
	public  function IndexLeadViewData(Request $request)
	{
		...
		return view("DepartmentAttendance/lead_index",compact('empData','departmentLists','designation'));
	}
