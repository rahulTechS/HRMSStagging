...
class SearchEngineEIBController extends Controller
{
    public static function getAgentFullName($empid)
	{
	 ...
	 public static function getTLName($empid)
		{
			$agentData = PreAgentPayoutEIBCard::where("employee_id",$empid)->orderby("id","DESC")->first(); 
			if($agentData != '')
			{
	...
	public function SearchEngineEIB(Request $request){
		
		return view("SearchEngine/SearchEngineEIB");
	   
	}
	Public function ListingSearchEngineEIB(Request $request){
		
		return view("SearchEngine/listingSearchEngineEIB");
	}
	
	public function SearchEngineFilterDataEIB(Request $request)
		{
			...
			$request->session()->put('rmslessthen_filter_inner_list_EIB',$rmslessthen);
			$request->session()->put('submissionday_filter_inner_list_EIB',$submissionday);
			$request->session()->put('Approvalratepercentage_filter_inner_list_EIB',$Approvalratepercentage);
			$request->session()->put('rmsbookingparday_filter_inner_list_EIB',$rmsbookingparday);
			$request->session()->put('RMsbookings_filter_inner_list_EIB',$RMsbookings);
			
			
		}
		public function resetSearchEngineFilterDataEIB(Request $request){
			$request->session()->put('searchEng_data_EIB','');
			$request->session()->put('searchEng_caption_EIB','');
			$request->session()->put('search_rest_agentdataEIB','');
		
		
		
		$request->session()->put('search_keywordIdValue_EIB','');	
		$request->session()->put('submission_type_search_EIB','');
		$request->session()->put('Card_type_search_EIB','');
		$request->session()->put('bureau_score_search_EIB','');
		$request->session()->put('bureau_segmentation_search_EIB','');
		$request->session()->put('mrs_score_search_EIB','');
		$request->session()->put('innter_engine_tl_EIB','');
		$request->session()->put('best_worse_engine_tl_EIB','');
		return redirect("SearchEngineEIB");
		}	
	...
	public function setKeywordXYEIB(Request $request)
	{
		...
		if($keywordId == 6)
		{
			...
			$collectionCardType = PreAgentPayoutEIBCard::groupBy('card_type')
							->selectRaw('count(*) as total, card_type')
							->get();
			
			return view("SearchEngine/setKeywordXYEIB",compact('keywordId','collectionCardType','bureauSegmentationArray'));
		}
		else if($keywordId == 9)
		{
			
			$bureauSegmentationArray = PreAgentPayoutEIBCard::groupBy('bureau_segmentation')
	...
	public function updateDataFromLoginEIB()
	{
		...
		*Cron Logs works
		*/
		$datas = PreAgentPayoutEIBCard::whereNull("update_by_login")->get();
		
		foreach($datas as $data)
		{
			$appId = $data->application_id;
			$detailsLogin = EibBankMis::where("application_no",$appId)->first();
			/* echo "<pre>";
			print_r($detailsLogin);
			exit; */
			if($detailsLogin !='')
			{
				$updateMe = PreAgentPayoutEIBCard::find($data->id);
	...
	function submitSearchEgnEIB(Request $request)
	{
		...
		else if($keywordIdValue == 6)
		{
			...
			$startDate = date("Y")."-".date("m")."-01";
			$request->session()->put('submission_type_search_EIB',$submission_type);
			$request->session()->put('Card_type_search_EIB',implode(",",$card_type));
			if($submission_type == 'submissions')
			{
		...
		else if($keywordIdValue == 7)
		{
			...
			{
				$detailsAgentsSubmissionTotal = PreAgentPayouttotal_cardCard::selectRaw("employee_id , count(employee_id) as total_card")
				->whereBetween("submission_date",[$startDate,$currentDate])
				->where("booking_status",2)
				->groupBy("employee_id")
				->get();
				
				$detailsAgentsSubmissionTotalB = PreAgentPayouttotal_cardCard::selectRaw("employee_id , count(employee_id) as total_card")
				->whereBetween("submission_date",[$startDate,$currentDate])
				->whereRaw($moreless1Html)
				->where("booking_status",2)
				->groupBy("employee_id")
			...
			else if($keywordIdValue == 8)
			{
				...
				else if($keywordIdValue == 9)
				{
					...
					$startDate = date("Y")."-".date("m")."-01";
					$request->session()->put('submission_type_search_EIB',$submission_type);
				    $request->session()->put('bureau_segmentation_search_EIB',implode(",",$bureau_segmentation));
					if($submission_type == 'submissions')
					{
						$submissionDONE = PreAgentPayoutEIBCard::selectRaw("employee_id , count(employee_id) as total_card")
						->whereBetween("submission_date",[$startDate,$currentDate])
		...
		if($keywordIdValue == 1 || $keywordIdValue ==3 || $keywordIdValue ==4 || $keywordIdValue == 5 || $keywordIdValue == 6 || $keywordIdValue == 7 || $keywordIdValue == 8 || $keywordIdValue == 9)
			{
					...
					elseif(($more_less == 'less' || $more_less == 'lessequal') && $submission_type == 'bookings'){
						//echo "h2";
					$restagent = PreAgentPayoutEIBCard::selectRaw("employee_id , count(employee_id) as total_card")
						->whereBetween("submission_date",[$startDate,$currentDate])
						->where("booking_status",2)
						->groupBy("employee_id")
						->get();
					...
					foreach($restagent as $ragent){
						$restdataarrya[]=$ragent->employee_id;
							...
							$tlagentdata=Employee_details::whereNotIn('emp_id',$restdataarrya)->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->get();
							if($tlagentdata!=''){
			...
			$request->session()->put('search_rest_agentdataEIB',$agentempid);	
		
		return redirect('SearchEngineEIB');
	}
	
	public function loadResultSEListingEIB(Request $request)
	{
		$employeeLists = array();
		$searchEng_caption = "No Search Keyword Selected.";
		$search_keywordIdValue = '';
		$agentdata=array();
		if(@$request->session()->get('search_rest_agentdataEIB') != '')
		{
			$agentdata = $request->session()->get('search_rest_agentdataEIB');
		}
		if(@$request->session()->get('searchEng_data_EIB') != '')
		{
			$employeeLists = $request->session()->get('searchEng_data_EIB');
		}
		if(@$request->session()->get('searchEng_caption_EIB') != '')
		{
			$searchEng_caption = $request->session()->get('searchEng_caption_EIB');
		}
		if(@$request->session()->get('search_keywordIdValue_EIB') != '')
		{
			$search_keywordIdValue = $request->session()->get('search_keywordIdValue_EIB');
		}
		...
		*/
		...
		return view("SearchEngine/loadResultSEListingEIB",compact('agentdata','employeeLists','searchEng_caption','search_keywordIdValue','tl_details','rangeIds','empIdList'));
	}
	...
	public static function getVintageData($empid = NULL)
	  ...
	  public static function getRangeId($empid = NULL)
	  {
			 ...
			 $getVintageData=PreAgentPayoutEIBCard::where("employee_id",$empid)->orderby("id","DESC")->first();
				if( $getVintageData != '')
				{
	...
	public function exportEmpSearchEngineReportEIB(Request $request)
		{
			...
			foreach ($selectedId as $sid) {
				...
				{
					$sheet->setCellValue('B'.$indexCounter, SearchEngineEIBController::getAgentFullName($sid))->getStyle('B'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
					$sheet->setCellValue('C'.$indexCounter, $sid)->getStyle('C'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
					$tlName = SearchEngineEIBController::getTLName($sid);
					$sheet->setCellValue('D'.$indexCounter, '')->getStyle('D'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				
					$sheet->setCellValue('E'.$indexCounter, SearchEngineEIBController::getTLName($sid))->getStyle('E'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				...
				$sheet->setCellValue('H'.$indexCounter, SearchEngineEIBController::getVintageData($sid))->getStyle('H'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('I'.$indexCounter, SearchEngineEIBController::getrecruiterName($sid))->getStyle('I'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('J'.$indexCounter, SearchEngineEIBController::getrecruiterCat($sid))->getStyle('J'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
	...
	public function SearchStartEngineEIB(Request $request)
	{
		...
		if($tl_name != '')
		{
			$request->session()->put('innter_engine_tl_EIB',$tl_name);
		}
		else
		{
			$request->session()->put('innter_engine_tl_EIB','');
		...
		if($processor != '')
		{
			$request->session()->put('innter_engine_processor_EIB',$processor);
		}
		else
		{
			$request->session()->put('innter_engine_processor_EIB','');
		...
		if($range_id != '')
		{
			$request->session()->put('innter_engine_rangeId_EIB',$range_id);
		}
		else
		{
			$request->session()->put('innter_engine_rangeId_EIB','');
	...
	public function ResetStartEngineEIB(Request $request)
	{
		$request->session()->put('innter_engine_tl_EIB','');
		$request->session()->put('innter_engine_processor_EIB','');
		$request->session()->put('innter_engine_rangeId_EIB','');
	...
	public function detailsSearchResultEIB(Request $request)
	{
		 $emp_id = $request->emp_id;
		  ...
		  $search_keywordIdValue = $request->search_keywordIdValue;
			...
			{
				$y_value = $request->session()->get('y_value_search_EIB');
				$submission_type = $request->session()->get('submission_type_search_EIB');
				$currentDate = date("Y-m-d");
				$startDate = date("Y-m-d",strtotime("-".$y_value." days ".$currentDate));
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->get();
				}
				else
				{
					$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->get();
				}
			...
			else if($search_keywordIdValue == 6)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_EIB');
				$Card_type_search =	$request->session()->get('Card_type_search_EIB');
				$Card_type_searchArr = explode(",",$Card_type_search);
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->whereIn("card_type",$Card_type_searchArr)->get();
				}
				else
				{
				$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->whereIn("card_type",$Card_type_searchArr)->get();	
				}
			...
			else if($search_keywordIdValue == 7)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_EIB');
				$moreless1Html = $request->session()->get('bureau_score_search_EIB');
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->whereRaw($moreless1Html)->get();
				}
				else
				{
				$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->whereRaw($moreless1Html)->get();	
				}
			...
			else if($search_keywordIdValue == 8)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_EIB');
				$moreless1Html = $request->session()->get('mrs_score_search_EIB');
			...
			else if($search_keywordIdValue == 9)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_EIB');
				$bureau_segmentation_search = $request->session()->get('bureau_segmentation_search_EIB');
				$bureau_segmentation_searchArr = explode(",",$bureau_segmentation_search);
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->whereIn("bureau_segmentation",$bureau_segmentation_searchArr)->get();
				}
				else
				{
				$detailsSubmissions = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->whereIn("bureau_segmentation",$bureau_segmentation_searchArr)->get();	
				}
			}
			else
			{
			...
			return view("SearchEngine/detailsSearchResultEIB",compact('detailsSubmissions'));
	}
	...
	public static function getBookingSearch1($empId=NULL)
		{
					...
					$currentDate = date("Y-m-d");
					$startDate = date("Y-m-d",strtotime("-".$y_value." days ".$currentDate));
					return $detailsAgents = PreAgentPayoutEIBCard::whereBetween("submission_date",[$startDate,$currentDate])
