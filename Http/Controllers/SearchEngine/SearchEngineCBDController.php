...
class SearchEngineCBDController extends Controller
{
    public static function getAgentFullName($empid)
	{
	 ...
	 public static function getTLName($empid)
		{
			$agentData = PreAgentPayoutCBDCard::where("employee_id",$empid)->orderby("id","DESC")->first(); 
			if($agentData != '')
			{
	...
	public function SearchEngineCBD(Request $request){
		
		return view("SearchEngine/SearchEngineCBD");
	   
	}
	Public function ListingSearchEngineCBD(Request $request){
		
		return view("SearchEngine/listingSearchEngineCBD");
	}
	
	public function SearchEngineFilterDataCBD(Request $request)
		{
			...
			$request->session()->put('rmslessthen_filter_inner_list_CBD',$rmslessthen);
			$request->session()->put('submissionday_filter_inner_list_CBD',$submissionday);
			$request->session()->put('Approvalratepercentage_filter_inner_list_CBD',$Approvalratepercentage);
			$request->session()->put('rmsbookingparday_filter_inner_list_CBD',$rmsbookingparday);
			$request->session()->put('RMsbookings_filter_inner_list_CBD',$RMsbookings);
			
			
		}
		public function resetSearchEngineFilterDataCBD(Request $request){
			$request->session()->put('searchEng_data_CBD','');
			$request->session()->put('searchEng_caption_CBD','');
			$request->session()->put('search_rest_agentdataCBD','');
		
		
		
		$request->session()->put('search_keywordIdValue_CBD','');	
		$request->session()->put('submission_type_search_CBD','');
		$request->session()->put('Card_type_search_CBD','');
		$request->session()->put('bureau_score_search_CBD','');
		$request->session()->put('bureau_segmentation_search_CBD','');
		$request->session()->put('mrs_score_search_CBD','');
		$request->session()->put('innter_engine_tl_CBD','');
		$request->session()->put('best_worse_engine_tl_CBD','');
		return redirect("SearchEngineCBD");
		}	
	...
	public function setKeywordXYCBD(Request $request)
	{
		...
		if($keywordId == 6)
		{
			...
			$collectionCardType = PreAgentPayoutCBDCard::groupBy('card_type')
							->selectRaw('count(*) as total, card_type')
							->get();
			
			return view("SearchEngine/setKeywordXYCBD",compact('keywordId','collectionCardType','bureauSegmentationArray'));
		}
		else if($keywordId == 9)
		{
			
			$bureauSegmentationArray = PreAgentPayoutCBDCard::groupBy('bureau_segmentation')
	...
	public function updateDataFromLoginCBD()
	{
		...
		*Cron Logs works
		*/
		$datas = PreAgentPayoutCBDCard::whereNull("update_by_login")->get();
		
		foreach($datas as $data)
		{
			$appId = $data->application_id;
			$detailsLogin = CBDBankMis::where("ref_no",$appId)->first();
			/* echo "<pre>";
			print_r($detailsLogin);
			exit; */
			if($detailsLogin !='')
			{
				$updateMe = PreAgentPayoutCBDCard::find($data->id);
	...
	function submitSearchEgnCBD(Request $request)
	{
		...
		else if($keywordIdValue == 6)
		{
			...
			$startDate = date("Y")."-".date("m")."-01";
			$request->session()->put('submission_type_search_CBD',$submission_type);
			$request->session()->put('Card_type_search_CBD',implode(",",$card_type));
			if($submission_type == 'submissions')
			{
		...
		else if($keywordIdValue == 7)
		{
			...
			else if($keywordIdValue == 8)
			{
				...
				else if($keywordIdValue == 9)
				{
					...
					$startDate = date("Y")."-".date("m")."-01";
					$request->session()->put('submission_type_search_CBD',$submission_type);
				    $request->session()->put('bureau_segmentation_search_CBD',implode(",",$bureau_segmentation));
					if($submission_type == 'submissions')
					{
						$submissionDONE = PreAgentPayoutCBDCard::selectRaw("employee_id , count(employee_id) as total_card")
						->whereBetween("submission_date",[$startDate,$currentDate])
		...
		if($keywordIdValue == 1 || $keywordIdValue ==3 || $keywordIdValue ==4 || $keywordIdValue == 5 || $keywordIdValue == 6 || $keywordIdValue == 7 || $keywordIdValue == 8 || $keywordIdValue == 9)
			{
					...
					elseif(($more_less == 'less' || $more_less == 'lessequal') && $submission_type == 'bookings'){
						//echo "h2";
					$restagent = PreAgentPayoutCBDCard::selectRaw("employee_id , count(employee_id) as total_card")
						->whereBetween("submission_date",[$startDate,$currentDate])
						->where("booking_status",2)
						->groupBy("employee_id")
						->get();
					...
					foreach($restagent as $ragent){
						$restdataarrya[]=$ragent->employee_id;
							...
							$tlagentdata=Employee_details::whereNotIn('emp_id',$restdataarrya)->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->get();
							if($tlagentdata!=''){
			...
			$request->session()->put('search_rest_agentdataCBD',$agentempid);	
		
		return redirect('SearchEngineCBD');
	}
	
	public function loadResultSEListingCBD(Request $request)
	{
		$employeeLists = array();
		$searchEng_caption = "No Search Keyword Selected.";
		$search_keywordIdValue = '';
		$agentdata=array();
		if(@$request->session()->get('search_rest_agentdataCBD') != '')
		{
			$agentdata = $request->session()->get('search_rest_agentdataCBD');
		}
		if(@$request->session()->get('searchEng_data_CBD') != '')
		{
			$employeeLists = $request->session()->get('searchEng_data_CBD');
		}
		if(@$request->session()->get('searchEng_caption_CBD') != '')
		{
			$searchEng_caption = $request->session()->get('searchEng_caption_CBD');
		}
		if(@$request->session()->get('search_keywordIdValue_CBD') != '')
		{
			$search_keywordIdValue = $request->session()->get('search_keywordIdValue_CBD');
		}
		...
		*/
		...
		return view("SearchEngine/loadResultSEListingCBD",compact('agentdata','employeeLists','searchEng_caption','search_keywordIdValue','tl_details','rangeIds','empIdList'));
	}
	...
	public static function getVintageData($empid = NULL)
	  ...
	  public static function getRangeId($empid = NULL)
	  {
			 ...
			 $getVintageData=PreAgentPayoutCBDCard::where("employee_id",$empid)->orderby("id","DESC")->first();
				if( $getVintageData != '')
				{
	...
	public function exportEmpSearchEngineReportCBD(Request $request)
		{
			...
			foreach ($selectedId as $sid) {
				...
				{
					$sheet->setCellValue('B'.$indexCounter, SearchEngineCBDController::getAgentFullName($sid))->getStyle('B'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
					$sheet->setCellValue('C'.$indexCounter, $sid)->getStyle('C'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
					$tlName = SearchEngineCBDController::getTLName($sid);
					$sheet->setCellValue('D'.$indexCounter, '')->getStyle('D'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				
					$sheet->setCellValue('E'.$indexCounter, SearchEngineCBDController::getTLName($sid))->getStyle('E'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				...
				$sheet->setCellValue('H'.$indexCounter, SearchEngineCBDController::getVintageData($sid))->getStyle('H'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('I'.$indexCounter, SearchEngineCBDController::getrecruiterName($sid))->getStyle('I'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('J'.$indexCounter, SearchEngineCBDController::getrecruiterCat($sid))->getStyle('J'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
	...
	public function SearchStartEngineCBD(Request $request)
	{
		...
		if($tl_name != '')
		{
			$request->session()->put('innter_engine_tl_CBD',$tl_name);
		}
		else
		{
			$request->session()->put('innter_engine_tl_CBD','');
		...
		if($processor != '')
		{
			$request->session()->put('innter_engine_processor_CBD',$processor);
		}
		else
		{
			$request->session()->put('innter_engine_processor_CBD','');
		...
		if($range_id != '')
		{
			$request->session()->put('innter_engine_rangeId_CBD',$range_id);
		}
		else
		{
			$request->session()->put('innter_engine_rangeId_CBD','');
	...
	public function ResetStartEngineCBD(Request $request)
	{
		$request->session()->put('innter_engine_tl_CBD','');
		$request->session()->put('innter_engine_processor_CBD','');
		$request->session()->put('innter_engine_rangeId_CBD','');
	...
	public function detailsSearchResultCBD(Request $request)
	{
		 $emp_id = $request->emp_id;
		  ...
		  $search_keywordIdValue = $request->search_keywordIdValue;
			...
			{
				$y_value = $request->session()->get('y_value_search_CBD');
				$submission_type = $request->session()->get('submission_type_search_CBD');
				$currentDate = date("Y-m-d");
				$startDate = date("Y-m-d",strtotime("-".$y_value." days ".$currentDate));
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->get();
				}
				else
				{
					$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->get();
				}
			...
			else if($search_keywordIdValue == 6)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_CBD');
				$Card_type_search =	$request->session()->get('Card_type_search_CBD');
				$Card_type_searchArr = explode(",",$Card_type_search);
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->whereIn("card_type",$Card_type_searchArr)->get();
				}
				else
				{
				$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->whereIn("card_type",$Card_type_searchArr)->get();	
				}
			...
			else if($search_keywordIdValue == 7)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_CBD');
				$moreless1Html = $request->session()->get('bureau_score_search_CBD');
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->whereRaw($moreless1Html)->get();
				}
				else
				{
				$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->whereRaw($moreless1Html)->get();	
				}
			...
			else if($search_keywordIdValue == 8)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_CBD');
				$moreless1Html = $request->session()->get('mrs_score_search_CBD');
			...
			else if($search_keywordIdValue == 9)
			{
				...
				$startDate = date("Y")."-".date("m")."-01";
				$submission_type = $request->session()->get('submission_type_search_CBD');
				$bureau_segmentation_search = $request->session()->get('bureau_segmentation_search_CBD');
				$bureau_segmentation_searchArr = explode(",",$bureau_segmentation_search);
				if($submission_type == 'bookings')
				{
					$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("booking_status",2)->where("employee_id",$emp_id)->whereIn("bureau_segmentation",$bureau_segmentation_searchArr)->get();
				}
				else
				{
				$detailsSubmissions = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])->where("employee_id",$emp_id)->whereIn("bureau_segmentation",$bureau_segmentation_searchArr)->get();	
				}
			}
			else
			{
			...
			return view("SearchEngine/detailsSearchResultCBD",compact('detailsSubmissions'));
	}
	...
	public static function getBookingSearch1($empId=NULL)
		{
					...
					$currentDate = date("Y-m-d");
					$startDate = date("Y-m-d",strtotime("-".$y_value." days ".$currentDate));
					return $detailsAgents = PreAgentPayoutCBDCard::whereBetween("submission_date",[$startDate,$currentDate])
