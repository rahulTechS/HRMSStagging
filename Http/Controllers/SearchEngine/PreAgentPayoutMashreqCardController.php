...
class PreAgentPayoutMashreqCardController extends Controller
{
    
	public function UpdatePrePayoutMashreqCardData(){
		
		/*
		*Cron Logs works
		*/
		$createCronLogs = new CronRunLogs();
		$createCronLogs->title = 'SearchEngine-MashreqCard';
		$createCronLogs->save();
		...
		*/
		...
		$_DepartmentData=DepartmentFormEntry::where("form_id",1)->whereNotNull('application_id')->whereNull('search_engine_status')->orderBy("id","DESC")->first();
		...
		if($_DepartmentData != ''){
			//$i=0;
				...
				*end code
				*/
				$PreAgentPayoutMashreqCardMOd = PreAgentPayoutMashreqCard::where('application_id',$_DepartmentData->application_id)->first();							
				if($PreAgentPayoutMashreqCardMOd!='')
				{
				$prepayoutOBJ = PreAgentPayoutMashreqCard::find($PreAgentPayoutMashreqCardMOd->id);
				//echo "h1<br>";
					
				}
				else{
					$prepayoutOBJ =new PreAgentPayoutMashreqCard();
				}
				...
				$Recruiter =RecruiterDetails::where("id",$agentData->recruiter)->first();
					  ...
					  $bookingdata=MashreqBookingMIS::where("instanceid",$_DepartmentData->application_id)->get()->count();
					  if($bookingdata>0){
						  $bookingmis=2;
					  }
					  else{
				...
				$prepayoutOBJ->application_id=$_DepartmentData->application_id;
				...
				$prepayoutOBJ->submission_date=date("Y-m-d",strtotime($_DepartmentData->submission_date));					
	...
	public function calculateVintageAgentPrePayoutmashreqData()
			{
				/*
				*Cron Logs works
				*/
				...
				*Cron Logs works
				*/
				$agentPayoutMod = PreAgentPayoutMashreqCard::whereNull("vintage_status")->get();
			     /*  echo '<pre>';
				...
				foreach($agentPayoutMod as $payout)
				{
						...
						if($dojEmp != '' && $dojEmp != NULL)
						{
							...
							$agentPUpdate = PreAgentPayoutMashreqCard::find($payout->id);
		...
		exit;
			...
			public function UpdateTimeRangePrePayoutMashreqData(){
				/*
		*Cron Logs works
		*/
		$createCronLogs = new CronRunLogs();
		...
		*/
			...
			foreach($data as $_time){
					...
					$PayoutData =PreAgentPayoutMashreqCard::whereRaw($whereraw)->whereNull("range_status")->get();
					foreach($PayoutData as $_newdata){
						$updateMod = PreAgentPayoutMashreqCard::find($_newdata->id);
...
public function UpdateCronMashreqCard()
{
			$data = PreAgentPayoutMashreqCard::where("use_cron",1)->first();
			...
			*booking check
			*/
			$bookingdata=MashreqBookingMIS::where("instanceid",$application_id)->get()->count();
			if($bookingdata >0)
			{
				$updateSubmission = PreAgentPayoutMashreqCard::find($data->id);
			...
			{
				$mtdData = 	MashreqMTDMIS::where("instanceid",$application_id)->get()->count();
				if($mtdData >0)
				{
				...
				{
					$mashreqBankMis = MashreqBankMIS::select("status")->where("rcms_id",$application_id)->first();
					
					if($mashreqBankMis != '')
					{
						$status = $mashreqBankMis->status;
						if($status == 'Decline' || $status == 'Declined')
						{
							$updateSubmission = PreAgentPayoutMashreqCard::find($data->id);
					
							$updateSubmission->use_cron = 2;
							$updateSubmission->submission_status=$status;
							$updateSubmission->save();
						}
					}
					else
...
public function updateBookingMISStatus()
{
	/*
		*Cron Logs works
		*/
		...
		*Cron Logs works
		*/
	$datas = MashreqBookingMIS::whereNull("update_search_eng")->limit(100)->orderBy("id","DESC")->get();
	/* echo "<pre>";
	print_r($datas);
	exit; */
	foreach($datas as $data)
	{
		...
		$checkBooking = PreAgentPayoutMashreqCard::where("application_id",$appId)->get()->count();
		if($checkBooking > 0)
		{
			if(PreAgentPayoutMashreqCard::where('application_id', $appId)->update([ 'use_cron' => 2, 'booking_status'=>2, 'submission_status'=>"Booked"]))
			{
				
				$update = MashreqBookingMIS::find($data->id);
