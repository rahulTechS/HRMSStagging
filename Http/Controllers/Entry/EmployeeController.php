...
class EmployeeController extends Controller
{
	...
	public function checkEmployeePost(Request $req)
	{
		...
		$getmatchempid =  Employee::where('employee_id',$username)->where("status",1)->get()->count();
		...
		if($getmatch >0)
		{
			...
			{
			$req->session()->put('company_RecruiterName_filter_inner_list',"");		
			...
			*/
			$logObj = new LoginLog();
			$logObj->user_id =$req->session()->get('EmployeeId');
			$logObj->ip =$ip;
			$logObj->status =1;
			$logObj->save();
			
			$userId = $req->session()->get('EmployeeId');
			/*
			*check employee degination
			*/
			/* if($employeeList->designation == 'Admin')
			{ */
		if($userId == 61 || $userId == 72 || $userId == 1)
		{
		...
		else if($getmatchempid>0){
			$employeeList = Employee::where('employee_id',$username)->first();
			$passwordFromUsername = Crypt::decrypt($employeeList->password);
			if( $passwordFromUsername == $password)
			{
			$req->session()->put('company_RecruiterName_filter_inner_list',"");		
	...
	public function dashboard(Request $req)
	{
		...
		if($empId == 1 || $empId == 60 || $empId == 61 || $empId == 72 || $empId == 71)
				{
					return redirect('parentdashboard');
	...
	public function leaveEmployee(Request $request)
	{
			...
			$request->session()->forget('ParentEmployeeId');
			return redirect('/');
	}
