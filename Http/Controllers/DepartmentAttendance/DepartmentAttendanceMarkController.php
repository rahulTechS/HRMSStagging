...
class DepartmentAttendanceMarkController extends Controller
{
	public  function departmentMarkAttendance(Request $request)
	{
        ...
        $departmentDetails = JobFunctionPermission::select('emp_id')->where("user_id",$loggedinUserid)->first();
        if($departmentDetails != '')
        {
            $empDetails=Employee_details::select('dept_id')->where("emp_id",$departmentDetails->emp_id)->first();
            if($empDetails!='')
            {
                $empData = Employee_details::select('emp_id','emp_name')->where('employee_details.dept_id',$empDetails->dept_id)->orderBy('id','desc')->get();
            }
        }
        else
        {
            $empData = Employee_details::select('emp_id','emp_name')->orderBy('id','desc')->take(5)->get();
        }
	...
	{
		$empDetails = Employee_details::select('dept_id')->where("emp_id",$empid)->orderBy('id','desc')->first();
		
		if($empDetails)
		{
			$departmentDetails = Department::select('department_name')->where("id",$empDetails->dept_id)->first();
			if($departmentDetails != '')
			{
	...
	{
		$empDetails = Employee_details::select('designation_by_doc_collection')->where("emp_id",$empid)->orderBy('id','desc')->first(); 
		//return $empDetails;

		if($empDetails)
		{
			$designationDetails = Designation::select('name')->where("id",$empDetails->designation_by_doc_collection)->first();
	...
	public static function getTeamLeader($empid)
	{
		...
		//return $empid;
		$empDetails = Employee_details::select('tl_id')->where("emp_id",$empid)->orderBy('id','desc')->first(); 


		if($empDetails)
		{
			$emp_details = Employee_details::select('emp_name')->where("id",$empDetails->tl_id)->first(); 
			if($emp_details!='')
			{
	...
	public static function getAttendanceStatus($emp_id,$gDate)
	{
		//return $gDate;
		$tdate = date('Y-m-d');
		// $attendanceData = EmpAttendance::select('attribute_value')->where('approve_status', 2)->where('emp_id',$emp_id)->where('attribute_code','attendance')->where('attendance_date',$gDate)->orderBy('id','desc')->first();


		$staff_ids=2;
		$attendanceData =  EmpAttendance::select('attribute_value')->where('emp_id',$emp_id)
		->where('attribute_code','attendance')
		->where('attendance_date',$gDate)
		->where(function($q) use($staff_ids){
		$q->where('approve_status',$staff_ids)
	...
	public static function getLoggedinUser($loggedinUserid)
	{
        ...
        if($departmentDetails != '')
        {
            $empDetails=Employee_details::select('emp_id')->where("emp_id",$departmentDetails->emp_id)->first();
            if($empDetails!='')
            {
				$employeeData=2;
            }
	...
	public  function Index(Request $request)
	{
        ...
        {
			$empData = Employee_details::select('emp_id','emp_name')->orderBy('id','desc')->get();			
        }

		$departmentLists =  Department::select('id','department_name')->where("status",1)->orderBy("id","DESC")->get();
		...
		if($departmentDetails != '')
		{
			...
			$loggedinEmpdetails = Employee_details::where('emp_id',$loggedinUserdetails->employee_id)->orderBy("id","DESC")->first();
				...
				$attendanceDetails = Attendance::where('status',1)->orderBy("id","DESC")->get();
		...
		return view("DepartmentAttendance/index",compact('empData','departmentLists','designation','tL_details','attendanceDetails','departmentid'));
	}
	...
	{		
		...
		$attendanceFilter='';
		if(!empty($request->session()->get('attendance_type')) && $request->session()->get('attendance_type') != 'All')
		{
			$teamlead = $request->session()->get('attendance_type');
			if($whererawAttendnce == '')
			{
				$whererawAttendnce = "attribute_value IN('".$teamlead."')";
				$attendanceFilter=1;
			}
			else
			{
				$whererawAttendnce .= " And attribute_value IN('".$teamlead."')";
				$attendanceFilter=1;
			}
		}
		...
		if(!empty($request->session()->get('emp_attendance_fromdate')) && $request->session()->get('emp_attendance_fromdate') != 'All')
		{
			$attendanceDateFilter=1;
			$datefrom = $request->session()->get('emp_attendance_fromdate');
			if($whererawAttendnce == '')
			{
				$whererawAttendnce = 'attendance_date >= "'.$datefrom.'"';
			}
			else
			{
				$whererawAttendnce .= ' And attendance_date >= "'.$datefrom.'"';
			}
		}
		if(!empty($request->session()->get('emp_attendance_todate')) && $request->session()->get('emp_attendance_todate') != 'All')
		{
			$attendanceDateFilter=1;
			$dateto = $request->session()->get('emp_attendance_todate');
			if($whererawAttendnce == '')
			{
				$whererawAttendnce = 'attendance_date <= "'.$dateto.'"';
			}
			else
			{
				$whererawAttendnce .= ' And attendance_date <= "'.$dateto.'"';
			}		
		}
		...
		$loggedinUserid=$request->session()->get('EmployeeId');
        if($whereraw != '' || $whererawAttendnce !='')
		{
			
			// echo "<pre>";
			// print_r($whereraw);
			// print_r($whererawAttendnce);
			// echo $attendanceDateFilter;
			// echo $attendanceFilter;
			// exit;
			...
			if($empData==1)
			{
				...
				$empData_details = Employee_details::select('emp_id','offline_status','doj')->orderBy('id','desc')->get();
				$lastworkingdate='';
				$offmarkEmp=array();
				foreach($empData_details as $emp)
				{
					if($emp->offline_status==2)
					{
						$offlineEmpData = EmpOffline::select('last_working_day_resign','last_working_day_terminate','created_at')->where('emp_id',$emp->emp_id)->orderBy('id','desc')->first();						
						if($offlineEmpData)
						{
							if($offlineEmpData->last_working_day_resign!=''|| $offlineEmpData->last_working_day_resign!=NULL)
							{
				...
				$attendanceArr=array();
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					
			
					if($attendanceDateFilter==1)
					{
						
						$empAttendanceData = EmpAttendance::whereRaw($whererawAttendnce)->orderBy('id','desc')->get();
					}
					else
					{
						$currDate = date('Y-m-d');
						$empAttendanceData = EmpAttendance::whereRaw($whererawAttendnce)->where('attendance_date',$currDate)->orderBy('id','desc')->get();
					}
					
					
					//dd($empAttendanceData);
					foreach($empAttendanceData as $empAtt)
					{
						$attendanceArr[]=$empAtt->emp_id;	
					}
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
				...
				$departmentDetails = JobFunctionPermission::select('emp_id')->where("user_id",$loggedinUserid)->first();
				$empDetails=Employee_details::select('dept_id')->where("emp_id",$departmentDetails->emp_id)->first();
				$empData_details = Employee_details::select('emp_id','offline_status','doj')->where('employee_details.dept_id',$empDetails->dept_id)->orderBy('id','desc')->get();
				$lastworkingdate='';
				$offmarkEmp=array();
				foreach($empData_details as $emp)
				{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('employee_details.dept_id',$empDetails->dept_id)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('employee_details.dept_id',$empDetails->dept_id)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
		...
		$holidayDetails = HolidayList::select('from_date','to_date')->where('from_date','LIKE',"%{$searchdate}%")->where('to_date','LIKE',"%{$searchdate}%")->get();
		$holidayarray = array();
	    ...
	    return view("DepartmentAttendance/listingAttendance",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}
	...
	public function searchAttendanceFilter(Request $request)
	{
		...
		//return $request->all();
			...
			$datefromAttendance = $request->input('datefromAttendance');
			$datetoAttendance = $request->input('datetoAttendance');
			...
			$attendanceShort='';
			if($request->input('attendanceShort')!=''){
			 
			 $attendanceShort=implode(",", $request->input('attendanceShort'));
			}
			//return "Test".$name;
			...
			$request->session()->put('emp_attendance_fromdate',$datefromAttendance);
			$request->session()->put('emp_attendance_todate',$datetoAttendance);
			$request->session()->put('attendance_type',$attendanceShort);
    ...
    public function resetAttendanceFilter(Request $request)
    {
		...
		$request->session()->put('attendance_type','');
			$request->session()->put('emp_attendance_fromdate','');
			$request->session()->put('emp_attendance_todate','');
	...
	public function exportAttendanceReport(Request $request)
	{
			...
			$parameters = $request->input(); 
				...
				foreach($list as $daysList)
				{
					...
					$h2 = getNameFromNumber($k);
					$sheet->setCellValue($h2.$indexCounter, strtoupper('In-Time'))->getStyle($h2.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');

					$h3 = getNameFromNumber($l);
					$sheet->setCellValue($h3.$indexCounter, strtoupper('Out-Time'))->getStyle($h3.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
					$j = $j+3;
					$k = $k+3;
					$l = $l+3;
				...
				foreach ($selectedId as $sid) 
				{
					...
					foreach($tlist as $daysList)
					{
						...
						$h2 = getNameFromNumber($k);
						$sheet->setCellValue($h2.$indexCounter, "Test1")->getStyle($h2.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');

						$h3 = getNameFromNumber($l);
						$sheet->setCellValue($h3.$indexCounter, "Test22")->getStyle($h3.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
	...
	public function markAttendanceCronOld(Request $request)
	{
		...
		else
		{
			...
			$usersessionId=$request->session()->get('EmployeeId');
			//$spec = array('101752');
			$empAttendanceData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->get();
			$newResult=array();
			foreach($empAttendanceData as $emp)
			{
				...
				if($empAttendanceData)
				{
					...
					$requestedLeaves = RequestedLeaves::where('emp_id',$emp->emp_id)->where('approved_reject_status',1)->where('final_status',1)->first();

					if($requestedLeaves)
					{
						$empAttendanceData2 = EmpAttendance::where('emp_id',$emp->emp_id)->where('attribute_code','attendance')->where('attendance_date',$todayDate)->orderBy('id','desc')->first();
						...
						if($requestedLeaves->updated_from_date==NULL && $requestedLeaves->updated_to_date==NULL)
						{
							if($requestedLeaves->from_date <= $todayDate && $requestedLeaves->to_date >= $todayDate)
							{
								$timestamp = strtotime($todayDate);
								$weakoffday = date('N', $timestamp);
								...
								$empAttendanceData2->emp_id = $requestedLeaves->emp_id;
								$empAttendanceData2->attribute_code = 'attendance';
								$empAttendanceData2->attribute_value = $requestedLeaveTypes->sort_title;
								$empAttendanceData2->attendance_date = $todayDate;
								$empAttendanceData2->created_at = date('Y-m-d H:i:s');
								$empAttendanceData2->status = 1;
								$empAttendanceData2->attendance_mark_by = $usersessionId;
								$empAttendanceData2->attendance_mark_on = date('Y-m-d');
								$empAttendanceData2->attendance_mark_using = 'Cron';
								$empAttendanceData2->save(); 
							}                       
						}
						else
						{
							if($requestedLeaves->updated_from_date <= $todayDate && $requestedLeaves->updated_to_date >= $todayDate)
							{
								$timestamp = strtotime($todayDate);
								$weakoffday = date('N', $timestamp);
	...
	public function markAttendanceCron(Request $request)
	{
		...
		//$todayDate = date('2025-01-18');
		


		$requestedLeavesPreCheck = RequestedLeaves::where('final_status',1)->where('approved_reject_status',1)->get();
		$newRowResult=array();
		$newEmpResult=array();
		...
		// {
			...
			foreach($empAttendanceData as $emp)
			{
				...
				if($empAttendanceData)
				{
					...
					$attendanceData->emp_id = $emp->emp_id;
					$attendanceData->attribute_code = 'attendance';
					$attendanceData->attribute_value = 'H';
					$attendanceData->attendance_date = $todayDate;
					$attendanceData->created_at = date('Y-m-d H:i:s');
					$attendanceData->status = 1;
					$attendanceData->attendance_mark_by = $usersessionId;
	...
	public function markAttendanceCron333(Request $request)  ///for whole emp
	{
		$todayDate = date('2024-04-12');
	...
	// new code for all departments tab listing start (14-1-2025)


	public function attendanceListingENBDData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',9)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',9)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',9)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',9)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',9)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',9)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',9)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',9)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceENBD'));    
	    return view("DepartmentAttendance/listingAttendanceENBD",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}


	public function attendanceListingMashreqData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',36)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',36)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',36)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',36)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',36)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',36)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',36)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',36)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceMashreq'));    
	    return view("DepartmentAttendance/listingAttendanceMashreq",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}

	public function attendanceListingDeemData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',8)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',8)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',8)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',8)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',8)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',8)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',8)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',8)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceDeem'));    
	    return view("DepartmentAttendance/listingAttendanceDeem",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}


	public function attendanceListingCBDData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',49)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',49)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',49)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',49)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',49)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',49)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',49)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',49)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceCBD'));    
	    return view("DepartmentAttendance/listingAttendanceCBD",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}



	public function attendanceListingEIBData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',52)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',52)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',52)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',52)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',52)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',52)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',52)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',52)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceEIB'));    
	    return view("DepartmentAttendance/listingAttendanceEIB",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}


	public function attendanceListingSCBData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',47)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',47)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',47)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',47)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',47)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',47)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',47)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',47)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceSCB'));    
	    return view("DepartmentAttendance/listingAttendanceSCB",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}


	public function attendanceListingDIBData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',48)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',48)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',48)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',48)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',48)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',48)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',48)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',48)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceDIB'));    
	    return view("DepartmentAttendance/listingAttendanceDIB",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}



	public function attendanceListingAafaqData(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        if($whereraw != '' || $whererawAttendnce !='')
		{
			...
			if($empData==1)
			{
				...
				if($attendanceFilter==1 || $attendanceDateFilter==1)
				{
					...
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereRaw($whereraw)->where('dept_id',43)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereRaw($whereraw)->where('dept_id',43)->whereNotIn('emp_id', $offmarkEmp)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
					else
					{
						//print_r($offmarkEmp);
						$empData = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',43)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						//dd($empData);
						->paginate($paginationValue);

						$reportsCount = Employee_details::whereNotIn('emp_id', $offmarkEmp)->where('dept_id',43)->whereIn('emp_id', $attendanceArr)->orderBy('id','desc')
						->get()->count();
					}
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',43)->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);

					$reportsCount = Employee_details::select('emp_id')->whereRaw($whereraw)->where('dept_id',43)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					->get()->count();
				}
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('id','emp_id','emp_name','doj')->where('dept_id',43)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);

				$reportsCount = Employee_details::select('emp_id')->where('dept_id',43)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
                ->get()->count();
			}
			else
			{
		...
		$empData->setPath(config('app.url/listingAttendanceAafaq'));    
	    return view("DepartmentAttendance/listingAttendanceAafaq",compact('empData','paginationValue','reportsCount','attendanceDate','holidayarray'));
	}



	// new code for all departments tab listing End (14-1-2025)
