...
class DepartmentAttendanceMarkController extends Controller
{
	...
	public  function Index(Request $request)
	{
		...
		return view("DepartmentAttendance/index",compact('empData','departmentLists','designation','tL_details','attendanceDetails'));
	}
	...
	{		
        ...
        if($whereraw != '')
		{
			...
			if($empData==1)
			{
				...
				{
					//print_r($offmarkEmp);
					$empData = Employee_details::select('emp_id','emp_name','doj')->whereRaw($whereraw)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
					//dd($empData);
					->paginate($paginationValue);
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData = Employee_details::select('emp_id','emp_name','doj')->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')->offset(10)->limit(10)
				->paginate($paginationValue);
			...
			else
			{
				...
				$empData = Employee_details::select('emp_id','emp_name','doj')->where('employee_details.dept_id',$empDetails->dept_id)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
				->paginate($paginationValue);
	...
	public function attendanceListingENBDDataOLD(Request $request)
	{		
		$whereraw = '';
		$whererawAttendnce = '';
		$whereraw1 = '';
        ...
        else
        {
			...
			if($empData==1)
			{
				...
				$empData_details = Employee_details::select('emp_id','offline_status','doj')->where('dept_id',9)->orderBy('id','desc')->get();
				$lastworkingdate='';
				$offmarkEmp=array();
				foreach($empData_details as $emp)
				{
					if($emp->offline_status==2)
					{
						$offlineEmpData = EmpOffline::select('last_working_day_resign','last_working_day_terminate','created_at')->where('emp_id',$emp->emp_id)->where('department',9)->orderBy('id','desc')->first();						
						if($offlineEmpData)
						{
							if($offlineEmpData->last_working_day_resign!=''|| $offlineEmpData->last_working_day_resign!=NULL)
							{
				...
				$empData = Employee_details::select('emp_id','emp_name','doj')->where('dept_id',9)->whereNotIn('emp_id', $offmarkEmp)->orderBy('id','desc')
				->paginate($paginationValue);
