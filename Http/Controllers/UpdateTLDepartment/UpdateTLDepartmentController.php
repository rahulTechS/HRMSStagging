...
namespace App\Http\Controllers\UpdateTLDepartment;
...
class UpdateTLDepartmentController extends Controller
{
    
       public function UpdateTLDepartment(Request $request)
		{			
			$Designation=Designation::where("status",1)->get();
			$empId=Employee_details::whereNull('tl_id')->get();
			$EmpName=Employee_details::whereNull('tl_id')->get();
			...
			return view("UpdateTLDepartment/UpdateTLDepartment",compact('Designation','empId','EmpName','design','jobfun','recdata'));
			
		}
	   
	   
	   
	   		public function UpdateTLDepartmentList(Request $request){
			...
			if(!empty($request->session()->get('onboading_page_limitall')))
				...
				else
				{
					...
					$empdetails = PayoutTlMapping::orderBy("id","DESC")->where("status",1)->paginate($paginationValue);
					$reportsCount = PayoutTlMapping::where("status",1)->get()->count();	
									
				
			
			
			$empdetails->setPath(config('app.url/UpdateTLDepartmentList'));
			
			
			return view("UpdateTLDepartment/UpdateTLList",compact('empdetails','paginationValue','reportsCount'));
		}
		public function updateUpdateTLDataDepartment(Request $request){
		$rowId=$request->rowId;
		$empLists = Employee_details::where('emp_id',$rowId)->first();
		$department = Department::where("status",1)->orderBy('id','DESC')->get();
		return view("UpdateTLDepartment/UpdateTLFormUpdate",compact('empLists','department'));
		
	}
	public function updateUpdateTLDataDepartmentPost(Request $req)
		{
			$inputData = $req->input();
			$EMPdetailsdata =  Employee_details::where("id",$req->input('rowId'))->first();
			$empdetails =  Employee_details::find($req->input('rowId'));			
			
			$empdetails->work_location=$req->input('location');
			$empdetails->designation_by_doc_collection=$req->input('designation');	
			$empdetails->dept_id=$req->input('dept_id');			
			if($empdetails->save()){
				$tldata=PayoutTlMapping::where("tl_id",$req->input('rowId'))->first();
				if($tldata!=''){
					$tlOBJ=PayoutTlMapping::find($tldata->id);
					$departmentdata = Department::where("id",$req->input('dept_id'))->first();
					$tlOBJ->bank=$departmentdata->department_name;
					$tlOBJ->bank_id=$req->input('dept_id');
					$tlOBJ->location=$req->input('location');
					$tlOBJ->movedate=$req->input('dateTLMOve');
					if($tlOBJ->save()){
						$logObj = new TLUpdateLog();
						$logObj->empid =$req->input('rowId');
						$logObj->old_dept =$EMPdetailsdata->dept_id;
						$logObj->current_dept =$req->input('dept_id');
						...
						$logObj->current_location =$req->input('location');
						$logObj->created_date =date("Y-m-d");
						$logObj->tlmovedate =$req->input('dateTLMOve');
						$logObj->createdBY =$req->session()->get('EmployeeId');			
						
						$logObj->title="Update TL Department";
						$logObj->save();
					}
			...
			$EMPupdate =  Employee_details::where("tl_id",$req->input('rowId'))->where("offline_status",1)->get();
			if($EMPupdate!=''){
				foreach($EMPupdate as $_data){
					$empdetailsupdate =  Employee_details::find($_data->id);			
					$empdetailsupdate->tl_id=NULL;
					$empdetailsupdate->save();
				}
	...
	public static function getTeamLeader($id = NULL)
	 ...
	 function getDesignationOfDocumentation(Request $request)
	  ...
	  public function getSalaryBreakupDocumentAjax(Request $request)
	   ...
	   public function setOffSetForEMPUpdateTLDepartment(Request $request)
	   {
...
public function OfferLetterReleasedStatus(Request $request){
	...
	public function updateVisaLedgerData(Request $request){
		...
		public function getdesigndataUpdateTLDepartment($rowId){
			
				$designationMod = Designation::where("department_id",$rowId)->where("status",1)->get();
				
				return view("UpdateTLDepartment/designationdropdown",compact('designationMod'));	
