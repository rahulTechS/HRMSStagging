...
class PreVisaController extends Controller
{
	...
	{
		$visaStageLists = VisaStage::where("visa_type",$visaTypeId)->where("status",1)->where("category",1)->orderBy("stage_order","ASC")->get();
		$visapermissiondata=VisaPermission::where("id",1)->first();
		

		return view("preVisaProcess/setPreVisaStage",compact('visaStageLists','documentCollectionId','visaTypeId','visapermissiondata'));
	}
	...
	public function empPreVisaPost(Request $req)
	{
		...
		*/
			...
			$data=DocumentCollectionDetails::where("id",$docId)->first();
			if($data!=''){
			$status=$data->visa_process_status;	
			if($status==1 || $status==0){
			$_documentCollectionMod->visa_inprogress_date =date("Y-m-d");;	
			}
			}
			if($_documentCollectionMod->save()){
				$onboardstatusdata=DocumentCollectionDetails::where("onboard_status",1)->where("id",$docId)->first();
				if($onboardstatusdata!=''){
				$onboardempdata=EmployeeOnboardData::where("document_id",$docId)->first();
				$empincid=EmployeeIncrement::where("id",1)->first();				
				if($onboardempdata==''){
				$incrementid=$empincid->increment_id;
				$EMPonDataOBJ=	new EmployeeOnboardData();
				$EMPonDataOBJ->emp_id=$incrementid;
				$EMPonDataOBJ->document_id=$docId;
				if($EMPonDataOBJ->save()){
				$logonboard= new EmployeeOnboardLogdata();
				$logonboard->document_id=$docId;
				$logonboard->emp_id=$incrementid;
				$logonboard->status=1;
				$logonboard->save();
				$incOBJ=EmployeeIncrement::find(1);
				$incOBJ->increment_id=$incrementid+1;
				$incOBJ->save();
				}			
				}
				}
	...
	{
		$requestData = $req->input();
		if(isset($requestData['changestatusdate']) ){
      $changestatusdate=$requestData['changestatusdate'];
		...
		$visaprocessObj->change_status_date = $changestatusdate;
		$visaprocessObj->createdBy = $req->session()->get('EmployeeId');
	...
	public function completeVisaProcess(Request $request)
	{
		...
		$docMod->visa_complete_date = date("Y-m-d");
		$docMod->save(); 
	...
	public function getcompleteProcessStages($docId=NULL,$visaId=NULL,$stageId=NULL){
		...
		if($visaProcessDetails!=''){
			$stageId1=$visaProcessDetails->id;
			$data=DocumentUploadVisaStage::where("visaprocess_id",$stageId1)->get();
			$i=1;
			...
			$response['change_status_date'] =$visaProcessDetails->change_status_date;
...
//print_r($response);exit;			
		...
		$visapermissiondata=VisaPermission::where("id",1)->first();
		return view("preVisaProcess/visaStagepopup",compact('visapermissiondata','visaProcessDetails','stageId','visaId','docId','value'));
