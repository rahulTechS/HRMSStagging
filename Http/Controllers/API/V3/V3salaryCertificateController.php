...
class V3salaryCertificateController extends Controller
{
	
	public function salaryCerificateList(Request $request)
	{
		$requestParameters = $request->input();
		
		if(isset($requestParameters['time']) && $requestParameters['time'] != ''&& isset($requestParameters['empId']) && $requestParameters['empId'] != ''&& isset($requestParameters['Token']) && $requestParameters['Token'] != '')
		{
			...
			$time = $requestParameters['time'];
				...
				if($checkToken != '' && trim($checkToken->Token) == trim($Token))
				{
					/* echo "yyy";
					exit; */
					$salaryCertificateList = array();
					if($time == 'All')
					{
						$datas = SalaryCertificate::where("emp_id",$empId)->get();
						$index = 0;
					    foreach($datas as $data)
						{
							$salaryCertificateList[$index]['month'] = $data->pay_slip_month;
							$salaryCertificateList[$index]['salaryShipPDF'] = 'https://www.hr-suranigroup.com/paySlips/'.$data->pay_slip_file;
							$index++;
						}
					}
					else
					{
						$datas = SalaryCertificate::where("emp_id",$empId)->where("pay_slip_month",$time)->get();
						$index = 0;
					    foreach($datas as $data)
						{
							$salaryCertificateList[$index]['month'] = $data->pay_slip_month;
					...
					$result['result'] = $salaryCertificateList;
		...
		if(isset($requestParameters['type']) && $requestParameters['type'] != ''&&  isset($requestParameters['managerId']) && $requestParameters['managerId'] != ''&& isset($requestParameters['empId']) && $requestParameters['empId'] != ''&& isset($requestParameters['Token']) && $requestParameters['Token'] != '')
		{
			...
			$managerId = 0;
				...
				if($checkToken != '' && trim($checkToken->Token) == trim($Token))
				{
					...
					$passportData = Passport::where("tl_id",$managerId)->where("passport_status",0)->orderBy('updated_at','desc')->skip($pageNo)
														->take($pageLimit)
														->get();
						$passportDataCount = Passport::where("tl_id",$managerId)->where("passport_status",0)->orderBy('updated_at','desc')->skip($pageNo)
														->take($pageLimit)
														->get()->count();					
