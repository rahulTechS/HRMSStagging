...
class V3AttendanceAPIController extends Controller
{
	...
	public function markAttendanceSelfie(Request $request)
	{
		$requestParameters = $request->input();
			...
			$dateAttendance = $requestParameters['dateAttendance'];
				...
				if($checkToken != '' && trim($checkToken->Token) == trim($Token))
				{
					...
					*/
					$filesParameters  = $request->file();
					/* echo "<pre>";
					print_r($filesParameters);
					exit; */
					if(count($filesParameters) > 0)
					{
									foreach($filesParameters as $key=>$file)
									{
										...
										if (!file_exists('attendanceSelfie/'.date('dMY'))) {
											mkdir('attendanceSelfie/'.date('dMY'), 0777, true);
										}
										if(file_exists(public_path('attendanceSelfie/'.date('dMY').'/'.$newFileName))){

											  unlink(public_path('attendanceSelfie/'.date('dMY').'/'.$newFileName));
											}  
										if($file->move(public_path('attendanceSelfie/'.date('dMY').'/'), $newFileName))
										{
													...
													$create->out_time =date("Y-m-d 06:00:00");
													$create->attendance_mark_using ='APP';
													$create->selfie =date('dMY').'/'.$newFileName;
													/* $update->pics =$newFileName; */
													$create->save();
													$result['responseCode'] = 200;
													$result['message'] = "SuccessFull.";
										...
										else
										{
											...
											$result['message'] = "Issue to upload selfie.";
										}
									}
					}
					else
