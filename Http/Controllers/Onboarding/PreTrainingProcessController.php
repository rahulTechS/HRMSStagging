...
class PreTrainingProcessController extends Controller
{
	...
	public function preTrainingProcess (Request $req)
	{
		...
		*/
		...
		$trainingTypeList = TrainingType::where("status",1)->whereIn("department_id",array($departmentId,1))->orderBy("id","DESC")->get();
	...
	public static function rollBackTraining(Request $request)
	{
		...
		{
			$finaljsondata = json_encode(array('DocData' =>$_tP), JSON_PRETTY_PRINT);
				$logObj = new DocumentCollectionDetailsLog();
...
//print_r($response);exit;			
	...
	public function getFilesForTrainingUpload(Request $request)
	{
		$processId = $request->processId;
		$documentlist = TrainingUploadDocument::where("training_id",$processId)->get();
		$stagestatus=TrainingProcess::where("id",$processId)->first();
		
		if($stagestatus!=''){
		$status=$stagestatus->stage_staus;
		}
		return view("Onboarding/Training/getFilesForTrainingUpload",compact('documentlist','status'));
	}
