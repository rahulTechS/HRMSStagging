...
if(!empty($request->session()->get('ReasonofAttrition_empoffboard_filter_list')) && $request->session()->get('ReasonofAttrition_empoffboard_filter_list') != 'All')
				...
				{
					//echo "hello";exit;
					$documentCollectiondetails = EmpOffline::where("condition_leaving",2)->whereRaw($whereraw)->orderBy("condition_leaving_date", "DESC")->paginate($paginationValue);
					//print_r($documentCollectiondetails);
				}
				else
				{
					//echo "hello1";
					$documentCollectiondetails = EmpOffline::where("condition_leaving",2)->orderBy("condition_leaving_date", "DESC")->paginate($paginationValue);
				...
				if($whereraw != '')
				{
					...
					$reportsCount = EmpOffline::where("condition_leaving",2)->whereRaw($whereraw)->get()->count();
				}
				else
				{
					$reportsCount = EmpOffline::where("condition_leaving",2)->get()->count();
				}
