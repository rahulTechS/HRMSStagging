...
class TabsController extends Controller
{
	   ...
	   public function summaryTabWithFullView(Request $request)
	   {
			...
			*/
		    ...
		    $stepsAll[8]['slagURL'] = 'finalizationOnboarding/'.$documentCollectionDetails->id;
	...
	public function tabvisaProcessPanel(Request $request)
	{
		...
		return view("Onboarding/RequestTabs/tabvisaProcessPanel",compact('documentCollectionDetails','totalCost','percentage','lastStageCompleted','visaStageD','totalDays','visaStageLists','visaTypeId'));
	}		  
...
public function tabtrainingProcessPanel(Request $request)
{
		$documentCollectId = $request->documentCollectionId;
		$documentCollectionDetails = DocumentCollectionDetails::where("id",$documentCollectId)->first();
		...
		*check for KYC as per department
		...
		return view("Onboarding/RequestTabs/tabtrainingProcessPanel",compact('documentCollectionDetails','percentage','lastStageCompleted','trainingStageD','totalDays','trainingStageLists','trainingTypeId','kycAttributeList'));
}
...
{
	return CancelationVisaProcess::where("document_id",$docId)->where("visa_type",$typeid)->count();
}

public static function startDateVisaProcess($docId,$typeid)
{
	$visaP =  CancelationVisaProcess::where("document_id",$docId)->where("visa_type",$typeid)->orderBy("id","ASC")->first();
	return date("d M Y",strtotime($visaP->created_at));
}

public static function endDateVisaProcess($docId,$typeid)
{
	$visaP =  CancelationVisaProcess::where("document_id",$docId)->where("visa_type",$typeid)->orderBy("id","DESC")->first();
...
public function codeGeneration(Request $request)
{
	...
	return view("Onboarding/RequestTabs/codeGeneration",compact('documentCollectionDetails'));
}
...
public static function endDateTProcess($docId,$typeid)
{
 ...
 public function finalizationOnboardingTab(Request $request)
	   {
			 $documentCollectionId = $request->documentCollectionId;
			$documentCollectionDetails = DocumentCollectionDetails::where("id",$documentCollectionId)->first();
		     return view("Onboarding/RequestTabs/finalizationOnboardingTab",compact('documentCollectionId','documentCollectionDetails'));
	   }
...
public function requestOnBoarding(Request $request)
{
			$documentCollectionId = $request->documentCollectionId;
			
		     return view("Onboarding/RequestTabs/requestOnBoarding",compact('documentCollectionId'));
}
