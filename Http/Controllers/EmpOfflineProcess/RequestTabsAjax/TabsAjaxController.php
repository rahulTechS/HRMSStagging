...
namespace App\Http\Controllers\EmpOfflineProcess\RequestTabsAjax;
...
class TabsAjaxController extends Controller
{
	   ...
	   public function offBoardsummaryTabWithFullViewAjax(Request $request)
	   {
			...
			$emp_details=Employee_details::where("emp_id",$documentCollectionDetails->emp_id)->first();
			/*
			...
			*/
			...
			$stepsAll = array();
			/*Step1*/
		    $stepsAll[0]['name'] = 'Request to OFB'; 
			if($documentCollectionDetails->condition_leaving == 1)
			{
			...
			else
			{
				...
				$OnboardingProgress = 'Request to OFB';
				$stepsAll[0]['Tab'] = 'active'; 
			}
			...
			/*Step2*/
			$stepsAll[1]['name'] = 'Exit Interview'; 
		    if($documentCollectionDetails->condition_leaving == 2)
			{
				$stepsAll[1]['stage'] = 'inprogress'; 
				$stepsAll[1]['Tab'] = 'active';
			}
			else if($documentCollectionDetails->condition_leaving > 2 || $documentCollectionDetails->condition_leaving == 3)
			{
				$completedStep++;
			...
			if($documentCollectionDetails->condition_leaving == 3 || $documentCollectionDetails->condition_leaving == 2|| $documentCollectionDetails->exit_interview_status>=1)
			{
				$stepsAll[2]['stage'] = 'inprogress'; 
				$stepsAll[2]['Tab'] = 'active'; 
			}
			else if($documentCollectionDetails->condition_leaving > 3 || $documentCollectionDetails->condition_leaving == 4 )
			{
				$completedStep++;
			...
			if($documentCollectionDetails->condition_leaving == 3 || $documentCollectionDetails->condition_leaving == 2 || $documentCollectionDetails->condition_leaving == 4 || $documentCollectionDetails->condition_leaving == 6 || $documentCollectionDetails->exit_interview_status>=1)
			{
				$stepsAll[3]['stage'] = 'inprogress'; 
				$stepsAll[3]['Tab'] = 'active'; 
			}
			else if($documentCollectionDetails->visa_process_status == 5 || $documentCollectionDetails->condition_leaving == 5)
			{
				$completedStep++;
...
if($documentCollectionDetails->condition_leaving == 3 || $documentCollectionDetails->condition_leaving == 2 || $documentCollectionDetails->condition_leaving ==5 || $documentCollectionDetails->condition_leaving ==4 || $documentCollectionDetails->exit_interview_status>=1)
			{
		    ...
		    if($documentCollectionDetails->bgverification_status == 1 && $documentCollectionDetails->onboard_status == 1)
			{
				$stepsAll[8]['stage'] = 'inprogress'; 
				$stepsAll[8]['Tab'] = 'active'; 
			}
			else if($documentCollectionDetails->bgverification_status == 1 && $documentCollectionDetails->onboard_status == 2)
			{
				$completedStep++;
		...
		/* 	echo '<pre>';
			...
			{
				if($documentCollectionDetails->bgverification_status == 1)
				{
			...
			return view("EmpOfflineProcess/RequestTabsAjax/summaryTabWithFullViewAjax",compact('emp_details','documentCollectionDetails','visaProcessLists','stepsAll','percentange','OnboardingProgress'));
	   }
	   ...
	   public function offBoardtab2EmpDetailsAjax(Request $request)
	   {
			...
			$Question=QuestionForLeaving::where("offline_id",$documentCollectId)->get();
			 return view("EmpOfflineProcess/RequestTabsAjax/ConditionforLeaving",compact('documentCollectionDetails','Question')); 
	   }
	    ...
	    public function offBoardtab3EmpDetailsAjax(Request $request)
		   {
			     ...
			     $documentCollectionDetails = EmpOffline::where("id",$documentCollectId)->first();
			$Question=QuestionForLeaving::where("offline_id",$documentCollectId)->get();
			 return view("EmpOfflineProcess/RequestTabsAjax/ExitInterview",compact('documentCollectionDetails','Question')); 
			  }
		  ...
		  public function onBoardtab4EmpDetailsAjax(Request $request)
		   ...
		   public function tab6EpmDetailsAjaxPayment(Request $request)
		   {
				 $documentCollectId = $request->documentCollectionId;
			     $documentCollectionDetails = EmpOffline::where("id",$documentCollectId)->first();
				 $SettelementAttributes=OffboardEMPData::where("emp_id",$documentCollectId)->first();
			
			 return view("EmpOfflineProcess/RequestTabsAjax/PaymentConfirm",compact('documentCollectionDetails','SettelementAttributes')); 
		   }
...
public function manageEmpAttrFromDocumentAttr(Request $request)
	   ...
	   public function tabonboardPanelAjax(Request $request)
		...
		public function OffBoardLogsTab(Request $request){
			  ...
			  $empOffboardData=OffboardEMPData::where("emp_id",$id)->first();
				...
				return view("EmpOfflineProcess/RequestTabsAjax/OffboardLogsDetails",compact('visadata','Documentdata','empOffboardData')); 
	...
	public static function getUserNameQuestion($id){
		$data = EmpOffline::where('id',$id)->orderBy("id","DESC")->first();
	  //print_r($data);
	  if($data != '')
	  {
...
// New code Start (29-05-2024)
	   ...
	   public static function getBookingsCount($empid)
	   {
		
			$deptData = Employee_details::where("emp_id",$empid)->orderBy('id','desc')->first();

			if($deptData)
			{
				if($deptData->dept_id==36)
				{
					$current_duration = date('Y-m');
					$toDate = date("Y-m-d");
					$fromDate = date("Y").'-'.date("m").'-'.'01';
					//return $fromDate.$toDate;


					//$bookingData = MashreqBookingMIS::where("emp_id",$empid)->where("dateofdisbursal",'LIKE',"%{$current_duration}%")->orderBy('id','desc')->get()->count();
					$bookingData = MashreqBookingMIS::where("emp_id",$empid)->whereDate('dateofdisbursal', '>=', $fromDate)->whereDate('dateofdisbursal', '<=', $toDate)->orderBy('id','desc')->get()->count();
					//return $bookingData;
					...
					else
					{
						...
						$bookingsDeptData = DepartmentFormEntry::where("emp_id",$empid)->whereDate('submission_date', '>=', $fromDate)->whereDate('submission_date', '<=', $toDate)->orderBy('id','desc')->get()->count();

						if($bookingsDeptData > 0)
						{
							return -1;
						}
						else
				...
				if($deptData->dept_id==49)
				{
					$status_arr = array("Approved","Pending with Onboarder","Pending with COC","Welcome Calling,Archive on Approval","Missing(Approved)");
					$current_duration = date('Y-m');
					$toDate = date("Y-m-d");
					$fromDate = date("Y").'-'.date("m").'-'.'01';
					//$bookingData = CBDBankMis::where("employee_id",$empid)->where("approval_date",'LIKE',"%{$current_duration}%")->whereIn("Status",$status_arr)->orderBy('id','desc')->get()->count();
					$bookingData = CBDBankMis::where("employee_id",$empid)->whereDate('approval_date', '>=', $fromDate)->whereDate('approval_date', '<=', $toDate)->whereIn("Status",$status_arr)->orderBy('id','desc')->get()->count();
					//return $bookingData;
					...
					{
						$bookingsDeptData = DepartmentFormEntry::where("emp_id",$empid)->whereDate('application_date', '>=', $fromDate)->whereDate('application_date', '<=', $toDate)->orderBy('id','desc')->get()->count();
				...
				if($deptData->dept_id==47)
				{
					$current_duration = date('Y-m');
					$toDate = date("Y-m-d");
					$fromDate = date("Y").'-'.date("m").'-'.'01';
					//$bookingData = SCBBankMis::where("NBO",$empid)->where("Current_Queue_Date",'LIKE',"%{$current_duration}%")->where("Status",'Approved')->orderBy('id','desc')->get()->count();
					$bookingData = SCBBankMis::where("NBO",$empid)->whereDate('Current_Queue_Date', '>=', $fromDate)->whereDate('Current_Queue_Date', '<=', $toDate)->where("Status",'Approved')->orderBy('id','desc')->get()->count();
					//return $bookingData;
		...
		public static function getCurrentTarget($empid)
		{
			$current_duration = date('Y-m');
			$targetData = MasterPayout::where("employee_id",$empid)->orderBy('sort_order','desc')->first();
			//return $bookingData;

			if($targetData)
			{
				return $targetData->agent_target;
			}
			else
			{
				return '--';
		...
		public static function getDept($empid)
		{
			$deptData = Employee_details::where("emp_id",$empid)->orderBy('id','desc')->first();
			//return $bookingData;
		...
		public static function getPrevoiusBookingsCount($empid)
	   	{
