	...
	public function addHRInterviewfrmPost(Request $rq)
	{
		//print_r($rq->input());exit;
			...
			if($detailsObj->save()){
				...
				/*if($rq->input('last_working_day_resign')!=''){
				$offlinedataupdate=EmpOffline::where("id",$rq->input('offboardingId'))->first();
				$empdataupdate=Employee_details::where("emp_id",$offlinedataupdate->emp_id)->first();
				if($empdataupdate!=''){
					$empOBJUpdate=Employee_details::find($empdataupdate->id);
					$empOBJUpdate->offline_status=2;
					$empOBJUpdate->pre_offline_status=3;
					$empOBJUpdate->save();
				}	
