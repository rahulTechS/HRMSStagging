...
class AllocateTLController extends Controller
{
    
       public function UpdateTLList(Request $request)
		{			
		
			$Designation=Designation::where("status",1)->get();
			$empId=Employee_details::whereNull('tl_id')->where("job_function",2)->get();
			...
			return view("UpdateTL/UpdateTL",compact('Designation','empId','EmpName','design','jobfun','recdata'));
			
		}
	   
	   
	   
	   		public function UpdateTLEmpListupdatetl(Request $request){
			...
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				...
				if(!empty($request->session()->get('UpdateTL_empid_emp_filter_inner_list')) && $request->session()->get('UpdateTL_empid_emp_filter_inner_list') != 'All')
				{
					$empId = $request->session()->get('UpdateTL_empid_emp_filter_inner_list');
				...
				if(!empty($request->session()->get('UpdateTL_fname_emp_filter_inner_list')) && $request->session()->get('UpdateTL_fname_emp_filter_inner_list') != 'All')
				{
					$cname = $request->session()->get('UpdateTL_fname_emp_filter_inner_list');
				...
				if(!empty($request->session()->get('UpdateTL_lname_emp_filter_inner_list')) && $request->session()->get('UpdateTL_lname_emp_filter_inner_list') != 'All')
				{
					$lname = $request->session()->get('UpdateTL_lname_emp_filter_inner_list');
				...
				if(!empty($request->session()->get('UpdateTL_design_emp_filter_inner_list')) && $request->session()->get('UpdateTL_design_emp_filter_inner_list') != 'All')
				{
					$design = $request->session()->get('UpdateTL_design_emp_filter_inner_list');
				...
				if(!empty($request->session()->get('UpdateTL_jobfunction_emp_filter_inner_list')) && $request->session()->get('UpdateTL_jobfunction_emp_filter_inner_list') != 'All')
				{
					$jobfunction = $request->session()->get('UpdateTL_jobfunction_emp_filter_inner_list');
				...
				if(!empty($request->session()->get('UpdateTL_RecruiterName_emp_filter_inner_list')) && $request->session()->get('UpdateTL_RecruiterName_emp_filter_inner_list') != 'All')
				{
					$RecruiterName = $request->session()->get('UpdateTL_RecruiterName_emp_filter_inner_list');
				...
				if(!empty($request->session()->get('UpdateTL_location_emp_filter_inner_list')) && $request->session()->get('UpdateTL_location_emp_filter_inner_list') != 'All')
				{
					$location = $request->session()->get('UpdateTL_location_emp_filter_inner_list');
					if($location!=''){
				...
				//echo $whereraw;//exit;
				if(!empty($request->session()->get('UpdateTL_visacompany_emp_filter_inner_list')) && $request->session()->get('UpdateTL_visacompany_emp_filter_inner_list') != 'All')
				{
					$companyvisa = $request->session()->get('UpdateTL_visacompany_emp_filter_inner_list');
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("offline_status",1)->paginate($paginationValue);
				$reportsCount = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("offline_status",1)->get()->count();
					$activeCount = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCount = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("offline_status",1)->paginate($paginationValue);
					$reportsCount = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->get()->count();	
					$activeCount = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("offline_status",1)->get()->count();
					$inactiveCount = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/UpdateTLEmpListupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/UpdateTLList",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCount','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCount','inactiveCount'));
		}
		public function updateUpdateTLDataupdatetl(Request $request){
		$rowId=$request->rowId;
		...
		return view("SalesTeamManagement/UpdateTL/UpdateTLFormUpdate",compact('empLists','tL_details'));
		
	}
	public function updateUpdateTLDataDataPostupdatetl(Request $req)
		{
			$inputData = $req->input();
		/* 	echo '<pre>';
			print_r($inputData);
		...
		public function searchbyempNameUpdateTLupdatetl(Request $request)
		{
			...
			$request->session()->put('UpdateTL_empid_emp_filter_inner_list',$empid);
			$request->session()->put('UpdateTL_fname_emp_filter_inner_list',$fname);
			$request->session()->put('UpdateTL_lname_emp_filter_inner_list',$lname);
			$request->session()->put('UpdateTL_scode_emp_filter_inner_list',$source);
			$request->session()->put('UpdateTL_location_emp_filter_inner_list',$location);
			$request->session()->put('UpdateTL_design_emp_filter_inner_list',$designation);
			$request->session()->put('UpdateTL_jobfunction_emp_filter_inner_list',$jobfunction);
			$request->session()->put('UpdateTL_RecruiterName_emp_filter_inner_list',$RecruiterName);
		...
		public function empFilterresetUpdateTLupdatetl(Request $request)
		{
			
			$request->session()->put('UpdateTL_fname_emp_filter_inner_list','');
			$request->session()->put('UpdateTL_lname_emp_filter_inner_list','');
			$request->session()->put('UpdateTL_scode_emp_filter_inner_list','');
			$request->session()->put('UpdateTL_location_emp_filter_inner_list','');
			$request->session()->put('UpdateTL_design_emp_filter_inner_list','');
			$request->session()->put('UpdateTL_empid_emp_filter_inner_list','');
			$request->session()->put('UpdateTL_jobfunction_emp_filter_inner_list','');
			$request->session()->put('UpdateTL_RecruiterName_emp_filter_inner_list','');
	...
	public static function getTeamLeader($id = NULL)
	   {
		...
		public function ViewDocumentPageEmpData(Request $request){
			$rowId=$request->rowId;
			$documentCollectiondetails = DocumentCollectionDetails::where("id",$rowId)->first();
			$cname=$documentCollectiondetails->emp_name;
			$request->session()->put('cname_emp_filter_inner_list',$cname);
			$request->session()->put('visaledger_redirect_filter_name',"visaledger_page");
			$request->session()->put('tab_onboarding_dasboard',"visaInProgress");
			
			//return redirect("/documentcollectionAjax");
			//redirect('documentcollectionAjax');
			$response['code'] = '200';
		   $response['message'] = "Data Saved Successfully.";
		   $response['docId'] = $rowId;
	   ...
	   public function setOffSetForEMPUpdateTL(Request $request)
	   {
	...
	public function uploadofferletterIncentiveLetterDocumentStartAjax(Request $request)
	   ...
	   public function visadeatlsformStartAjax(Request $request)
	   {
		...
		public function documentcollectionbyfilter(Request $request)
		{
			...
			{
				$special_status  = array_filter($special_status);
			}
			if($special_status!=''){
			$special_status=implode(",", $special_status);
			}
			else{
				$special_status='';
			}
			...
			if(!empty($request->session()->get('onboading_page_limit')))
				...
				if(!empty($request->session()->get('onboard_special_status_filter_inner_list')) && $request->session()->get('onboard_special_status_filter_inner_list') != 'All')
				{
					$specialStatusArray = $request->session()->get('onboard_special_status_filter_inner_list');
					 ...
					 $specialStatus = explode(",",$specialStatusArray);
					
					 if(in_array(1,$specialStatus))
						 {
						  if($whereraw == '')
							{
								$whereraw = 'visa_process_status IN (1,0)  And onboard_status = 2 ';
							}
							else
							{
								$whereraw .= ' And visa_process_status IN (1,0) and onboard_status = 2 ';
							}
						 }

					if(in_array(2,$specialStatus))
						 {
							  if($whereraw == '')
								{
									$whereraw = " visa_process_status = 2 And onboard_status =1  And visa_inprogress_date<'".date('Y-m-d', strtotime("-30 days"))."'";
								}
								else
								{
									$whereraw .= "And visa_process_status = 2 And onboard_status =1  And visa_inprogress_date<'".date('Y-m-d', strtotime("-30 days"))."'";
								}
						 }
						 if(in_array(3,$specialStatus))
						 {
							  if($whereraw == '')
								{
									$whereraw = " (visa_process_status IN (1,0) OR visa_process_status = 4) And visa_complete_date<'".date('Y-m-d', strtotime("-10 days"))."'" ;
								}
								else
								{
									$whereraw .= " And (visa_process_status IN (1,0) OR visa_process_status = 4) And visa_complete_date<'".date('Y-m-d', strtotime("-10 days"))."'" ;
								}
						 }
						 if(in_array(4,$specialStatus))
						 {
							  if($whereraw == '')
								{
									$whereraw = " offer_letter_details_date IS NULL And created_at<'".date('Y-m-d', strtotime("-7 days"))."'" ;
								}
								else
								{
									$whereraw .= " And offer_letter_details_date IS NULL And created_at<'".date('Y-m-d', strtotime("-7 days"))."'" ;
								}
						 }
						 if(in_array(5,$specialStatus))
						 {
							  if($whereraw == '')
								{
									$whereraw = " offer_letter_replease_date IS NULL And offer_letter_details_date<'".date('Y-m-d', strtotime("-7 days"))."'" ;
								}
								else
								{
									$whereraw .= " And offer_letter_replease_date IS NULL And offer_letter_details_date<'".date('Y-m-d', strtotime("-7 days"))."'" ;
								}
						 }
						 if(in_array(6,$specialStatus))
						 {
							  if($whereraw == '')
								{
									$whereraw = " visa_process_status = 2 And visa_inprogress_date<'".date('Y-m-d', strtotime("-60 days"))."'" ;
								}
								else
								{
									$whereraw .= " And visa_process_status = 2 And visa_inprogress_date<'".date('Y-m-d', strtotime("-60 days"))."'" ;
								}
						 }
						 if(in_array(7,$specialStatus))
						 {
							  if($whereraw == '')
								{
								...
								{
									$whereraw .= " And bgverification_response_date IS NULL And bgverification_initiated_date<'".date('Y-m-d', strtotime("-7 days"))."'" ;
								}
						 }
...
public function OfferLetterReleasedStatus(Request $request){
	...
	public function updatevisaLedgerDataPost(Request $req)
		...
		public function updateUpdateTLDataAssignTL(Request $req)
		{
		...
		public function listingUpdateTLListENBDTLupdatetl(Request $request){
			...
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",9)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountenbd = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",9)->where("offline_status",1)->get()->count();
					$activeCountenbd = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",9)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountenbd = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",9)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("dept_id",9)->where("job_function",2)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountenbd = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",9)->get()->count();	
					$activeCountenbd = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",9)->where("offline_status",1)->get()->count();
					$inactiveCountenbd = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",9)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListENBDTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListENBDTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountenbd','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountenbd','inactiveCountenbd'));
		}
		public function listingUpdateTLListdeemTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",8)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountdeem = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("dept_id",8)->where("job_function",2)->where("offline_status",1)->get()->count();
					$activeCountdeem = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("dept_id",8)->where("job_function",2)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountdeem = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("dept_id",8)->where("job_function",2)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("dept_id",8)->where("job_function",2)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountdeem = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",8)->get()->count();	
					$activeCountdeem = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",8)->where("offline_status",1)->get()->count();
					$inactiveCountdeem = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",8)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListdeemTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListdeemTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountdeem','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountdeem','inactiveCountdeem'));
		}
		public function listingUpdateTLListmashreqTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined" )
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",36)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountmashreq = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",36)->where("offline_status",1)->get()->count();
					$activeCountmashreq = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",36)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountmashreq = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",36)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",36)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountmashreq = Employee_details::where("offline_status",1)->where("job_function",2)->whereNull('tl_id')->where("dept_id",36)->get()->count();	
					$activeCountmashreq = Employee_details::where('status',1)->where("job_function",2)->whereNull('tl_id')->where("dept_id",36)->where("offline_status",1)->get()->count();
					$inactiveCountmashreq = Employee_details::where('status',2)->where("job_function",2)->whereNull('tl_id')->where("dept_id",36)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListmashreqTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListmashreqTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountmashreq','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountmashreq','inactiveCountmashreq'));
		}
		public function listingUpdateTLListaafaqTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountaafaq = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->where("offline_status",1)->get()->count();
					$activeCountaafaq = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountaafaq = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountaafaq = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->get()->count();	
					$activeCountaafaq = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->where("offline_status",1)->get()->count();
					$inactiveCountaafaq = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",43)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListaafaqTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListaafaqTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountaafaq','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountaafaq','inactiveCountaafaq'));
		}
		public function listingUpdateTLListdibTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountdib = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->where("offline_status",1)->get()->count();
					$activeCountdib = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountdib = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountdib = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->get()->count();	
					$activeCountdib = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->where("offline_status",1)->get()->count();
					$inactiveCountdib = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",46)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListdibTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListdibTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountdib','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountdib','inactiveCountdib'));
		}
		public function listingUpdateTLListscbTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountscb = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->where("offline_status",1)->get()->count();
					$activeCountscb = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountscb = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountscb = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->get()->count();	
					$activeCountscb = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->where("offline_status",1)->get()->count();
					$inactiveCountscb = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",47)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListscbTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListscbTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountscb','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountscb','inactiveCountscb'));
		}
		public function listingUpdateTLListcbdTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountcbd = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->get()->count();
					$activeCountcbd = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountcbd = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountcbd = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->get()->count();	
					$activeCountcbd = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->get()->count();
					$inactiveCountcbd = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",49)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListcbdTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListcbdTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountcbd','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountcbd','inactiveCountcbd'));
		}
		public function getEMPDeptIdupdatetl($empid,$tlid){
			...
			if($empdetails !='' && $tldetails!=''){
				...
				return view("SalesTeamManagement/UpdateTL/designationdropdown",compact('designationMod'));	
				}
			}
			
	
		}
		public function listingUpdateTLListeibTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->paginate($paginationValue);
				$reportsCounteib = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->get()->count();
					$activeCounteib = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCounteib = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->paginate($paginationValue);
					$reportsCounteib = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->get()->count();	
					$activeCounteib = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->get()->count();
					$inactiveCounteib = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",52)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListeibTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListeibTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCounteib','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCounteib','inactiveCounteib'));
		}
		public function listingUpdateTLListhsbcTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->where("offline_status",1)->paginate($paginationValue);
				$reportsCounthsbc = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->where("offline_status",1)->get()->count();
					$activeCounthsbc = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCounthsbc = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->where("offline_status",1)->paginate($paginationValue);
					$reportsCounthsbc = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->get()->count();	
					$activeCounthsbc = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->where("offline_status",1)->get()->count();
					$inactiveCounthsbc = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",54)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListhsbcTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListhsbcTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCounthsbc','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCounthsbc','inactiveCounthsbc'));
		}
	
	

		public function listingUpdateTLListuabTLupdatetl(Request $request){
			//$request->session()->put('design_emp_filter_inner_list','');
			$deptID = '';
			if(!empty($request->session()->get('onboading_page_limitall')) && $request->session()->get('onboading_page_limitall')!="undefined")
				{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->where("offline_status",1)->paginate($paginationValue);
				$reportsCountuab = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->where("offline_status",1)->get()->count();
					$activeCountuab = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->where("offline_status",1)->where('status',1)->get()->count();
					$inactiveCountuab = Employee_details::whereRaw($whereraw)->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->where("offline_status",1)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->where("offline_status",1)->paginate($paginationValue);
					$reportsCountuab = Employee_details::where("offline_status",1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->get()->count();	
					$activeCountuab = Employee_details::where('status',1)->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->where("offline_status",1)->get()->count();
					$inactiveCountuab = Employee_details::where('status',2)->whereNull('tl_id')->where("job_function",2)->where("dept_id",56)->where("offline_status",1)->get()->count();					
				}
			
			$departmentLists = Department::where("status",1)->orderBy('id','DESC')->get();
			$empdetails->setPath(config('app.url/listingUpdateTLListuabTLupdatetl'));
			Cache::put('empdetails', $empdetails, now()->addMinutes(30));
			
			return view("SalesTeamManagement/UpdateTL/listingUpdateTLListuabTL",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCountuab','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCountuab','inactiveCountuab'));
		}
