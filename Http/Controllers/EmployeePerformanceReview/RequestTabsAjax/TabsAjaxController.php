...
namespace App\Http\Controllers\EmployeePerformanceReview\RequestTabsAjax;
...
use App\Models\Changesalary\Visa_stages_change_salary;
...
class TabsAjaxController extends Controller
{
	   ...
	   public function warningLettersummaryTabWithFullViewAjax(Request $request)
	   {
		    $empId = $request->empid;
			$id = $request->id;
			...
			*/
			...
			$visaProcessLists = '';
			return view("EmployeePerformanceReview/RequestTabsAjax/summaryTabWithFullViewAjax",compact('documentCollectionDetails','visaProcessLists','stepsAll','percentange','OnboardingProgress'));
	   }
	...
	->orderBy('warning_letter_requests.id', 'asc')->get();
			 ...
			 return view("EmployeePerformanceReview/RequestTabsAjax/ConditionforLeaving",compact('documentCollectionDetails','warningletterEmpDetails')); 
	   }
...
public function offBoardtab3EmpDetailsAjax(Request $request)
{
	...
	->select('employee_details.*', 'warning_letter_requests.*')
	->where('warning_letter_requests.emp_id', $empId)
	->where('warning_letter_requests.id', $rowid)
	->orderBy('warning_letter_requests.id', 'desc')->first();
	...
	$currentRowDetails = WarningLetterRequest::
	join('employee_details', 'employee_details.emp_id', '=', 'warning_letter_requests.emp_id')
	->select('employee_details.*', 'warning_letter_requests.*')
	->where('warning_letter_requests.emp_id', $empId)
	->where('warning_letter_requests.id', $rowid)
	...
	return view("EmployeePerformanceReview/RequestTabsAjax/ExitInterview",compact('documentCollectionDetails','warningletterEmpDetails','currentRowDetails')); 
}
		  ...
		  public function onBoardtab4EmpDetailsAjax(Request $request)
			{
				...
				->orderBy('warning_letter_requests.id', 'asc')->get();
			 	...
			 	return view("EmployeePerformanceReview/RequestTabsAjax/ExitInterview22",compact('documentCollectionDetails','warningletterEmpDetails')); 
			    ...
			    //return view("EmployeePerformanceReview/RequestTabsAjax/FullandFinalSettelement",compact('documentCollectionDetails')); 
		   }
		   public function tab6EpmDetailsAjaxPayment(Request $request)
		   {
			 ...
			 return view("ChangeSalaryNew/RequestTabsAjax/PaymentConfirm",compact('documentCollectionDetails','empdata')); 
		   }
	...
	public function tabCancelvisaProcessPanelAjax(Request $request)
	{
		...
		$documentCollectionDetails = Employee_details_change_salary::orderBy("id","DESC")->where("offline_status",1)->where('emp_id',$empid)->first();
		...
		/*
				...
				*/
				...
				echo $typeId;
				exit; */
				$visaProcessDetailAll =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->get();
				...
				*/
				$visaStageD = '';
				$lastStageCompleted =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("stage_staus",2)->orderBy("id","DESC")->first();
				if($lastStageCompleted != '')
				{
					$visaStageId = $lastStageCompleted->visa_stage;
					$visaStageD = VisaStage::where("id",$visaStageId)->where("category",4)->first();



					$completedStageOrderNew = CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("stage_staus",2)->count();


					$completedStageOrder = $visaStageD->stage_order;  // 24
					...
					//return $completedStageOrder;
					$p1 = $completedStageOrderNew/$totalStagesCount;
					$p2 = round($p1*100);
					$percentage = $p2;
					$firstStageCompleted =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->orderBy("id","ASC")->first();
				...
				*/
				...
				$lastStageCompletedList =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->whereIn("stage_staus",array(1,2))->orderBy("id","DESC")->get();
				...
				*working
				*/
				$visaTypeModChecked =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->orderBy("id","DESC")->first();
		...
		return view("ChangeSalaryNew/RequestTabsAjax/tabvisaProcessPanelAjax",compact('documentCollectionDetails','totalCost','percentage','lastStageCompleted','visaStageD','totalDays','visaStageLists','visaTypeId'));
	}		  
...
public function warningLetterFormData(Request $request)
{
		$empId = $request->empid;
		$rowid = $request->rowid;

		$empData = WarningLetterRequest::join('employee_details', 'employee_details.emp_id', '=', 'warning_letter_requests.emp_id')
		->select('employee_details.*', 'warning_letter_requests.*')
		->where('warning_letter_requests.emp_id', $empId)
		->where('warning_letter_requests.id', $rowid)
		...
		return view("EmployeePerformanceReview/RequestTabsAjax/OffboardPerformanceDetailspopup",compact('empData')); 
	  
}	

	   public function thirdWarnLetterFormData(Request $request)
	   {
			$empId = $request->empid;

			$empData = WarningLetterRequest::
			join('employee_details', 'employee_details.emp_id', '=', 'warning_letter_requests.emp_id')
			->select('employee_details.*', 'warning_letter_requests.*')
			->where('warning_letter_requests.emp_id', $empId)
			->orderBy('warning_letter_requests.id', 'desc')->first();		
			...
			return view("EmployeePerformanceReview/RequestTabsAjax/OffboardPerformanceDetailspopupNew",compact('empData')); 


		 	//return view("EmployeePerformanceReview/RequestTabsAjax/thirdwarnletterModal",compact('empData')); 	
	 }	
