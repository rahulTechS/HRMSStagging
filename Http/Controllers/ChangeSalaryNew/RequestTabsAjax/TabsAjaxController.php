...
namespace App\Http\Controllers\ChangeSalaryNew\RequestTabsAjax;
...
define('ApprovedAccessUsers', array(101456,101042,100762,102723));
...
class TabsAjaxController extends Controller
{
	   ...
	   public function offBoardsummaryTabWithFullViewAjax(Request $request)
	   {
			...
			*/
			...
			$visaProcessLists = '';
			return view("ChangeSalaryNew/RequestTabsAjax/summaryTabWithFullViewAjax",compact('documentCollectionDetails','visaProcessLists','stepsAll','percentange','OnboardingProgress','changesalarydetails'));
	   }
	   ...
	   public function offBoardtab2EmpDetailsAjax(Request $request)
	   {
		...
		->where('change_salary_request.id', $rowid)
		->orderBy('employee_details.id', 'desc')->first();
			...
			$empdata = ChangeSalary::where("emp_id",$empId)->orderBy('id','DESC')->first();
			 ...
			 //$documentCollectionDetails = EmpOffline::where("id",$documentCollectId)->first();
			$Question='';
			 return view("ChangeSalaryNew/RequestTabsAjax/ConditionforLeaving",compact('documentCollectionDetails','Question','salrychnageinfo','empdata')); 
	   }
	    ...
	    public function offBoardtab3EmpDetailsAjax(Request $request)
		{
			...
			$usersids = array(101456,101058,101042,100762,101466,101549);


			//$Question=QuestionForLeaving::where("offline_id",$documentCollectId)->get();
			$Question='';

				return view("ChangeSalaryNew/RequestTabsAjax/ExitInterview",compact('documentCollectionDetails','Question','empdata','salrychnageinfo','rowid','visiblebtn')); 
		}
		  ...
		  public function onBoardtab4EmpDetailsAjax(Request $request)
			{
			    ...
			    return view("ChangeSalaryNew/RequestTabsAjax/FullandFinalSettelement",compact('documentCollectionDetails')); 
		   }
	...
	{
		$resultids = explode("-",$request->documentCollectionId);
		...
		/*
				...
				*/
				...
				echo $typeId;
				exit; */
				$visaProcessDetailAll =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("rowid",$rowid)->get();
				...
				*/
				$visaStageD = '';
				$lastStageCompleted =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("rowid",$rowid)->where("stage_staus",2)->orderBy("id","DESC")->first();
				if($lastStageCompleted != '')
				{
					...
					$completedStageOrderNew = CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("rowid",$rowid)->where("stage_staus",2)->count();
					...
					$p2 = round($p1*100);
					$percentage = $p2;
					$firstStageCompleted =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("rowid",$rowid)->orderBy("id","ASC")->first();
				...
				*/
				...
				$lastStageCompletedList =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("rowid",$rowid)->whereIn("stage_staus",array(1,2))->orderBy("id","DESC")->get();
				...
				*working
				*/
				$visaTypeModChecked =  CancelationVisaProcessChangeSalary::where("document_id",$empid)->where("rowid",$rowid)->orderBy("id","DESC")->first();
		...
		return view("ChangeSalaryNew/RequestTabsAjax/tabvisaProcessPanelAjax",compact('documentCollectionDetails','totalCost','percentage','lastStageCompleted','visaStageD','totalDays','visaStageLists','visaTypeId','rowid','salaryRequestdata'));
	}		  
	...
	public function offerLetterIncentiveForm($id=NULL)
	   ...
	   public function tabonboardPanelAjax(Request $request)
		  ...
		  public function Cancelvisadetailedform(Request $request){
		   ...
		   public function changeSalaryLogsTabData(Request $request)
		   {
			   $empid = $request->empid;
			   $rowid = $request->rowid;
			   ...
			   $changesalaryRequestData = Change_Salary_logs::where('emp_id',$empid)->orderBy('id','desc')->get();
   
   
			   
	 
			   return view("ChangeSalaryNew/RequestTabsAjax/ChangeSalaryLogsDetails",compact('changesalaryRequestData')); 
...
public function ExitInterviewempPerformance(Request $request){
		...
		$requestTypes = SalaryRequest::where("status",1)->get();
			...
			return view("ChangeSalaryNew/RequestTabsAjax/OffboardPerformanceDetailspopup",compact('empData','requestTypes','rowid')); 
