...
namespace App\Http\Controllers\EmpChangeSalary;
...
class EmpChangeSalaryController extends Controller
{
       ...
       public function EmpOffBoardProcess(Request $req)
	   {
		...
		return view("EmpChangeSalary/EmpChangeSalaryIndex",compact('ReasonsForLeavingDetails','departmentLists','tL_details','empId','Designation'));
	   }
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
		...
		return view("EmpChangeSalary/listingEmpOfflineProcessConditionforLeaving",compact('departmentLists','productDetails','paginationValue','designationDetails','documentCollectiondetails','reportsCount'));
	   }
...
if(!empty($request->session()->get('ReasonofAttrition_empoffboard_filter_list')) && $request->session()->get('ReasonofAttrition_empoffboard_filter_list') != 'All')
				...
				if($whereraw != '')
				{
					...
					$documentCollectiondetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->where("offline_status",1)->paginate($paginationValue);
				...
				else
				{
					...
					$documentCollectiondetails = Employee_details::orderBy("id","DESC")->where("offline_status",1)->paginate($paginationValue);
		...
		return view("EmpChangeSalary/listingEmpOfflineProcessAll",compact('departmentLists','productDetails','designationDetails','documentCollectiondetails','reportsCount','filterList','salaryBreakUpdetails','paginationValue'));
	   }
	   
	   public function listingIncrementData(Request $request)
	   {
		    $whereraw = '';
			$whereraw1 = '';
			$selectedFilter['CNAME'] = '';
...
if(!empty($request->session()->get('ReasonofAttrition_empoffboard_filter_list')) && $request->session()->get('ReasonofAttrition_empoffboard_filter_list') != 'All')
				...
				{
					//echo "hello";exit;
					$documentCollectiondetails = EmpOffline::whereIn("condition_leaving",array(3,4,5))->whereRaw($whereraw)->orderBy("settelement_date", "DESC")->paginate($paginationValue);
				}
				else
				{					
					$documentCollectiondetails = EmpOffline::whereIn("condition_leaving",array(3,4,5))->orderBy("settelement_date", "DESC")->paginate($paginationValue);
...
if(!empty($request->session()->get('ReasonofAttrition_empoffboard_filter_list')) && $request->session()->get('ReasonofAttrition_empoffboard_filter_list') != 'All')
				...
				if($whereraw != '')
				{
					...
					$documentCollectiondetails = EmpOffline::whereIn("condition_leaving",array(3,4,5,6))->whereRaw($whereraw)->orderBy("exit_interview_date", "DESC")->paginate($paginationValue);
				}
				else
				{
					
					$documentCollectiondetails = EmpOffline::whereIn("condition_leaving",array(3,4,5,6))->orderBy("exit_interview_date", "DESC")->paginate($paginationValue);
...
public function addpaymentconfirmation($offboardingId)
	{
		...
		return view("EmpChangeSalary/addpaymentconfirmationForm",compact('empData'));
	}
...
public static function getsSttelementHr($id)
	...
	public function getleavingTypePopupData($offboardId)
	{
		...
		$emp_details = Employee_details::orderBy("id","DESC")->where('emp_id',$offboardId)->where("offline_status",1)->first();
		$requestTypes = SalaryRequest::where("status",1)->get();

		
		return view("EmpChangeSalary/LeavingType",compact('emp_details','requestTypes'));
	}

	public function getleavingTypePopupData2($offboardId)
	{
		//$offlinedata = EmpOffline::where("id",$offboardId)->first();
		...
		return view("EmpChangeSalary/LeavingType2",compact('emp_details','requestTypes'));
	}
...
public function offboardDataFilterfnf(Request $request)
 ...
 if(!empty($request->session()->get('onboarding_department_filter')) && $request->session()->get('onboarding_department_filter') != '')
	   ...
	   public function listingMolData(Request $request)
	   {
		    $whereraw = '';
			$whereraw1 = '';
			$selectedFilter['CNAME'] = '';
...
if(!empty($request->session()->get('ReasonofAttrition_empoffboard_filter_list')) && $request->session()->get('ReasonofAttrition_empoffboard_filter_list') != 'All')
		 ...
		 $salaryBreakUpdetails =  SalaryBreakup::where("status",1)->orderBy("id","DESC")->get();
		return view("EmpChangeSalary/offboardFullandFinalSettelement",compact('CandidateRecruiterArray','companyvisaArray','VintageArray','departmentLists','productDetails','designationDetails','documentCollectiondetails','reportsCount','filterList','salaryBreakUpdetails','paginationValue','selectedFilter','CandidateNameArray','CandidateEmailArray','DesignationArray','OpeningArray','StatusArray','DepartmentArray'));
	   }
