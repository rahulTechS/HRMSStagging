		 ...
		 $salaryBreakUpdetails =  SalaryBreakup::where("status",1)->orderBy("id","DESC")->get();
				...
				*/
				...
				foreach($documentColectionId as $doc){
					$visastageId = Visaprocess::where("document_id",$doc->id)->orderBy('id','DESC')->first()->visa_stage;
					$stage=VisaStage::where("id",$visastageId)->orderBy('id','DESC')->first();
					if($stage!=''){
						$visastagearray[$visastageId]=$stage->stage_name;
					}
