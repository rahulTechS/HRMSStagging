...
class InterviewProcessController extends Controller
{
	...
	{
		//echo "hello";exit;where('offline_status',"!=",2)
		$jobRecruiterDetails=RecruiterDetails::where("status",1)->where("offline_status","!=",2)->get();
	...
	public function addInterviewProcessPost0(Request $rq)
	{
		...
		$filesAttributeInfo = array();
			...
			$obj->interview_city = $rq->input('interview_city');
	...
	public function interviewList(Request $request){
			...
			if(!empty($request->session()->get('offset_hiring_filter')))
				...
				$whereraw='';
				 ...
				 $jobfunctiondetails = JobFunctionPermission::where("user_id",$empId)->first();
				 ...
				 if($jobfunctiondetails!='' && ($jobfunctiondetails->job_function_id==3)){
					 ...
					 if($dept!=''){
						...
						if($whereraw == '')
							{
								$whereraw = 'department = "'.$dept->dept_id.'"';
							}
							else
							{
								$whereraw .= ' And department = "'.$dept->dept_id.'"';
							}
					 }
				 }
				...
				if(!empty($request->session()->get('source_from_list_filter')) && $request->session()->get('source_from_list_filter') != 'All')
				{
					$recruiter = $request->session()->get('source_from_list_filter');
					 $selectedFilter['recruiter'] = $recruiter;
					 if($whereraw == '')
					{
						$whereraw = "source_from IN('".$recruiter."')";
					}
					else
					{
						$whereraw .= " And source_from IN('".$recruiter."')";
					}
				}
				//echo "hlo";exit;
			...
			return view("InterviewProcess/InterviewListingAll",compact('InterviewList','paginationValue','reportsCount'));
		}
		...
		public static function getcurrentInterview($id)
			...
			public static function getDesignationNameTL($nam)
			{	
				$data = Employee_details::where("id",$nam)->first();
			  //$data = Department::where('id',$id)->first();
			  if($data != '')
	...
	public function UpdateInterviewForm($rowId=NULL)
	{
		...
		return view("InterviewProcess/UpdateInterviewForm",compact('cList','empdetailsdata','SalesExecutive','deptid','tL_details','jobRecruiterDetails','jobOpning','visaTypeList','interviewdetail1','interviewdetail2','interviewdetail3','interviewdetail4','interviewList'));
	}
...
public function updateInterviewProcessPostStep3(Request $rq)
	...
	public function updateInterviewProcessPostStep4(Request $rq)
	{
		...
		$obj->country = $rq->input('country');
			...
			$detailsObj->interviewer_tl = $rq->input('interviewer_tl');
			$detailsObj->rating = $rq->input('rating');
			$detailsObj->interviewer_tlsm = $rq->input('interviewer_tlsm');
		...
		$DocumentCollectionDetails->country = $data->country;
		...
		$DocumentCollectionDetails->ticket_status=1;
		...
		if($data->attached_cv!=''){
			$objDocument1 = new DocumentCollectionDetailsValues();
			$objDocument1->document_collection_id = $LastInsertId;
			$objDocument1->attribute_code = 14;
			$objDocument1->attribute_value = $data->attached_cv;
			
			$imagePath = public_path('interviewcv/'.$data->attached_cv);
			$newPath = public_path('documentCollectionFiles/'.$data->attached_cv);
			File::copy($imagePath, $newPath);
			$objDocument1->save();	
			}
			}
	...
	public function interviewListenbd(Request $request){
			...
			return view("InterviewProcess/InterviewListingenbd",compact('InterviewList','paginationValue','reportsCountenbd'));
		}
	public function interviewListdeem(Request $request){
			...
			return view("InterviewProcess/InterviewListingdeem",compact('InterviewList','paginationValue','reportsCountdeem'));
		}
			...
			return view("InterviewProcess/InterviewListingmashreq",compact('InterviewList','paginationValue','reportsCountmashreq'));
		}
		public function interviewListaafaq(Request $request){
			...
			return view("InterviewProcess/InterviewListingaafaq",compact('InterviewList','paginationValue','reportsCountaafaq'));
		}
		...
		$status='';
			...
			$sourceFrom='';
			if($request->interviewSourceFrom!=''){
			$sourceFrom = array_filter($request->interviewSourceFrom);
			$sourceFrom=implode(",", $sourceFrom);
			}
			...
			$request->session()->put('source_from_list_filter',$sourceFrom);
		...
		public function interviewFilterreset(Request $request)
		{
			...
			$request->session()->put('source_from_list_filter','');
		}
...
public static function getFinalDiscussionDate($id)
	...
	public static function getinterviewerNameFD($id)
			...
			$interview4=InterviewDetailsProcess::where('interview_id',$id)->where("interview_type",'final discussion')->first();
			 ...
			 $eecruiter =RecruiterDetails::where("id",$interview4->interviewer_name)->first();
			  
			  if($eecruiter != '')
			  {
				
			  return $eecruiter->name;
			  }
			  else
			  {
			  return '';
...
public function interviewListDIB(Request $request){
			...
			return view("InterviewProcess/InterviewListingDIB",compact('InterviewList','paginationValue','reportsCountDIB'));
		}
//SCB
...
public function interviewListSCB(Request $request){
			...
			return view("InterviewProcess/InterviewListingSCB",compact('InterviewList','paginationValue','reportsCountSCB'));
		}	
...
public function interviewListCBD(Request $request){
			...
			return view("InterviewProcess/InterviewListingCBD",compact('InterviewList','paginationValue','reportsCountCBD'));
		}
...
public function filterInterviewNameSuggestion(Request $request)
{
	$suggestionName =  $request->name;
	
	$interviewNameList = InterviewProcess::select("id","name")->where("name","like","%".$suggestionName."%")->get();
	
	return view("InterviewProcess/filterInterviewNameSuggestion",compact('interviewNameList'));
}
public function interviewListEIB(Request $request){
			...
			if(!empty($request->session()->get('offset_hiring_filter')))
				...
				if($whereraw != '')
				{
					$InterviewList = InterviewProcess::whereRaw($whereraw)->where('department',52)->orderBy("id","DESC")->paginate($paginationValue);
					$reportsCountEIB = InterviewProcess::whereRaw($whereraw)->where('department',52)->get()->count();	
				}
				else{
					$InterviewList = InterviewProcess::where('department',52)->orderBy("id","DESC")->paginate($paginationValue);
					$reportsCountEIB = InterviewProcess::where('department',52)->get()->count();	
				}					
				
				
				$InterviewList->setPath(config('app.url/interviewListEIB'));
				
				
				
				
			
			return view("InterviewProcess/InterviewListingeib",compact('InterviewList','paginationValue','reportsCountEIB'));
		}
public function interviewListHSBC(Request $request){
			...
			if(!empty($request->session()->get('offset_hiring_filter')))
				...
				if($whereraw != '')
				{
					$InterviewList = InterviewProcess::whereRaw($whereraw)->where('department',54)->orderBy("id","DESC")->paginate($paginationValue);
					$reportsCountHSBC = InterviewProcess::whereRaw($whereraw)->where('department',54)->get()->count();	
				}
				else{
					$InterviewList = InterviewProcess::where('department',54)->orderBy("id","DESC")->paginate($paginationValue);
					$reportsCountHSBC = InterviewProcess::where('department',54)->get()->count();	
				}					
				
				
				$InterviewList->setPath(config('app.url/interviewListHSBC'));
				
				
				
				
			
			return view("InterviewProcess/InterviewListinghsbc",compact('InterviewList','paginationValue','reportsCountHSBC'));
		}	




		public function interviewListUAE(Request $request){
			...
			if(!empty($request->session()->get('offset_hiring_filter')))
				...
				if($whereraw != '')
				{
					$InterviewList = InterviewProcess::whereRaw($whereraw)->where('department',56)->orderBy("id","DESC")->paginate($paginationValue);
					$reportsCountUAE = InterviewProcess::whereRaw($whereraw)->where('department',56)->get()->count();	
				}
				else{
					$InterviewList = InterviewProcess::where('department',56)->orderBy("id","DESC")->paginate($paginationValue);
					$reportsCountUAE = InterviewProcess::where('department',56)->get()->count();	
				}					
				
				
				$InterviewList->setPath(config('app.url/interviewListUAE'));
				
				
				
				
			
			return view("InterviewProcess/InterviewListinguae",compact('InterviewList','paginationValue','reportsCountUAE'));
		}	
