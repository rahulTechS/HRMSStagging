...
namespace App\Http\Controllers\VisaLedger;
...
class VisaLedgerController extends Controller
{
    
       public function VisaLedger(Request $req)
	   {
				/*
				*getting all document colection id for visa processdata
				*start code
				*/
				$whereraw = '';
				...
				$documentCId = array();
				foreach($documentColectionId as $_doc)
				{
					$documentCId[] = $_doc->id;
				}
				/*
		...
		*start code
		*/
				$jobopeningdata = array();
        		$documentC = DocumentCollectionDetails::whereIn("id",$documentCId)->get();
				foreach($documentC as $_c)
				{
					$jobopeningMOd = JobOpening::where("id",$_c->job_opening)->first();
					if($jobopeningMOd != '')
					{
					$departmentName = Department::where("id",$_c->department)->first()->department_name;
					$jobopeningdata[$_c->job_opening] = $jobopeningMOd->name.'-'.$departmentName.'-'.$_c->location;
					}
				}
		...
		*/
				...
				foreach($documentC as $_c)
				{
					$departmentMod = Department::where("id",$_c->department)->first();
					if($departmentMod != '')
					{
					$department[$_c->department] = $departmentMod->department_name;
					}
				}
		...
		*/
				$visaTypeArray = array();
				$visatypeLists = Visaprocess::whereIn('document_id',$documentCId)->get();
				foreach($visatypeLists as $visaT)
				{
						$visaTypeDetails = visaType::where("id",$visaT->visa_type)->first();
					if($visaTypeDetails != '')
					{
						$visaTypeArray[$visaT->visa_type] = $visaTypeDetails->title;
					}
				}
		...
		*/
		...
		return view("VisaLedger/VisaLedger",compact('jobopeningdata','department','visaTypeArray'));
	   }
	   
	   
	   
	   public function listingVisaLedgerList(Request $request)
	   {
		    $whereraw = '';
			
		
			if(!empty($request->session()->get('onboading_page_limitall')))
				{
				...
				*/
				$whereraw = '';
				$whereraw1 = '';
				if(!empty($request->session()->get('visaL_name')) && $request->session()->get('visaL_name') != 'All' && $request->session()->get('visaL_name') != '')
				{
					$cname = $request->session()->get('visaL_name');
					 if($whereraw == '')
					{
				...
				if(!empty($request->session()->get('visaL_jobOpening')) && $request->session()->get('visaL_jobOpening') != 'All' && $request->session()->get('visaL_jobOpening') != '')
				{
					$visaL_jobOpening = $request->session()->get('visaL_jobOpening');
					 if($whereraw == '')
					{
						$whereraw = 'job_opening IN('.$visaL_jobOpening.')';
					}
					else
					{
						$whereraw .= ' And job_opening IN('.$visaL_jobOpening.')';
					}
				}
				
				if(!empty($request->session()->get('visaL_department')) && $request->session()->get('visaL_department') != 'All' && $request->session()->get('visaL_department') != '')
				{
					$visaL_department = $request->session()->get('visaL_department');
					 if($whereraw == '')
					{
						$whereraw = 'department IN('.$visaL_department.')';
					}
					else
					{
						$whereraw .= ' And department IN('.$visaL_department.')';
					}
				}
				if(!empty($request->session()->get('visaL_datefrom')) && $request->session()->get('visaL_datefrom') != 'All' && $request->session()->get('visaL_datefrom') != '')
				{
					$visaL_datefrom = $request->session()->get('visaL_datefrom');
					 if($whereraw1 == '')
					{
						$whereraw1 = 'closing_date_by_system <="'.$visaL_datefrom.' 00:00:00"';
					}
					else
					{
						$whereraw1 .= ' And closing_date_by_system <="'.$visaL_datefrom.' 00:00:00"';
					}
				}
				if(!empty($request->session()->get('visaL_dateto')) && $request->session()->get('visaL_dateto') != 'All' && $request->session()->get('visaL_dateto') != '')
				{
					$visaL_dateto = $request->session()->get('visaL_dateto');
					 if($whereraw1 == '')
					{
						$whereraw1 = 'closing_date_by_system >="'.$visaL_dateto.' 00:00:00"';
					}
					else
					{
						$whereraw1 .= ' And closing_date_by_system >="'.$visaL_dateto.' 00:00:00"';
					}
				}
				if(!empty($request->session()->get('visaL_visaType')) && $request->session()->get('visaL_visaType') != 'All' && $request->session()->get('visaL_visaType') != '')
				{
					$visaL_visaType = $request->session()->get('visaL_visaType');
					 if($whereraw1 == '')
					{
						$whereraw1 = 'visa_type IN('.$visaL_visaType.')';
					}
					else
					{
						$whereraw1 .= ' And visa_type IN('.$visaL_visaType.')';
					}
				}
				//echo $whereraw1;exit;
				if($whereraw != '')
				{
					...
					$documentColectionIdMod = DocumentCollectionDetails::whereRaw($whereraw)->where("visa_process_status",array(2,4))->get();
				}
				else
				{
					$documentColectionIdMod = DocumentCollectionDetails::whereIn("visa_process_status",array(2,4))->get();	
				}
				
				$documentCId = array();
				foreach($documentColectionIdMod as $_doc)
				{
					$documentCId[] = $_doc->id;
				}
				/* echo count($documentColectionId);
				exit; */
				/*
				*getting all document colection id for visa processdata
				*end code
				*/
					if(count($documentCId) > 0)
					{
						if($whereraw1 != '')
						{
							
							$documentCollectiondetails = Visaprocess::orderBy('closing_date','DESC')->whereRaw($whereraw1)->whereIn('document_id',$documentCId)->paginate($paginationValue);
					
							$reportsCount = Visaprocess::whereRaw($whereraw1)->whereIn('document_id',$documentCId)->get()->count();
						}
						else
						{
							$documentCollectiondetails = Visaprocess::orderBy('closing_date','DESC')->whereIn('document_id',$documentCId)->paginate($paginationValue);
					
							$reportsCount = Visaprocess::whereIn('document_id',$documentCId)->get()->count();
						}
					}
					else
					{
				...
				$documentCollectiondetails->setPath(config('app.url/listingVisaLedgerList'));
				
		
		
		return view("VisaLedger/VisaLedgerList",compact('documentCollectiondetails','reportsCount','paginationValue'));
	   }
	   
	   
	   public function visalsearchfilter(Request $request)
	   {
		  $parameterInput =  $request->input();
		  /* echo "<pre>";
		  print_r($parameterInput);
		  exit; */
		  $candidateName = $parameterInput['candidateName'];
		  $dateto = $parameterInput['dateto'];
		  $datefrom = $parameterInput['datefrom'];
		  if(isset($parameterInput['jobOpening']))
		  {
			  
			  $jobopeningArray = $parameterInput['jobOpening'];
			  sort($jobopeningArray);
			  $jobopeningstr = implode(",",$jobopeningArray);
		  ...
		  if(isset($parameterInput['department']))
		  {
			   $departmentArray = $parameterInput['department'];
			   sort($departmentArray);
			   $departmentStr = implode(",",$departmentArray);
		  }
		  else
		  {
			  $departmentStr = '';
		  ...
		  if(isset($parameterInput['visaType']))
		  {
			  $visaTypeArray = $parameterInput['visaType'];
			   sort($visaTypeArray);
			   $visaTypeStr = implode(",",$visaTypeArray);
		  }
		  else
		  {
			  $visaTypeStr = '';
		  }
		  $request->session()->put('visaL_name',$candidateName);
		  $request->session()->put('visaL_dateto',$dateto);
		  $request->session()->put('visaL_datefrom',$datefrom);
		  $request->session()->put('visaL_jobOpening',$jobopeningstr);
		  $request->session()->put('visaL_department',$departmentStr);
		  $request->session()->put('visaL_visaType',$visaTypeStr);
			$response['code'] = '200';
	   ...
	   public function visalReset(Request $request)
	   {
		    $request->session()->put('visaL_name','');
		    $request->session()->put('visaL_dateto','');
		    $request->session()->put('visaL_datefrom','');
		    $request->session()->put('visaL_jobOpening','');
		    $request->session()->put('visaL_department','');
		    $request->session()->put('visaL_visaType','');
			 return redirect('listingVisaLedgerList');
			echo "yes";
			exit;
	   }
	   public function exportDocReportvisaL(Request $request){
		$parameters = $request->input(); 
	         $selectedId = $parameters['selectedIds'];
			 
	        $filename = 'Visa_Ledger_report_'.date("d-m-Y").'.xlsx';
			...
			$sheet->setCellValue('B'.$indexCounter, strtoupper('Final Date'))->getStyle('B'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('C'.$indexCounter, strtoupper('Candidate Name'))->getStyle('C'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('D'.$indexCounter, strtoupper('Visa Type Name'))->getStyle('D'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('E'.$indexCounter, strtoupper('Visa Stage Name'))->getStyle('E'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			$sheet->setCellValue('F'.$indexCounter, strtoupper('Cost'))->getStyle('F'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			...
			$sheet->setCellValue('I'.$indexCounter, strtoupper('Document Id'))->getStyle('I'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
			
			$sn = 1;
			foreach ($selectedId as $sid) {
				//echo $sid;
				 $misData = Visaprocess::where("id",$sid)->first();
				 
				 
				 if(!empty($misData->closing_date)){
				 $date=date("d-M-Y",strtotime(str_replace("/","-",$misData->closing_date)));
				 }
				 else{
					 $date='';
				 }
				 $name = DocumentCollectionDetails::where("id",$misData->document_id)->first();
				 $empName=$name->emp_name;
				 $visatype=visaType::where("id",$misData->visa_type)->first()->title;
				 $visa_stage = VisaStage::where("id",$misData->visa_stage)->first()->stage_name;
				 if($misData->cost!=''){
				 $cost=$misData->cost;
				 }
				 else{
				 ...
				 $jobOpning=JobOpening::where("id",$name->job_opening)->first();
				 $jobname=$jobOpning->name;
				 $department=$name->department;
				 $docId=DocumentCollectionDetails::where("id",$misData->document_id)->first();
				 $fdocId=$docId->id;
				 $indexCounter++; 	
				...
				$sheet->setCellValue('C'.$indexCounter, strtoupper($empName))->getStyle('C'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('D'.$indexCounter, $visatype)->getStyle('D'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('E'.$indexCounter, $visa_stage)->getStyle('E'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				$sheet->setCellValue('F'.$indexCounter, "AED ".$cost)->getStyle('F'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
				...
				$sheet->setCellValue('I'.$indexCounter, $fdocId)->getStyle('I'.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top');
	 ...
	 function getDesignationOfDocumentation(Request $request)
	  ...
	  public function getSalaryBreakupDocumentAjax(Request $request)
	   ...
	   public function setOffSetForOnboardingVisaLedger(Request $request)
	   {
