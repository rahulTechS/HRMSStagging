...
class CompanyController extends Controller
{
    ...
    public function addDepartment()
    {
        ...
        return view("Company/addDepartmentType",compact('divisonDetails'));
    }
	...
	{
		return view("Company/DepartmentType"); 
	}
	public function setOffSetForDepartmentType(Request $request)
			{
				$offset = $request->offset;
				$request->session()->put('offset_department_filter',$offset);
				 
			}

    public function departmentListData(Request $request)
    {
		if(!empty($request->session()->get('offset_department_filter')))
				{
					$paginationValue = $request->session()->get('offset_department_filter');
				}
				else
				{
					$paginationValue = 10;
			...
			$departmentDetails =  Department::whereIn("status",array(1,2))->orderBy("id","DESC")->paginate($paginationValue);
			$reportsCount = Department::whereIn("status",array(1,2))->get()->count();
			
		
	   
       return view("Company/DepartmentList",compact('departmentDetails','paginationValue','reportsCount'));
    }
