<?php
namespace App\Http\Controllers\EmpProcessNew;
...
class EmpProcessController extends Controller
{
		...
		{
			$attributeListing = EmpExportSetting::where("status",1)->orderBy("id","DESC")->get();
					  
			return view("EmpProcess/filterEmployee",compact('attributeListing'));
		...
		public function viewEmployeeData($empid=NULL)
	{
		...
		$empDetailsSection3 = Attributes::join('employee_attribute', 'employee_attribute.attribute_code', '=', 'attributes.attribute_code')
			...
			return view("EmpProcess/viewEmployee",compact('empDetails'),compact('empRequiredDetails','empDetailsSection2','empDetailsSection3','empDetailsSection4','kycSection5'));
	
	}
	
		public function EmpProcessList(Request $request)
		{			
			return view("EmpProcess/EmpList");
...
$request->session()->put('scode_emp_filter_inner_list','All');
		...
		public function AjaxEmpList(Request $request){
			...
			if(!empty($request->session()->get('offset_emp_filter')))
				...
				if(!empty($request->session()->get('fname_emp_filter_inner_list')) && $request->session()->get('fname_emp_filter_inner_list') != 'All')
				{
					$fname = $request->session()->get('fname_emp_filter_inner_list');
					 ...
					 if($whereraw == '')
					{
						$whereraw = 'first_name = "'.$fname.'"';
					}
					else
					{
						$whereraw .= ' And first_name = "'.$fname.'"';
					}
				}
				if(!empty($request->session()->get('lname_emp_filter_inner_list')) && $request->session()->get('lname_emp_filter_inner_list') != 'All')
				{
					$lname = $request->session()->get('lname_emp_filter_inner_list');
					 ...
					 if($whereraw == '')
					{
						$whereraw = 'last_name = "'.$lname.'"';
					}
					else
					{
						$whereraw .= ' And last_name = "'.$lname.'"';
					}
				}
				if(!empty($request->session()->get('design_emp_filter_inner_list')) && $request->session()->get('design_emp_filter_inner_list') != 'All')
				{
					$design = $request->session()->get('design_emp_filter_inner_list');
					 ...
					 if($whereraw == '')
					{
						$whereraw = 'job_role = "'.$design.'"';
					}
					else
					{
						$whereraw .= ' And job_role = "'.$design.'"';
					}
				}
				if(!empty($request->session()->get('scode_emp_filter_inner_list')) && $request->session()->get('scode_emp_filter_inner_list') != 'All')
				{
					$scode = $request->session()->get('scode_emp_filter_inner_list');
					 ...
					 if($whereraw == '')
					{
						$whereraw = 'source_code = "'.$scode.'"';
					}
					else
					{
						$whereraw .= ' And source_code = "'.$scode.'"';
					}
				...
				if(!empty($request->session()->get('location_emp_filter_inner_list')) && $request->session()->get('location_emp_filter_inner_list') != 'All')
				{
					$location = $request->session()->get('location_emp_filter_inner_list');
					 ...
					 if($whereraw == '')
					{
						$whereraw = 'work_location = "'.$location.'"';
					}
					else
					{
				...
				*/
				...
				if($whereraw != '')
				{
				$empdetails = Employee_details::orderBy("id","DESC")->whereRaw($whereraw)->paginate($paginationValue);
				$reportsCount = Employee_details::whereRaw($whereraw)->get()->count();
					$activeCount = Employee_details::whereRaw($whereraw)->where('status',1)->get()->count();
					$inactiveCount = Employee_details::whereRaw($whereraw)->where('status',2)->get()->count();				
				}
				else
				{
					$empdetails = Employee_details::orderBy("id","DESC")->paginate($paginationValue);
					$reportsCount = Employee_details::get()->count();	
					$activeCount = Employee_details::where('status',1)->get()->count();
					$inactiveCount = Employee_details::where('status',2)->get()->count();					
				}
			...
			return view("EmpProcess/AjaxEmpList",compact('VisaUnderCompany','LocationArray','VintageArray','empdetails','paginationValue','departmentLists','deptID','reportsCount','empIdArray','selectedFilter','f_nameArray','l_nameArray','departmentArray','sourcecodeArray','designationArray','activeCount','inactiveCount'));
		}
...
//print_r($filesAttributeInfo['37']);exit;
		...
		public function UpdateEMPPersonalData(Request $request)
		   {
				...
				return view("EmpProcess/UpdatePersonalDetailForm",compact('attributesDetailsbd','attributesDetailscd','attributesDetailspd','attributesDetailsvd','emp_details','attribute','design','cList','percentage'));
		   }
	...
	protected function toAlpha($data){
		...
		public static function getArributeName($code)
			{
				//$name = Employee_details::where("id",$id)->first();
				$name =EmpExportSetting::where("attribute_code",$code)->where("status",1)->first();
				if($name != '')
				{
		...
		public function exportEmpReport(Request $request){
			...
			//print_r($parameters);exit;
	         $selectedCode = $parameters['selectedCode'];
			 
			  $selectedId = $parameters['selectedIds'];
			 //print_r($selectedCode);exit;
			...
			$sheet->setCellValue($cellName.$indexCounter, strtoupper(EmpProcessController::getArributeName($_code)))->getStyle($cellName.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top'); 
			...
			foreach ($selectedId as $sid) {
				...
				foreach($selectedCode as $_code)
				{
					...
					$attributes = Attributes::where('attribute_code',$_code)->first();
					$newcellName = strtoupper($this->toAlpha($Indexval));
					//print_r($attributes->attrbute_type_id);exit;
					if($attributes->attrbute_type_id == 4){
						 $empattributesMod = Employee_attribute::where('emp_id',$empId)->where('attribute_code',$_code)->first();
						 if(!empty($empattributesMod)){
						 $date=date("d-M-Y",strtotime(str_replace("/","-",$empattributesMod->attribute_values)));
						 }
						 else{
							 $date='';
						 }
						 ...
						 $col_data = Employee_attribute::where('emp_id',$empId)->where('attribute_code',$_code)->first();
						 if(!empty($col_data)){
						 $date=$col_data->attribute_values;
						 }
						 else{
							 $date='';
						 }
					...
					$sheet->setCellValue($newcellName.$indexCounter, $date)->getStyle($newcellName.$indexCounter)->getAlignment()->setHorizontal('center')->setVertical('top'); 
					
					//$newcellName = $Indexval+1;
					$Indexval = $Indexval+1;
			...
			$cellIndexval = 5;
			$startCellName ='F'; 
			foreach($selectedCode as $_code1)
			{
			
			$cell = strtoupper($this->toAlpha($cellIndexval));
			//$sheet->mergeCells($startCellName.$indexCounter.':'.$cellName.$indexCounter);
			$spreadsheet->getActiveSheet()->getStyle($startCellName.$Counter.':'.$cell.$Counter)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('cbddf7');
			$sheet->getColumnDimension($cellName)->setAutoSize(true);
			$cellIndexval = $cellIndexval+1;
