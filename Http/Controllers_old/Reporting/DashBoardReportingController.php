...
exit; */			 
		   ...
		   */
		   ...
		   $StatusValues = array();
				...
				else
				{
				   ...
				   {
					   $AgentlistValues[] = $Agent->first_name;
					   $detailsAsPerTL[$Agent->id]['login'] =	MainMisReport::whereDate("submission_format",">=",$fromDate)->whereDate("submission_format","<=",$toDate)->where("employee_id",$Agent->id)->where("TL",$TLID)->whereIn("file_source",$typeSource)->whereIn("ALE_NALE",$listedValueArray)->get()->count();
					   foreach($enbdStatus as $status)
					   {
	 ...
	 {
		$_enbdCards = ENBDCardsMisReport::where("CURRENTACTIVITY",array("Single Data Entry","Document Verification","Underwriting","Detail Data Entry","HOLD RCC","VERIFICATION DETAIL","HOLD SOURCING"))
